            ...
            match fetch_issue_comments(repo, number, runner) {
                ...
                Err(error) => {
                    ...
                    desktop_log::warn(format_args!(
                        "IDEOCODE-desktop: failed to refresh comments for GitHub issue {repo}#{number}: {error:#}"
                    ));
                    existing_comments
                }
            }
...
fn ideocode_data_dir() -> PathBuf {
    ...
    if let Some(home) = std::env::var_os("HOME") {
        return PathBuf::from(home).join(".IDEOCODE");
    }
    PathBuf::from(".IDEOCODE")
}
...
mod tests {
    ...
    fn unique_issue_cache_temp_root(name: &str) -> PathBuf {
        ...
        std::env::temp_dir().join(format!(
            "IDEOCODE-desktop-issue-cache-{name}-{}-{nanos}",
            std::process::id()
        ))
    }
