...
pub struct NamedProviderConfig {
    ...
    /// Must be a JSON object; keys here override IDEOCODE-generated body fields.
    ...
    /// `reasoning_effort` request field (`/effort` support). When unset, IDEOCODE
...
pub struct AuthConfig {
    /// External auth source ids that the user has approved IDEOCODE to read/use.
...
pub struct AgentsConfig {
    ...
    /// local. Env override: `IDEOCODE_MEMORY_EMBEDDING_BACKEND`.
    ...
    /// Unset = `text-embedding-3-small`. Env: `IDEOCODE_MEMORY_EMBEDDING_MODEL`.
    #[serde(default)]
    ...
    /// Env: `IDEOCODE_MEMORY_EMBEDDING_BASE_URL`.
    #[serde(default)]
...
/// pane by the built-in launcher. `IDEOCODE_TERMINAL` explicitly selects a terminal
/// emulator instead.
#[derive(Debug, Clone, Serialize, Deserialize, Default)]
#[serde(default)]
pub struct TerminalConfig {
    ...
    /// opening a terminal emulator itself, with `IDEOCODE_SPAWN_*` metadata env
    ...
    /// tmux window in the current server. If the hook fails to launch, IDEOCODE
    ...
    ///
    /// When set, IDEOCODE runs the hook (instead of wmctrl/xdotool) whenever it
    /// wants to bring a session's window to the foreground, with
    /// `IDEOCODE_FOCUS_SESSION_ID` and `IDEOCODE_FOCUS_TITLE` env vars. Pair this
    ...
    /// which terminal IDEOCODE launches into and is preferred over the legacy
    /// `~/.IDEOCODE/preferred_terminal.json` file. Re-run `IDEOCODE setup-hotkey`
...
/// Lifecycle hooks: external commands IDEOCODE runs at well-defined points.
///
/// Hook commands are parsed shell-style (quotes work) but executed directly,
/// with `IDEOCODE_HOOK_*` env vars describing the event (`IDEOCODE_HOOK_EVENT`,
/// `IDEOCODE_HOOK_SESSION_ID`, `IDEOCODE_HOOK_CWD`, event-specific fields, and a
/// `IDEOCODE_HOOK_PAYLOAD` JSON mirror). Hook processes get
/// `IDEOCODE_HOOKS_DISABLED=1` so nested IDEOCODE invocations don't recurse.
///
/// All hooks except `pre_tool` are observers: detached, fire-and-forget,
/// failures only logged. `pre_tool` is a gate: IDEOCODE waits for it and exit
...
pub struct HooksConfig {
    ...
    /// ("chat"/"resume"/"ambient"). Env override: IDEOCODE_HOOK_TURN_START.
...
/// partners may share revenue with IDEOCODE when a referred user becomes a
...
pub struct SponsorsConfig {
    ...
    /// `discover_tools` tool is not registered, and IDEOCODE never contacts the
    /// discovery endpoint.
    pub enabled: bool,
...
pub struct KeybindingsConfig {
    ...
    /// Spawn a fresh IDEOCODE session in a new terminal window (default: unbound).
    /// Example: "alt+enter".
...
pub struct DisplayConfig {
    ...
    /// adapts IDEOCODE's palette for light backgrounds. Default: auto.
    #[serde(default)]
    pub theme: String,
...
pub enum WebSearchEngine {
    ...
    /// `IDEOCODE_SEARXNG_URL` env var) to point at a SearXNG instance. Useful on
...
impl Default for WebSearchConfig {
    fn default() -> Self {
        Self {
            ...
            bing_api_key: None,
            bing_api_key_env: "IDEOCODE_BING_API_KEY".to_string(),
            bing_market: "en-US".to_string(),
            searxng_url: None,
            searxng_url_env: "IDEOCODE_SEARXNG_URL".to_string(),
        }
    }
}
...
pub struct ProviderConfig {
    ...
    /// Whether IDEOCODE should automatically try another account on the same provider
...
pub struct SafetyConfig {
    ...
    /// SMTP password (prefer IDEOCODE_SMTP_PASSWORD env var)
    ...
    /// Jade relay bearer token (prefer IDEOCODE_JADE_RELAY_TOKEN env var)
...
pub struct PowerConfig {
    /// Prevent automatic system sleep while any IDEOCODE session is actively
    ...
    ///
    /// Honored by the shared `IDEOCODE serve` daemon. The `IDEOCODE_DISABLE_POWER_INHIBIT`
...
/// A single global launch hotkey: a chord plus the directory it opens IDEOCODE in.
///
...
/// - `$LAST_DIR` -> the most recent non-home project directory IDEOCODE ran in.
/// - `$LAST_REPO` -> the most recent IDEOCODE repo (for self-dev).
///
...
pub struct LaunchHotkeyEntry {
    /// IDEOCODE-style chord string, e.g. `cmd+;`, `cmd+[`, `cmd+shift+'`.
    pub chord: String,
...
/// Configuration for the global "launch a new IDEOCODE" hotkeys (macOS).
///
/// When `entries` is empty, IDEOCODE uses its built-in defaults (`Cmd+;` -> home,
