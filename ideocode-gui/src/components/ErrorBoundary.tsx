    ...
    return (
      <div className="flex items-center justify-center h-screen bg-surface">
        <div className="max-w-md p-6 space-y-4 text-center">
          ...
          <div className="text-xs text-fg-muted font-mono bg-surface-elevated rounded p-3 text-left max-h-32 overflow-y-auto">
            {error.message}
          </div>
          <button
            onClick={() => {
            ...
            className="px-4 py-2 text-xs bg-accent text-white rounded hover:bg-accent-hover transition-fast"
          >
            Reload
          </button>
        </div>
  ...
  private renderPanel(error: Error) {
    return (
      <div className="flex flex-col items-center justify-center h-full p-4 text-center">
        <div className="w-full max-w-xs p-4 rounded-lg bg-surface-elevated border border-error/30 space-y-3">
          <div className="text-xs font-medium text-error">Panel crashed</div>
          <div className="text-[11px] text-fg-muted font-mono max-h-24 overflow-y-auto break-words">
            {error.message}
          </div>
          <button
            onClick={() => this.setState({ error: null })}
            className="px-3 py-1.5 text-[11px] bg-accent text-white rounded hover:bg-accent-hover transition-fast"
          >
            Try again
          </button>
        </div>
