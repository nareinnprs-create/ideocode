...
impl Config {
    ...
    pub fn create_default_config_file() -> anyhow::Result<PathBuf> {
        ...
        let default_content = r#"# IDEOCODE configuration file
# Location: ~/.IDEOCODE/config.toml
#
# Environment variables override these settings.
# Run `/config` in IDEOCODE to see current settings.
...
# Spawn a fresh IDEOCODE session in a new terminal window, reusing the current
# session's working directory. Companion to the system-wide launch hotkeys.
# `IDEOCODE setup-hotkey` installs these three global launch hotkeys on macOS,
...
#   Cmd+;        new IDEOCODE in your home directory
#   Cmd+'        new IDEOCODE in your last project directory
#   Cmd+Shift+'  new IDEOCODE self-dev session (last IDEOCODE repo)
...
# How to apply the transcript inside IDEOCODE: insert|append|replace|send
mode = "send"
...
# Set false here or set IDEOCODE_NO_EMOJI=1 for ASCII fallbacks.
emoji = true
...
# bing_api_key_env = "IDEOCODE_BING_API_KEY"
# bing_api_key = ""
...
# this. Configure here or via the IDEOCODE_SEARXNG_URL environment variable, then
...
# extended/full additionally emit ignorable _IDEOCODE/* extension notifications.
profile = "standard"
# Tool profile requested when `IDEOCODE acp` starts the daemon itself.
...
#   claude        = Claude via OAuth/subscription (token in ~/.IDEOCODE/auth.json)
#   anthropic-api = Claude via direct Anthropic API key (ANTHROPIC_API_KEY env
#                   or ~/.config/IDEOCODE/anthropic.env). API-key mode does NOT fall
...
# to match this value. Also overridable per-launch via IDEOCODE_STREAM_IDLE_TIMEOUT_SECS.
...
# Env override: IDEOCODE_SWARM_MODEL
# swarm_model = "inherit"
#
...
# Env override: IDEOCODE_SWARM_SPAWN_MODE
...
# Env override: IDEOCODE_SWARM_MAX_CONCURRENT_AGENTS
...
# Env override: IDEOCODE_SWARM_STRIP_LAYOUT
...
# Env override: IDEOCODE_MEMORY_MODEL
...
# dormant instead of degrading to the no-LLM path. Env: IDEOCODE_MEMORY_SIDECAR_ENABLED
...
# Env override: IDEOCODE_MEMORY_EMBEDDING_BACKEND
...
# Set IDEOCODE_TERMINAL to force a supported terminal emulator instead.
...
#
# When set, IDEOCODE runs `<spawn_hook> <IDEOCODE-binary> <args...>` instead of
# opening a terminal emulator itself. The hook receives IDEOCODE_SPAWN_* env vars
# describing the spawn so multiplexers/wrappers can decide where it appears:
#   IDEOCODE_SPAWN_KIND        - "swarm-agent", "resume", "selfdev", "restart", ...
#   IDEOCODE_SPAWN_SESSION_ID  - session the window will run
#   IDEOCODE_SPAWN_TITLE       - suggested window/tab title
#   IDEOCODE_SPAWN_CWD         - session working directory (also the hook's cwd)
#   IDEOCODE_SPAWN_PROGRAM     - IDEOCODE binary path
#   IDEOCODE_SPAWN_COMMAND     - full shell-escaped command line
#   IDEOCODE_SPAWN_SWARM_ID / IDEOCODE_SPAWN_COORDINATOR_SESSION_ID (swarm spawns)
# If the hook fails to start, IDEOCODE falls back to built-in terminal detection.
# Env override: IDEOCODE_SPAWN_HOOK (set empty to disable a config hook).
#
# Examples:
#   spawn_hook = "tmux new-window"                # tmux window per agent
#   spawn_hook = "kitty @ launch --type=tab --"   # kitty tab per agent
#   spawn_hook = "~/bin/IDEOCODE-spawn-router"       # custom placement script
# spawn_hook = ""
#
# External command used to focus/raise an existing session window, replacing
# the built-in wmctrl/xdotool title search. Receives IDEOCODE_FOCUS_SESSION_ID
# and IDEOCODE_FOCUS_TITLE env vars. Pair with spawn_hook so the program that
# placed the window also brings it to the front.
# Env override: IDEOCODE_FOCUS_HOOK (set empty to disable a config hook).
#
# Example:
#   focus_hook = "~/bin/IDEOCODE-focus-router"
# focus_hook = ""
#
# macOS only: terminal that the Cmd+; launch hotkey and in-app session spawns
# open IDEOCODE into. One of: ghostty, iterm2, wezterm, warp, alacritty, vscode,
# terminal (Apple Terminal). Preferred over the legacy
# ~/.IDEOCODE/preferred_terminal.json file. After changing this, re-run
# `IDEOCODE setup-hotkey` so the generated launcher script (Cmd+;) picks it up.
# preferred = "ghostty"
...
[hooks]
# Lifecycle hooks: external commands IDEOCODE runs at well-defined points so other
# programs can observe or gate agent behavior. Commands are parsed shell-style
# (quotes work) but executed directly, with IDEOCODE_HOOK_* env vars describing
# the event:
#   IDEOCODE_HOOK_EVENT       - "turn_start", "turn_end", "session_start",
#                            "session_end", "pre_tool", "post_tool"
#   IDEOCODE_HOOK_SESSION_ID  - the session the event belongs to
#   IDEOCODE_HOOK_CWD         - session working directory (also the hook's cwd)
#   IDEOCODE_HOOK_PAYLOAD     - JSON mirror of all fields
# Hook processes get IDEOCODE_HOOKS_DISABLED=1 so nested IDEOCODE calls don't recurse.
#
# All hooks except pre_tool are observers: detached, fire-and-forget, failures
# only logged. Env overrides: IDEOCODE_HOOK_TURN_START, IDEOCODE_HOOK_TURN_END,
# IDEOCODE_HOOK_SESSION_START, IDEOCODE_HOOK_SESSION_END, IDEOCODE_HOOK_PRE_TOOL,
# IDEOCODE_HOOK_POST_TOOL (set empty to disable a config hook).
#
...
# think/stream window before any tool call. Extra fields: IDEOCODE_HOOK_MODEL,
# IDEOCODE_HOOK_SOURCE ("chat"/"resume"/"ambient").
# turn_start = "~/bin/IDEOCODE-turn-start"
#
# Runs when an agent turn completes. Extra fields: IDEOCODE_HOOK_STATUS
# ("ok"/"error"), IDEOCODE_HOOK_DURATION_MS, IDEOCODE_HOOK_MODEL,
# IDEOCODE_HOOK_LAST_ASSISTANT_TEXT (first 4000 chars), IDEOCODE_HOOK_ERROR.
# turn_end = "~/bin/IDEOCODE-turn-notify"
#
# Runs when a session becomes active. Extra: IDEOCODE_HOOK_SOURCE
# ("create"/"attach"/"resume").
# session_start = ""
#
# Runs when a session closes normally. Extra: IDEOCODE_HOOK_SOURCE ("close").
# session_end = ""
#
# Gate hook before every tool call. Receives IDEOCODE_HOOK_TOOL_NAME and the tool
# input JSON on stdin (truncated copy in IDEOCODE_HOOK_TOOL_INPUT). Exit 0 allows
...
# pre_tool = "~/bin/IDEOCODE-tool-policy"
#
# Max milliseconds to wait for pre_tool before failing open (default: 5000).
# pre_tool_timeout_ms = 5000
#
# Runs after each tool call. Extra fields: IDEOCODE_HOOK_TOOL_NAME,
# IDEOCODE_HOOK_STATUS, IDEOCODE_HOOK_DURATION_MS, IDEOCODE_HOOK_OUTPUT_BYTES,
# IDEOCODE_HOOK_ERROR.
# post_tool = ""
...
[power]
# Prevent automatic system sleep while any IDEOCODE session is actively working.
...
# Set IDEOCODE_DISABLE_POWER_INHIBIT=1 to force-disable regardless of this setting.
...
# ntfy_topic = "IDEOCODE-ambient-your-secret-topic"
...
# email_from = "IDEOCODE@example.com"
# email_smtp_host = "smtp.gmail.com"
# email_smtp_port = 587
# Password via env: IDEOCODE_SMTP_PASSWORD (preferred) or config below
# email_password = ""
...
# telegram_bot_token = ""  # From @BotFather (prefer IDEOCODE_TELEGRAM_BOT_TOKEN env var)
...
# discord_bot_token = ""     # From Discord Developer Portal (prefer IDEOCODE_DISCORD_BOT_TOKEN env var)
...
# IDEOCODE_JADE_RELAY_API_BASE, IDEOCODE_JADE_RELAY_TOKEN, IDEOCODE_JADE_RELAY_TOKEN_ID,
# IDEOCODE_JADE_RELAY_USER_ID, IDEOCODE_JADE_RELAY_SESSION_ID.
...
# developer tools from IDEOCODE's hosted partner directory. Some partners may
