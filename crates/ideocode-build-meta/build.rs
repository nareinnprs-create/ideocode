...
// Build metadata generator for the IDEOCODE workspace.
//
...
fn main() {
    ...
    let dirty = match std::env::var("IDEOCODE_BUILD_GIT_DIRTY") {
        Ok(value) => matches!(
    ...
    let raw_log = std::env::var("IDEOCODE_BUILD_CHANGELOG_RAW")
        .ok()
    ...
    let is_release = std::env::var("IDEOCODE_RELEASE_BUILD").is_ok();
    ...
    println!("cargo:rustc-env=IDEOCODE_GIT_HASH={}", git_hash);
    println!("cargo:rustc-env=IDEOCODE_GIT_DATE={}", git_date);
    println!("cargo:rustc-env=IDEOCODE_VERSION={}", version);
    println!("cargo:rustc-env=IDEOCODE_SEMVER={}", build_semver);
    println!("cargo:rustc-env=IDEOCODE_BASE_SEMVER={}", base_semver);
    println!("cargo:rustc-env=IDEOCODE_UPDATE_SEMVER={}", update_semver);
    println!("cargo:rustc-env=IDEOCODE_GIT_TAG={}", git_tag);
    println!("cargo:rustc-env=IDEOCODE_CHANGELOG={}", changelog);
    println!("cargo:rustc-env=IDEOCODE_PKG_VERSION={}", pkg_version);

    // Forward IDEOCODE_RELEASE_BUILD env var if set (CI sets this for release binaries)
    if std::env::var("IDEOCODE_RELEASE_BUILD").is_ok() {
        println!("cargo:rustc-env=IDEOCODE_RELEASE_BUILD=1");
    }
    ...
    // Since `IDEOCODE-build-meta` sits at the bottom of the crate graph
    ...
    //   * Release/dist builds set IDEOCODE_RELEASE_BUILD=1 and IDEOCODE_BUILD_SEMVER,
    ...
    println!("cargo:rerun-if-env-changed=IDEOCODE_RELEASE_BUILD");
    println!("cargo:rerun-if-env-changed=IDEOCODE_BUILD_SEMVER");
    // Allow callers to force a metadata refresh (e.g. install scripts) without a
    // full clean, by bumping this env var.
    println!("cargo:rerun-if-env-changed=IDEOCODE_BUILD_GIT_HASH");
    println!("cargo:rerun-if-env-changed=IDEOCODE_BUILD_GIT_DATE");
    println!("cargo:rerun-if-env-changed=IDEOCODE_BUILD_GIT_DIRTY");
    println!("cargo:rerun-if-env-changed=IDEOCODE_BUILD_GIT_TAG");
}

/// Workspace root, derived from this crate's manifest dir (`crates/IDEOCODE-build-meta`).
fn repo_root() -> PathBuf {
    let manifest_dir = std::env::var("CARGO_MANIFEST_DIR")
        .map(PathBuf::from)
        .unwrap_or_else(|_| PathBuf::from("."));
    // crates/IDEOCODE-build-meta -> crates -> <repo root>
    manifest_dir
        .parent()
        .and_then(|p| p.parent())
...
fn explicit_build_semver_override() -> Option<String> {
    std::env::var("IDEOCODE_BUILD_SEMVER")
        .ok()
...
fn resolve_build_semver(base_version: (u32, u32, u32)) -> Result<String, String> {
    ...
    // function of HEAD, so the emitted IDEOCODE_SEMVER/IDEOCODE_VERSION only change when
    ...
    // turn invalidated `IDEOCODE-build-meta` and force-recompiled the entire crate
...
fn metadata_value(key: &str) -> Option<String> {
    let path = std::env::var("IDEOCODE_BUILD_METADATA_FILE").ok()?;
