...
fn shell_command_quotes_single_quotes_for_handterm_exec() {
    let command = shell_command(&[
        "/tmp/IDEOCODE binary".to_string(),
        "--resume".to_string(),
        "session'quote".to_string(),
    ]);
    assert_eq!(
        command,
        "'/tmp/IDEOCODE binary' '--resume' 'session'\"'\"'quote'"
    );
}

#[test]
fn resume_invocation_args_includes_socket_when_present() {
    let args = resume_invocation_args("ses_123", Some("/tmp/IDEOCODE-test.sock"));
    assert_eq!(
        args,
        vec![
            ...
            "ses_123".to_string(),
            "--socket".to_string(),
            "/tmp/IDEOCODE-test.sock".to_string()
        ]
    );
}
...
/// Pin IDEOCODE_HOME to a tempdir containing a `builds/current/IDEOCODE` binary so
...
/// other tests mutating IDEOCODE_HOME in parallel. Returns the guards that keep
...
fn pinned_resume_test_home() -> (
    ...
    std::fs::write(current.join("IDEOCODE"), b"#!/bin/sh\n").expect("write fake IDEOCODE binary");
    let home = EnvVarGuard::set_path("IDEOCODE_HOME", temp.path());
    (env_lock, temp, home)
}
...
fn gather_ambient_info_filters_to_session_reminders_when_ambient_disabled() {
    ...
    // Read synchronously while the temporary IDEOCODE_HOME and its files are pinned:
    // an unrelated in-flight cache refresh can otherwise overwrite the cleared
    // process-global cache with data loaded under another test's IDEOCODE_HOME.
...
fn fresh_session_command_includes_fresh_spawn_and_socket() {
    ...
    assert_eq!(command.title.as_deref(), Some("IDEOCODE · new session"));
    assert_eq!(
        command.args,
        vec![
            "--fresh-spawn".to_string(),
