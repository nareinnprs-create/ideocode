...
This document captures the initial direction for a desktop application for IDEOCODE under these constraints:
...
The goal is to make the desktop client a first-class IDEOCODE surface without forking the IDEOCODE runtime or turning the app into a heavyweight IDE clone.
...
Build IDEOCODE Desktop as a small Rust desktop client with a custom GPU-rendered UI. The app should connect to a local IDEOCODE server/daemon that owns sessions, tools, agent execution, persistence, and permissions.
...
| Backend/runtime | Existing Rust IDEOCODE server/session runtime |
| Process model | Desktop frontend + local IDEOCODE daemon/server |
...
IDEOCODE Desktop should not start as a full IDE and should not look like a conventional chatbot.
...
1. Existing IDEOCODE server channel, if compatible with desktop needs.
...
A debug HUD should exist in the prototype before real IDEOCODE integration is considered complete.
...
- connects to local IDEOCODE server/daemon
- lists sessions
...
```text
crates/
  IDEOCODE-desktop-protocol/   # shared protocol/event types if not already covered by server types
  IDEOCODE-desktop-ui/         # UI tree, layout, text/cache abstractions, renderer-agnostic pieces
  IDEOCODE-desktop-renderer/   # wgpu renderer and GPU resources
  IDEOCODE-desktop/            # app shell, platform window, protocol client, product UI
```
...
- small serialization/protocol crates already consistent with IDEOCODE
...
4. Reuse the existing IDEOCODE server protocol or introduce a desktop-specific event protocol crate?
...
Only after that should the real IDEOCODE event stream be connected.
