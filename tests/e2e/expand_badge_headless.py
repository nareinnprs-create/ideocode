...
Run from repo root after building IDEOCODE:
...
def main():
    binary = os.environ.get("IDEOCODE_E2E_BIN")
    if not binary:
        candidates = [
            REPO / "target" / "selfdev" / "IDEOCODE",
            Path.home() / ".IDEOCODE" / "builds" / "current" / "IDEOCODE",
        ]
        binary = next((str(p) for p in candidates if p.exists()), None)
    if not binary:
        raise SystemExit("No IDEOCODE binary found. Set IDEOCODE_E2E_BIN or build first.")
    ...
    try:
        ...
        env.setdefault("IDEOCODE_CLIENT_SELFDEV_MODE", "1")
