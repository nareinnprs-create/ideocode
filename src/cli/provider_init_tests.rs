...
fn test_provider_choice_arg_values() {
    assert_eq!(ProviderChoice::IDEOCODE.as_arg_value(), "IDEOCODE");
...
async fn explicit_anthropic_api_choice_pins_api_key_over_available_oauth() {
    ...
    assert_eq!(
        std::env::var("IDEOCODE_RUNTIME_PROVIDER").ok().as_deref(),
        Some("claude-api")
    );
...
fn test_init_provider_ideocode_delegates_runtime_profile_to_wrapper() {
    ...
    // (e.g. a pinned anthropic api-key route) re-pins IDEOCODE_RUNTIME_PROVIDER
    // during MultiProvider construction and breaks the assertions below.
    let dir = TempDir::new().expect("temp dir");
    let saved_home = std::env::var("IDEOCODE_HOME").ok();
    crate::env::set_var("IDEOCODE_HOME", dir.path());
    crate::subscription_catalog::clear_runtime_env();
    crate::env::remove_var("IDEOCODE_OPENROUTER_MODEL");
    ...
    let provider = runtime
        .block_on(init_provider(&ProviderChoice::IDEOCODE, None))
        .expect("init IDEOCODE provider");

    assert_eq!(provider.name(), "IDEOCODE Subscription");
    ...
    assert_eq!(
        std::env::var("IDEOCODE_ACTIVE_PROVIDER").ok().as_deref(),
        Some("openrouter")
    );
    assert_eq!(
        std::env::var("IDEOCODE_RUNTIME_PROVIDER").ok().as_deref(),
    ...
    assert_eq!(
        std::env::var("IDEOCODE_INITIAL_PROVIDER_EXPLICIT")
            .ok()
            .as_deref(),
        Some("1")
    );
    ...
    crate::env::remove_var("IDEOCODE_INITIAL_PROVIDER_EXPLICIT");
    match saved_home {
        Some(home) => crate::env::set_var("IDEOCODE_HOME", home),
...
fn choice_for_login_provider_round_trips_core_targets() {
    assert_eq!(
        choice_for_login_provider(provider_catalog::IDEOCODE_LOGIN_PROVIDER),
        Some(ProviderChoice::IDEOCODE)
    );
    assert_eq!(
...
fn apply_login_provider_profile_env_preserves_compatible_profile_for_auto_spawn() {
    ...
    assert_eq!(
        std::env::var("IDEOCODE_PROVIDER_PROFILE_ACTIVE")
            .ok()
            .as_deref(),
        Some("1")
    );
...
async fn auto_provider_uses_config_default_named_no_auth_provider() {
    ...
    let saved: Vec<(String, Option<String>)> = [
        ...
        "IDEOCODE_OPENROUTER_ALLOW_NO_AUTH",
        "IDEOCODE_OPENROUTER_ENV_FILE",
        "IDEOCODE_OPENROUTER_DEFAULT_MODEL",
        "IDEOCODE_OPENROUTER_CACHE_NAMESPACE",
        "IDEOCODE_PROVIDER_PROFILE_ACTIVE",
        "IDEOCODE_PROVIDER_PROFILE_NAME",
        "IDEOCODE_NAMED_PROVIDER_PROFILE",
    ...
    crate::env::set_var("IDEOCODE_HOME", dir.path());
    crate::env::set_var("IDEOCODE_NON_INTERACTIVE", "1");
    for key in [
        "ANTHROPIC_API_KEY",
        "OPENAI_API_KEY",
        "OPENROUTER_API_KEY",
...
async fn auto_provider_noninteractive_skips_untrusted_external_auth_instead_of_blocking() {
    ...
    let saved: Vec<(String, Option<String>)> = [
        "IDEOCODE_HOME",
        "IDEOCODE_NON_INTERACTIVE",
        "IDEOCODE_DEFERRED_AUTH_BOOTSTRAP",
        "ANTHROPIC_API_KEY",
        "OPENAI_API_KEY",
        "OPENROUTER_API_KEY",
        "GITHUB_TOKEN",
