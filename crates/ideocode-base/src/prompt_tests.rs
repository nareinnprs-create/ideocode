...
fn test_skill_prompt_integration() {
    ...
    assert!(prompt.contains("Your name is IDEOCODE."));
...
fn test_session_context_includes_time_timezone_and_system_info() {
    ...
    assert!(context.contains("IDEOCODE version: "));
...
fn test_prompt_overlay_files_are_loaded_from_project_and_global_IDEOCODE_dirs() {
    ...
    let project_dir = tempfile::TempDir::new().unwrap();
    std::fs::create_dir_all(project_dir.path().join(".IDEOCODE")).unwrap();
    std::fs::write(
        project_dir.path().join(".IDEOCODE/prompt-overlay.md"),
...
fn test_preferred_tools_files_are_loaded_from_project_and_global_IDEOCODE_dirs() {
    ...
    std::fs::create_dir_all(project_dir.path().join(".IDEOCODE")).unwrap();
    std::fs::write(
        project_dir.path().join(".IDEOCODE/preferred-tools.md"),
    ...
    let direct_content = direct.0.unwrap();
    assert!(
        direct_content.contains("Project Preferred Tools (.IDEOCODE/preferred-tools.md)"),
    ...
    assert!(
        direct_content.contains("Global Preferred Tools (~/.IDEOCODE/preferred-tools.md)"),
...
fn test_non_selfdev_prompt_leaves_selfdev_guidance_to_the_tool_schema() {
    ...
    assert!(!prompt.contains("You are working on the IDEOCODE codebase itself."));
}
...
fn test_selfdev_prompt_uses_full_selfdev_instructions() {
    ...
    assert!(prompt.contains("You are working on the IDEOCODE codebase itself."));
    assert!(prompt.contains("launched from the TUI/root IDEOCODE context"));
...
fn test_selfdev_prompt_uses_desktop_focus_for_desktop_working_dir() {
    let desktop_dir = std::path::Path::new("/tmp/IDEOCODE/crates/IDEOCODE-desktop/src");
...
fn test_split_selfdev_prompt_defaults_to_tui_focus_for_repo_root() {
    let repo_dir = std::path::Path::new("/tmp/IDEOCODE");
    let (split, _info) = build_system_prompt_split(None, &[], true, None, Some(repo_dir));
    assert!(
        split
            .static_part
            .contains("launched from the TUI/root IDEOCODE context")
    );
