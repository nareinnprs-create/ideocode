...
fn macos_launcher_script_shows_alerts_and_uses_terminal_launcher() {
    let script = macos_launcher_script(
        ...
        "/tmp/IDEOCODE",
        Path::new("/Users/test/Applications/IDEOCODE.app"),
    );
    assert!(script.contains("display alert \"IDEOCODE launch failed\""));
    assert!(script.contains("IDEOCODE setup-launcher"));
