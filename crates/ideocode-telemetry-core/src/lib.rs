...
const TELEMETRY_ENDPOINT: &str = "https://telemetry.IDEOCODE.sh/v1/event";
...
const DEFAULT_DISCOVERY_ENDPOINT: &str = "https://api.IDEOCODE.sh/v1/discovery";
...
pub fn is_enabled() -> bool {
    if std::env::var("IDEOCODE_NO_TELEMETRY").is_ok() || std::env::var("DO_NOT_TRACK").is_ok() {
...
/// Its presence is equivalent to setting `IDEOCODE_NO_TELEMETRY=1`, but persists
...
pub fn opt_out_forced_by_env() -> bool {
    std::env::var("IDEOCODE_NO_TELEMETRY").is_ok() || std::env::var("DO_NOT_TRACK").is_ok()
}
    ...
    F: FnMut(Value) + Send + 'static,
{
    ...
    std::thread::Builder::new()
        .name("IDEOCODE-telemetry".to_string())
        .spawn(move || {
...
pub fn record_install_if_first_run() {
    ...
    // fresh ~/.IDEOCODE (so a new telemetry_id) on every job, which would otherwise
...
fn show_first_run_notice() {
    ...
    eprintln!("{dim}");
    eprintln!("  IDEOCODE collects anonymous usage statistics (install count, version, OS,");
