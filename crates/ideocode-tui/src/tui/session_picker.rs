...
pub enum PickerResult {
    ...
    /// to IDEOCODE. This is never emitted by ordinary Enter/resume behavior.
...
impl SessionPicker {
    ...
    fn ensure_selected_preview_loading(&mut self) {
        ...
        let _ = std::thread::Builder::new()
            .name("IDEOCODE-session-preview-loader".to_string())
            .spawn(move || {
    ...
    fn render_claude_takeover_confirmation(&self, frame: &mut Frame) {
        ...
        let body = vec![
            ...
            Line::from(""),
            Line::from(Span::styled(
                "IDEOCODE will prepare the transcript first, then ask Claude to exit.",
