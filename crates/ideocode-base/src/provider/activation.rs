...
impl RuntimeProviderId {
    ...
    pub const fn label(self) -> &'static str {
        match self {
            Self::IDEOCODE => "IDEOCODE Subscription",
...
impl ProviderActivation {
    ...
    pub fn azure_openai(model: Option<String>) -> Self {
        ...
        if let Some(model) = model.filter(|value| !value.trim().is_empty()) {
            activation.with_model_hint("IDEOCODE_OPENROUTER_MODEL", model)
        } else {
            activation
        }
    }
    ...
    pub fn apply_env(&self) -> Result<()> {
        ...
        match self.runtime_id {
            ...
            RuntimeProviderId::OpenAiCompatible => {
                if std::env::var_os("IDEOCODE_OPENROUTER_TRANSPORT_STATE").is_none() {
                    crate::env::set_var("IDEOCODE_OPENROUTER_TRANSPORT_STATE", "direct-api-key");
                }
            }
            _ => {
        ...
        match self.selection {
            RuntimeSelection::Initial(active_provider) => {
                ...
                crate::env::set_var("IDEOCODE_ACTIVE_PROVIDER", active_key_for_log);
                crate::env::set_var("IDEOCODE_INITIAL_PROVIDER_EXPLICIT", "1");
            }
...
pub fn select_initial_runtime_provider_key(provider_key_raw: &str) {
    crate::env::set_var("IDEOCODE_ACTIVE_PROVIDER", provider_key_raw);
    crate::env::set_var("IDEOCODE_INITIAL_PROVIDER_EXPLICIT", "1");
    crate::logging::auth_event(
...
mod tests {
    ...
    fn azure_activation_preserves_identity_while_using_openrouter_slot() {
        ...
        // (e.g. anthropic_tests sets IDEOCODE_RUNTIME_PROVIDER=claude); without
        ...
        assert_eq!(
            std::env::var("IDEOCODE_OPENROUTER_MODEL").as_deref(),
            Ok("gpt-4.1-mini")
        );
    }
}
