...
1. What is using the IDEOCODE server's memory?
...
```bash
IDEOCODE debug 'server:memory-incident'
```
...
```bash
python scripts/analyze_runtime_memory_log.py --days 1 --json > /tmp/IDEOCODE-memory-analysis.json
```
...
2. Run `IDEOCODE debug 'swarm:list'` and inspect the largest live swarm.
...
```bash
IDEOCODE debug 'server:memory-incident' > /tmp/before.json
IDEOCODE debug 'allocator:purge'
IDEOCODE debug 'server:memory-incident' > /tmp/after.json
```
...
1. Run `IDEOCODE debug 'server:memory'` for the full attribution walk.
