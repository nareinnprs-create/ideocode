...
fn bench_root() -> PathBuf {
    std::env::var("MEMORY_BENCH_DIR")
        .map(PathBuf::from)
        .unwrap_or_else(|_| dirs_home().join("IDEOCODE-memory-bench"))
}
...
fn cmd_queries(args: &[String]) -> Result<()> {
    ...
    let sessions_dir = opts
        .get("sessions")
        .cloned()
        .unwrap_or_else(|| format!("{}/.IDEOCODE/sessions", dirs_home().display()));
...
// ---------------- LLM judge (direct Anthropic via IDEOCODE Sidecar) ----------------
...
fn cmd_metrics(args: &[String]) -> Result<()> {
    ...
    // For `prod_hybrid`: seed a temp IDEOCODE_HOME project graph with the corpus and
