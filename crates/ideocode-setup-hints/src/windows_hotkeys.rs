...
//! low-level keyboard hook in the first-party Rust listener (`IDEOCODE setup-hotkey
...
/// Returns `None` for keys with no stable VK code. IDEOCODE `cmd` maps to Alt
...
/// Translate a canonical IDEOCODE key token into a Win32 virtual-key code.
...
/// User-facing chord rendering for Windows notices: IDEOCODE's `cmd` modifier
...
fn ps_dir_expr(dir: &str, last_dir_file: &str, last_repo_file: &str) -> String {
    match dir {
        "$HOME" => "(Resolve-IDEOCODEDir $null)".to_string(),
        "$LAST_DIR" => format!("(Resolve-IDEOCODEDir {})", ps_quote(last_dir_file)),
        "$LAST_REPO" => format!("(Resolve-IDEOCODEDir {})", ps_quote(last_repo_file)),
        path => format!("(Resolve-IDEOCODEFixedDir {})", ps_quote(path)),
    }
}

/// Render the full hotkey-listener PowerShell script.
///
/// `launch_exe`/`launch_args` describe the terminal command that opens IDEOCODE
/// (e.g. `wt.exe` + profile args, or `alacritty -e IDEOCODE`); `{DIR}` never
...
pub(crate) fn render_windows_listener_ps1(
    ...
    for (index, entry) in entries.iter().enumerate() {
        ...
        registrations.push_str(&format!(
            "Register-IDEOCODEHotkey -Id 0x{id:X} -Mods 0x{mods:X} -Vk 0x{vk:X} -Label {label}\n",
    ...
    Some(format!(
        r#"# IDEOCODE global launch hotkey listener
# Auto-generated by IDEOCODE setup-hotkey from [launch_hotkeys] config. Runs at
# login via a startup shortcut. Do not edit; re-run `IDEOCODE setup-hotkey`.
...
function Register-IDEOCODEHotkey {{
    ...
    if ([HotKeyHelper]::RegisterHotKey([IntPtr]::Zero, $Id, $Mods -bor $MOD_NOREPEAT, $Vk)) {{
        ...
        Write-Warning "IDEOCODE: failed to register hotkey $Label (already claimed?)"
    }}
}}

# Resolve a dynamic launch dir from a tracking file, falling back to $HOME.
function Resolve-IDEOCODEDir {{
    param([string]$File)
    if ($File) {{
        try {{
...
function Resolve-IDEOCODEFixedDir {{
    param([string]$Dir)
    if ($Dir -and (Test-Path -LiteralPath $Dir -PathType Container)) {{ return $Dir }}
    return $env:USERPROFILE
}}
...
if ($script:RegisteredIds.Count -eq 0) {{
    Write-Error "IDEOCODE: no launch hotkeys could be registered"
    exit 1
}}
...
mod tests {
    ...
    fn args_for(entry: &WindowsHotkey) -> String {
        ...
        if entry.self_dev {
            format!(r#"-e "C:\IDEOCODE.exe" --spawn-hotkey "{hotkey}" self-dev"#)
        } else {
            format!(r#"-e "C:\IDEOCODE.exe" --spawn-hotkey "{hotkey}""#)
        }
    }
    ...
    fn listener_script_registers_each_entry_and_dispatches_dirs() {
        let entries = vec![
            hk("win+shift+f23", "C:\\Users\\u\\IDEOCODE", "IDEOCODE", false),
        ...
        let script = render_windows_listener_ps1(
            ...
            "wt.exe",
            args_for,
            "C:\\Users\\u\\.IDEOCODE\\hotkey\\last_dir",
            "C:\\Users\\u\\.IDEOCODE\\hotkey\\last_repo",
        )
        .unwrap();

        // Three registrations with distinct ids.
        assert_eq!(script.matches("Register-IDEOCODEHotkey").count(), 3 + 1); // 3 calls + fn def
