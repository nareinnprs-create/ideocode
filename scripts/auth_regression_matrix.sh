...
bin=${IDEOCODE_AUTH_MATRIX_BIN:-}
out_dir=${IDEOCODE_AUTH_MATRIX_OUT:-"$repo_root/target/auth-test-reports"}
prompt=${IDEOCODE_AUTH_MATRIX_PROMPT:-"Reply with exactly AUTH_TEST_OK and nothing else. Do not call tools."}
providers=${IDEOCODE_AUTH_MATRIX_PROVIDERS:-"claude copilot openrouter deepseek zai alibaba-coding-plan openai-compatible"}
mode=${IDEOCODE_AUTH_MATRIX_MODE:-configured}
keep_going=${IDEOCODE_AUTH_MATRIX_KEEP_GOING:-1}
per_command_timeout=${IDEOCODE_AUTH_MATRIX_TIMEOUT:-90}

usage() {
  cat <<'EOF'
Usage: scripts/auth_regression_matrix.sh [options]

Runs IDEOCODE auth-test across the auth/provider matrix and writes one JSON report per provider.
...
Environment equivalents:
  IDEOCODE_AUTH_MATRIX_BIN=/path/to/IDEOCODE
  IDEOCODE_AUTH_MATRIX_OUT=target/auth-test-reports
  IDEOCODE_AUTH_MATRIX_PROVIDERS="claude deepseek zai"
  IDEOCODE_AUTH_MATRIX_MODE=configured|all
  IDEOCODE_AUTH_MATRIX_LOGIN=1
  IDEOCODE_AUTH_MATRIX_NO_SMOKE=1
  IDEOCODE_AUTH_MATRIX_NO_TOOL_SMOKE=1
  IDEOCODE_AUTH_MATRIX_KEEP_GOING=0
  IDEOCODE_AUTH_MATRIX_TIMEOUT=90
...
if [[ "${IDEOCODE_AUTH_MATRIX_LOGIN:-0}" == "1" ]]; then
  extra_args+=(--login)
fi
if [[ "${IDEOCODE_AUTH_MATRIX_NO_SMOKE:-0}" == "1" ]]; then
  extra_args+=(--no-smoke)
fi
if [[ "${IDEOCODE_AUTH_MATRIX_NO_TOOL_SMOKE:-0}" == "1" ]]; then
  extra_args+=(--no-tool-smoke)
fi
...
if [[ "$mode" == "configured" ]]; then
  ...
  if ! run_IDEOCODE auth-test --all-configured --no-smoke --no-tool-smoke --json --output "$configured_json" >/tmp/IDEOCODE-auth-matrix-discovery.out 2>/tmp/IDEOCODE-auth-matrix-discovery.err; then
    if [[ -s "$configured_json" ]]; then
      ...
      cat /tmp/IDEOCODE-auth-matrix-discovery.err >&2 || true
