...
/// Identifies a session to resume, across the agent backends IDEOCODE can import
/// from. This is pure data (only ids/paths) with no UI dependency; it lives in
/// `IDEOCODE-session-types` so the foundation/import layer can match on it without
/// depending on any `IDEOCODE-tui-*` crate. The session-picker UI re-exports it.
...
impl ResumeTarget {
    pub fn stable_id(&self) -> &str {
        match self {
            Self::IDEOCODESession { session_id } => session_id,
