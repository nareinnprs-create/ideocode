...
//! When one of those overlaps a key IDEOCODE wants (the classic case is a tiling WM
//! binding `Cmd+J`/`Cmd+K` to window focus, which shadows IDEOCODE's prompt
//! navigation), the keystroke never reaches the terminal and the IDEOCODE binding
...
fn apply_modifier(token: &str, mods: &mut Mods, hyper_mods: Mods) -> bool {
    match token.trim().to_ascii_lowercase().as_str() {
        ...
        // "fn" is not representable as a IDEOCODE modifier; ignore it so the rest of
