...
fn swarm_status_debounce_member_threshold() -> usize {
    ...
    CACHED
        .get_or_init(|| {
            let configured = std::env::var("IDEOCODE_SWARM_STATUS_DEBOUNCE_MEMBER_THRESHOLD")
                .ok()
...
fn swarm_status_debounce_ms() -> u64 {
    ...
    CACHED
        .get_or_init(|| {
            let configured = std::env::var("IDEOCODE_SWARM_STATUS_DEBOUNCE_MS")
                .ok()
...
pub(super) fn swarm_task_heartbeat_interval() -> Duration {
    Duration::from_secs(configured_positive_u64(
        "IDEOCODE_SWARM_TASK_HEARTBEAT_SECS",
...
pub(super) fn swarm_task_stale_after() -> Duration {
    Duration::from_secs(configured_positive_u64(
        "IDEOCODE_SWARM_TASK_STALE_AFTER_SECS",
...
pub(super) fn swarm_task_sweep_interval() -> Duration {
    Duration::from_secs(configured_positive_u64(
        "IDEOCODE_SWARM_TASK_SWEEP_INTERVAL_SECS",
...
pub(super) fn swarm_terminal_member_retention() -> Duration {
    Duration::from_secs(configured_positive_u64(
        "IDEOCODE_SWARM_TERMINAL_MEMBER_RETENTION_SECS",
...
pub(super) fn swarm_terminal_member_gc_interval() -> Duration {
    Duration::from_secs(configured_positive_u64(
        "IDEOCODE_SWARM_TERMINAL_MEMBER_GC_INTERVAL_SECS",
...
pub(super) fn swarm_status_broadcast_terminal_retention() -> Duration {
    Duration::from_secs(configured_positive_u64(
        "IDEOCODE_SWARM_STATUS_BROADCAST_TERMINAL_SECS",
...
pub(super) fn swarm_idle_worker_reap_after() -> Option<Duration> {
    let secs = std::env::var("IDEOCODE_SWARM_IDLE_WORKER_REAP_SECS")
        .ok()
        .and_then(|value| value.trim().parse::<u64>().ok())
...
mod tests {
    ...
    fn idle_worker_reap_window_env_zero_disables() {
        ...
        // it.
        unsafe {
            std::env::set_var("IDEOCODE_SWARM_IDLE_WORKER_REAP_SECS", "0");
        }
        assert_eq!(super::swarm_idle_worker_reap_after(), None);
        unsafe {
            std::env::set_var("IDEOCODE_SWARM_IDLE_WORKER_REAP_SECS", "90");
        }
        assert_eq!(
        ...
        unsafe {
            std::env::remove_var("IDEOCODE_SWARM_IDLE_WORKER_REAP_SECS");
        }
    ...
    /// sending. Swarms below `IDEOCODE_SWARM_STATUS_DEBOUNCE_MEMBER_THRESHOLD`
