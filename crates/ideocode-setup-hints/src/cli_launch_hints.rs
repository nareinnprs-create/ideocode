...
//! intercepting shell commands. The hook invokes a hidden, fast IDEOCODE command;
...
pub(super) fn maybe_notify(source: &str) -> Result<()> {
    ...
    *shown = shown.saturating_add(1);
    ...
    let body = format!(
        "{} is open. Press {} anytime to launch IDEOCODE.",
        source.label(),
        display
    );
    send_desktop_notification("IDEOCODE shortcut", &body);
    Ok(())
}
...
fn trusted_ideocode_executable() -> Result<PathBuf> {
    ...
    #[cfg(not(windows))]
    {
        if let Some(home) = dirs::home_dir() {
            let launcher = home.join(".local").join("bin").join("IDEOCODE");
            if launcher.is_file() {
                return Ok(launcher);
            }
        }
    ...
    std::env::current_exe().context("resolving the IDEOCODE executable for lifecycle hooks")
}
    ...
    #[cfg(windows)]
    {
        ...
        let script = format!(
            "$title=\"{}\";$body=\"{}\";\
             ...
             [Windows.UI.Notifications.ToastNotificationManager]::CreateToastNotifier('IDEOCODE').Show([Windows.UI.Notifications.ToastNotification]::new($xml))",
            ps_quote(title),
            ps_quote(body)
        );
...
mod tests {
    ...
    fn inserts_session_start_hook_without_replacing_existing_hooks() {
        ...
        let command = "'/trusted/IDEOCODE' setup-hotkey --notify-cli-launch claude";
    ...
    fn managed_hook_update_is_idempotent() {
        let mut value = json!({});
        let command = "'/trusted/IDEOCODE' setup-hotkey --notify-cli-launch codex";
        assert!(upsert_hook(&mut value, command).unwrap());
        assert!(!upsert_hook(&mut value, command).unwrap());
            ...
            "hooks": {
                "SessionStart": [{
                    ...
                    "hooks": [
                        {
                            "type": "command",
                            "command": "IDEOCODE setup-hotkey --notify-cli-launch old",
                            "timeout": 30
                        },
    ...
    fn rejects_malformed_hook_shape_instead_of_clobbering_it() {
        ...
        assert!(
            upsert_hook(
                &mut value,
                "'/trusted/IDEOCODE' setup-hotkey --notify-cli-launch claude"
            )
            .is_err()
        );
    }
...
"#,
        ...
        assert!(!command.starts_with("IDEOCODE "));
    ...
    fn unix_hook_executable_quoting_handles_spaces_and_single_quotes() {
        assert_eq!(
            quote_hook_executable(Path::new("/tmp/IDEOCODE's bin/IDEOCODE")),
            "'/tmp/IDEOCODE'\\''s bin/IDEOCODE'"
        );
    }
}
