...
impl App {
    ...
    pub(super) fn maybe_begin_onboarding_flow_on_startup(&mut self) {
        ...
        // spawned by developers (e.g. the niri `IDEOCODE self-dev` hotkey) and that
    ...
    /// only counts interactive `IDEOCODE` launches (TTY-gated) and can be reset
    ...
    ///
    /// These are launched by developers working on IDEOCODE itself (for example the
    /// niri `IDEOCODE self-dev` hotkey). That launch path bypasses
    ...
    pub(super) fn begin_onboarding_flow_at_login(&mut self) {
        ...
        if had_imports {
            self.set_status_notice(
                "Welcome to IDEOCODE: review detected logins (arrows/hl to move, Enter to choose)",
            );
        } else {
            self.set_status_notice(
    ...
    fn onboarding_finish_import_review(&mut self) {
        ...
        // target the agent repair brief at the right `IDEOCODE auth-test --provider`.
    ...
    /// with a large `~/.IDEOCODE/sessions` directory. Doing it inline on Enter made
    ...
    pub(super) fn onboarding_prepare_recent_project_review(&mut self) -> bool {
        let Some(repository) = self.onboarding_recent_project_path() else {
            self.onboarding_show_suggestions();
            self.set_status_notice(
                "No recent Git repository found. Start IDEOCODE inside a project to review it.",
            );
            return false;
        };
        self.onboarding_finish();
    ...
    /// 1. A IDEOCODE login that exists but expired (they clearly use it).
    ...
    fn onboarding_other_provider_rows(skip: Option<&str>) -> (Vec<String>, Vec<String>) {
        ...
        let providers: [(&str, &str, AuthState); 8] = [
            ...
            ("IDEOCODE subscription", "IDEOCODE", status.IDEOCODE),
    ...
    /// commands the agent can run (`IDEOCODE auth-test --json`, `IDEOCODE login
    /// --api-key-stdin`, `IDEOCODE provider add`), copy it to the clipboard, and
