...
pub(crate) fn debug_control_allowed() -> bool {
    ...
    if std::env::var("IDEOCODE_DEBUG_CONTROL")
        .ok()
...
pub(crate) fn embedding_idle_unload_secs() -> u64 {
    parse_embedding_idle_unload_secs(
        std::env::var("IDEOCODE_EMBEDDING_IDLE_UNLOAD_SECS")
            .ok()
            .as_deref(),
    )
}
...
pub(crate) fn swarm_id_for_dir(dir: Option<PathBuf>) -> Option<String> {
    if let Ok(sw_id) = std::env::var("IDEOCODE_SWARM_ID") {
...
pub(crate) fn startup_headless_recovery_test_delay() -> Option<std::time::Duration> {
    let raw = std::env::var("IDEOCODE_TEST_HEADLESS_STARTUP_RECOVERY_DELAY_MS").ok()?;
...
mod newer_binary_tests {
    ...
    fn reports_update_when_candidate_is_strictly_newer() {
        let candidates = vec![(PathBuf::from("/x/stable/IDEOCODE"), Some(t(200)))];
        assert!(newer_binary_available(
            Some(t(100)),
            Some(std::path::Path::new("/x/current/IDEOCODE")),
            candidates,
        ));
    }
    ...
    fn ignores_candidate_that_is_not_newer() {
        let candidates = vec![(PathBuf::from("/x/stable/IDEOCODE"), Some(t(100)))];
        assert!(!newer_binary_available(
            Some(t(100)),
            Some(std::path::Path::new("/x/current/IDEOCODE")),
            candidates,
    ...
    fn never_reloads_into_self_even_if_paths_were_equal() {
        ...
        let candidates = vec![(PathBuf::from("/x/current/IDEOCODE"), Some(t(999)))];
        assert!(!newer_binary_available(
            Some(t(100)),
            Some(std::path::Path::new("/x/current/IDEOCODE")),
            candidates,
    ...
    fn suppresses_update_when_candidate_mtime_unavailable() {
        ...
        let candidates = vec![(PathBuf::from("/x/stable/IDEOCODE"), None)];
        assert!(!newer_binary_available(
            Some(t(100)),
            Some(std::path::Path::new("/x/current/IDEOCODE")),
            candidates,
    ...
    fn reports_update_if_any_candidate_is_newer() {
        let candidates = vec![
            (PathBuf::from("/x/stable/IDEOCODE"), None),
            (PathBuf::from("/x/shared/IDEOCODE"), Some(t(300))),
        ];
        assert!(newer_binary_available(
            Some(t(100)),
            Some(std::path::Path::new("/x/current/IDEOCODE")),
    ...
    fn newer_server_is_not_outdated_by_older_channel_binary() {
        ...
        assert!(!newer_binary_available(
            Some(t(300)),
            Some(std::path::Path::new("/x/builds/versions/dev/IDEOCODE")),
            candidates,
        ));
    }
...
mod reload_target_tests {
    ...
    fn same_binary_is_always_used() {
        ...
        let decision = guarded_reload_target(
            candidate("/x/current/IDEOCODE"),
            Path::new("/x/current/IDEOCODE"),
            Some(Path::new("/x/current/IDEOCODE")),
            Some(Path::new("/x/current/IDEOCODE")),
            Some(t(200)),
            Some(t(100)),
        );
    ...
    fn newer_candidate_is_used() {
        ...
        let decision = guarded_reload_target(
            candidate("/x/shared-server/IDEOCODE"),
            Path::new("/x/builds/versions/new/IDEOCODE"),
            Some(Path::new("/x/builds/versions/old/IDEOCODE")),
            Some(Path::new("/x/builds/versions/old/IDEOCODE")),
            Some(t(100)),
            Some(t(200)),
        );
        match decision {
            ReloadTargetDecision::UseCandidate((path, _)) => {
                assert_eq!(path, PathBuf::from("/x/shared-server/IDEOCODE"));
            }
    ...
    fn equal_mtime_candidate_is_used() {
        ...
        let decision = guarded_reload_target(
            candidate("/x/shared-server/IDEOCODE"),
            Path::new("/x/builds/versions/same/IDEOCODE"),
            Some(Path::new("/x/builds/versions/current/IDEOCODE")),
            Some(Path::new("/x/builds/versions/current/IDEOCODE")),
            Some(t(100)),
            Some(t(100)),
        );
    ...
    fn strictly_older_candidate_is_blocked_and_uses_current_exe() {
        ...
        let decision = guarded_reload_target(
            candidate("/x/shared-server/IDEOCODE"),
            Path::new("/x/builds/versions/old-0.14.3/IDEOCODE"),
            Some(Path::new("/x/builds/versions/new/IDEOCODE")),
            Some(Path::new("/x/builds/versions/new/IDEOCODE")),
            Some(t(300)),
            Some(t(100)),
        );
        match decision {
            ReloadTargetDecision::DowngradeBlockedUseCurrent((path, _)) => {
                assert_eq!(path, PathBuf::from("/x/builds/versions/new/IDEOCODE"));
            }
    ...
    fn unreadable_candidate_mtime_is_treated_as_downgrade() {
        let decision = guarded_reload_target(
            candidate("/x/shared-server/IDEOCODE"),
            Path::new("/x/builds/versions/unknown/IDEOCODE"),
            Some(Path::new("/x/builds/versions/new/IDEOCODE")),
            Some(Path::new("/x/builds/versions/new/IDEOCODE")),
            Some(t(300)),
            None,
    ...
    fn downgrade_without_current_exe_falls_back_to_candidate() {
        ...
        let decision = guarded_reload_target(
            candidate("/x/shared-server/IDEOCODE"),
            Path::new("/x/builds/versions/old/IDEOCODE"),
            None,
            None,
            None,
            Some(t(100)),
...
mod pick_newest_candidate_tests {
    ...
    fn other_flavor_wins_when_strictly_newer() {
        ...
        let chosen = pick_newest_candidate([
            entry(
                "/x/versions/old-selfdev/IDEOCODE",
                "shared-server",
                Some(t(100)),
            ),
            entry("/x/versions/new-release/IDEOCODE", "stable", Some(t(200))),
        ])
        .expect("a candidate");
        assert_eq!(chosen.0, PathBuf::from("/x/versions/new-release/IDEOCODE"));
    }
    ...
    fn own_flavor_wins_on_tie() {
        ...
        .expect("a candidate");
        assert_eq!(chosen.0, PathBuf::from("/x/versions/selfdev/IDEOCODE"));
    }
    ...
    fn own_flavor_wins_when_strictly_newer() {
        let chosen = pick_newest_candidate([
            entry(
                "/x/versions/fresh-selfdev/IDEOCODE",
                "shared-server",
                Some(t(300)),
            ),
            entry("/x/versions/release/IDEOCODE", "stable", Some(t(200))),
    ...
    fn duplicate_canonical_paths_collapse() {
        ...
        let chosen = pick_newest_candidate([
            entry("/x/versions/same/IDEOCODE", "shared-server", Some(t(100))),
            entry("/x/versions/same/IDEOCODE", "stable", Some(t(999))),
        ])
        .expect("a candidate");
...
mod newest_reload_candidate_integration_tests {
    ...
    /// Install a release-archive-style version dir: a tiny `IDEOCODE` wrapper
    /// script plus the real `IDEOCODE-linux-x86_64.bin` payload, with independently
...
mod deleted_suffix_tests {
    ...
    fn strips_linux_deleted_marker() {
        let p = PathBuf::from("/home/u/.IDEOCODE/builds/versions/abc/IDEOCODE (deleted)");
        assert_eq!(
            strip_deleted_suffix(p),
            PathBuf::from("/home/u/.IDEOCODE/builds/versions/abc/IDEOCODE")
        );
    }

    #[test]
    fn leaves_normal_paths_untouched() {
        let p = PathBuf::from("/home/u/.IDEOCODE/builds/versions/abc/IDEOCODE");
    ...
    fn only_strips_trailing_marker() {
        ...
        let p = PathBuf::from("/home/u/IDEOCODE (deleted)/IDEOCODE");
        assert_eq!(strip_deleted_suffix(p.clone()), p);
    }
}
