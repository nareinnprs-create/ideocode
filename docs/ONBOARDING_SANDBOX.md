...
- `IDEOCODE_HOME` redirects IDEOCODE-owned state such as `~/.IDEOCODE` into a sandbox directory.
- `IDEOCODE_HOME` also redirects app config into `IDEOCODE_HOME/config/IDEOCODE`.
- `IDEOCODE_RUNTIME_DIR` redirects sockets and other ephemeral runtime files.
...
That gives you a clean IDEOCODE launch with isolated state.
...
# Or do it all in one shot: reset, seed, and launch IDEOCODE
scripts/onboarding_sandbox.sh fresh-real --with-transcripts
```

How it works: when `IDEOCODE_HOME` is set, IDEOCODE resolves every external credential
and transcript lookup to `$IDEOCODE_HOME/external/<same-relative-path-as-$HOME>`.
...
```bash
scripts/onboarding_sandbox.sh IDEOCODE
```
...
# Log into a provider without touching your normal IDEOCODE config
...
# Run arbitrary IDEOCODE commands in the sandbox
scripts/onboarding_sandbox.sh IDEOCODE auth status
scripts/onboarding_sandbox.sh IDEOCODE pair
```

## Reusable local auth fixtures

For repeated login testing, use local auth fixtures. A fixture is a copy of a
sandbox `IDEOCODE_HOME` after you have put it into an interesting state, for
...
scripts/onboarding_sandbox.sh fixture-load normal-openai
scripts/onboarding_sandbox.sh auth-status
scripts/onboarding_sandbox.sh IDEOCODE auth-test --provider openai
```
...
- `IDEOCODE_ONBOARDING_SANDBOX`: select which sandbox receives the fixture.
- `IDEOCODE_ONBOARDING_DIR`: use an explicit sandbox directory.
- `IDEOCODE_AUTH_FIXTURE_DIR`: use a fixture store outside the repo, for example
  `~/.local/share/IDEOCODE-auth-fixtures`.
...
- no real IDEOCODE config files are reused
...
When using fixtures, the sandbox is still isolated from your normal IDEOCODE state,
...
This sandbox is designed to isolate IDEOCODE-owned state and trusted external-import state. If you later decide to test explicit import/reuse flows from external tools, do that intentionally and treat it as a separate test case from first-run onboarding.
