...
_DATE_RE = re.compile(r"IDEOCODE-(\d{4}-\d{2}-\d{2})\.log$")
...
def _select_log_files(log_dir: str, days: int) -> list[tuple[str, str]]:
    ...
    pattern = os.path.join(log_dir, "IDEOCODE-*.log")
...
def mine_usage(log_dir: str = "~/.IDEOCODE/logs", days: int = 7) -> UsageProfile:
    ...
    if not files:
        return _default_profile(
            f"No IDEOCODE-YYYY-MM-DD.log files under {resolved!r}; using defaults."
        )
