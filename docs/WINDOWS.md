...
| `IDEOCODE update` | Supported with SHA-256 verification |
...
4. Installs immutable, stable, and launcher copies under `%LOCALAPPDATA%\IDEOCODE`.
5. Adds `%LOCALAPPDATA%\IDEOCODE\bin` to the user `PATH`.
...
IDEOCODE can also offer these options interactively after launch.
...
- Launcher: `%LOCALAPPDATA%\IDEOCODE\bin\IDEOCODE.exe`
- Stable binary: `%LOCALAPPDATA%\IDEOCODE\builds\stable\IDEOCODE.exe`
- Versioned binary: `%LOCALAPPDATA%\IDEOCODE\builds\versions\<version>\IDEOCODE.exe`
- User data and configuration: `%USERPROFILE%\.IDEOCODE`

### Verify an installation

```powershell
IDEOCODE --version
Get-Command IDEOCODE
...
```powershell
Get-AuthenticodeSignature (Get-Command IDEOCODE).Source | Format-List Status,StatusMessage,SignerCertificate
```
...
Unix domain sockets are replaced by Windows named pipes under `crates/IDEOCODE-base/src/transport/windows.rs`. Platform-specific filesystem, process, update, and replacement behavior is selected at compile time with `#[cfg(windows)]`, so Windows support does not add runtime branching to Unix builds.

Windows launch-hotkey setup is implemented in `crates/IDEOCODE-setup-hints/src/windows_setup.rs` and is only installed after explicit user consent.
...
- IDEOCODE version from `IDEOCODE --version`
- Installation method
...
Do not upload private configuration, credentials, session transcripts, or `.IDEOCODE` authentication files.
