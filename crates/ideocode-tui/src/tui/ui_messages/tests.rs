...
fn render_compact_launch_and_divergence_notices_as_one_line() {
    ...
    let notices = [
        DisplayMessage::system(
            "Configured IDEOCODE launch hotkeys (niri):\nSuper+; → IDEOCODE (/home/user/project)\n\nBound system-wide.",
        )
        .with_title("Launch hotkeys"),
        DisplayMessage::system(
            "Update diverged. Press Ctrl+Y to let a IDEOCODE agent merge local and upstream (or run `git pull` / `git rebase` yourself).",
        )
        .with_title("Update"),
    ];
...
fn render_assistant_message_plan_card_keeps_nested_fences_inside() {
    ...
    let bash_line = lines
        .iter()
        .map(extract_line_text)
        .find(|line| line.contains("cargo test -p IDEOCODE-tui"))
...
fn render_system_message_uses_connection_card_for_reconnect_status() {
    let msg = DisplayMessage::system(
        "⚡ Connection lost - retrying (attempt 2, 7s) - connection reset by server · resume: IDEOCODE --resume koala",
    )
    .with_title("Connection");
    ...
    assert!(plain.contains("IDEOCODE --resume koala"));
}
...
fn render_swarm_message_centered_mode_caps_wrap_width_for_long_notifications() {
    ...
    let msg = DisplayMessage::swarm(
        "File activity",
        "/home/jeremy/IDEOCODE/src/tui/ui_messages.rs - moss just edited this file while you were working nearby, so the notification should still read as centered in wide layouts.",
    );
...
fn render_tool_message_shows_intent_and_technical_preview_on_one_line() {
    ...
    let msg = DisplayMessage {
        ...
        tool_data: Some(crate::message::ToolCall {
            ...
            name: "bash".to_string(),
            input: serde_json::json!({
                "command": "cargo test -p IDEOCODE render_background_task --lib",
...
fn render_tool_message_shows_discovery_browse_results_and_rationale() {
    ...
    assert!(
        plain.contains("IDEOCODE partners with tool providers to make their tools discoverable"),
        "{plain}"
    );
    assert!(
...
fn batched_discovery_renders_first_use_disclosure_inline_once() {
    ...
    assert_eq!(
        plain
            .matches("IDEOCODE partners with tool providers to make their tools discoverable")
            .count(),
        1,
...
fn render_tool_message_shows_selected_discovery_setup() {
    ...
    assert!(
        !plain.contains("IDEOCODE partners with tool providers"),
...
fn render_tool_message_shows_catalog_suggestion_receipt_and_trust_line() {
    ...
    assert!(plain.contains("IDEOCODE maintainers only"), "{plain}");
...
fn discovery_cards_wrap_within_narrow_transcript_width() {
    let msg = discovery_message(
        "Catalog suggestion submitted.\n\nStatus: received for IDEOCODE maintainer review.",
        serde_json::json!({
            "action": "suggest",
