...
pub(super) fn debug_message_timeout_secs() -> Option<u64> {
    let raw = std::env::var("IDEOCODE_DEBUG_MESSAGE_TIMEOUT_SECS").ok()?;
...
pub(super) async fn execute_debug_command(
    ...
    if trimmed == "reload" {
        let repo_dir = crate::build::get_repo_dir()
            .ok_or_else(|| anyhow::anyhow!("Could not find IDEOCODE repository directory"))?;

        let target_binary = crate::build::find_dev_binary(&repo_dir)
            .unwrap_or_else(|| build::release_binary_path(&repo_dir));
        if !target_binary.exists() {
            return Err(anyhow::anyhow!(format!(
                "No binary found at {}. Run 'IDEOCODE self-dev --build' first, or build with 'scripts/dev_cargo.sh build --profile selfdev -p IDEOCODE --bin IDEOCODE' and publish current.",
...
mod tests {
    ...
    /// which set `IDEOCODE_HOME` under `storage::lock_test_env`). Two mutexes
    ...
    async fn debug_tool_selfdev_reload_returns_promptly_for_direct_execution() {
        ...
        let _test_session = EnvGuard::set("IDEOCODE_TEST_SESSION", "1");
        let _debug_control = EnvGuard::set("IDEOCODE_DEBUG_CONTROL", "1");
