...
pub(super) const SUPPORT_EMAIL: &str = "support@IDEOCODE.sh";
pub(super) const SUPPORT_SUBJECT: &str = "IDEOCODE support";
...
fn gather_diagnostics(app: &App) -> SupportDiagnostics {
    ...
    // Account identity is only present when a IDEOCODE subscription is
    ...
    let (account_id, account_email, tier) = if has_subscription {
        (
            load_env_value_from_env_or_config(cat::IDEOCODE_ACCOUNT_ID_ENV, cat::IDEOCODE_ENV_FILE),
            load_env_value_from_env_or_config(cat::IDEOCODE_ACCOUNT_EMAIL_ENV, cat::IDEOCODE_ENV_FILE),
...
mod tests {
    ...
    fn support_mailto_url_is_correctly_encoded() {
        ...
        assert!(url.starts_with("mailto:support@IDEOCODE.sh?subject=IDEOCODE%20support&body="));
