...
The default uses IDEOCODE's normal toolset so Discovery competes with built-in
...
The runner uses a dedicated IDEOCODE server with:
...
```text
x-IDEOCODE-discovery-benchmark: 1
```
