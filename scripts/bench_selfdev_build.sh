...
#   2. leaf touch        - 1-line change in the root `IDEOCODE` bin crate
#   3. tui touch         - 1-line change in IDEOCODE-tui (largest UI crate)
#   4. core touch        - 1-line change in IDEOCODE-app-core (mid-stack crate)
#   5. commit blast      - simulate HEAD moving (IDEOCODE_BUILD_GIT_HASH change),
#                          which reruns IDEOCODE-build-meta's build script and
...
echo "5. commit blast radius (simulated HEAD move via IDEOCODE_BUILD_GIT_HASH):"
fake_hash="bench$(date +%s | tail -c 4)"
start=$(date +%s.%N)
if IDEOCODE_BUILD_GIT_HASH="$fake_hash" "${build_cmd[@]}" >/tmp/bench_selfdev_build_commit-blast.log 2>&1; then
    end=$(date +%s.%N)
...
echo "    IDEOCODE-build-meta embeds GIT_HASH via env! and IDEOCODE-base/app-core/"
