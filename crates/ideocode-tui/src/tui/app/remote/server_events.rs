...
fn allow_runtime_identity_mismatch() -> bool {
    std::env::var_os("IDEOCODE_ALLOW_SERVER_VERSION_MISMATCH").is_some()
}

/// Parse a IDEOCODE version string into an orderable `(major, minor, patch)`, but
...
fn client_release_version() -> String {
    if (cfg!(test) || cfg!(debug_assertions))
        && let Some(v) = std::env::var_os("IDEOCODE_TEST_CLIENT_VERSION_OVERRIDE")
    {
        ...
        ServerEvent::History {
            ...
            if should_defer_history_for_runtime_identity(
                ...
                if client_detected_stale {
                    ...
                    app.push_display_message(DisplayMessage::system(
                        "ℹ Connected server binary differs from the installed client channel. Reloading the server before applying remote session state. Set IDEOCODE_ALLOW_SERVER_VERSION_MISMATCH=1 only for intentional compatibility testing."
                            .to_string(),
                    ));
                }
                app.update_terminal_title();
