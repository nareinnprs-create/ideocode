...
/// `IDEOCODE_OPENAI_WS_IDLE_RECONNECT_SECS` (0 disables the idle reconnect entirely,
...
impl OpenAIProvider {
    ...
    fn new_inner(credentials: CodexCredentials, browser_only: bool) -> Self {
        ...
        let mut model = if browser_only {
            ...
            std::env::var("IDEOCODE_OPENAI_MODEL")
        ...
        let prompt_cache_key = std::env::var("IDEOCODE_OPENAI_PROMPT_CACHE_KEY")
            .ok()
            .map(|v| v.trim().to_string())
            .filter(|v| !v.is_empty());
        let prompt_cache_retention = std::env::var("IDEOCODE_OPENAI_PROMPT_CACHE_RETENTION")
            .ok()
            .map(|v| v.trim().to_string())
            .filter(|v| !v.is_empty());
    ...
    fn load_max_output_tokens() -> Option<u32> {
        let raw = std::env::var("IDEOCODE_OPENAI_MAX_OUTPUT_TOKENS").ok();
    ...
    /// `IDEOCODE_OPENAI_API_BASE`, `OPENAI_BASE_URL`, `OPENAI_API_BASE`.
    ///
