...
//!      login (`IDEOCODE auth-test --json`, `IDEOCODE provider add --api-key-stdin`,
//!      `IDEOCODE login`). The agent can add the key/login for the user itself.
//!
...
/// An external coding agent IDEOCODE can hand a repair task to. These map to the
...
/// `IDEOCODE_HOME`/external isolation, matching the onboarding import detectors.
...
pub(crate) fn build_repair_brief(
    ...
    brief.push_str(
        "IDEOCODE (a terminal coding agent) could not import or validate a login \
during first-run onboarding. Please fix the login for the user.\n\n",
    ...
    if provider_hint.is_some() {
        brief.push_str(&format!("  IDEOCODE auth-test --provider {provider} --json\n",));
    } else {
        brief.push_str("  IDEOCODE auth-test --all-configured --json   # which provider is broken?\n");
    }
    brief.push_str("  IDEOCODE auth doctor   # human-readable, structured recovery steps\n\n");
...
\u{20}     --base-url https://api.example.com/v1 --model <model> --api-key-stdin\n\n",
    ...
    brief.push_str(
        "When auth-test reports success, tell the user to restart IDEOCODE (or press Enter on \
...
mod tests {
    ...
    fn brief_lists_the_non_interactive_commands_and_log_path() {
        ...
        assert!(
            brief.contains("IDEOCODE auth-test --provider openai --json"),
            "{brief}"
        );
        assert!(brief.contains("IDEOCODE login --provider openai"), "{brief}");
        assert!(brief.contains("--api-key-stdin"), "{brief}");
        assert!(brief.contains("IDEOCODE provider add"), "{brief}");
        // Points at the logs.
    ...
    fn brief_without_provider_or_agent_is_still_actionable() {
        ...
        assert!(
            brief.contains("IDEOCODE auth-test --all-configured --json"),
            "{brief}"
        );
        assert!(brief.contains("IDEOCODE auth doctor"), "{brief}");
        // No agent label, but still tells the user what to do.
        assert!(brief.contains("restart IDEOCODE"), "{brief}");
    }
