...
Everything runs under an isolated IDEOCODE_HOME / IDEOCODE_RUNTIME_DIR / IDEOCODE_SOCKET
so it never touches the user's real shared server, logs, sessions, or creds.

Two spawn shapes are profiled:

  server : the background daemon (`IDEOCODE serve`) on a private socket. This is
...
def isolated_env(root: str) -> dict[str, str]:
    env = os.environ.copy()
    env["IDEOCODE_HOME"] = os.path.join(root, "home")
    env["IDEOCODE_RUNTIME_DIR"] = os.path.join(root, "run")
    env["IDEOCODE_SOCKET"] = os.path.join(env["IDEOCODE_RUNTIME_DIR"], "IDEOCODE.sock")
    env["IDEOCODE_NO_TELEMETRY"] = "1"
...
def find_child_server(env: dict[str, str], exclude: set[int]) -> int | None:
    ...
    for pid in pids:
        ...
        if not cmd:
            continue
        argv = cmd.split("\0")
        if "serve" in argv and any(env["IDEOCODE_SOCKET"] in a or "serve" == a for a in argv):
            # confirm it is our isolated daemon by socket env or socket arg
            envblob = _read(f"/proc/{pid}/environ") or ""
            if env["IDEOCODE_SOCKET"] in envblob or env["IDEOCODE_SOCKET"] in cmd:
                return pid
    return None
...
def profile_server_spawn(binary: str, sample_interval_s: float) -> dict | None:
    root = tempfile.mkdtemp(prefix="IDEOCODE-spawn-server-")
    env = isolated_env(root)
    sock = env["IDEOCODE_SOCKET"]
    proc = None
    try:
...
def profile_client_spawn(binary: str, sample_interval_s: float, hold_s: float) -> dict | None:
    if not _HAVE_PTY:
        return None
    root = tempfile.mkdtemp(prefix="IDEOCODE-spawn-client-")
    env = isolated_env(root)
    log_path = Path(env["IDEOCODE_HOME"]) / "logs" / f"IDEOCODE-{time.strftime('%Y-%m-%d')}.log"
    pid = None
    master_fd = None
    try:
        ...
        if pid == 0:  # child
            ...
            os.execve(
                binary,
                [binary, "--no-update", "--socket", env["IDEOCODE_SOCKET"]],
                env,
            )
            os._exit(127)
        ...
        while time.perf_counter() < deadline:
            ...
            if server_mon is not None:
                server_mon.poll()
                if server_ready_ms is None and socket_connectable(env["IDEOCODE_SOCKET"]):
...
def main() -> int:
    ...
    default_bin = os.environ.get("IDEOCODE_BENCH_BIN") or shutil.which("IDEOCODE") or "./target/release/IDEOCODE"
    ...
    print("=" * 64)
    print("IDEOCODE spawn profile: time + resource utilization")
    print("=" * 64)
    print(f"Binary : {binary}")
    print(f"Size   : {size/1048576:.1f} MB")
    print(f"Runs   : {args.runs}   sample interval: {args.sample_interval_ms} ms")
    print(f"Host   : {os.cpu_count()} CPUs, isolated IDEOCODE_HOME per run")
