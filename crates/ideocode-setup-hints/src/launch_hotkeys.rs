...
//!   [`ResolvedLaunchHotkey`]s. An empty config reproduces IDEOCODE's historical
...
/// Build a IDEOCODE launch snippet that pauses on non-zero exit so the user can
...
pub(crate) struct ResolvedLaunchHotkey {
    /// IDEOCODE-style chord string, e.g. `cmd+;`.
    pub chord: String,
    ...
    /// fallback) before launching IDEOCODE.
    ...
    pub dir: String,
    /// Extra CLI args passed to IDEOCODE (e.g. `self-dev`).
    pub args: Vec<String>,
...
fn script_name_for(chord: &str, index: usize) -> String {
    ...
    format!("launch_IDEOCODE_{index}_{slug}.sh")
}

/// The built-in default entries, used when config has none. Mirrors IDEOCODE's
...
/// Map a IDEOCODE key token onto a `global_hotkey` `Code`. Returns `None` for
...
mod tests {
    ...
    fn shell_command_includes_cd_and_exe() {
        ...
        let cmd = shell_command_for(&resolved[0], "/bin/IDEOCODE");
        assert!(cmd.contains("/Users/jeremy/proj"));
        assert!(cmd.contains("/bin/IDEOCODE"));
    }
}
