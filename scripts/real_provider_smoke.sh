...
expect=${IDEOCODE_TRACE_EXPECT:-DONE}
cargo_exec="$repo_root/scripts/cargo_exec.sh"

echo "=== Real Provider Smoke ==="
echo "Provider: ${provider}"

if [[ "${IDEOCODE_REAL_PROVIDER_TEST_API:-1}" == "1" ]]; then
  if [[ "${provider}" == "claude" && "${IDEOCODE_USE_DIRECT_API:-0}" != "1" ]]; then
    echo ""
    ...
    echo ""
    echo "Test 1: Skipping test_api (provider=${provider}, IDEOCODE_USE_DIRECT_API=${IDEOCODE_USE_DIRECT_API:-0})"
  fi
fi
...
set +e
output=$(IDEOCODE_HOME="$workdir" PATH="$repo_root/target/release:$PATH" \
  IDEOCODE run --no-update --trace --provider "$provider" "$prompt" 2>&1)
status=$?
set -e
