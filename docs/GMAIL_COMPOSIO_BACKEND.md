...
|---|---|---|---|
| `direct` (default) | Local Google OAuth tokens (`IDEOCODE login google`) | No third party in the loop | Unverified-app warning; 7-day refresh-token expiry in Google "Testing" mode |
...
- `IDEOCODE_GMAIL_BACKEND=direct` (or unset) -> direct Google backend.
- `IDEOCODE_GMAIL_BACKEND=composio` -> Composio backend (requires `COMPOSIO_API_KEY`).

If `composio` is requested but `COMPOSIO_API_KEY` is missing, IDEOCODE warns and
...
1. IDEOCODE calls Composio's `POST /connected_accounts/link` (hosted "Connect
...
4. IDEOCODE polls `GET /connected_accounts/{id}` until the connection is `ACTIVE`,
   then persists it to `~/.IDEOCODE/composio_gmail.json`.
...
3. Export the variables:
   ```bash
   export IDEOCODE_GMAIL_BACKEND=composio
...
- `direct`: honors the access tier chosen at `IDEOCODE login google`
