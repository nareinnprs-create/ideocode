...
fn preferences_path() -> Result<PathBuf> {
    if let Ok(path) = std::env::var("IDEOCODE_DESKTOP_STATE") {
        return Ok(PathBuf::from(path));
    }

    if let Ok(path) = std::env::var("IDEOCODE_HOME") {
        return Ok(PathBuf::from(path).join("config/IDEOCODE/desktop-state.json"));
    }

    if let Ok(path) = std::env::var("XDG_CONFIG_HOME") {
        return Ok(PathBuf::from(path).join("IDEOCODE/desktop-state.json"));
    }

    let home = std::env::var_os("HOME")
        .map(PathBuf::from)
        .context("HOME is not set")?;
    Ok(home.join(".config/IDEOCODE/desktop-state.json"))
}
...
mod tests {
    ...
    fn saves_and_loads_preferences() -> Result<()> {
        ...
        unsafe {
            std::env::set_var("IDEOCODE_DESKTOP_STATE", &path);
        }
    ...
    fn preference_temp_paths_are_unique_and_hidden_next_to_target() {
        let path = PathBuf::from("/tmp/IDEOCODE-desktop-prefs/state.json");
