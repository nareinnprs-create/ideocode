  ...
  return (
    <div className="flex flex-col h-full" role="region" aria-label="Hooks panel">
      <div className="px-1 pt-1 flex items-center justify-between">
        ...
        <button onClick={() => setShowAdd(!showAdd)} aria-label="Add hook" aria-expanded={showAdd} className="flex items-center gap-1 px-2 py-1 text-xs text-accent hover:text-accent-hover transition-fast rounded hover:bg-surface-elevated">
          <Plus size={14} /> Add
        </button>
      </div>
      {showAdd && (
        <div className="mx-3 mb-2 p-2 rounded bg-surface-elevated border border-border-subtle space-y-1.5">
          ...
          <button onClick={handleAdd} disabled={!command.trim()}
            className="w-full px-3 py-1.5 text-xs bg-accent text-white rounded hover:bg-accent-hover disabled:opacity-50 transition-fast">Create Hook</button>
        </div>
      )}
          ...
          <div key={h.id} className="px-3 py-2 border-b border-border-subtle hover:bg-surface-elevated transition-fast group">
            <div className="flex items-center gap-2">
              <button onClick={() => toggleHook(h.id)} className="shrink-0">
                {h.enabled ? <ToggleRight size={16} className="text-success" /> : <ToggleLeft size={16} className="text-fg-muted" />}
              </button>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <span className={`text-[10px] px-1.5 py-0.5 rounded ${EVENT_COLORS[h.event]}`}>{EVENT_LABELS[h.event]}</span>
                  <span className="text-xs font-medium text-fg-primary">{h.name}</span>
                </div>
                <div className="text-[11px] text-fg-muted font-mono truncate mt-0.5">{h.command}</div>
                <div className="text-[10px] text-fg-muted mt-0.5">
                  {h.async ? "async" : "sync"} · {h.timeout}s timeout
                  {h.matcher.patterns.length > 0 && ` · patterns: ${h.matcher.patterns.join(", ")}`}
                </div>
              </div>
              <button onClick={() => removeHook(h.id)} aria-label="Delete hook" className="p-1 text-fg-muted hover:text-error opacity-0 group-hover:opacity-100 transition-fast">
                <Trash2 size={11} />
              </button>
            </div>
          </div>
