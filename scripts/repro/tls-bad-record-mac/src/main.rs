...
//
// We then wrap that error exactly like IDEOCODE's anthropic provider does
...
fn classify(error_str: &str) -> bool {
    // VERBATIM copy of IDEOCODE's shared is_transient_transport_error
    // (crates/IDEOCODE-base/src/provider/routing.rs @ v0.24.0).
...
async fn main() -> anyhow::Result<()> {
    ...
    // ---- 4. Client (mirrors IDEOCODE's reqwest+rustls config) ----
