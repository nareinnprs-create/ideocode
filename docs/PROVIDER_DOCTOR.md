...
tracks (`IDEOCODE provider-test-coverage`), but as an interactive command you can run
...
```bash
# Validate IDEOCODE's own wiring for a provider, no API key, no spend:
IDEOCODE provider-doctor cerebras --tier offline

# Validate the key + live model catalog (needs a key, negligible spend):
IDEOCODE provider-doctor cerebras --tier catalog

# Full readiness, including real chat, streaming, and tool calls (spends balance):
IDEOCODE provider-doctor cerebras --tier full

# Pin a specific model and emit JSON for scripting/CI:
IDEOCODE provider-doctor cerebras --model gpt-oss-120b --tier full --json
```
...
| --- | --- | --- | --- | --- |
| `offline` | no | no | IDEOCODE-side wiring against a synthetic catalog | catalog reload, picker rendering, fallback labeling, and model-switch routing bugs for this provider |
...
12. `real_IDEOCODE_tool_smoke` - an end-to-end tool smoke passed (full tier)
...
`IDEOCODE provider-test-coverage` shows a cumulative "Recorded spend" footer
...
1. **"My picker is broken / shows the wrong models."**
   ...
   `model_switch_route` fail, it's a IDEOCODE-side routing bug for that provider:
...
2. **"It won't connect / says auth failed."**
   ...
   `IDEOCODE login --provider <provider>`.
...
`IDEOCODE provider-test-coverage`. Lighter tiers record the API-dependent
checkpoints as skipped, so they never over-credit a pair.

`IDEOCODE provider-test-coverage` renders the same 11 checkpoints as an 11-stage
...
```
  ...
  6/11   nvidia-nim / gemma-4-31b  failed at `streaming reply`; run `IDEOCODE provider-doctor nvidia-nim --model gemma-4-31b --tier full`; last tested 2 days ago ...
```
