...
fn save_and_load_api_key() {
    ...
    let file = dir.path().join("IDEOCODE").join("cursor.env");
...
fn cursor_auth_file_path_respects_IDEOCODE_home() {
    ...
    // provider correctly looked under `$IDEOCODE_HOME/external/...`.
    ...
    assert!(
        ...
        "cursor auth path should be under IDEOCODE_HOME/external, got {}",
        path.display()
    );
