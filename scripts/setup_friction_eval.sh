...
#              install, does `IDEOCODE` resolve in a brand-new login/interactive
...
#   Section C  retention - an upgrade must preserve ~/.IDEOCODE config and auth,
...
# Probe: does `IDEOCODE` resolve and run in a fresh shell of the given kind, with
...
probe_shell() { # probe_shell <home> <shell-cmd...>
  ...
  ENV="$home/.profile" \
  PATH="/usr/bin:/bin" \
  "$@" 'command -v IDEOCODE >/dev/null 2>&1 && IDEOCODE --version' 2>/dev/null </dev/null
}
...
launcher="$home_a/.local/bin/IDEOCODE"
[ -x "$launcher" ]; check "launcher exists and is executable" \
  "executable at ~/.local/bin/IDEOCODE" "missing or not executable: $launcher" "$?"

ver=$("$launcher" --version 2>/dev/null || true)
[ "$ver" = "IDEOCODE 1.2.3" ]; check "launcher runs and reports the installed version" \
  "IDEOCODE 1.2.3" "${ver:-<no output>}" "$?"

# The success message must not dead-end the user: either IDEOCODE is already
# resolvable or the copy explicitly says future shells will have it.
printf '%s' "$install_out" | grep -q "Run 'IDEOCODE' to get started\|Future terminal sessions will have IDEOCODE on PATH automatically"
check "install output gives a working next step (no dead end)" \
  "a 'run IDEOCODE' or 'future sessions' line" "neither line found in installer output" "$?"
...
probe_case() { # probe_case <label> <binary> <shell-cmd...>
  ...
  [ "$out" = "IDEOCODE 1.2.3" ]
  check "$label" "IDEOCODE resolves and prints 'IDEOCODE 1.2.3'" "${out:-<not found on PATH>}" "$?"
}

probe_case "bash login shell (bash -lc) finds IDEOCODE"        bash bash -lc
probe_case "bash interactive shell (bash -ic) finds IDEOCODE"  bash bash -ic
probe_case "sh login shell (sh -lc) finds IDEOCODE"            sh   sh -lc
probe_case "fish shell (fish -c) finds IDEOCODE"               fish fish -c
probe_case "zsh login shell (zsh -lc) finds IDEOCODE"          zsh  zsh -lc
...
for rc in .bashrc .profile .zshenv .config/fish/config.fish; do
  file="$home_a/$rc"
  [ -f "$file" ] || continue
  # Each install appends one "# Added by IDEOCODE installer" stanza when missing;
  # a correct idempotency guard leaves exactly one after any number of runs.
  count=$(grep -cF "# Added by IDEOCODE installer" "$file" || true)
  [ "$count" -le 1 ]
  check "~/$rc has at most one IDEOCODE PATH stanza after 3 installs" \
...
mkdir -p "$home_c/.IDEOCODE"
printf 'model = "kept"\n' > "$home_c/.IDEOCODE/config.toml"
printf '{"kept":true}\n' > "$home_c/.IDEOCODE/auth.json"

run_install "$home_c" "1.3.0" >/dev/null 2>&1

ver=$("$home_c/.local/bin/IDEOCODE" --version 2>/dev/null || true)
[ "$ver" = "IDEOCODE 1.3.0" ]; check "upgrade switches the launcher to the new version" \
  "IDEOCODE 1.3.0" "${ver:-<no output>}" "$?"

[ "$(cat "$home_c/.IDEOCODE/config.toml" 2>/dev/null)" = 'model = "kept"' ]
check "upgrade preserves ~/.IDEOCODE/config.toml" \
  "file unchanged" "missing or modified" "$?"

[ "$(cat "$home_c/.IDEOCODE/auth.json" 2>/dev/null)" = '{"kept":true}' ]
check "upgrade preserves ~/.IDEOCODE/auth.json" \
  "file unchanged" "missing or modified" "$?"

[ -x "$home_c/.IDEOCODE/builds/versions/1.2.3/IDEOCODE" ] && [ -x "$home_c/.IDEOCODE/builds/versions/1.3.0/IDEOCODE" ]
check "both immutable version binaries kept (rollback possible)" \
  "versions/1.2.3 and versions/1.3.0 both executable" \
  "$(ls "$home_c/.IDEOCODE/builds/versions" 2>/dev/null | tr '\n' ' ')" "$?"

stable_ver=$(cat "$home_c/.IDEOCODE/builds/stable-version" 2>/dev/null || true)
[ "$stable_ver" = "1.3.0" ]; check "stable channel marker points at the new version" \
  "1.3.0" "${stable_ver:-<missing>}" "$?"

# A second post-upgrade login shell must still resolve IDEOCODE (PATH survives
# upgrades, not just fresh installs).
out=$(probe_shell "$home_c" bash -lc)
[ "$out" = "IDEOCODE 1.3.0" ]; check "post-upgrade login shell still finds IDEOCODE" \
  "IDEOCODE 1.3.0" "${out:-<not found on PATH>}" "$?"
...
# uninstall.sh pkills running IDEOCODE servers; neuter that inside the sandbox so
...
[ ! -e "$home_c/.local/bin/IDEOCODE" ] && [ ! -d "$home_c/.IDEOCODE/builds" ]
...
if command -v pwsh >/dev/null 2>&1; then
  broadcast_block=$(awk "/<<'IDEOCODE_PS_BROADCAST_EOF'/{inblock=1; next} /^IDEOCODE_PS_BROADCAST_EOF\$/{inblock=0} inblock" "$install_sh")
  printf '%s' "$broadcast_block" > "$work/broadcast.ps1"
  parse_errors=$(IDEOCODE_EVAL_PS_FILE="$work/broadcast.ps1" pwsh -NoProfile -NonInteractive -Command '
    $errors = $null
    [System.Management.Automation.Language.Parser]::ParseFile($env:IDEOCODE_EVAL_PS_FILE, [ref]$null, [ref]$errors) | Out-Null
    $errors.Count
...
printf 'C:\\Tools;%s\\' "$(printf '%s' "$work/localappdata/IDEOCODE/bin" | tr '[:lower:]' '[:upper:]')" > "$win_state/user-path"
...
#   2. write the user PATH  -> persist $IDEOCODE_NEW_USER_PATH
#   3. broadcast            -> record that a broadcast happened
args="$*"
case "$args" in
  *GetEnvironmentVariable*) cat "$EVAL_WIN_STATE/user-path" ;;
  *SetEnvironmentVariable*) printf '%s' "$IDEOCODE_NEW_USER_PATH" > "$EVAL_WIN_STATE/user-path" ;;
