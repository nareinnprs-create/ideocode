...
- Avoid cyclic dependencies and hidden coupling through broad `IDEOCODE-core` re-exports.
...
Examples: `IDEOCODE-session-types`, `IDEOCODE-side-panel-types`, `IDEOCODE-selfdev-types`, `IDEOCODE-background-types`.
...
### `IDEOCODE-core` is for genuinely shared primitives

`IDEOCODE-core` should contain:
...
`IDEOCODE-core` should not accumulate every extracted DTO indefinitely. Once a cluster grows, split it into a focused domain crate.
...
4. Validate.
   ...
   - [ ] Did `cargo check --profile selfdev -p <type-crate> -p IDEOCODE --bin IDEOCODE` pass?
...
| --- | --- | --- | --- |
| Usage DTOs | `cargo check --profile selfdev -p IDEOCODE-usage-types -p IDEOCODE --bin IDEOCODE` | Prefer exact tests under usage/copilot usage modules. Avoid bare `usage` as a required gate because it selects display/UI tests too. | DTO crate owns report and local counter contracts. Runtime fetch/cache/display stay root. |
| Gateway DTOs | `cargo check --profile selfdev -p IDEOCODE-gateway-types -p IDEOCODE --bin IDEOCODE` | Focus gateway persistence/auth tests by exact test names when available. | Pairing/token HTTP/WebSocket behavior stays root. |
| Ambient DTOs | `cargo check --profile selfdev -p IDEOCODE-ambient-types -p IDEOCODE --bin IDEOCODE` | Scheduler/type consumers only. | Ambient DTO crate owns usage records only. Queue/runtime/prompt behavior stays root. |
| Ambient behavior modules | `cargo check --profile selfdev -p IDEOCODE --bin IDEOCODE` | `cargo test --profile selfdev -p IDEOCODE ambient::ambient_tests --lib`; `cargo test --profile selfdev -p IDEOCODE ambient::scheduler::tests --lib`; `cargo test --profile selfdev -p IDEOCODE ambient::runner::runner_tests --lib` | Avoid bare `ambient` as a required gate for module-only refactors because it selects cross-module TUI/config state tests. |
| Memory activity DTOs | `cargo check --profile selfdev -p IDEOCODE-memory-types -p IDEOCODE-core -p IDEOCODE --bin IDEOCODE` | `cargo test --profile selfdev -p IDEOCODE runtime_memory_log --lib`; `cargo test --profile selfdev -p IDEOCODE tui::info_widget::tests --lib` | `memory::activity` currently matches no tests, so use consumer tests. |
| Goal/todo/catchup core DTOs | `cargo check --profile selfdev -p IDEOCODE-core -p IDEOCODE --bin IDEOCODE` | Exact goal/todo/catchup filters if behavior changes. | Currently small/stable enough to leave in `IDEOCODE-core`; revisit if churn grows. |


## Compile baseline observations

Measured on 2026-04-30 with `scripts/dev_cargo.sh check --profile selfdev -p IDEOCODE --bin IDEOCODE` after the compile-speed boundary doc commit. This is a coarse mtime-touch benchmark, not a full statistical study, but it is enough to guide priorities.
...
| Touch `crates/IDEOCODE-core/src/usage_types.rs` | ~65.35s | Editing `IDEOCODE-core` invalidates broad downstream dependents. Avoid adding high-churn domain DTOs to `IDEOCODE-core`. |

Implication: the compile-speed target is not simply "move things out of root". Moving stable, low-churn contracts out of root is good, but putting many high-churn domain DTOs into `IDEOCODE-core` can be counterproductive because `IDEOCODE-core` has high fan-out. Prefer focused leaf crates such as `IDEOCODE-usage-types`, `IDEOCODE-gateway-types`, and `IDEOCODE-ambient-types` for domain DTOs that are likely to change.

## `IDEOCODE-core` fan-out audit

At this checkpoint, the root crate is the only direct Cargo dependency on `IDEOCODE-core`, but root re-exports many `IDEOCODE-core` modules and root is the high-cost recompilation target. A touch to `IDEOCODE-core` invalidated broad downstream checks in the baseline above. Therefore `IDEOCODE-core` should be treated as a high-fan-out crate even if Cargo.toml direct dependents are currently few.
...
1. Move clustered, likely-changing domain DTOs from `IDEOCODE-core` to focused leaf crates.
2. Keep stable general utilities in `IDEOCODE-core`.
3. Avoid adding new domain DTOs to `IDEOCODE-core` unless they are very stable or temporary staging.

| Module | Current contents | Preferred long-term home | Notes |
| --- | --- | --- | --- |
| `ambient_usage_types` | Ambient scheduler usage records/rate limit DTOs | moved to `IDEOCODE-ambient-types` | Compatibility re-export remains in root module. |
| `catchup_types` | Catch-up persisted state and rendered brief DTOs | `IDEOCODE-catchup-types` or stay in core | Small and low churn. Split only if catch-up grows. |
| `copilot_usage_types` | Local Copilot usage counters | moved to `IDEOCODE-usage-types` | Compatibility re-export remains in root module. |
| `gateway_types` | Paired device and pairing code persisted records | moved to `IDEOCODE-gateway-types` | Pairing/token behavior remains root. |
| `goal_types` | Goal state, milestones, status, updates | `IDEOCODE-goal-types` or `IDEOCODE-task-types` | Larger domain. Worth splitting if goal/tool work grows. |
| `memory_types` | Memory activity DTOs | moved to `IDEOCODE-memory-types` | Memory has enough domain weight for its own type crate. |
| `todo_types` | Todo item DTO | `IDEOCODE-task-types`, `IDEOCODE-todo-types`, or core | Tiny. Could join goal/catchup task-state crate. |
| `usage_types` | Provider usage report DTOs | moved to `IDEOCODE-usage-types` | Runtime fetch/cache/display remain root. |
...
- public report DTOs in `IDEOCODE-usage-types`
...
- WebSocket relay
- public gateway DTOs in `IDEOCODE-gateway-types`
...
- `IDEOCODE-core` contains only true primitives or documented temporary staging modules.
