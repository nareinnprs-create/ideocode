...
struct SkillInput {
    ...
    /// `list` shows both loaded skills and the IDEOCODE-endorsed catalog.
...
impl SkillTool {
        ...
        let skill = registry.get(&name).ok_or_else(|| {
                ...
                .find(|endorsed| endorsed.name == name)
            {
                match endorsed.install {
                    ...
                    None => anyhow::anyhow!(
                        "Skill '{}' is endorsed but not installed (source: {}). Install it into ~/.IDEOCODE/skills/{}/SKILL.md, then run skill_manage reload_all.",
                        name,
                        endorsed.source,
                        name
                    ),
    ...
    async fn list_skills(&self, working_dir: Option<&std::path::Path>) -> Result<ToolOutput> {
        ...
        let mut output = if skills.is_empty() {
            ...
            - ~/.IDEOCODE/skills/<skill-name>/SKILL.md (global)\n\
            - ./.IDEOCODE/skills/<skill-name>/SKILL.md (project-local)\n\
...
/// Append the curated IDEOCODE-endorsed skill catalog to `output`, grouped by
...
fn append_endorsed_skills(output: &mut String, installed: &std::collections::HashSet<&str>) {
    ...
    output.push_str("\nEndorsed skills (recommended by IDEOCODE)\n");
...
mod tests {
    ...
    fn create_test_tool_with_skill(name: &str) -> (SkillTool, tempfile::TempDir) {
        ...
        let skill_dir = temp_dir.path().join(".IDEOCODE").join("skills").join(name);
