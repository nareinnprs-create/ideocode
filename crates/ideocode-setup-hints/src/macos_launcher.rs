...
const MACOS_APP_ICON_FILE_NAME: &str = "IDEOCODE.icns";
const MACOS_APP_ICON_BYTES: &[u8] = include_bytes!(concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/../../assets/app-icons/IDEOCODE.icns"
));
...
pub(super) fn install_macos_app_launcher() -> Result<(PathBuf, MacTerminalKind)> {
    ...
    let launcher_path = macos_dir.join("IDEOCODE-launcher");
...
<dict>
    ...
    <string>IDEOCODE</string>
    <key>CFBundleDisplayName</key>
    <string>IDEOCODE</string>
    <key>CFBundleIdentifier</key>
    <string>com.IDEOCODE.launcher</string>
    <key>CFBundleVersion</key>
    <string>{version}</string>
    <key>CFBundleShortVersionString</key>
    <string>{version}</string>
    <key>CFBundleExecutable</key>
    <string>IDEOCODE-launcher</string>
...
fn macos_launcher_script(terminal: MacTerminalKind, exe_path: &str, app_dir: &Path) -> String {
    ...
    let terminal_failure_message = escape_applescript_text(&format!(
        "IDEOCODE could not open {}.\n\nTry rerunning:\nIDEOCODE setup-launcher\n\nLauncher log:\n~/.IDEOCODE/launcher/macos-launcher.log",
        terminal.label()
    ));
...
LOG_DIR="$HOME/.IDEOCODE/launcher"
LOG_FILE="$LOG_DIR/macos-launcher.log"
mkdir -p "$LOG_DIR" >/dev/null 2>&1 || true

show_missing_executable() {{
  /usr/bin/osascript <<'APPLESCRIPT' >/dev/null 2>&1 || true
display alert "IDEOCODE launch failed" message "{missing_message}" as critical
APPLESCRIPT
}}

show_terminal_launch_failure() {{
  /usr/bin/osascript <<'APPLESCRIPT' >/dev/null 2>&1 || true
display alert "IDEOCODE launch failed" message "{terminal_failure_message}" as critical
APPLESCRIPT
}}
