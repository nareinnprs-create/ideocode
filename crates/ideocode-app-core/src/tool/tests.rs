...
async fn registry_execute_pre_tool_hook_blocks_and_allows() {
    ...
    match prev {
        Some(value) => crate::env::set_var("IDEOCODE_HOOK_PRE_TOOL", value),
        None => crate::env::remove_var("IDEOCODE_HOOK_PRE_TOOL"),
    }
    crate::config::invalidate_config_cache();
...
async fn gemini_build_tools_from_registry_definitions_omits_const_keywords() {
    // Moved from IDEOCODE-base/src/provider/gemini_tests.rs: this is the one test
