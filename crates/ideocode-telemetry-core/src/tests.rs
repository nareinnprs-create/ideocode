...
// flip `IDEOCODE_NO_TELEMETRY` / `DO_NOT_TRACK`, while the session tests drive the
...
fn telemetry_endpoint_uses_production_custom_domain() {
    assert_eq!(TELEMETRY_ENDPOINT, "https://telemetry.IDEOCODE.sh/v1/event");
}
...
fn test_onboarding_step_milestone_key_includes_provider_and_method() {
    assert_eq!(
        onboarding_step_milestone_key("auth_success", Some("IDEOCODE"), Some("API key")),
        "auth_success_IDEOCODE_api_key"
    );
    assert_eq!(
