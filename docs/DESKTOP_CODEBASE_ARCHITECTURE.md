...
This document translates the current IDEOCODE TUI architecture into a concrete codebase plan for a future custom desktop app.
...
```text
IDEOCODE server/runtime/protocol
...
```text
crates/
  IDEOCODE-protocol/             # eventually extracted from src/protocol.rs
  IDEOCODE-client-core/          # surface-independent client state/reducers/view models
  IDEOCODE-desktop-ui/           # custom UI tree, layout, input routing, style tokens
  IDEOCODE-desktop-renderer/     # wgpu renderer, display list, glyph/image atlases
  IDEOCODE-desktop-platform/     # winit/AppKit/Linux shell abstraction, menus, clipboard
  IDEOCODE-desktop/              # product app: windows, panels, protocol client, composition
```

Initial implementation may keep some of these as modules inside `crates/IDEOCODE-desktop` to reduce early friction, but the boundaries should be designed as if they were separate crates.
...
```text
IDEOCODE-desktop
  -> IDEOCODE-desktop-platform
  -> IDEOCODE-desktop-renderer
  -> IDEOCODE-desktop-ui
  -> IDEOCODE-client-core
  -> IDEOCODE-protocol
```

`IDEOCODE-client-core` must not depend on:
...
`IDEOCODE-desktop-ui` should not depend on the IDEOCODE server runtime. It can depend on client-core view models and generic UI types.

`IDEOCODE-desktop-renderer` should not know what a IDEOCODE session is. It renders display lists, text runs, images, clips, and primitives.
...
```text
crates/IDEOCODE-protocol/src/lib.rs
  Request
  ServerEvent
  HistoryMessage
...
`IDEOCODE-desktop` should be product composition, not renderer internals.

Suggested modules:

```text
crates/IDEOCODE-desktop/src/
  main.rs
...
`IDEOCODE-desktop-ui` is the framework-like internal layer, but it is product-owned and small.

```text
crates/IDEOCODE-desktop-ui/src/
  lib.rs
  id.rs
  geometry.rs
  color.rs
...
`IDEOCODE-desktop-renderer` should know nothing about IDEOCODE.

```text
crates/IDEOCODE-desktop-renderer/src/
  lib.rs
  gpu.rs
  surface.rs
  pipeline.rs
...
```text
crates/IDEOCODE-desktop-platform/src/
  lib.rs
  event.rs
  window.rs
  clipboard.rs
...
Once the desktop reducer shape is clear, extract shared pieces from TUI and desktop into `IDEOCODE-client-core`:
...
At that point the TUI can gradually become another presentation of `IDEOCODE-client-core`, but it does not have to be converted all at once.
...
> Desktop always connects to a local IDEOCODE server/daemon. It does not embed the agent runtime in-process.
...
```text
crates/IDEOCODE-desktop-ui       # pure-ish UI/layout model
crates/IDEOCODE-desktop-renderer # wgpu display-list renderer
crates/IDEOCODE-desktop          # fake-data app shell
```

Then connect to the server protocol.

Only after the desktop reducer/view model shape is proven should shared `IDEOCODE-client-core` extraction begin. This avoids prematurely extracting the wrong abstraction from the current TUI.
