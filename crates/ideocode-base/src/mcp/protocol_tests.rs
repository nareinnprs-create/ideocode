...
fn load_for_dir_without_project_does_not_load_process_cwd_config() {
    ...
    crate::env::set_var("IDEOCODE_HOME", home.path());
...
fn test_load_project_locals_merge_order() {
    // `.IDEOCODE/mcp.json` loads first, then `.mcp.json` overrides same-named
    // servers, then `.claude/mcp.json`.
    let temp = tempfile::tempdir().expect("tempdir");
    let project = temp.path();
    std::fs::create_dir_all(project.join(".IDEOCODE")).unwrap();
    std::fs::create_dir_all(project.join(".claude")).unwrap();
    std::fs::write(
        project.join(".IDEOCODE/mcp.json"),
        r#"{"servers":{"shared-name":{"command":"IDEOCODE-bin"},"IDEOCODE-only":{"command":"a"}}}"#,
    )
    .unwrap();
    std::fs::write(
        project.join(".mcp.json"),
    ...
    assert_eq!(
        ...
        "claude-bin",
        ".mcp.json must override .IDEOCODE/mcp.json for same-named servers"
    );
    assert!(config.servers.contains_key("IDEOCODE-only"));
