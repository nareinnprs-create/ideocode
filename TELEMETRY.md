...
| `version` | `"0.6.0"` | IDEOCODE version |
...
| `version` | `"0.9.1"` | Current IDEOCODE version |
| `from_version` | `"0.8.1"` | Previously recorded IDEOCODE version |
...
is also sent to the discovery API as the `x-IDEOCODE-discovery-request-id` header,
...
The benchmark runner sets `IDEOCODE_DISCOVERY_BENCHMARK=1`. Discovery requests then
carry `x-IDEOCODE-discovery-benchmark: 1`, and the corresponding telemetry event has
`benchmark_run: true`.
...
The UUID is randomly generated on first run and stored at `~/.IDEOCODE/telemetry_id`. It is not derived from your machine, username, email, or any identifiable information.

## How It Works

1. On first launch, IDEOCODE generates a random UUID and sends an `install` event
2. When a session begins, IDEOCODE sends a `session_start` event
3. When a session ends normally, IDEOCODE sends a `session_end` event with coarse session metrics
4. When auth succeeds, IDEOCODE sends a coarse `auth_success` event for activation-funnel analysis
5. When IDEOCODE detects a version change, it sends an `upgrade` event
6. On best-effort crash/signal handling, IDEOCODE sends a `session_crash` event
7. IDEOCODE may also send one-off onboarding milestone events and explicit feedback events when triggered
...
9. If a request fails (offline, firewall, etc.), IDEOCODE silently continues - no retries, no queuing
