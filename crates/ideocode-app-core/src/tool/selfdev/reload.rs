...
impl SelfDevTool {
    pub(super) async fn do_reload(
        ...
        // a local debug/selfdev checkout may not have target/release/IDEOCODE).
        ...
        if !SelfDevTool::is_test_session() && !target_binary.exists() {
            return Ok(ToolOutput::new(
                format!(
                    "No binary found at {}.\n\
                     Run 'IDEOCODE self-dev --build' first, or build with 'scripts/dev_cargo.sh build --profile selfdev -p IDEOCODE --bin IDEOCODE' and then try reload again.",
