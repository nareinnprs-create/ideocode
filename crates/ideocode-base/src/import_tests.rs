...
fn repeated_external_resume_reuses_the_imported_snapshot() {
    ...
    let first = resolve_resume_target_to_IDEOCODE(&target).unwrap();
    std::fs::remove_file(&transcript).unwrap();
    let second = resolve_resume_target_to_IDEOCODE(&target).unwrap();
...
fn cached_imported_session_preserves_existing_history_verbatim() {
    ...
    legacy.append_stored_message(StoredMessage {
        id: "IDEOCODE-continuation".to_string(),
        role: Role::Assistant,
        content: vec![ContentBlock::Text {
            text: "continued inside IDEOCODE".to_string(),
            cache_control: None,
        }],
        display_role: None,
        timestamp: None,
...
/// `resolve_resume_target_to_IDEOCODE`, which must import the transcript and hand
/// back a resumable `imported_cc_<id>` IDEOCODE session. This guards the full
...
/// Regression for silent data loss: the picker hides the imported IDEOCODE session
...
/// user already resumed and continued that imported session inside IDEOCODE, a
...
fn test_reimporting_claude_session_preserves_ideocode_continuation() {
    ...
    // User resumes inside IDEOCODE and appends a IDEOCODE-only follow-up message.
    let mut session = Session::load(&imported_id).unwrap();
    session.append_stored_message(StoredMessage {
        id: "IDEOCODE-continuation".to_string(),
        role: Role::User,
        content: vec![ContentBlock::Text {
            text: "IDEOCODE-only follow up".to_string(),
            cache_control: None,
        }],
        display_role: None,
        timestamp: None,
    ...
    assert_eq!(
        after.messages.len(),
        3,
        "IDEOCODE-side continuation must not be clobbered by re-import"
    );
    let preserved = after.messages.iter().flat_map(|m| m.content.iter()).any(
        |block| matches!(block, ContentBlock::Text { text, .. } if text == "IDEOCODE-only follow up"),
    );
    assert!(
        preserved,
        "the IDEOCODE-only follow up message must be preserved"
    );
}
