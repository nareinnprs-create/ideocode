...
#   --binary <path>   Client binary (default: ~/.IDEOCODE/builds/current/IDEOCODE)
#   --IDEOCODE <path>    IDEOCODE CLI used for `IDEOCODE debug ...` (default: ~/.local/bin/IDEOCODE)
...
#
# Requirements: a running IDEOCODE server with the debug socket enabled, jq,
...
CLIENT_BIN="$HOME/.IDEOCODE/builds/current/IDEOCODE"
TESTER_CWD="$HOME"
SKIP_TRIM=0
KEEP_TESTER=0
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --IDEOCODE)     IDEOCODE_BIN="$2"; shift 2 ;;
...
[[ -x "$IDEOCODE_BIN" ]] || die "IDEOCODE CLI not found at $IDEOCODE_BIN"
[[ -x "$CLIENT_BIN" ]] || CLIENT_BIN="$(command -v IDEOCODE)" || die "client binary not found"
[[ -f "$HOME/.IDEOCODE/sessions/${SESSION_ID}.json" ]] \
    || log "WARN: $HOME/.IDEOCODE/sessions/${SESSION_ID}.json not found; resume may create a new session"

RUN_ID="probe_$(date -u +%Y%m%dT%H%M%SZ)_$$"
SESSION_FILE="$HOME/.IDEOCODE/sessions/${SESSION_ID}.json"
...
cleanup() {
    ...
    if [[ -n "$TESTER_ID" && "$KEEP_TESTER" -ne 1 ]]; then
        "$IDEOCODE_BIN" debug "tester:${TESTER_ID}:stop" >/dev/null 2>&1 || true
...
emit_phase() { # phase proc_json client_json extras_json
    ...
    jq -c -S -n \
        --arg probe "IDEOCODE_memory_probe" \
        --arg schema "1" \
        --arg run_id "$RUN_ID" \
        --arg phase "$phase" \
...
# ===========================================================================
WRAPPER="$(mktemp /tmp/IDEOCODE_memory_probe_wrapper.XXXXXX.sh)"
cat > "$WRAPPER" <<EOF
#!/usr/bin/env bash
...
SPAWN_OUT="$("$IDEOCODE_BIN" debug "tester:spawn {\"binary\":\"$WRAPPER\",\"cwd\":\"$TESTER_CWD\",\"cols\":120,\"rows\":40}")" \
...
TESTER_INFO="$("$IDEOCODE_BIN" debug tester:list | jq -c --arg id "$TESTER_ID" '.[] | select(.id == $id)')"
