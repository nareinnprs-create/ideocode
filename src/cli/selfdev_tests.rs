...
fn set_socket_test_env(socket_path: &Path, runtime_dir: &Path) -> EnvVarGuard {
    let guard = EnvVarGuard::capture(&["IDEOCODE_SOCKET", "IDEOCODE_RUNTIME_DIR"]);
...
impl TestEnvGuard {
    fn new() -> anyhow::Result<Self> {
        ...
        let temp_home = tempfile::Builder::new()
            .prefix("IDEOCODE-selfdev-test-home-")
            .tempdir()?;
        let env = EnvVarGuard::capture(&["IDEOCODE_HOME", "IDEOCODE_TEST_SESSION"]);
...
fn test_launcher_dir_ignores_blank_overrides_and_uses_home_default() {
    let (_lock, _env, temp) = isolated_launcher_env();
    set_var("IDEOCODE_INSTALL_DIR", "   ");
    set_var("IDEOCODE_HOME", "\t");
...
fn default_launcher_dir(home: &Path) -> PathBuf {
    if cfg!(windows) {
        home.join("AppData").join("Local").join("IDEOCODE").join("bin")
    } else {
...
fn test_selfdev_build_command_prefers_repo_wrapper_when_present() {
    ...
    assert!(command.contains("dev_cargo.sh' build --profile selfdev -p IDEOCODE --bin IDEOCODE"));
    assert!(!command.contains("IDEOCODE-desktop"));
    assert!(build.display.contains("-p IDEOCODE --bin IDEOCODE"));
    assert!(!build.display.contains("IDEOCODE-desktop"));
}
...
fn test_selfdev_build_command_falls_back_to_cargo_when_wrapper_missing() {
    ...
    assert_eq!(build.args.first().map(String::as_str), Some("-lc"));
    let command = build.args.get(1).expect("shell command");
    assert!(command.contains("cargo build --profile selfdev -p IDEOCODE --bin IDEOCODE"));
    assert!(!command.contains("IDEOCODE-desktop"));
}
...
fn test_selfdev_build_command_can_target_all() {
    ...
    assert!(
        build
            .display
            .contains("-p IDEOCODE-desktop --bin IDEOCODE-desktop")
    );
}
