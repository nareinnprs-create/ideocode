...
impl SkillRegistry {
    ...
    ///
    /// Holds GLOBAL skills only (plugins, `~/.IDEOCODE/skills/`,
    ...
    /// `~/.IDEOCODE/skills/`, and `~/.agents/skills/`.
    ///
    ...
    pub fn load_global() -> Result<Self> {
        ...
        // first, so explicit IDEOCODE/agents skills with the same name win below.
    ...
    /// `./.IDEOCODE/skills/`, `./.agents/skills/`, and `./.claude/skills/`.
    ///
    ...
    /// plugin never prevents IDEOCODE's own skills from loading.
    ...
    /// Reload the shared global skill sources (plugins, `~/.IDEOCODE/skills/`,
...
/// A skill recommended/curated by IDEOCODE that the user may want to install.
...
pub struct EndorsedSkill {
    ...
    /// Grouping label used to organize the endorsed list (e.g. "IDEOCODE",
    /// "NVIDIA CUDA-X").
...
/// Curated list of skills endorsed by IDEOCODE. Used by the `/skills` command to
...
pub const ENDORSED_SKILLS: &[EndorsedSkill] = &[
    EndorsedSkill {
        ...
        category: "IDEOCODE",
        source: "bundled in IDEOCODE repo (.IDEOCODE/skills/optimization)",
        install: None,
    },
    EndorsedSkill {
        ...
        category: "IDEOCODE",
        source: "bundled with IDEOCODE / Claude Code skills",
        install: None,
    },
    EndorsedSkill {
...
/// Return the curated list of skills endorsed by IDEOCODE.
...
mod tests {
    ...
    fn project_overlay_is_session_scoped_and_composes_over_globals() {
        let temp = tempfile::tempdir().expect("tempdir");
        write_test_skill(temp.path(), ".IDEOCODE", "session-skill");
    ...
    fn registry_contains_reports_loaded_skills() {
        let temp = tempfile::tempdir().expect("tempdir");
        write_test_skill(temp.path(), ".IDEOCODE", "present-skill");
    ...
    fn explicit_ideocode_skill_wins_over_plugin_skill_with_same_name() {
        ...
        // Explicit IDEOCODE skill with the same name.
        write_test_skill(temp.path(), ".IDEOCODE", "shared-name");
        ...
        registry.load_plugin_skills_from_root(&plugins_root);
        registry
            .load_from_dir(&temp.path().join(".IDEOCODE/skills"))
        ...
        assert_eq!(
            ...
            "explicit IDEOCODE skill must override the plugin-provided one"
        );
    }
