...
fn sibling_socket_path_roundtrip() {
    let main = std::path::PathBuf::from("/tmp/IDEOCODE.sock");
    let debug = std::path::PathBuf::from("/tmp/IDEOCODE-debug.sock");
...
fn cleanup_socket_pair_removes_main_and_debug_files() {
    ...
    let main = dir.join(format!("IDEOCODE-test-{}.sock", stamp));
    let debug = dir.join(format!("IDEOCODE-test-{}-debug.sock", stamp));
...
async fn reap_stale_socket_removes_dead_socket_pair_and_lock() {
    ...
    let socket = temp.path().join("IDEOCODE.sock");
    let debug = temp.path().join("IDEOCODE-debug.sock");
...
fn existing_server_start_errors_are_detected() {
    assert!(server_start_matches_existing_server(
        "Error: Another IDEOCODE server process is already running for runtime dir /run/user/1000"
    ));
    assert!(server_start_matches_existing_server(
        "Error: Refusing to replace active server socket at /run/user/1000/IDEOCODE.sock"
    ));
    assert!(!server_start_matches_existing_server(
