...
//! launch hotkeys (e.g. `Cmd+[`, `Cmd+]`) to "open IDEOCODE here". The ranking is a
...
pub struct RankOptions {
    ...
    /// the user's home directory, which is noise from launching IDEOCODE at `$HOME`.
...
/// IDEOCODE in. Produced by [`build_launch_hotkey_plan`] from a ranking, then
/// persisted to config so the mapping is baked once and does not move around.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct PlannedHotkey {
    /// IDEOCODE-style chord string, e.g. `cmd+;` or `cmd+[`.
    pub chord: String,
    /// Absolute directory the hotkey opens IDEOCODE in.
    pub dir: String,
...
/// Scan a IDEOCODE sessions directory and extract one [`SessionLocation`] per
...
/// Compute the baked launch-hotkey plan from a IDEOCODE sessions directory.
///
...
mod tests {
    ...
    fn plan_assigns_top_repo_home_then_next_repos() {
        ...
        assert_eq!(plan[0].dir, "/u/jeremy/IDEOCODE");
    ...
    fn plan_skips_home_if_it_appears_in_ranking() {
        ...
        // Top repo slot is IDEOCODE (home filtered out), then home gets cmd+'.
        assert_eq!(plan[0].dir, "/u/jeremy/IDEOCODE");
        assert_eq!(plan[1].dir, "/u/jeremy");
    ...
    fn collect_reads_working_dirs_and_skips_junk() {
        ...
        write(
            "session_a.json",
            r#"{"working_dir":"/Users/jeremy/IDEOCODE-github","id":"a"}"#,
        );
        write(
            "session_b.json",
        ...
        assert_eq!(
            dirs,
            vec!["/Users/jeremy/IDEOCODE-github", "/Users/jeremy/scrollwm"]
        );
