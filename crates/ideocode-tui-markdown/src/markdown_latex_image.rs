...
impl Toolchain {
    fn from_environment() -> Self {
        Self {
            latex: std::env::var_os("IDEOCODE_LATEX_COMMAND")
                .map(PathBuf::from)
                .unwrap_or_else(|| PathBuf::from("latex")),
            dvipng: std::env::var_os("IDEOCODE_DVIPNG_COMMAND")
                .map(PathBuf::from)
                .unwrap_or_else(|| PathBuf::from("dvipng")),
            pdflatex: std::env::var_os("IDEOCODE_PDFLATEX_COMMAND")
                .map(PathBuf::from)
                .unwrap_or_else(|| PathBuf::from("pdflatex")),
            pdftocairo: std::env::var_os("IDEOCODE_PDFTOCAIRO_COMMAND")
                .map(PathBuf::from)
...
fn cache_dir() -> Result<PathBuf, String> {
    dirs::cache_dir()
        .map(|path| path.join("IDEOCODE").join("latex"))
...
fn run_command<const N: usize>(
    ...
    let output_path = working_dir.join(".IDEOCODE-command-output.log");
...
mod tests {
    ...
    fn missing_toolchain_returns_an_error_without_panicking() {
        ...
        let missing = PathBuf::from("/definitely/missing/IDEOCODE-latex-command");
