...
fn test_loading_preview_refreshes_search_index_for_picker_filtering() {
    ...
    let previous_home = std::env::var("IDEOCODE_HOME").ok();
...
fn onboarding_banner_renders_prompt_and_both_action_rows() {
    ...
    assert!(
        text.contains("Welcome to IDEOCODE"),
    ...
    let welcome_y = lines
        .iter()
        .position(|line| line.contains("Welcome to IDEOCODE"))
        .expect("welcome row");
    let review_y = lines
        .iter()
...
//
//   cargo test -p IDEOCODE-tui --lib --release -- --ignored --nocapture benchmark_resume_op
//
