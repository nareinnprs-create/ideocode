...
//! consumes them. These live in `IDEOCODE-render-core` (a backend-neutral, pure
//! crate) rather than in `IDEOCODE-tui-markdown` so the foundation/streaming layer
//! can format reasoning lines without depending on any `IDEOCODE-tui-*` crate.
