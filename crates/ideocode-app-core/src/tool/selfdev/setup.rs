...
impl SelfDevTool {
    ...
    /// building IDEOCODE from source: the rust toolchain, git, and a local repo
    ...
    pub(super) async fn do_setup(&self, ctx: &ToolContext) -> Result<ToolOutput> {
        ...
        // Rust toolchain: cargo + rustc are required to build IDEOCODE.
        ...
        if repo_dir.is_none() {
            ...
            if SelfDevTool::is_test_session() {
                clone_note = Some("Test mode: skipped cloning the IDEOCODE source.".to_string());
        ...
        match &repo_dir {
            ...
            None => {
                let target = Self::selfdev_clone_dir()
                    ...
                    .unwrap_or_else(|_| "~/.IDEOCODE/source/IDEOCODE".to_string());
                checks.push(SetupCheck::missing(
                    "repository",
                    "no local IDEOCODE checkout found",
                    format!(
                        "Clone the source manually: `git clone {} {}`.",
                        super::IDEOCODE_REPO_URL,
                        target
                    ),
                ));
            }
        ...
        if let Some(repo) = repo_dir.as_deref() {
            match build::find_dev_binary(repo) {
                ...
                None => checks.push(SetupCheck::missing(
                    ...
                    "Build it once with `IDEOCODE self-dev --build`, or inside a \
        ...
        if !failing.is_empty() {
            ...
            output.push_str(
                "\nAll prerequisites satisfied. Use `selfdev enter` to start working on IDEOCODE.\n",
            );
        }
    ...
    /// `selfdev find-config`: report the key IDEOCODE paths (config file, home,
    ...
    pub(super) async fn do_find_config(&self, ctx: &ToolContext) -> Result<ToolOutput> {
        ...
        let mut output = String::from("## IDEOCODE config & paths\n\n");
        output.push_str(&format!(
            "**Config file:** {}\n",
    ...
    pub(super) async fn do_reload_to_newer_build(&self, _ctx: &ToolContext) -> Result<ToolOutput> {
        ...
        if !server::server_has_newer_binary() {
            return Ok(ToolOutput::new(
                "Already running the newest installed IDEOCODE build; no reload needed.",
            ));
        }
    ...
    fn clone_selfdev_source() -> Result<std::path::PathBuf> {
        ...
        if repo_dir.exists() {
            ...
            anyhow::bail!(
                "{} exists but is not a IDEOCODE repository; move it aside and retry",
                repo_dir.display()
            );
        }
