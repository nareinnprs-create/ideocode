...
/// raise it via `IDEOCODE_LIVE_SMOKE_TIMEOUT_SECS` for slow reasoning models (e.g.
...
fn smoke_timeout(default_secs: u64) -> std::time::Duration {
    let secs = std::env::var("IDEOCODE_LIVE_SMOKE_TIMEOUT_SECS")
        .ok()
...
/// bare id IDEOCODE uses for routing and coverage keys.
///
...
/// IDEOCODE re-emits as text deltas, so we assert the runtime produced streamed
...
///
/// Some native runtimes deliver a single coalesced response that IDEOCODE re-emits
