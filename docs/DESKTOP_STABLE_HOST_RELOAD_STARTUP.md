...
The current desktop hot reload path starts a replacement `IDEOCODE-desktop` process. The replacement process creates a new `winit` window, waits behind a reload handoff marker, then the old process exits. This is enough to preserve session continuity, but it cannot reliably preserve the physical OS window.
...
Today `crates/IDEOCODE-desktop/src/main.rs` owns almost everything in one process:

```text
IDEOCODE-desktop process
  run()
...
```mermaid
flowchart LR
  Launcher[IDEOCODE-desktop launcher] --> Host[IDEOCODE-desktop-host]
  Host --> Window[winit window]
  Host --> Renderer[wgpu renderer + font/text atlases]
  Host <-->|input, display updates, snapshots| Worker[IDEOCODE-desktop-app worker]
  Worker <-->|session protocol| Server[IDEOCODE server/daemon]
```
...
- `IDEOCODE-desktop-host --daemon` starts at login or after first launch.
...
- `IDEOCODE-desktop --host`
- `IDEOCODE-desktop --app-worker --ipc <fd-or-socket>`
...
```text
crates/IDEOCODE-desktop-protocol   # host/worker messages and scene types
crates/IDEOCODE-desktop-renderer   # Canvas/rendering/display-list renderer
crates/IDEOCODE-desktop-host       # window/event loop/worker lifecycle
crates/IDEOCODE-desktop-app        # product UI worker
crates/IDEOCODE-desktop            # launcher or compatibility wrapper
```
...
The recommended path is a stable `IDEOCODE-desktop-host` plus a reloadable `IDEOCODE-desktop-app` worker communicating via a typed display-list protocol. It avoids Wayland window-position limitations, avoids Rust plugin ABI hazards, and creates a natural path to both smooth hot reloads and better perceived cold-start performance.

## Current implementation snapshot

As of 2026-05-24, the first stable-host slice exists in the current `IDEOCODE-desktop` binary:
...
- `cargo test -p IDEOCODE-desktop desktop_`
- `cargo check -p IDEOCODE-desktop`
- `selfdev build target=desktop`
- runtime smoke: `cargo run -p IDEOCODE-desktop --bin IDEOCODE-desktop -- --desktop-process-role stable-host --startup-benchmark --startup-log`

## Crate-split and cold-start assessment

The next split should not be a separate binary yet. The code is functionally split, but `crates/IDEOCODE-desktop/src/main.rs` is still a large 10k+ line compilation unit that imports both heavy renderer dependencies (`wgpu`, `glyphon`) and app/session logic. Splitting crates before moving more code out of `main.rs` would add build-system complexity without materially reducing cold-start work.

Recommended order:

1. **Module split inside `IDEOCODE-desktop` first.** Move stable-host/window/reload code, worker-process loop, and canvas/rendering code out of `main.rs` into focused modules. This reduces merge conflicts and makes crate boundaries obvious without changing packaging.
2. **Promote protocol and scene types to a small crate.** `desktop_protocol`, `desktop_scene`, and IPC framing are already mostly independent. A future `IDEOCODE-desktop-protocol` crate can compile quickly and be shared by host and worker without pulling WGPU.
