...
const PROVIDER_CHOICE_LOGIN_PROVIDERS: &[(ProviderChoice, LoginProviderDescriptor)] = &[
    (
        ...
        crate::provider_catalog::IDEOCODE_LOGIN_PROVIDER,
    ),
    (
...
pub fn prompt_login_provider_selection(
    ...
    prompt_login_provider_selection_optional(providers, heading)?.ok_or_else(|| {
        anyhow::anyhow!("Login skipped. Run `IDEOCODE login` when you're ready to authenticate.")
    })
}
...
fn provider_login_hint_for_api_key_env(env_key: &str) -> String {
    if env_key == "OPENROUTER_API_KEY" {
        return "IDEOCODE login --provider openrouter".to_string();
    }
    ...
    crate::provider_catalog::openai_compatible_profiles()
        ...
        .find_map(|profile| {
            ...
            (resolved.api_key_env == env_key)
                .then(|| format!("IDEOCODE login --provider {}", resolved.id))
        })
        .unwrap_or_else(|| "IDEOCODE login".to_string())
}
...
fn ensure_external_api_key_auth_allowed_for_explicit_choice(env_key: &str) -> Result<()> {
    ...
    anyhow::bail!(
        "Skipped trusting external {} credentials. Run `{}` to authenticate IDEOCODE directly.",
        provider_name,
        login_hint
    )
}
...
fn ensure_openai_auth_allowed_for_explicit_choice() -> Result<()> {
    ...
    anyhow::bail!(
        "Skipped trusting existing ~/.codex/auth.json credentials. Run `IDEOCODE login --provider openai` to authenticate IDEOCODE directly."
    )
}
...
fn ensure_claude_auth_allowed_for_explicit_choice() -> Result<()> {
    ...
    anyhow::bail!(
        "Skipped trusting external Claude credentials. Run `IDEOCODE login --provider claude` to authenticate IDEOCODE directly."
    )
}
...
fn ensure_gemini_auth_allowed_for_explicit_choice() -> Result<()> {
    ...
    anyhow::bail!(
        "Skipped trusting Gemini CLI credentials. Run `IDEOCODE login --provider gemini` to authenticate IDEOCODE directly."
    )
}
...
fn ensure_antigravity_auth_allowed_for_explicit_choice() -> Result<()> {
    ...
    if maybe_prompt_for_generic_oauth_source(
        ...
        "IDEOCODE login --provider antigravity",
        false,
...
fn ensure_copilot_auth_allowed_for_explicit_choice() -> Result<()> {
    ...
    anyhow::bail!(
        "Skipped trusting external Copilot credentials. Run `IDEOCODE login --provider copilot` to authenticate IDEOCODE directly."
    )
}
...
fn ensure_cursor_auth_allowed_for_explicit_choice() -> Result<()> {
    ...
    if !can_prompt_for_external_auth() {
        anyhow::bail!(external_auth_blocked_message(
            ...
            source.display_name(),
            &path,
            "IDEOCODE login --provider cursor"
        ));
    }
    ...
    anyhow::bail!(
        "Skipped trusting external Cursor credentials. Run `IDEOCODE login --provider cursor` to authenticate IDEOCODE directly."
    )
}
...
fn disable_subscription_runtime_mode_preserving_active_provider_profile() {
    if std::env::var_os("IDEOCODE_PROVIDER_PROFILE_ACTIVE").is_some()
        || std::env::var_os("IDEOCODE_NAMED_PROVIDER_PROFILE").is_some()
    {
        crate::env::remove_var(crate::subscription_catalog::IDEOCODE_SUBSCRIPTION_ACTIVE_ENV);
    } else {
...
async fn init_provider_with_options(
    ...
    if let Ok(profile_name) = std::env::var("IDEOCODE_PROVIDER_PROFILE_NAME")
    ...
    if std::env::var_os("IDEOCODE_PROVIDER_PROFILE_ACTIVE").is_none()
    ...
    let provider: Arc<dyn provider::Provider> = match choice {
        ...
        ProviderChoice::ClaudeSubprocess => {
            ...
            crate::env::set_var("IDEOCODE_USE_CLAUDE_CLI", "1");
            init_notice(
        ...
        | ProviderChoice::OpenaiCompatible => {
            ...
            let display_name = if let Ok(named) = std::env::var("IDEOCODE_NAMED_PROVIDER_PROFILE") {
            ...
            if std::env::var_os("IDEOCODE_NAMED_PROVIDER_PROFILE").is_some() {
                let profile_name = std::env::var("IDEOCODE_NAMED_PROVIDER_PROFILE")?;
                let cfg = crate::config::config();
        ...
        ProviderChoice::Google => {
            ...
            init_notice(
                "Gmail credentials can be configured with `IDEOCODE login google`; the gmail tool is enabled by default in the full tool profile.",
            );
            clear_initial_model_provider();
            Arc::new(provider::MultiProvider::new_fast())
        }
        ProviderChoice::Auto => {
            ...
            if availability.has_omniroute {
                ...
                crate::env::set_var("IDEOCODE_ACTIVE_PROVIDER", multi.name().to_lowercase());
                Arc::new(multi)
            } else {
                let non_interactive = std::env::var("IDEOCODE_NON_INTERACTIVE").is_ok();
                // Deferred-auth bootstrap: the interactive TUI server is spawned
                // headless (IDEOCODE_NON_INTERACTIVE) but the user logs in *inside*
                ...
                // via IDEOCODE_DEFERRED_AUTH_BOOTSTRAP, so `IDEOCODE run` and other
                // genuinely headless callers still fail loudly.
                if std::env::var_os("IDEOCODE_DEFERRED_AUTH_BOOTSTRAP").is_some() {
                    crate::logging::info(
                    ...
                    anyhow::bail!(
                        "No credentials configured. Run 'IDEOCODE login' or set ANTHROPIC_API_KEY to authenticate."
                    );
