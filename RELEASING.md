...
Tag push (v*)
    ...
    └─► release (after platform jobs finish)
          ...
          ├─► Update Homebrew formula (nareinnprs-create/homebrew-ideocode)
          └─► Update AUR package (IDEOCODE-bin)
```
...
- **Homebrew**: Updates `Formula/IDEOCODE.rb` in `nareinnprs-create/homebrew-ideocode` with new SHA256 hashes
- **AUR**: Updates `PKGBUILD` and `.SRCINFO` in the `IDEOCODE-bin` AUR repo
...
`cargo-zigbuild` can cross-compile pure Rust code to macOS, but IDEOCODE depends on crates that link against macOS system frameworks:
...
The bottleneck is compiling IDEOCODE itself (120k lines of Rust). Dependencies are cached and don't need recompilation. The `build.rs` timestamp causes a full recompile of the main crate on every build.
