...
pub const REGION_ENV: &str = "IDEOCODE_BEDROCK_REGION";
#[cfg(not(feature = "aws-sdk"))]
const NO_AWS_SDK_SUPPORT: &str =
    "IDEOCODE was built without AWS Bedrock support (feature `bedrock` disabled)";
...
impl BedrockProvider {
    pub fn new() -> Self {
        let model =
            std::env::var("IDEOCODE_BEDROCK_MODEL").unwrap_or_else(|_| DEFAULT_MODEL.to_string());
        let provider = Self {
    ...
    pub fn has_credentials() -> bool {
        let explicitly_enabled = std::env::var("IDEOCODE_BEDROCK_ENABLE")
            .ok()
    ...
    async fn sdk_config() -> aws_types::SdkConfig {
        ...
        if let Some(profile) = profile {
            ...
            // which could otherwise override an explicit IDEOCODE Bedrock profile.
    ...
    async fn validate_credentials_if_requested() -> Result<()> {
        let validate = std::env::var("IDEOCODE_BEDROCK_VALIDATE_STS")
            .ok()
    ...
    fn classify_error_message(raw: &str) -> String {
            ...
            || lower.contains("rate exceeded")
        {
            ...
            "AWS region is missing. Set AWS_REGION or IDEOCODE_BEDROCK_REGION."
        } else {
    ...
    fn inference_config() -> Option<InferenceConfiguration> {
        let max_tokens = std::env::var("IDEOCODE_BEDROCK_MAX_TOKENS")
            .ok()
            .and_then(|v| v.trim().parse::<i32>().ok())
            .filter(|v| *v > 0);
        let temperature = std::env::var("IDEOCODE_BEDROCK_TEMPERATURE")
            .ok()
            .and_then(|v| v.trim().parse::<f32>().ok())
            .filter(|v| (0.0..=1.0).contains(v));
        let top_p = std::env::var("IDEOCODE_BEDROCK_TOP_P")
            .ok()
            .and_then(|v| v.trim().parse::<f32>().ok())
            .filter(|v| (0.0..=1.0).contains(v));
        let stop_sequences = std::env::var("IDEOCODE_BEDROCK_STOP_SEQUENCES")
            .ok()
            .map(|v| {
                v.split(',')
                    .map(str::trim)
...
mod tests {
    ...
    async fn bedrock_live_smoke_test() {
        if std::env::var("IDEOCODE_BEDROCK_LIVE_TEST").ok().as_deref() != Some("1") {
            return;
        }
