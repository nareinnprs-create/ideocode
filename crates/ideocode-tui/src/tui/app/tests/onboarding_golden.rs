...
//   cargo test -p IDEOCODE-tui onboarding_golden -- --nocapture
...
fn onboarding_golden_walks_every_phase() {
    ...
    // 5. Suggestions (resting state).
    {
        ...
        assert!(text.contains("Welcome to IDEOCODE onboarding"), "{text}");
    }
}
...
fn onboarding_golden_walks_failure_and_async_states() {
    ...
    let assert_guided_polish = |title: &str, text: &str| {
        assert!(
            text.contains("Welcome to IDEOCODE onboarding"),
...
fn onboarding_golden_telemetry_settings_page() {
    ...
    assert!(text.contains("Helps IDEOCODE the most"), "caption 1: {text}");
    assert!(
