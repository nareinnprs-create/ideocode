...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Analyze IDEOCODE runtime memory JSONL logs for growth, spikes, attribution, and optimization hints"
    )
    parser.add_argument("paths", nargs="*", help="Specific JSONL files or directories to analyze")
    parser.add_argument(
        "--log-dir",
        help="Directory containing runtime memory JSONL logs (default: ~/.IDEOCODE/logs/memory or $IDEOCODE_HOME/logs/memory)",
    )
...
def default_log_dir() -> Path:
    IDEOCODE_home = os.environ.get("IDEOCODE_HOME")
    if IDEOCODE_home:
        return Path(IDEOCODE_home).expanduser() / "logs" / "memory"
    return Path.home() / ".IDEOCODE" / "logs" / "memory"
    ...
    if runaway_population:
        ...
        actions = [
            {
                ...
                "commands": ["IDEOCODE debug 'server:memory-incident'", "IDEOCODE debug 'swarm:list'"],
            },
            {
                "priority": 2,
            ...
            },
            {
                ...
                "commands": ["IDEOCODE debug 'allocator:purge'"],
            },
        ]
        ...
        actions = [
            {
                ...
                "commands": ["IDEOCODE debug 'allocator:purge'", "IDEOCODE debug 'server:memory-incident'"],
            },
            {
                "priority": 2,
                "action": "If retained pages repeatedly regrow, inspect allocation churn and allocator decay.",
                "commands": ["IDEOCODE debug 'allocator'", "IDEOCODE debug 'allocator:decay:1000'"],
            },
        ]
        ...
        actions = [
            {
                ...
                "commands": ["IDEOCODE debug 'server:memory'"],
            }
        ]
        ...
        actions = [
            ...
            },
            {
                ...
                "commands": [
                    "IDEOCODE debug 'allocator:profile:on'",
                    "IDEOCODE debug 'allocator:profile:dump /tmp/IDEOCODE-server.heap'",
                ],
            },
        ]
