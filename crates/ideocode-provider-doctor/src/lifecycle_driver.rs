...
impl AuthLifecycleDriver {
    ...
    pub(crate) fn run_openai_compatible_fixture(
        ...
        // (`handle_notify_auth_changed`): once the live catalog lands, IDEOCODE
    ...
    fn user_visible_transcript(
        ...
        if spec.auth_path.shows_paste_prompt() {
            ...
            transcript.push(format!(
                "**{} API key saved.**\n\nStored at `{}`.\nFetching models now. IDEOCODE will switch to an accessible model returned by the live catalog and show the catalog diff when discovery finishes.",
                spec.provider_label,
...
mod tests {
    ...
    fn live_cerebras_api_key() -> Option<LiveTestApiKey> {
        std::env::var("IDEOCODE_AUTH_LIFECYCLE_CEREBRAS_API_KEY")
            .ok()
    ...
    fn live_opencode_zen_api_key() -> Option<LiveTestApiKey> {
        if let Some(secret) = std::env::var("IDEOCODE_AUTH_LIFECYCLE_OPENCODE_API_KEY")
            .ok()
            .map(|value| value.trim().to_string())
            .filter(|value| !value.is_empty())
        {
    ...
    async fn cerebras_live_opt_in_catalog_lifecycle_uses_isolated_sandbox() {
        if !env_truthy("IDEOCODE_AUTH_LIFECYCLE_LIVE") {
            eprintln!(
                "skipping live Cerebras auth lifecycle test; set IDEOCODE_AUTH_LIFECYCLE_LIVE=1 and IDEOCODE_AUTH_LIFECYCLE_CEREBRAS_API_KEY"
            );
            return;
        }
    ...
    async fn opencode_zen_live_opt_in_tool_call_smoke() {
        if !env_truthy("IDEOCODE_OPENCODE_ZEN_LIVE_TOOL_TEST") {
            eprintln!(
                "skipping live OpenCode Zen tool-call smoke; set IDEOCODE_OPENCODE_ZEN_LIVE_TOOL_TEST=1 and provide OPENCODE_API_KEY"
            );
            return;
        }
        let api_key = live_opencode_zen_api_key().expect(
            "IDEOCODE_OPENCODE_ZEN_LIVE_TOOL_TEST=1 requires OPENCODE_API_KEY or IDEOCODE_AUTH_LIFECYCLE_OPENCODE_API_KEY",
        );
        let model = std::env::var("IDEOCODE_OPENCODE_ZEN_LIVE_TOOL_MODEL")
            .ok()
            .map(|value| value.trim().to_string())
            .filter(|value| !value.is_empty())
    ...
    async fn issue_driven_openai_compatible_live_target_smoke() {
        let Some(provider_id) = std::env::var("IDEOCODE_ISSUE_DRIVEN_LIVE_PROVIDER")
            .ok()
            .map(|value| value.trim().to_string())
            .filter(|value| !value.is_empty())
        else {
            eprintln!(
                "skipping issue-driven live provider test; set IDEOCODE_ISSUE_DRIVEN_LIVE_PROVIDER to an OpenAI-compatible profile id"
            );
            return;
        };
        ...
        let requested_model = std::env::var("IDEOCODE_ISSUE_DRIVEN_LIVE_MODEL")
            .ok()
            .map(|value| value.trim().to_string())
            .filter(|value| !value.is_empty())
            .or_else(|| resolved.default_model.clone());
        let spend_smoke = env_truthy("IDEOCODE_ISSUE_DRIVEN_LIVE_COMPLETION_SMOKE");
