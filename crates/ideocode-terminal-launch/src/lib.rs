...
pub struct TerminalCommand {
    ...
    /// Exported as `IDEOCODE_SPAWN_KIND` to spawn hooks and spawned terminals.
    pub kind: Option<String>,
    /// The IDEOCODE session this terminal will run, when known.
    /// Exported as `IDEOCODE_SPAWN_SESSION_ID`.
    pub session_id: Option<String>,
    /// Extra metadata env entries (e.g. `IDEOCODE_SPAWN_SWARM_ID`) exported to
    /// spawn hooks and spawned terminals. Applied after the first-class
    /// `IDEOCODE_SPAWN_*` keys, so entries here win on key collisions.
    ...
    /// and are also exported under a `IDEOCODE_CLIENT_*` prefix so spawn/focus
...
///
/// The IDEOCODE server process is long-lived and captures these at *its* startup,
...
fn resume_terminal_candidates_with_client_env(
    ...
    // IDEOCODE_TERMINAL and configured spawn hooks still take precedence.
...
pub fn resume_terminal_candidates() -> Vec<String> {
    let configured_terminal = std::env::var("IDEOCODE_TERMINAL").ok();
...
pub fn resume_terminal_candidates() -> Vec<String> {
    let mut candidates = Vec::new();
    if let Ok(term) = std::env::var("IDEOCODE_TERMINAL") {
        push_unique_terminal(&mut candidates, term);
    }
...
/// The `IDEOCODE_SPAWN_*` metadata env exported to spawn hooks and to terminals
/// launched by the built-in fallback:
///
/// - `IDEOCODE_SPAWN_KIND`: why this spawn happened ("resume", "selfdev",
///   "swarm-agent", ...), when known.
/// - `IDEOCODE_SPAWN_SESSION_ID`: the IDEOCODE session the window will run.
/// - `IDEOCODE_SPAWN_TITLE`: the suggested window/tab title.
/// - `IDEOCODE_SPAWN_CWD`: the working directory for the session.
/// - `IDEOCODE_SPAWN_PROGRAM`: path of the IDEOCODE binary to execute.
/// - `IDEOCODE_SPAWN_COMMAND`: the full command line, shell-escaped, for hooks
///   (like tmux) that take a single shell-command string.
///
/// `TerminalCommand::extra_env` entries (e.g. `IDEOCODE_SPAWN_SWARM_ID`,
/// `IDEOCODE_SPAWN_COORDINATOR_SESSION_ID`) are appended last and win collisions.
fn spawn_metadata_env(command: &TerminalCommand, cwd: &Path) -> Vec<(String, String)> {
    let mut env: Vec<(String, String)> = Vec::new();
    if let Some(kind) = &command.kind {
        env.push(("IDEOCODE_SPAWN_KIND".to_string(), kind.clone()));
    }
    if let Some(session_id) = &command.session_id {
        env.push(("IDEOCODE_SPAWN_SESSION_ID".to_string(), session_id.clone()));
    }
    if let Some(title) = &command.title {
        env.push(("IDEOCODE_SPAWN_TITLE".to_string(), title.clone()));
    }
    env.push((
        "IDEOCODE_SPAWN_CWD".to_string(),
        cwd.to_string_lossy().into_owned(),
    ));
    env.push((
        "IDEOCODE_SPAWN_PROGRAM".to_string(),
        command.program.to_string_lossy().into_owned(),
    ));
    env.push((
        "IDEOCODE_SPAWN_COMMAND".to_string(),
    ...
    // `IDEOCODE_CLIENT_<NAME>` alias so hooks can explicitly distinguish the
    // client's terminal from the server's.
    for (key, value) in &command.client_terminal_env {
        env.push((key.clone(), value.clone()));
        env.push((format!("IDEOCODE_CLIENT_{key}"), value.clone()));
    }
...
/// convention), so `hook --flag` becomes `hook --flag <IDEOCODE> <args...>`.
/// The hook runs in the session working directory with the full
/// `IDEOCODE_SPAWN_*` metadata env set (see [`spawn_metadata_env`]); hooks that
/// need a single shell-command string (tmux, kitty `@ launch`) can use
/// `$IDEOCODE_SPAWN_COMMAND` instead of the appended argv.
pub fn build_hook_spawn_command(
    ...
    if command.fresh_spawn {
        cmd.env("IDEOCODE_FRESH_SPAWN", "1");
    }
...
fn build_spawn_command(term: &str, command: &TerminalCommand, cwd: &Path) -> Option<Command> {
    let title = command.title.as_deref().unwrap_or("IDEOCODE");
    ...
    match term {
        ...
        "terminal" => {
            ...
            // means split/resume spawns silently fail to launch IDEOCODE. Use AppleScript's
    ...
    // not inherit this env, matching the existing IDEOCODE_FRESH_SPAWN caveat.
    for (key, value) in spawn_metadata_env(command, cwd) {
        cmd.env(key, value);
    }
...
mod tests {
    ...
    fn spawn_metadata_env_reexports_client_terminal_env_with_native_and_client_keys() {
        ...
        // inherited/stale value) and under a `IDEOCODE_CLIENT_*` alias so hooks can
        // distinguish the client's terminal from the server's.
        let command = TerminalCommand::new("/usr/local/bin/IDEOCODE", vec!["--resume".to_string()])
            .kind("swarm-agent")
            .session_id("ses_405")
        ...
        assert_eq!(
            lookup("IDEOCODE_CLIENT_ZELLIJ_SESSION_NAME").as_deref(),
            Some("sessionB")
        );
    ...
    fn spawn_metadata_env_without_client_env_has_no_client_keys() {
        ...
        let env = spawn_metadata_env(&command, Path::new("/tmp/work"));
        assert!(
            !env.iter().any(|(k, _)| k.starts_with("IDEOCODE_CLIENT_")),
            "no client terminal env should produce no IDEOCODE_CLIENT_* keys"
        );
    }
    ...
    fn missing_tmux_binary_falls_back_to_detected_emulator() {
        let _guard = ENV_LOCK.lock().unwrap();
        let previous_terminal = std::env::var_os("IDEOCODE_TERMINAL");
        unsafe {
            std::env::remove_var("IDEOCODE_TERMINAL");
        }
        let command =
            TerminalCommand::new("/usr/local/bin/IDEOCODE", vec![]).client_terminal_env(vec![
                (
                    "TMUX".to_string(),
                    "/tmp/tmux-1000/default,123,0".to_string(),
                ),
        ...
        match previous_terminal {
            Some(value) => unsafe { std::env::set_var("IDEOCODE_TERMINAL", value) },
            None => unsafe { std::env::remove_var("IDEOCODE_TERMINAL") },
        }
    ...
    fn shell_command_quotes_arguments() {
        ...
        #[cfg(unix)]
        assert_eq!(shell, "'IDEOCODE' 'it'\"'\"'s ok'");
    }
    ...
    fn windows_cmd_fallback_runs_ideocode_under_cmd_k() {
        ...
        let args: Vec<String> = cmd
            ...
            .map(|arg| arg.to_string_lossy().into_owned())
            .collect();
        assert_eq!(&args[..5], ["/C", "start", "IDEOCODE", "cmd.exe", "/K"]);
        assert!(args[5].contains(r#""C:\Program Files\IDEOCODE\IDEOCODE.exe""#));
        assert!(args[5].contains("self-dev"));
    }

    #[test]
    #[cfg(unix)]
    fn macos_terminal_inner_script_runs_IDEOCODE() {
        let command = TerminalCommand::new(
            std::path::PathBuf::from("/usr/local/bin/IDEOCODE"),
            vec!["--resume".to_string(), "abc-123".to_string()],
        );
        let script = macos_terminal_inner_script(&command, Path::new("/work/dir"));
        assert_eq!(
            script,
            "cd '/work/dir' && exec '/usr/local/bin/IDEOCODE' '--resume' 'abc-123'"
        );
        // Must actually exec IDEOCODE, not the broken `open -a Terminal <file>` form.
        assert!(script.contains("exec '/usr/local/bin/IDEOCODE'"));
    }
    ...
    fn macos_terminal_inner_script_injects_fresh_spawn() {
        let command =
            TerminalCommand::new(std::path::PathBuf::from("/usr/local/bin/IDEOCODE"), vec![])
                .fresh_spawn();
        let script = macos_terminal_inner_script(&command, Path::new("/tmp"));
        assert_eq!(
            script,
            "cd '/tmp' && exec env IDEOCODE_FRESH_SPAWN=1 '/usr/local/bin/IDEOCODE'"
        );
    }
    ...
    fn macos_terminal_applescript_uses_do_script() {
        ...
        assert!(applescript.contains("'/usr/local/bin/IDEOCODE'"));
    }
    ...
    fn hook_spawn_command_appends_program_args_and_exports_metadata() {
        ...
        .title("🦊 IDEOCODE ses_abc")
        .kind("swarm-agent")
        .session_id("ses_abc")
        .spawn_env("IDEOCODE_SPAWN_SWARM_ID", "swarm-1")
        .fresh_spawn();
        ...
        assert_eq!(
            args,
            vec!["--flag", "/usr/local/bin/IDEOCODE", "--resume", "ses_abc"]
        );
        assert_eq!(
            cmd.get_current_dir(),
        ...
        assert_eq!(
            env_value(&cmd, "IDEOCODE_SPAWN_KIND").as_deref(),
            Some("swarm-agent")
        );
        assert_eq!(
            env_value(&cmd, "IDEOCODE_SPAWN_SESSION_ID").as_deref(),
            Some("ses_abc")
        );
        assert_eq!(
            env_value(&cmd, "IDEOCODE_SPAWN_TITLE").as_deref(),
            Some("🦊 IDEOCODE ses_abc")
        );
        assert_eq!(
            env_value(&cmd, "IDEOCODE_SPAWN_CWD").as_deref(),
            Some("/work/dir")
        );
        assert_eq!(
            env_value(&cmd, "IDEOCODE_SPAWN_PROGRAM").as_deref(),
            Some("/usr/local/bin/IDEOCODE")
        );
        #[cfg(unix)]
        assert_eq!(
            env_value(&cmd, "IDEOCODE_SPAWN_COMMAND").as_deref(),
            Some("'/usr/local/bin/IDEOCODE' '--resume' 'ses_abc'")
        );
        assert_eq!(
