...
async fn removing_server_session_clears_active_pid_marker() {
    ...
    let home = tempfile::tempdir().expect("create temporary IDEOCODE_HOME");
    let _home_guard = ScopedEnvVar::set("IDEOCODE_HOME", home.path());
...
fn server_identity_reads_name_from_env() {
    let _guard = crate::storage::lock_test_env();
    let _server_name_guard = ScopedEnvVar::set("IDEOCODE_SERVER_NAME", "mount-cloud/john");
    let _server_display_name_guard = ScopedEnvVar::set("IDEOCODE_SERVER_DISPLAY_NAME", "ignored");
...
fn configure_test_env(root: &tempfile::TempDir) -> EnvGuard {
    ...
    let prev_runtime_dir = std::env::var_os("IDEOCODE_RUNTIME_DIR");
    let prev_socket = std::env::var_os("IDEOCODE_SOCKET");
    let home_dir = root.path().join("home");
    let runtime_dir = root.path().join("runtime");
    std::fs::create_dir_all(&home_dir).expect("create home dir");
    std::fs::create_dir_all(&runtime_dir).expect("create runtime dir");
    crate::env::set_var("IDEOCODE_HOME", &home_dir);
    crate::env::set_var("IDEOCODE_RUNTIME_DIR", &runtime_dir);
    crate::env::remove_var("IDEOCODE_SOCKET");
    EnvGuard {
        prev_home,
        prev_runtime_dir,
...
#[allow(
    ...
    reason = "test intentionally serializes process-wide IDEOCODE_HOME/env state across async recovery assertions"
)]
...
#[allow(
    clippy::await_holding_lock,
    reason = "test intentionally serializes process-wide IDEOCODE_HOME/env state across async startup assertions"
)]
async fn startup_ready_signal_is_not_blocked_by_headless_recovery_delay() -> Result<()> {
    ...
    let temp = tempfile::TempDir::new()?;
    let _env = configure_test_env(&temp);
    let _delay_guard = ScopedEnvVar::set("IDEOCODE_TEST_HEADLESS_STARTUP_RECOVERY_DELAY_MS", "500");
    ...
    let _ready_fd_guard = ScopedEnvVar::set("IDEOCODE_READY_FD", write_fd.to_string());
