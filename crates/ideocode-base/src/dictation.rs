...
const CLIENT_TITLE_PREFIXES: &[&str] = &["IDEOCODE:d:", "IDEOCODE:c:"];
...
pub async fn run_configured() -> Result<DictationRun> {
    ...
    if command.is_empty() {
        anyhow::bail!(
            "Dictation is not configured. Set `[dictation].command` in `~/.IDEOCODE/config.toml`."
        );
    }
...
pub fn remember_last_focused_session(session_id: &str) -> Result<()> {
    ...
    std::fs::write(&path, session_id).context("failed to persist last focused IDEOCODE session")?;
...
pub fn last_focused_session() -> Result<Option<String>> {
    ...
    let session_id = match std::fs::read_to_string(path) {
        ...
        Err(err) => return Err(err).context("failed to read last focused IDEOCODE session"),
    };
    if session_id.is_empty() {
        return Ok(None);
    }
...
fn extract_session_short_name_from_window_title(title: &str) -> Option<String> {
    let (_, rest) = title
        .split_once("IDEOCODE/")
        .or_else(|| title.split_once("IDEOCODE "))?;
