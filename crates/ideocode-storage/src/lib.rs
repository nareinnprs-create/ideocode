...
///
/// Can be overridden with `$IDEOCODE_RUNTIME_DIR`.
...
fn fallback_runtime_dir() -> PathBuf {
    std::env::temp_dir().join(format!("IDEOCODE-{}", runtime_user_discriminator()))
}
...
/// to `~/.IDEOCODE/state` (respecting `IDEOCODE_HOME`).
///
/// When `IDEOCODE_RUNTIME_DIR` is set (tests and sandboxed temp servers), it
/// takes precedence so isolated runs never touch the real IDEOCODE home.
...
/// Resolve IDEOCODE's app-owned config directory.
///
/// Default location is the platform config dir + `IDEOCODE` (for example
/// `~/.config/IDEOCODE` on Linux). When `IDEOCODE_HOME` is set, sandbox this under
/// `$IDEOCODE_HOME/config/IDEOCODE` so self-dev/tests do not leak into the user's
/// real config directory.
pub fn app_config_dir() -> Result<PathBuf> {
    if let Ok(path) = std::env::var("IDEOCODE_HOME") {
        return Ok(PathBuf::from(path).join("config").join("IDEOCODE"));
    }

    let config_dir =
        dirs::config_dir().ok_or_else(|| anyhow::anyhow!("No config directory found"))?;
    Ok(config_dir.join("IDEOCODE"))
}

/// Resolve a path under the user's home directory, but sandbox it under
/// `$IDEOCODE_HOME/external/` when `IDEOCODE_HOME` is set.
///
...
fn harden_secret_file_permissions_windows(path: &Path) {
    ...
    // Read-time hardening is opportunistic, while IDEOCODE's own secret writes
...
fn schedule_windows_path_hardening(path: &Path, directory: bool) {
    ...
    if std::thread::Builder::new()
        .name("IDEOCODE-windows-acl-harden".to_string())
        .spawn(|| {
...
///
/// IDEOCODE intentionally avoids mutating these files. We also reject obvious risky
...
mod windows_hardening_tests {
    ...
    fn first_path_starts_one_worker_and_repeated_paths_are_coalesced() {
        ...
        let now = Instant::now();
        let directory = Path::new(r"C:\Users\test\.IDEOCODE");
    ...
    fn recently_attempted_paths_are_not_requeued() {
        ...
        let file = PathBuf::from(r"C:\Users\test\.IDEOCODE\auth.json");
        state
            .files
