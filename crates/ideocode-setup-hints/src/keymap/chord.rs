...
/// [`KeyChord::normalize_key`]). Modifiers use IDEOCODE's vocabulary where the
...
impl KeyChord {
    ...
    /// IDEOCODE's cmd-first canonical order for consistency with `display`).
    ...
    /// Parse a IDEOCODE-style binding string such as `ctrl+k`, `alt+right`, or
    /// `cmd+shift+[` into a chord. Mirrors IDEOCODE's own keybinding grammar so the
    ...
    pub fn parse(raw: &str) -> Option<Self> {
        ...
        for part in raw.split('+').map(str::trim).filter(|s| !s.is_empty()) {
            match part.to_ascii_lowercase().as_str() {
                ...
                // IDEOCODE treats alt/option/meta as Alt.
    ...
    /// single vocabulary shared with IDEOCODE's own keybinding parser.
    pub fn normalize_key(raw: &str) -> String {
        let k = raw.trim().to_ascii_lowercase();
        match k.as_str() {
            // Arrows (ghostty/kitty style -> IDEOCODE style)
