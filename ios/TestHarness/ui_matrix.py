...
IOS = HERE.parent
BUNDLE = "com.IDEOCODE.mobile"
APP = IOS / ".build-ios/Build/Products/Debug-iphonesimulator/IDEOCODEMobile.app"
PORT = 7643
TOKEN = "mocktoken0123456789abcdef"
CRED = ('[{"host":"127.0.0.1","port":7643,"token":"%s","serverName":'
        '"mock-IDEOCODE","serverVersion":"mock-0.32.0","pairedAt":770000000}]' % TOKEN)
...
def build_app(device):
    ...
    r = sh(
        "xcodebuild build -project IDEOCODEMobile.xcodeproj -scheme IDEOCODEMobile "
...
def seed_and_launch(device, collect_perf=True):
    ...
    (appsup / "IDEOCODE-servers.json").write_text(CRED + "\n")
