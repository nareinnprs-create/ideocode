...
$testRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("IDEOCODE-windows-setup-eval-{0}" -f ([guid]::NewGuid().ToString('N')))
...
$envNames = @(
    ...
    'TMP',
    'PATH',
    'IDEOCODE_INSTALL_PS1_IMPORT_ONLY',
    'IDEOCODE_UNINSTALL_PS1_IMPORT_ONLY',
    'IDEOCODE_SKIP_SERVER_RELOAD',
    'IDEOCODE_INSTALL_SKIP_BINARY_VALIDATION',
    'IDEOCODE_DISABLE_ENV_BROADCAST',
    'IDEOCODE_WINDOWS_SETUP_SKIP_EXTERNALS'
)
$originalEnv = @{}
...
function Assert-PathCount([string]$PathValue, [string]$Entry, [int]$ExpectedCount, [string]$Message) {
    $entryKey = ConvertTo-IDEOCODEPathKey $Entry
    $count = 0
    foreach ($candidate in (Split-IDEOCODEPathList $PathValue)) {
        if ((ConvertTo-IDEOCODEPathKey $candidate) -eq $entryKey) { $count += 1 }
    }
...
function New-IsolatedWindowsProfile([string]$Name) {
    ...
    $IDEOCODEHome = Join-Path $root 'IDEOCODE Home'
    $temp = Join-Path $root 'Temp'
    foreach ($dir in @($local, $profile, $appData, $IDEOCODEHome, $temp)) {
    ...
    $env:APPDATA = $appData
    $env:IDEOCODE_HOME = $IDEOCODEHome
    $env:TEMP = $temp
    $env:TMP = $temp
    ...
    return [pscustomobject]@{
        ...
        UserProfile = $profile
        AppData = $appData
        IDEOCODEHome = $IDEOCODEHome
        Temp = $temp
        InstallDir = Join-Path $local 'IDEOCODE\bin'
        LauncherPath = Join-Path $local 'IDEOCODE\bin\IDEOCODE.exe'
        BuildsDir = Join-Path $local 'IDEOCODE\builds'
        SetupHintsPath = Join-Path $IDEOCODEHome 'setup_hints.json'
        HotkeyDir = Join-Path $IDEOCODEHome 'hotkey'
        StartupShortcutPath = Join-Path $appData 'Microsoft\Windows\Start Menu\Programs\Startup\IDEOCODE-hotkey.lnk'
    }
}

function Set-InstallScriptProfileGlobals($Profile) {
    $script:IDEOCODEHome = $Profile.IDEOCODEHome
...
try {
    $env:IDEOCODE_INSTALL_PS1_IMPORT_ONLY = '1'
    $env:IDEOCODE_SKIP_SERVER_RELOAD = '1'
    $env:IDEOCODE_INSTALL_SKIP_BINARY_VALIDATION = '1'
    . $installScript -SkipAlacrittySetup -SkipHotkeySetup

    Invoke-Case 'release_lookup_avoids_unauthenticated_github_api' {
        Assert-Equal 'v1.2.3' (Resolve-IDEOCODEReleaseTagFromUri 'https://github.com/1jehuang/IDEOCODE/releases/tag/v1.2.3') 'release redirect parser should extract the stable tag'
        Assert-Equal 'v1.2.3-rc.1' (Resolve-IDEOCODEReleaseTagFromUri 'https://github.com/1jehuang/IDEOCODE/releases/tag/v1.2.3-rc.1?source=latest') 'release redirect parser should stop before query parameters'
        Assert-Equal $true (Test-IDEOCODEReleaseTag 'v1.2.3') 'stable release tags should validate'
        Assert-Equal $false (Test-IDEOCODEReleaseTag 'latest') 'unversioned release labels should not validate'
        ...
        Assert-Contains $scriptText 'IDEOCODE.sh/releases' 'installer should include independent static release metadata'
        ...
        function Invoke-WebRequest {
            ...
            if ($Uri -eq 'https://github.com/1jehuang/IDEOCODE/releases/latest') {
                throw 'simulated GitHub block'
            }
            throw "unexpected URI: $Uri"
        }
        try {
            Assert-Equal 'v1.2.3' (Get-LatestIDEOCODEReleaseTag) 'static metadata should cover a blocked GitHub release lookup'
            $bases = @(Get-IDEOCODEReleaseDownloadBases 'v1.2.3')
            Assert-Equal 'https://mirror.example/releases/v1.2.3' $bases[0] 'configured mirror should be preferred'
            Assert-Equal 'https://github.com/1jehuang/IDEOCODE/releases/download/v1.2.3' $bases[1] 'GitHub should remain the final fallback'
        } finally {
    ...
    Invoke-Case 'path_persistence_and_deduplication' {
        ...
        $pathUpdate = Resolve-IDEOCODEPathUpdate -InstallDir $profile.InstallDir -CurrentPath $currentPath
        ...
        Assert-PathCount $pathUpdate.Path $profile.InstallDir 1 'updated PATH should contain exactly one IDEOCODE launcher dir'
        Assert-Equal 2 $pathUpdate.RemovedManagedEntries 'PATH update should remove both stale IDEOCODE launcher entries before re-adding one'
        ...
        $first = Set-IDEOCODEUserPath -InstallDir $profile.InstallDir -CurrentPath 'C:\Tools' -SetUserPathAction $setPathAction -BroadcastAction $broadcastAction
        ...
        $second = Set-IDEOCODEUserPath -InstallDir $profile.InstallDir -CurrentPath $script:appliedPath -SetUserPathAction $setPathAction -BroadcastAction $broadcastAction
    ...
    Invoke-Case 'wm_settingchange_broadcast_contract' {
        ...
        $env:IDEOCODE_DISABLE_ENV_BROADCAST = '1'
        Assert-Equal $false (Send-IDEOCODEEnvironmentChangedBroadcast) 'broadcast helper should honor the deterministic no-mutate opt-out'
        $env:IDEOCODE_DISABLE_ENV_BROADCAST = $null
    ...
    function Set-IDEOCODEUserPath {
        ...
        $update = Resolve-IDEOCODEPathUpdate -InstallDir $InstallDir -CurrentPath $script:mockUserPath
        if ($update.Changed) {
    ...
    Invoke-Case 'clean_install_isolated_profile_and_opt_out' {
        ...
        $source = Join-Path $profile.Root 'IDEOCODE-v1.exe'
        ...
        Assert-PathExists (Join-Path $profile.BuildsDir 'stable\IDEOCODE.exe') 'clean install should populate the stable build channel'
        Assert-PathExists (Join-Path $profile.BuildsDir 'versions\0.0.1-eval\IDEOCODE.exe') 'clean install should populate the immutable versioned build'
        ...
        Assert-PathExists $profile.SetupHintsPath 'clean install should write setup hints into isolated IDEOCODE_HOME'
    ...
    Invoke-Case 'upgrade_and_idempotency_do_not_duplicate_path' {
        ...
        Set-InstallScriptProfileGlobals $profile
        $sourceV1 = Join-Path $profile.Root 'IDEOCODE-v1.exe'
        $sourceV2 = Join-Path $profile.Root 'IDEOCODE-v2.exe'
    ...
    Invoke-Case 'copilot_key_mapping_and_spaces_non_ascii_paths' {
        ...
        Set-InstallScriptProfileGlobals $profile
        $env:IDEOCODE_WINDOWS_SETUP_SKIP_EXTERNALS = '1'
        $IDEOCODEExe = Join-Path $profile.Root '路径 With Spaces\IDEOCODE.exe'
        New-Item -ItemType Directory -Path (Split-Path -Parent $IDEOCODEExe) -Force | Out-Null
        Set-Content -Path $IDEOCODEExe -Value 'fake exe' -NoNewline
        New-Item -ItemType Directory -Path $profile.HotkeyDir -Force | Out-Null
        Set-Content -Path (Join-Path $profile.HotkeyDir 'IDEOCODE-hotkey.ps1') -Value 'legacy listener' -Force

        $ok = Install-IDEOCODEHotkey -IDEOCODEExePath $IDEOCODEExe
        Assert-Equal $true $ok 'hotkey install should succeed using the deterministic external-command skip hook'
        $vbsPath = Join-Path $profile.HotkeyDir 'IDEOCODE-hotkey-launcher.vbs'
        Assert-PathMissing $vbsPath 'hotkey install should remove the legacy hidden VBScript trampoline'
        $shortcutScriptPath = Join-Path $profile.HotkeyDir 'IDEOCODE-hotkey-shortcut.ps1'
        Assert-PathExists $shortcutScriptPath 'hotkey install should render the deterministic Startup shortcut script under the isolated IDEOCODE_HOME'
        ...
        Assert-Contains $shortcutScript $IDEOCODEExe 'Startup shortcut should preserve spaces and non-ASCII characters in the IDEOCODE path'
        Assert-PathMissing (Join-Path $profile.HotkeyDir 'IDEOCODE-hotkey.ps1') 'hotkey upgrade should remove the legacy PowerShell listener'
        $scriptText = Get-Content -LiteralPath $installScript -Raw
    ...
    Invoke-Case 'missing_windows_terminal_is_not_required' {
        ...
        $IDEOCODEExe = Join-Path $profile.Root 'No Windows Terminal\IDEOCODE.exe'
        New-Item -ItemType Directory -Path (Split-Path -Parent $IDEOCODEExe) -Force | Out-Null
        Set-Content -Path $IDEOCODEExe -Value 'fake exe' -NoNewline

        Assert-Equal $true (Install-IDEOCODEHotkey -IDEOCODEExePath $IDEOCODEExe) 'hotkey setup should not require Windows Terminal to be installed or active'
        $scriptText = Get-Content -LiteralPath $installScript -Raw
    ...
    Invoke-Case 'launcher_rollback_failure_preserves_existing_install' {
        ...
        $threw = $false
        try {
            Install-IDEOCODELauncher -SourcePath $missingSource -LauncherPath $profile.LauncherPath | Out-Null
        } catch {
            $threw = $true
        }
        ...
        $tempLaunchers = @(Get-ChildItem -LiteralPath $profile.InstallDir -Filter '.IDEOCODE-launcher-*.tmp.exe' -Force -ErrorAction SilentlyContinue)
    ...
    $env:IDEOCODE_UNINSTALL_PS1_IMPORT_ONLY = '1'
    . $uninstallScript
    ...
    function Remove-IDEOCODEUserPath {
        ...
        $update = Resolve-IDEOCODEPathRemoval -InstallDir $InstallDir -CurrentPath $script:uninstallUserPath
        if ($update.Changed) {
    ...
    Invoke-Case 'uninstall_guards_purge_paths_and_process_scope' {
        ...
        Assert-Equal $true (Test-IDEOCODESafePurgePath $profile.IDEOCODEHome) 'dedicated IDEOCODE data directories should be purgeable'
        Assert-Equal $false (Test-IDEOCODESafePurgePath $profile.UserProfile) 'the user profile must never be accepted as a purge target'
        Assert-Equal $false (Test-IDEOCODESafePurgePath $profile.Root) 'a parent workspace must never be accepted as a purge target'
        Assert-Equal $true (Test-IDEOCODEManagedExecutablePath -ExecutablePath $profile.LauncherPath -LauncherPath $profile.LauncherPath -BuildsDir $profile.BuildsDir) 'the installed launcher should be recognized as managed'
        Assert-Equal $true (Test-IDEOCODEManagedExecutablePath -ExecutablePath (Join-Path $profile.InstallDir '.IDEOCODE-launcher-old-a1b2c3.exe') -LauncherPath $profile.LauncherPath -BuildsDir $profile.BuildsDir) 'a renamed live-upgrade launcher should be recognized as managed'
        Assert-Equal $false (Test-IDEOCODEManagedExecutablePath -ExecutablePath (Join-Path $profile.InstallDir 'other-tool.exe') -LauncherPath $profile.LauncherPath -BuildsDir $profile.BuildsDir) 'unrelated executables beside the launcher must not be terminated'
        Assert-Equal $true (Test-IDEOCODEManagedExecutablePath -ExecutablePath (Join-Path $profile.BuildsDir 'stable\IDEOCODE.exe') -LauncherPath $profile.LauncherPath -BuildsDir $profile.BuildsDir) 'installed version binaries should be recognized as managed'
        Assert-Equal $false (Test-IDEOCODEManagedExecutablePath -ExecutablePath (Join-Path $profile.Root 'development\IDEOCODE.exe') -LauncherPath $profile.LauncherPath -BuildsDir $profile.BuildsDir) 'unrelated development binaries must not be terminated'
    }
    ...
    Invoke-Case 'uninstall_cleanup_removes_binaries_path_and_keeps_user_data' {
        ...
        New-Item -ItemType Directory -Path $profile.IDEOCODEHome -Force | Out-Null
        New-Item -ItemType Directory -Path $profile.HotkeyDir -Force | Out-Null
        ...
        $oldLauncherPath = Join-Path $profile.InstallDir '.IDEOCODE-launcher-old-a1b2c3.exe'
        Set-Content -Path $oldLauncherPath -Value 'previous running launcher' -NoNewline
        Set-Content -Path (Join-Path $profile.BuildsDir 'stable\IDEOCODE.exe') -Value 'stable build' -NoNewline
        Set-Content -Path (Join-Path $profile.IDEOCODEHome 'config.toml') -Value 'kept = true' -NoNewline
        Set-Content -Path (Join-Path $profile.HotkeyDir 'IDEOCODE-hotkey.ps1') -Value 'legacy listener' -NoNewline
        ...
        $exitCode = Invoke-IDEOCODEUninstall -InstallDir $profile.InstallDir -Yes
        ...
        Assert-PathMissing (Join-Path $profile.HotkeyDir 'IDEOCODE-hotkey.ps1') 'uninstall should remove legacy launch-hotkey artifacts'
        Assert-PathExists (Join-Path $profile.IDEOCODEHome 'config.toml') 'uninstall without -Purge should keep user data'
        ...
        Assert-Equal 'C:\Keep' $script:uninstallUserPath 'uninstall should remove all IDEOCODE-managed PATH variants and keep unrelated entries'
