          ...
          <>
            <div className="grid grid-cols-2 gap-2">
              <div className="p-2.5 rounded-lg bg-surface-elevated border border-border-subtle">
                <div className="text-[10px] text-fg-muted uppercase tracking-wider">Tokens</div>
                <div className="text-lg font-semibold text-fg-primary mt-0.5">{formatTokens(totalTokens)}</div>
              </div>
              <div className="p-2.5 rounded-lg bg-surface-elevated border border-border-subtle">
                <div className="text-[10px] text-fg-muted uppercase tracking-wider">Cost</div>
                <div className="text-lg font-semibold text-fg-primary mt-0.5">${estimatedCost.toFixed(2)}</div>
              </div>
              <div className="p-2.5 rounded-lg bg-surface-elevated border border-border-subtle">
                <div className="text-[10px] text-fg-muted uppercase tracking-wider flex items-center gap-1">
                  <MessageSquare size={9} /> Messages
                </div>
                <div className="text-lg font-semibold text-fg-primary mt-0.5">{messageCount}</div>
              </div>
              <div className="p-2.5 rounded-lg bg-surface-elevated border border-border-subtle">
                <div className="text-[10px] text-fg-muted uppercase tracking-wider">Input / Output</div>
            ...
            <div className="grid grid-cols-3 gap-2">
              <div className="p-2 rounded-lg bg-surface-elevated border border-border-subtle text-center">
                <Calendar size={12} className="mx-auto text-fg-muted mb-0.5" />
                <div className="text-sm font-semibold text-fg-primary">{activeDays}</div>
                <div className="text-[9px] text-fg-muted">Active Days</div>
              </div>
              <div className="p-2 rounded-lg bg-surface-elevated border border-border-subtle text-center">
                <MessageSquare size={12} className="mx-auto text-fg-muted mb-0.5" />
                <div className="text-sm font-semibold text-fg-primary">{sessionCount}</div>
                <div className="text-[9px] text-fg-muted">Sessions</div>
              </div>
              <div className="p-2 rounded-lg bg-surface-elevated border border-border-subtle text-center">
                <Flame size={12} className="mx-auto text-fg-muted mb-0.5" />
                <div className="text-sm font-semibold text-fg-primary">{streak}</div>
                <div className="text-[9px] text-fg-muted">Streak</div>
              </div>
            </div>
            ...
            {dailyData.length > 0 && (
              <div>
                <div className="text-[11px] font-medium text-fg-secondary mb-2">Daily Token Trend</div>
                <div className="flex items-end gap-1 h-16">
                  {dailyData.map(([day, tokens]) => (
                    <div key={day} className="flex-1 flex flex-col items-center gap-0.5 min-w-0">
                      ...
                      />
                      <span className="text-[7px] text-fg-muted truncate w-full text-center">{day}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {modelUsage.length > 0 && (
              <div>
                <div className="text-[11px] font-medium text-fg-secondary mb-2">Model Usage Ranking</div>
                  ...
                  {modelUsage.map(([model, tokens], i) => (
                    <div key={model} className="space-y-0.5">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[11px] text-fg-primary flex-1 truncate">{model}</span>
                        <span className="text-[10px] text-fg-muted font-mono">{formatTokens(tokens)}</span>
                      </div>
                      <div className="w-full h-1 rounded bg-surface-elevated overflow-hidden">
                        <div
            ...
            <div>
              <div className="text-[11px] font-medium text-fg-secondary mb-2">Token Breakdown</div>
            ...
            <div>
              <div className="text-[11px] font-medium text-fg-secondary mb-2">Cost Breakdown</div>
              <div className="space-y-1.5">
                <div className="flex items-center gap-2">
                  <span className="text-[11px] text-fg-primary flex-1">Input cost (prompt)</span>
                  <span className="text-[10px] text-fg-muted font-mono">
                    ${((inputTokens / 1000) * 0.001).toFixed(4)}
                  </span>
                </div>
                <div className="w-full h-1.5 rounded bg-surface-elevated overflow-hidden">
                  <div
                ...
                <div className="flex items-center gap-2">
                  <span className="text-[11px] text-fg-primary flex-1">Output cost (completion)</span>
                  <span className="text-[10px] text-fg-muted font-mono">
