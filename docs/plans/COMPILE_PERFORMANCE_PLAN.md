...
- Warm `scripts/dev_cargo.sh build --release -p IDEOCODE --bin IDEOCODE --quiet`: **~47.3s**
...
- `clang + lld` links the release `IDEOCODE` binary successfully here.
...
- Use `scripts/dev_cargo.sh` for local self-dev builds:
  ...
  - uses the dedicated Cargo `selfdev` profile for `IDEOCODE` self-dev build/reload paths
  - can still opt into `mold` via `IDEOCODE_FAST_LINKER=mold`
...
- cold self-dev `IDEOCODE` build
- warm touched-file self-dev `IDEOCODE` build
...
- 2026-03-30: removed the per-build `build.rs` timestamp/build-number churn from local source
  builds. `IDEOCODE_VERSION` for source builds is now stable per `Cargo.toml` version + git hash,
  while UI/version build-time display comes from the binary mtime at runtime. Validation on this
  machine: two no-op release-IDEOCODE runs measured **221.688s then 0.559s**, confirming the main
...
- 2026-04-09: introduced a dedicated Cargo `selfdev` profile for self-dev iteration. On this
  machine, the warm local `IDEOCODE` self-dev build path dropped from about **56.1s** for
  `scripts/dev_cargo.sh build --release -p IDEOCODE --bin IDEOCODE --quiet` to about **16.0s** for
  `scripts/dev_cargo.sh build --profile selfdev -p IDEOCODE --bin IDEOCODE --quiet`, while keeping the
...
- 2026-04-18: added `scripts/bench_selfdev_checkpoints.sh` to standardize cold/warm self-dev
  ...
  - warm `selfdev-IDEOCODE` touched-file measurement on `src/tool/read.rs` failed because the
    `sccache`-wrapped rustc process terminated with signal 15 during the `IDEOCODE` crate build
...
- 2026-04-18: updated `scripts/bench_selfdev_checkpoints.sh` to keep running after individual
  ...
  - warm touched-file `selfdev-IDEOCODE` build: **59.898s**
...
- 2026-04-18: added `--skip-cold` to `scripts/bench_selfdev_checkpoints.sh` so warm-only
  ...
  - warm touched-file `selfdev-IDEOCODE` build: **18.844s**
...
- 2026-04-18: additional warm-only checkpoint on a broader shared edit target with
  ...
  - warm touched-file `selfdev-IDEOCODE` build: **18.969s**
- 2026-04-18: additional warm-only checkpoint on a heavy tool-path file with
  ...
  - warm touched-file `selfdev-IDEOCODE` build: **21.400s**
- 2026-04-18: additional warm-only checkpoint on a provider-heavy file with
  ...
  - warm touched-file `selfdev-IDEOCODE` build: **21.386s**
- 2026-04-18: additional warm-only checkpoint on the shared provider module with
  ...
  - warm touched-file `selfdev-IDEOCODE` build: **17.917s**
- 2026-04-18: additional warm-only checkpoint on the agent entry module with
  ...
  - warm touched-file `selfdev-IDEOCODE` build: **30.928s**
- 2026-04-18: additional warm-only checkpoint on the memory tool with
  ...
  - warm touched-file `selfdev-IDEOCODE` build: **12.798s**
- 2026-04-18: additional warm-only checkpoint on session search with
  ...
  - warm touched-file `selfdev-IDEOCODE` build: **12.874s**
- 2026-04-18: additional warm-only checkpoint on the browser tool with
  ...
  - warm touched-file `selfdev-IDEOCODE` build: **18.874s**
- 2026-04-28: diagnosed the repeated self-dev `IDEOCODE` lib build `SIGTERM` on this 16 GiB,
  ...
  `IDEOCODE_SELFDEV_LOW_MEMORY=off` to disable, or `IDEOCODE_SELFDEV_LOW_MEMORY=on` to force. Initial
...
- 2026-05-21: rechecked the same failure mode during overnight TUI test triage. `journalctl -u
  earlyoom` showed repeated `SIGTERM` events against root `IDEOCODE` `rustc` processes at about
...
- 2026-05-05: trimmed root compile surface by replacing broad `tokio/full` with explicit used
  features, aligning IDEOCODE-owned `crossterm` dependencies on 0.29, and replacing `qr2term` with
  direct `qrcode` rendering. This removed the duplicate `crossterm 0.28` path from the `IDEOCODE`
  tree while preserving login QR output. Validation: `cargo check --profile selfdev -p IDEOCODE --bin
  IDEOCODE`, `cargo test --profile selfdev login_qr --lib -- --nocapture`, and coordinated
  `selfdev build` passed.
- 2026-05-05: removed unused `reqwest/blocking` from `IDEOCODE-provider-core`; static search showed
  no blocking API usage in that crate. Validation: `cargo check --profile selfdev -p
  IDEOCODE-provider-core` and full `cargo check --profile selfdev -p IDEOCODE --bin IDEOCODE` passed.
- 2026-05-03: added `IDEOCODE_DEV_FEATURE_PROFILE` to `scripts/dev_cargo.sh` so compile-speed probes and
  ...
  this machine: `IDEOCODE_DEV_FEATURE_PROFILE=minimal scripts/dev_cargo.sh check -p IDEOCODE --lib --quiet` passed.
- 2026-05-03: disabled Cargo auto-discovery for root binary targets and moved developer-only helper
  ...
  explicit probe coverage via `cargo check --all-targets -p IDEOCODE --features dev-bins`. Validation showed
  `cargo check --all-targets -p IDEOCODE` skips those three bins, while adding `--features dev-bins` includes them.
- 2026-05-03: moved the self-dev build/version/channel support implementation out of the root crate and
  into `crates/IDEOCODE-build-support`, leaving `src/build.rs` as a re-export facade. This cuts another
  stable, high-fanout support subsystem out of the root compile unit while preserving existing call sites
  (`crate::build::*`). Validation: `cargo check -p IDEOCODE-build-support`, `cargo test -p IDEOCODE-build-support`,
  and `cargo check -p IDEOCODE --lib` passed during the split.
- 2026-05-03: moved the pure keybinding parser/matcher/types from `src/tui/keybind.rs` into
  `IDEOCODE-tui-core::keybind`, leaving root TUI config-loading wrappers in place. This creates a reusable
  cache boundary for a low-coupling TUI helper module while preserving the existing `crate::tui::keybind::*`
  API. Validation: `cargo check -p IDEOCODE-tui-core`, `cargo test -p IDEOCODE-tui-core`, and
  `cargo check -p IDEOCODE --lib` passed.

Warm-only touched-file checkpoints captured so far on this machine:

| Touched file | Warm `cargo check` | Warm `selfdev-IDEOCODE` build |
| --- | ---: | ---: |
...
- warm touched-file `selfdev-IDEOCODE` build: **12.798s to 30.928s**
...
- 2026-03-24: moved the heavy ONNX/tokenizer implementation into the new
  `crates/IDEOCODE-embedding` workspace crate.
...
- 2026-05-23: reverted that default-feature split because embedding-backed
  ...
  need compile-speed probes can use `IDEOCODE_DEV_FEATURE_PROFILE=minimal` or
  `IDEOCODE_DEV_FEATURE_PROFILE=pdf` to skip the local inference stack. Full local
  inference remains available explicitly via `--features embeddings` or
  `IDEOCODE_DEV_FEATURE_PROFILE=full` when testing non-default feature paths.
  Validation target: `cargo tree -p IDEOCODE --edges normal --depth 1` should
  include both `IDEOCODE-pdf` and `IDEOCODE-embedding`; `--no-default-features` should
  include neither.

- 2026-03-24: moved PDF extraction behind the new `crates/IDEOCODE-pdf` workspace
...
- 2026-03-24: moved Azure bearer-token retrieval behind the new
  `crates/IDEOCODE-azure-auth` workspace crate so the Azure SDK no longer lives
...
- 2026-03-24: moved email notification / IMAP reply transport behind the new
  `crates/IDEOCODE-notify-email` workspace crate.
...
- 2026-03-25: landed the first provider boundary slice with
  `crates/IDEOCODE-provider-metadata`.
...
- 2026-03-25: landed the next provider-core slice with `crates/IDEOCODE-provider-core`.
...
- 2026-03-25: landed the first provider-implementation support crate with
  `crates/IDEOCODE-provider-openrouter`.
...
- 2026-03-25: landed the next provider-implementation support crate with
  `crates/IDEOCODE-provider-gemini`.
...
- 2026-03-30: moved the pure OpenAI tool-schema normalization helpers into
  `crates/IDEOCODE-provider-core/src/openai_schema.rs`.
...
- 2026-05-05: moved provider catalog-refresh diffing into
  `IDEOCODE-provider-core::catalog_refresh` and re-exported it from the root provider facade.
...
- 2026-05-05: split the stable provider pricing tables/helpers into
  `IDEOCODE-provider-core::pricing`, leaving `src/provider/pricing.rs` as a thin facade for root-only
...
- Validation: `cargo test -p IDEOCODE-provider-core --quiet`, `cargo test -p IDEOCODE pricing:: --quiet`,
  `cargo check -p IDEOCODE --quiet`, and `cargo check -p IDEOCODE --features embeddings --quiet` pass.
- 2026-05-05: moved provider failover prompt/decision/classifier contracts and provider
  selection/fallback-order contracts into `IDEOCODE-provider-core`, leaving root provider modules as
  facades for env/runtime/account state. This continues shrinking `src/provider/mod.rs` support
  surfaces toward an eventual `IDEOCODE-provider` runtime crate.
- Validation: `cargo test -p IDEOCODE-provider-core --quiet`, focused root provider selection/failover
  tests, and `cargo check -p IDEOCODE --quiet` pass.
- 2026-05-05: moved the Copilot `PremiumMode` provider-control enum into `IDEOCODE-provider-core`
...
- Validation: `cargo check -p IDEOCODE-provider-core --quiet` and `cargo check -p IDEOCODE --quiet` pass.
- 2026-05-05: moved provider-native tool result DTOs/sender aliases into `IDEOCODE-provider-core`.
...
- 2026-05-05: moved stable provider model constants, static provider/model classification,
  ...
  `IDEOCODE-provider-core::models`. Root `src/provider/models.rs` now layers dynamic account catalogs,
  runtime availability, and cache hydration on top of those core helpers.
- Validation: `cargo test -p IDEOCODE-provider-core models:: --quiet`,
  `cargo check -p IDEOCODE-provider-core --quiet`, and `cargo check -p IDEOCODE --quiet` pass.
- 2026-05-05: moved the global `Provider` trait and `EventStream` alias into `IDEOCODE-provider-core`.
  ...
  future `IDEOCODE-provider` runtime crate can be introduced safely.
- Validation: `cargo check -p IDEOCODE-provider-core --quiet` and `cargo check -p IDEOCODE --quiet` pass.
...
- 2026-05-05: moved the stable provider-facing `ToolDefinition` contract from `src/message.rs` into
  `IDEOCODE-message-types` and re-exported it from the root message facade. This is a prerequisite for
  shrinking the provider trait and tool registry surfaces away from root-crate-only message types.
- Validation: `cargo test -p IDEOCODE-message-types --quiet` and `cargo check -p IDEOCODE --quiet` pass.
- 2026-05-05: introduced `IDEOCODE-tool-types` for stable tool execution output DTOs and moved
...
- Validation: `cargo check -p IDEOCODE-tool-types --quiet`, `cargo test -p IDEOCODE-tool-types --quiet`,
  and `cargo check -p IDEOCODE --quiet` pass.
- 2026-05-05: added `IDEOCODE-tool-core` for runtime tool contracts and moved `Tool`, `ToolContext`,
  `ToolExecutionMode`, and `StdinInputRequest` out of `src/tool/mod.rs`. `IDEOCODE-tool-types` stays
...
- 2026-05-05: also moved the shared tool intent schema helper into `IDEOCODE-tool-core`, keeping the
  root `src/tool/mod.rs` module focused on registry composition rather than shared schema contracts.
- Validation: `cargo check -p IDEOCODE-tool-core --quiet`, `cargo check -p IDEOCODE-tool-types --quiet`,
  and `cargo check -p IDEOCODE --quiet` pass.
- 2026-05-05: moved provider streaming contracts `StreamEvent` and `ConnectionPhase` from
  `src/message.rs` into `IDEOCODE-message-types`, again preserving root facade re-exports. Together
  with `ToolDefinition`, this materially reduces the root-only surface of the provider trait and
  prepares a future `IDEOCODE-provider` crate.
- Validation: `cargo check -p IDEOCODE-message-types --quiet`, `cargo test -p IDEOCODE-message-types --quiet`,
  and `cargo check -p IDEOCODE --quiet` pass.
- 2026-05-05: moved core conversation DTOs `Message`, `ContentBlock`, `Role`, and `CacheControl`
  into `IDEOCODE-message-types`, while keeping root-only redaction/generated-image/session helpers in
...
- 2026-05-05: moved pure message helpers for fresh-user-turn detection, stable message hashing,
  tool ID sanitization, and the missing-tool-output constant into `IDEOCODE-message-types`. Root keeps
  secret redaction and generated-image visual context because those still depend on regex/env/fs/base64
  integration details.
- Validation: `cargo check -p IDEOCODE-message-types --quiet`, focused root message helper tests, and
  `cargo check -p IDEOCODE --quiet` pass.
- 2026-05-05: moved the provider split-system dynamic-context insertion helper and its tests into
  `IDEOCODE-message-types`. This removes another pure message transformation from `src/provider/mod.rs`
  and keeps preparing the provider trait for an eventual runtime crate split.
- Validation: `cargo test -p IDEOCODE-message-types dynamic_context --quiet`,
  `cargo check -p IDEOCODE-message-types --quiet`, and `cargo check -p IDEOCODE --quiet` pass.

- 2026-05-05: moved the server lightweight-control request classifier from
  `src/server/client_lifecycle.rs` into `IDEOCODE-protocol::Request::is_lightweight_control_request`.
...
- Validation: `cargo check -p IDEOCODE-protocol --quiet` and `cargo check -p IDEOCODE --quiet` pass.
- 2026-05-05: moved swarm task-control action parsing, assignment-message formatting, and status
  eligibility/error policy from `src/server/comm_control.rs` into `IDEOCODE-plan`. This keeps plan/task
...
- Validation: `cargo test -p IDEOCODE-plan --quiet` and `cargo check -p IDEOCODE --quiet` pass.

- 2026-03-30: moved the workspace-map subsystem into the new `crates/IDEOCODE-tui-workspace` crate.
...
- Reason: this is a safe first `IDEOCODE-tui` foothold because the workspace map code is already mostly self-contained and
...
- 2026-03-30: landed the first prompt-overlay seam for system-prompt customization without a rebuild.
  IDEOCODE now loads `~/.IDEOCODE/prompt-overlay.md` and `./.IDEOCODE/prompt-overlay.md` into the
...
- a future `IDEOCODE-provider-core` / provider-implementation split is still the most promising next
...
- **Done:** `IDEOCODE-provider-metadata` for stable login/profile catalog data and pure selection logic.
- **Done:** `IDEOCODE-provider-core` for shared HTTP client plus route/cost/core provider value types.
- **Done:** `IDEOCODE-provider-openrouter` for OpenRouter-specific catalog/cache/ranking/model-spec support.
- **Done:** `IDEOCODE-provider-gemini` for Gemini Code Assist schema/types and pure model support helpers.
- **Done:** `IDEOCODE-provider-core::openai_schema` for pure OpenAI schema adaptation / strict-normalization helpers.
...
- **Done:** `IDEOCODE-tui-workspace` for workspace-map model + widget rendering.
- **Not done yet:** broader `IDEOCODE-tui` extraction for markdown, mermaid, info widgets, and the shared renderer.
...
2. **Break the cheap back-edges first.** Eliminate the 1-2 ref couplings (image helpers, single config
   ...
   (`IDEOCODE-core`, `IDEOCODE-tui-*`) or inverting them behind small traits. Re-run the analyzer; watch the
   SCC shrink.
...
```
IDEOCODE (root: cli + bin)        depends on
  -> IDEOCODE-tui (tui + video_export)   depends on
       -> IDEOCODE-app-core (server/tool/agent SCC + leaves)  depends on
            -> IDEOCODE-base (provider/auth/config/session/message/memory foundation)
```
...
| IDEOCODE-base | 1.126 GiB | FLOOR: bottom crate, fewest internal deps |
| IDEOCODE-app-core | 1.176 GiB | base + 0.050 |
| IDEOCODE-tui | 1.280 GiB | app-core + 0.104 (98K loc adds only +0.104) |
| IDEOCODE (root cli) | 0.664 GiB | thin shell, fast incremental for cli iteration |
...
`4aec863e` (Phase B), `f649daeb` (test import), `85c96735` (Phase C IDEOCODE-tui), `2591c0e5` (test-support
...
A further split of `IDEOCODE-tui` (the current 1.280 GiB max) was analyzed and deliberately **not** done:
...
- **But the peak is floor-bound, not tui-bound.** `IDEOCODE-base` alone is already 1.126 GiB because every
...
2. **Stopped git activity from forcing full-tree recompiles** (`crates/IDEOCODE-build-meta/build.rs`, commit
   `8d87b2c0`). This was the dominant inner-loop tax. `IDEOCODE-build-meta` embeds version/git metadata that
   every crate consumes via `env!("IDEOCODE_*")`. It (a) declared `.git/HEAD` + `.git/index` as
   ...
   keeping `Cargo.toml` + `IDEOCODE_RELEASE_BUILD`/`IDEOCODE_BUILD_SEMVER`/`IDEOCODE_BUILD_GIT_*` env triggers so
   ...
   | build after a real code edit (`IDEOCODE-base/src/lib.rs`) | ~20s | ~20s (correctly unchanged) |
...
scripts/dev_cargo.sh build --release -p IDEOCODE --bin IDEOCODE --quiet
scripts/dev_cargo.sh build --profile selfdev -p IDEOCODE --bin IDEOCODE --quiet
scripts/dev_cargo.sh --print-setup
```

For narrower feature-set probes, set `IDEOCODE_DEV_FEATURE_PROFILE` instead of spelling out Cargo flags:

```bash
IDEOCODE_DEV_FEATURE_PROFILE=minimal scripts/dev_cargo.sh check -p IDEOCODE --lib --quiet
IDEOCODE_DEV_FEATURE_PROFILE=pdf scripts/dev_cargo.sh build --profile selfdev -p IDEOCODE --bin IDEOCODE --quiet
IDEOCODE_DEV_FEATURE_PROFILE=full scripts/dev_cargo.sh check -p IDEOCODE --lib --quiet
```

This is especially useful because default `IDEOCODE` enables both `embeddings` and `pdf`; in the current
...
cargo check --all-targets -p IDEOCODE --features dev-bins --quiet
```
...
- can inject a named feature profile via `IDEOCODE_DEV_FEATURE_PROFILE` unless explicit feature args are present
...
```bash
IDEOCODE_FAST_LINKER=lld scripts/dev_cargo.sh build --release -p IDEOCODE --bin IDEOCODE
IDEOCODE_FAST_LINKER=mold scripts/dev_cargo.sh build --release -p IDEOCODE --bin IDEOCODE
IDEOCODE_FAST_LINKER=system scripts/dev_cargo.sh build --release -p IDEOCODE --bin IDEOCODE
```

### sccache: non-incremental only

`sccache` cannot cache incremental compilation units. All of IDEOCODE's common
...
```bash
IDEOCODE_SCCACHE=on   scripts/dev_cargo.sh build --profile selfdev ...   # force-enable
IDEOCODE_SCCACHE=off  scripts/dev_cargo.sh build --profile release-lto ... # force-disable
...
- 2026-05-29: made sccache incremental-aware. Validation on this machine:
  ...
  for `--profile release-lto`; `IDEOCODE_SCCACHE=on` and `CARGO_INCREMENTAL=0` both
  re-enable it for selfdev. A clean `IDEOCODE-azure-auth` rebuild under the old
  always-on sccache showed `0/54` cache hits, confirming the prior wasted overhead.

### Remote build host fast-fail / fast-recovery

When `IDEOCODE_REMOTE_CARGO=1` (commonly set in `~/.config/IDEOCODE/remote-build.env`),
`dev_cargo.sh` offloads builds to `IDEOCODE_REMOTE_HOST` via `scripts/remote_build.sh`.
...
```bash
IDEOCODE_REMOTE_TCP_TIMEOUT=1            # first-probe TCP timeout (seconds, fractional ok)
IDEOCODE_REMOTE_RECOVERY_TCP_TIMEOUT=0.3 # probe timeout while host was recently down
IDEOCODE_REMOTE_DOWN_TTL=300             # how long to keep using the recovery timeout
IDEOCODE_REMOTE_TCP_PROBE=0              # disable the pre-probe; use SSH preflight only
IDEOCODE_REMOTE_CARGO=0                  # disable remote builds entirely for one command
```
...
- It never touches a `target/<profile>` dir that has an active `rustc`/`cargo`
  ...
  activity window (default 20min, `IDEOCODE_CLEAN_ACTIVE_WINDOW_MIN`).
...
a static `jobs = 6`. rustc on the large root `IDEOCODE` crate peaks around 2.5-3 GiB
...
- `select_build_jobs()` reads `MemAvailable` and divides by a per-job memory budget
  (default **2048 MiB**, `IDEOCODE_BUILD_MIB_PER_JOB`), then clamps into `[1, nproc]`.
...
- Explicit `IDEOCODE_BUILD_JOBS` / `CARGO_BUILD_JOBS` always win; invalid values warn and
...
```bash
IDEOCODE_BUILD_JOBS=2            # hard override the job count for one command
IDEOCODE_BUILD_MIB_PER_JOB=2048  # memory budget per rustc job (default)
...
- 2026-05-29: added adaptive sizing. Verified via `--print-setup` and a stubbed-cargo
  ...
  cargo process. A real `dev_cargo.sh check -p IDEOCODE-logging` build succeeded.
...
scripts/bench_compile.sh release-IDEOCODE --runs 3
scripts/bench_compile.sh selfdev-IDEOCODE --runs 3
scripts/bench_compile.sh build -- --package IDEOCODE --bin test_api
...
`-Ztime-passes` on `IDEOCODE-base` (selfdev profile, single-threaded, clean unit):
...
- `IDEOCODE-base` clean recompile: **25.3s -> 12.7s** (`-Zthreads=4`)
...
Controls: `IDEOCODE_PARALLEL_FRONTEND`, `IDEOCODE_FRONTEND_THREADS`, `IDEOCODE_DEV_TOOLCHAIN`.

### WIN 2 — prefer mold over lld for the bin link

The `IDEOCODE` binary is **~300 MB** of `.text` (statically links aws-sdk, `tract`
...
### NEGATIVE — splitting provider/auth out of `IDEOCODE-base`
...
- Executed it in a worktree: created `IDEOCODE-provider-stack`, moved all 114 files,
...
~49k LOC of test code. They add **+5.7s (+23%)** to a `cargo test -p IDEOCODE-base`
...
(reload copies the binary to `~/.IDEOCODE/builds/current/`). Bad trade for zero gain.
...
- sccache is deliberately **skipped** for incremental builds (`maybe_enable_sccache`
  ...
  on requires `IDEOCODE_SCCACHE=on` (and a non-incremental profile to be useful). ✓
...
```
stale: changed ".../IDEOCODE-provider-core/src/lib.rs"
...
agent was editing files in the same shared worktree** (`/home/jeremy/IDEOCODE`)
between builds. Cargo keys freshness on mtime, so any concurrent edit to a low,
high-fanout crate (e.g. `IDEOCODE-provider-core`, `IDEOCODE-base`) invalidates that
