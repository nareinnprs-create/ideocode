...
This mirrors the existing **`browser`** tool (`crates/IDEOCODE-app-core/src/tool/browser.rs`):
one registered tool, an `action: String` that selects a sub-operation, with optional
typed params. It gives IDEOCODE a closed control loop (*see screen → decide → act*)
...
```
crates/IDEOCODE-macos-control/        (new) cfg(target_os = "macos") platform crate
...
crates/IDEOCODE-app-core/src/tool/computer.rs   (new) ComputerTool
  └─ thin dispatch layer: parse input -> call IDEOCODE-macos-control -> ToolOutput
  └─ registered in crates/IDEOCODE-app-core/src/tool/mod.rs base_tools()
```
...
1. `AXIsProcessTrustedWithOptions([kAXTrustedCheckOptionPrompt: true])` — shows the
   system dialog *and auto-adds IDEOCODE to the Accessibility list* (toggled off).
...
1. `IDEOCODE-macos-control` crate: permissions, screenshot, AX read, AX action,
