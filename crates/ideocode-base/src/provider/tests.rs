...
fn with_clean_provider_test_env<T>(f: impl FnOnce() -> T) -> T {
    ...
    // Concrete provider runtimes live downstream (IDEOCODE-provider-*-runtime),
    ...
    let prev_subscription =
        std::env::var_os(crate::subscription_catalog::IDEOCODE_SUBSCRIPTION_ACTIVE_ENV);
    ...
    if let Some(prev_subscription) = prev_subscription {
        crate::env::set_var(
            crate::subscription_catalog::IDEOCODE_SUBSCRIPTION_ACTIVE_ENV,
            prev_subscription,
        );
    } else {
...
fn standard_openrouter_catalog_refresh_fires_when_named_profile_owns_slot() {
    with_clean_provider_test_env(|| {
        ...
        runtime.block_on(async {
            ...
            crate::env::set_var("IDEOCODE_OPENROUTER_CACHE_NAMESPACE", "mynvidia");
            ...
            assert!(
                ...
                "standard OpenRouter refresh must fire even when a named profile sets IDEOCODE_OPENROUTER_* env"
            );
        });
    });
}
...
/// (IDEOCODE-provider-{gemini,cursor,antigravity}-runtime) and therefore cannot
