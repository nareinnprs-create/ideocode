...
//! [`AuthStatus`] plus the `IDEOCODE_RUNTIME_PROVIDER` override on its own, keyed on
...
pub struct ResolvedProviderAuth {
    ...
    /// True when the active credential was pinned via `IDEOCODE_RUNTIME_PROVIDER`
...
///
/// `runtime_provider` is the raw `IDEOCODE_RUNTIME_PROVIDER` value (if any); it
