...
#[test]
#[ignore = "local performance benchmark over the real ~/.IDEOCODE session corpus"]
fn bench_real_session_search_corpus() {
    if std::env::var("IDEOCODE_SESSION_SEARCH_BENCH_REAL")
        .ok()
        .as_deref()
        != Some("1")
    {
...
fn external_codex_sessions_are_searchable_without_IDEOCODE_session_dir() {
    with_temp_home(|home| {
        ...
        std::fs::remove_dir_all(home.join("sessions")).expect("remove IDEOCODE sessions dir");
