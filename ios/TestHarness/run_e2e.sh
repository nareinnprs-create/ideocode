...
DEVICE="iPhone 17"
PUSH_DEMO=""
BUNDLE_ID="com.IDEOCODE.mobile"
PORT=7643
SHOT_DIR="${TMPDIR:-/tmp}/IDEOCODE-ios-e2e"
...
xcodebuild build \
  -project IDEOCODEMobile.xcodeproj \
  -scheme IDEOCODEMobile \
  -destination "platform=iOS Simulator,name=$DEVICE" \
  -derivedDataPath .build-ios >/dev/null
APP=".build-ios/Build/Products/Debug-iphonesimulator/IDEOCODEMobile.app"
...
mkdir -p "$APPSUP"
printf '%s\n' \
  '[{"host":"127.0.0.1","port":7643,"token":"mocktoken0123456789abcdef","serverName":"mock-IDEOCODE","serverVersion":"mock-0.32.0","pairedAt":770000000}]' \
  > "$APPSUP/IDEOCODE-servers.json"
