...
fn test_direct_chutes_ignores_legacy_openrouter_catalog_cache() {
    with_clean_provider_test_env(|| {
        ...
        with_env_var("HOME", &home, || {
            let cache_dir = temp_home.path().join(".IDEOCODE").join("cache");
...
fn test_state_space_openrouter_default_survives_switch_to_nvidia_nim() {
    with_clean_provider_test_env(|| {
        ...
        assert!(
            std::env::var_os("IDEOCODE_OPENROUTER_API_BASE").is_none(),
...
fn test_active_compatible_route_treats_claude_like_bare_model_as_provider_local() {
    with_clean_provider_test_env(|| {
        with_env_var("OPENROUTER_API_KEY", "test-openrouter-key", || {
            with_env_var("IDEOCODE_OPENROUTER_PROVIDER_FEATURES", "0", || {
                with_env_var(
...
fn test_config_default_provider_openai_compatible_keeps_gpt_model_provider_local() {
    with_clean_provider_test_env(|| {
        with_env_var(
            ...
            "https://compat.example.test/v1",
            || {
                with_env_var("IDEOCODE_OPENAI_COMPAT_API_KEY_NAME", "OPENAI_API_KEY", || {
...
fn test_openai_auth_mode_prefixed_model_switch_changes_credentials() {
    with_clean_provider_test_env(|| {
        let prev_runtime = std::env::var_os("IDEOCODE_RUNTIME_PROVIDER");
        ...
        if let Some(prev_runtime) = prev_runtime {
            crate::env::set_var("IDEOCODE_RUNTIME_PROVIDER", prev_runtime);
        } else {
            crate::env::remove_var("IDEOCODE_RUNTIME_PROVIDER");
        }
    });
...
fn test_initial_openai_provider_can_switch_to_anthropic_auth_routes() {
    with_clean_provider_test_env(|| {
        ...
        // for each pin is covered by IDEOCODE-provider-anthropic-runtime's
...
fn test_config_default_model_with_credential_prefix_applies_model_and_pin() {
    ...
    for (spec, expect_oauth) in [
        ...
        with_clean_provider_test_env(|| {
            ...
            // covered by IDEOCODE-provider-anthropic-runtime's tests.
            if expect_oauth {
                assert_ne!(
                    anthropic.credential_mode(),
                    IDEOCODE_provider_core::CredentialMode::ApiKey,
...
fn isolate_openrouter_autodetect_env_or() -> Vec<OrEnvVarGuard> {
    let mut guards = vec![
        OrEnvVarGuard::remove("IDEOCODE_OPENROUTER_API_BASE"),
        OrEnvVarGuard::remove("IDEOCODE_OPENROUTER_API_KEY_NAME"),
        OrEnvVarGuard::remove("IDEOCODE_OPENROUTER_ENV_FILE"),
        OrEnvVarGuard::remove("IDEOCODE_OPENROUTER_DYNAMIC_BEARER_PROVIDER"),
        OrEnvVarGuard::remove("IDEOCODE_OPENROUTER_MODEL"),
        OrEnvVarGuard::remove("IDEOCODE_OPENROUTER_CACHE_NAMESPACE"),
        OrEnvVarGuard::remove("IDEOCODE_OPENROUTER_ALLOW_NO_AUTH"),
        OrEnvVarGuard::remove("IDEOCODE_OPENROUTER_TRANSPORT_STATE"),
        OrEnvVarGuard::remove("IDEOCODE_OPENROUTER_PROVIDER_FEATURES"),
        OrEnvVarGuard::remove("IDEOCODE_OPENROUTER_MODEL_CATALOG"),
        OrEnvVarGuard::remove("IDEOCODE_OPENROUTER_AUTH_HEADER"),
        OrEnvVarGuard::remove("IDEOCODE_OPENROUTER_AUTH_HEADER_NAME"),
        OrEnvVarGuard::remove("IDEOCODE_OPENROUTER_STATIC_MODELS"),
        OrEnvVarGuard::remove("IDEOCODE_ACTIVE_PROVIDER"),
        OrEnvVarGuard::remove("IDEOCODE_RUNTIME_PROVIDER"),
        OrEnvVarGuard::remove("IDEOCODE_NAMED_PROVIDER_PROFILE"),
        OrEnvVarGuard::remove("IDEOCODE_PROVIDER_PROFILE_NAME"),
        OrEnvVarGuard::remove("IDEOCODE_PROVIDER_PROFILE_ACTIVE"),
        OrEnvVarGuard::remove("IDEOCODE_OPENAI_COMPAT_API_BASE"),
        OrEnvVarGuard::remove("IDEOCODE_OPENAI_COMPAT_API_KEY_NAME"),
        OrEnvVarGuard::remove("IDEOCODE_OPENAI_COMPAT_ENV_FILE"),
        OrEnvVarGuard::remove("IDEOCODE_OPENAI_COMPAT_SETUP_URL"),
        OrEnvVarGuard::remove("IDEOCODE_OPENAI_COMPAT_DEFAULT_MODEL"),
        OrEnvVarGuard::remove("IDEOCODE_OPENAI_COMPAT_LOCAL_ENABLED"),
