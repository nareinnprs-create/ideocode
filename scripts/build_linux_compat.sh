...
artifact="${IDEOCODE_COMPAT_ARTIFACT:-IDEOCODE-linux-x86_64}"
profile="${IDEOCODE_COMPAT_PROFILE:-release}"
image="${IDEOCODE_COMPAT_IMAGE:-quay.io/pypa/manylinux2014_x86_64}"
cache_root="${IDEOCODE_COMPAT_CACHE_DIR:-$HOME/.cache/IDEOCODE-linux-compat}"
...
# metadata file (read by IDEOCODE-build-meta/build.rs through
# IDEOCODE_BUILD_METADATA_FILE). The repo is bind-mounted into the container and
...
docker run --rm \
  ...
  -e IDEOCODE_RELEASE_BUILD="${IDEOCODE_RELEASE_BUILD:-1}" \
  -e IDEOCODE_BUILD_SEMVER="${IDEOCODE_BUILD_SEMVER:-}" \
  -e IDEOCODE_BUILD_METADATA_FILE=/IDEOCODE-build-meta \
  -e IDEOCODE_COMPAT_PROFILE="$profile" \
  -e IDEOCODE_COMPAT_TARGET="$target" \
  -e HOST_UID="$host_uid" \
  -e HOST_GID="$host_gid" \
  -v "$repo_root:/work" \
  -v "$metadata_file:/IDEOCODE-build-meta:ro" \
  -v "$out_dir:/out" \
  ...
  bash -lc '
    ...
    if [[ ! -x /root/.cargo/bin/cargo ]]; then
      curl https://sh.rustup.rs -sSf | sh -s -- -y --profile minimal --default-toolchain stable
	    ...
	    # (IDEOCODE_BUILD_METADATA_FILE=/IDEOCODE-build-meta) is the primary source of
...
WRAPPER
	    ...
	    # these sibling libraries and sets LD_LIBRARY_PATH for the IDEOCODE process.
