...
fn test_recover_session_without_tools_preserves_debug_and_canary_flags() {
    ...
    app.session.working_dir = Some("/tmp/IDEOCODE-test".to_string());
    ...
    assert_eq!(app.session.working_dir.as_deref(), Some("/tmp/IDEOCODE-test"));
...
fn test_selfdev_command_spawns_session_in_test_mode() {
    ...
    let prev_test = std::env::var_os("IDEOCODE_TEST_SESSION");
    ...
    if let Some(prev_test) = prev_test {
        crate::env::set_var("IDEOCODE_TEST_SESSION", prev_test);
    } else {
        crate::env::remove_var("IDEOCODE_TEST_SESSION");
    }
}
