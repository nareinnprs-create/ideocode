...
Runs a full-feature selfdev IDEOCODE build with Cargo timings enabled and summarizes
...
Options:
  ...
  --package <name>          Cargo package to build (default: IDEOCODE)
  --bin <name>              Cargo binary to build (default: IDEOCODE)
  --feature-profile <name>  IDEOCODE_DEV_FEATURE_PROFILE for dev_cargo.sh (default: default)
...
Notes:
  ...
  - The "IDEOCODE serial stack" summary is not a formal Cargo critical path. It is a
    focused view of the known long-pole crates: IDEOCODE-base, IDEOCODE-app-core,
    IDEOCODE-tui, root IDEOCODE lib, and IDEOCODE bin.
USAGE
}
...
PY
  ...
  IDEOCODE_DEV_FEATURE_PROFILE="$feature_profile" \
...
def is_IDEOCODE_stack_unit(unit: dict[str, Any]) -> bool:
    name = unit.get("name")
    target = unit.get("target") or ""
    if name in {"IDEOCODE-base", "IDEOCODE-app-core", "IDEOCODE-tui"} and "build script" not in target:
        return True
    if name == "IDEOCODE" and (target == "" or f'bin "{bin_name}"' in target):
        return True
    return False

IDEOCODE_stack = sorted([unit for unit in units if is_IDEOCODE_stack_unit(unit)], key=lambda unit: float(unit.get("start", 0.0)))
stack_span = None
if IDEOCODE_stack:
    stack_span = max(float(unit["end"]) for unit in IDEOCODE_stack) - min(float(unit.get("start", 0.0)) for unit in IDEOCODE_stack)
stack_sum = sum(float(unit.get("duration", 0.0)) for unit in IDEOCODE_stack)
stack_frontend_sum = sum(float(unit.get("frontend_duration") or 0.0) for unit in IDEOCODE_stack)
stack_codegen_sum = sum(float(unit.get("codegen_duration") or 0.0) for unit in IDEOCODE_stack)
