...
fn startup_shortcut_path() -> PathBuf {
    startup_dir().join("IDEOCODE-hotkey.lnk")
}
...
Get-CimInstance Win32_Process -ErrorAction SilentlyContinue |
  Where-Object {{
    ($_.ProcessId -ne $current) -and ($_.ProcessId -ne $PID) -and (
      (($_.Name -eq 'powershell.exe' -or $_.Name -eq 'pwsh.exe') -and $_.CommandLine -like '*IDEOCODE-hotkey*') -or
      (($_.Name -eq 'IDEOCODE.exe') -and $_.CommandLine -like '*--listen-windows-hotkey*')
    )
  }} |
...
pub(super) fn uninstall_windows_hotkey_listener() -> Result<()> {
    ...
    state.save()?;
    eprintln!("  \x1b[32m✓\x1b[0m Removed IDEOCODE Windows launch-hotkey listener");
    Ok(())
}
...
fn launch_windows_hotkey(entry: &WindowsHotkey) -> Result<()> {
    ...
    if !launched {
        anyhow::bail!("no terminal found to launch IDEOCODE");
    }
    Ok(())
}
...
fn windows_native_hotkey_loop(entries: Vec<WindowsHotkey>) -> Result<()> {
    ...
    const MUTEX_NAME: &str = "Local\\IDEOCODELaunchHotkeyListener";
...
/// interactive `IDEOCODE setup-hotkey` flow to install it.
...
fn nudge_hotkey(state: &mut SetupHintsState) -> bool {
    ...
    eprintln!(
        "\x1b[36m│\x1b[0m \x1b[1m💡 Set up global keys to launch IDEOCODE?\x1b[0m                  \x1b[36m│\x1b[0m"
    );
    eprintln!(
    ...
    eprintln!(
        "\x1b[36m│\x1b[0m    Opens IDEOCODE in {:<39}    \x1b[36m│\x1b[0m",
        ...
        "y" | "yes" => {
            ...
            match create_hotkey_shortcut(using_alacritty) {
                Ok(()) => {
                    ...
                    let _ = state.save();
                    eprintln!(
                        "  \x1b[32m✓\x1b[0m Created hotkeys (\x1b[1mAlt+;\x1b[0m and \x1b[1mCopilot\x1b[0m) → {} + IDEOCODE",
                        terminal_name
                    );
                    eprintln!();
                    true
                ...
                Err(e) => {
                    ...
                    eprintln!(
                        "    You can set it up manually later with: \x1b[1mIDEOCODE setup-hotkey\x1b[0m"
                    );
                    eprintln!();
                    false
                }
...
fn nudge_alacritty(state: &mut SetupHintsState) -> bool {
    ...
    eprintln!("\x1b[36m┌─────────────────────────────────────────────────────────────┐\x1b[0m");
    eprintln!(
        "\x1b[36m│\x1b[0m \x1b[1m💡 Alacritty: the fastest terminal for IDEOCODE\x1b[0m               \x1b[36m│\x1b[0m"
    );
    eprintln!(
        "\x1b[36m│\x1b[0m                                                             \x1b[36m│\x1b[0m"
    );
                ...
                Ok(()) => {
                    ...
                    if state.hotkey_configured {
                        ...
                        match create_hotkey_shortcut(true) {
                            Ok(()) => {
                                eprintln!(
                                    "  \x1b[32m✓\x1b[0m Hotkeys updated: \x1b[1mAlt+;\x1b[0m and \x1b[1mCopilot\x1b[0m → Alacritty + IDEOCODE"
                                );
                            }
                            Err(e) => {
...
fn prompt_try_it_out(installed_alacritty: bool) {
    ...
    eprintln!(
        "\x1b[32m│\x1b[0m    Press \x1b[1mAlt+;\x1b[0m or the \x1b[1mCopilot key\x1b[0m to launch IDEOCODE.       \x1b[32m│\x1b[0m"
    );
    eprintln!(
        "\x1b[32m│\x1b[0m    The listener is native IDEOCODE, no AutoHotkey required.          \x1b[32m│\x1b[0m"
    );
    ...
    eprintln!(
        "\x1b[32m│\x1b[0m    \x1b[90m(Starting IDEOCODE normally in 3 seconds...)\x1b[0m                 \x1b[32m│\x1b[0m"
    );
    eprintln!("\x1b[32m└─────────────────────────────────────────────────────────────┘\x1b[0m");
    eprintln!();
...
mod tests {
    ...
    fn startup_shortcut_uses_native_listener_without_vbscript_or_bypass() {
        let script = render_startup_shortcut_script(
            Path::new(r"C:\Users\O'Hara\Startup\IDEOCODE-hotkey.lnk"),
            Path::new(r"C:\Program Files\IDEOCODE O'Hara\IDEOCODE.exe"),
        );
...
pub(super) fn run_setup_hotkey_windows() -> Result<()> {
    ...
    eprintln!(
        "  Setting up global launch hotkeys → {} + IDEOCODE...",
        terminal_name
    );
...
pub(super) fn create_windows_desktop_shortcut(state: &mut SetupHintsState) -> Result<()> {
    ...
    let shortcut_path = format!("{}\\Desktop\\IDEOCODE.lnk", desktop_dir);
...
$shortcut.Description = "IDEOCODE - AI coding agent"
$shortcut.Save()
Write-Output "OK"
"#,
