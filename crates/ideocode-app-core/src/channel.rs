...
impl MessageChannel for JadeRelayChannel {
    ...
    fn is_reply_enabled(&self) -> bool {
        ...
        // IDEOCODE session directly. Keep this channel for outbound notifications
    ...
    async fn reply_loop(&self, runner: AmbientRunnerHandle) {
        ...
        let device_id = format!("IDEOCODE-{}", host);
        logging::info(&format!(
...
mod tests {
    ...
    ///   cargo test -p IDEOCODE-app-core relay_live -- --ignored --nocapture
    #[tokio::test]
    ...
    async fn test_relay_live_roundtrip() {
        ...
        ch.heartbeat("IDEOCODE-test-device").await;
