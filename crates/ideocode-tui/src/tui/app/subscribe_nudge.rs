...
//!     IDEOCODE account credentials, never in replay/test runtimes.
//!
...
pub(super) const RATE_LIMIT_NUDGE_LINE: &str =
    "Get more tokens with a IDEOCODE subscription: /subscribe";
/// Status-line tail shared by trigger B.
const SUPPORT_NUDGE_NOTICE: &str = "Support IDEOCODE: /subscribe";
...
/// Persisted nudge bookkeeping (one small JSON file under the IDEOCODE dir).
...
pub(super) fn subscribe_pitch_markdown() -> String {
    let mut message = String::from("Subscribe to IDEOCODE\n\n");
    ...
    message.push_str("\nStart: /login IDEOCODE (browser approval, no keys in the terminal)\n");
...
impl App {
    ...
    pub(super) fn claim_subscribe_nudge(&mut self, trigger: SubscribeNudgeTrigger) -> bool {
        ...
        // Never pitch existing IDEOCODE account holders.
    ...
    pub(super) fn show_subscribe_pitch(&mut self) {
        ...
        self.set_status_notice("Subscribe: /login IDEOCODE to start");
    }
}
...
mod tests {
    ...
    fn long_task_message_reads_support_wording_with_elapsed() {
        ...
        assert_eq!(
            message,
            "✦ IDEOCODE just worked 1h 23m for you. Support IDEOCODE: /subscribe"
        );
