...
/// Directory name under the durable state dir (`~/.IDEOCODE/state`).
const SWARM_STATE_DIR: &str = "swarm";
/// Pre-0.36 location under the runtime dir (tmpfs on Linux, wiped on reboot).
const LEGACY_SWARM_STATE_DIR: &str = "IDEOCODE-swarm-state";
...
fn dormant_plan_retention() -> Duration {
    Duration::from_secs(
        std::env::var("IDEOCODE_SWARM_DORMANT_PLAN_RETENTION_SECS")
            .ok()
...
/// `IDEOCODE_RUNTIME_DIR`. Never let those tests fall through to the real
/// `~/.IDEOCODE/state/swarm`: that leaked synthetic `swarm-1` plans/members into
/// live user state during ordinary `cargo test` runs. Tests that need an
/// isolated explicit location still set `IDEOCODE_RUNTIME_DIR` and use the normal
/// resolver; otherwise use a process-local temp directory.
#[cfg(test)]
fn state_dir() -> PathBuf {
    if std::env::var_os("IDEOCODE_RUNTIME_DIR").is_some() {
        storage::durable_state_dir().join(SWARM_STATE_DIR)
    } else {
        std::env::temp_dir()
            .join(format!("IDEOCODE-test-state-{}", std::process::id()))
