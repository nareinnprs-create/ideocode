...
The ambient agent has access to a subset of IDEOCODE tools plus ambient-specific tools.
...
// Tool: end_ambient_cycle
{
    "summary": "Merged 3 duplicate memories, pruned 2 stale facts,
                extracted memories from crashed session IDEOCODE-red-fox-1234",
    "memories_modified": 8,
    "compactions": 2,
    "proactive_work": null,
    "next_schedule": {
...
```
You are the ambient agent for IDEOCODE. You operate autonomously without
...
```toml
# In project-level .IDEOCODE/config.toml
[ambient]
