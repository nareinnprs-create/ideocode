...
Compares IDEOCODE's swarm (multi-agent coordination) with single-agent performance
...
Environment:
    Requires IDEOCODE server running with debug_control enabled:
        touch ~/.IDEOCODE/debug_control
        IDEOCODE serve
"""
...
def main():
    ...
    if not os.path.exists(DEBUG_SOCKET):
        ...
        print("Make sure IDEOCODE server is running with debug_control enabled:")
        print("  touch ~/.IDEOCODE/debug_control")
        print("  IDEOCODE serve")
        sys.exit(1)
