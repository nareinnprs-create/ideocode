...
/// Load a value only from the saved env file under the IDEOCODE config dir,
...
mod tests {
    ...
    fn loads_api_key_with_zero_width_space_from_config_file() {
        ...
        std::fs::write(
            ...
            "IDEOCODE_PROVIDER_FOO_API_KEY=\u{200B}sk-mykey123\n",
        )
        .expect("write env file");

        assert_eq!(
            load_api_key_from_env_or_config("IDEOCODE_PROVIDER_FOO_API_KEY", "provider-foo.env")
                .as_deref(),
            Some("sk-mykey123")
        );
    }
