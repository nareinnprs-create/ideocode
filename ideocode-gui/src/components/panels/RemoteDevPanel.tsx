...
export function RemoteDevPanel() {
  ...
  const statusColor = (status: string) => {
    const colors: Record<string, string> = {
      ...
      disconnected: "bg-surface-elevated text-fg-muted",
    };
  ...
  return (
    <div className="flex flex-col h-full" role="region" aria-label="Remote dev panel">
      <div className="px-1 pt-1 flex items-center justify-between">
        ...
        <button onClick={() => setShowAdd(!showAdd)} aria-label="Add remote host" aria-expanded={showAdd} className="flex items-center gap-1 px-2 py-1 text-xs text-accent hover:text-accent-hover transition-fast rounded hover:bg-surface-elevated">
          <Plus size={14} /> Add
        </button>
      </div>
      {showAdd && (
        <div className="mx-3 mb-2 p-2 rounded bg-surface-elevated border border-border-subtle space-y-1.5">
          ...
          <div className="flex gap-1.5">
            <input type="text" placeholder="Port" value={port} onChange={(e) => setPort(e.target.value)}
              className="w-20 p-1.5 text-xs font-mono bg-surface-elevated border border-border-subtle rounded text-fg-primary focus:outline-none focus:border-accent" />
          ...
          <button onClick={handleAdd} disabled={!name.trim() || !host.trim()}
            className="w-full px-3 py-1.5 text-xs bg-accent text-white rounded hover:bg-accent-hover disabled:opacity-50 transition-fast">Add Host</button>
        </div>
      )}
      <div className="flex-1 overflow-y-auto">
        {hosts.length === 0 ? (
          ...
          <div key={h.id} className="px-3 py-2 border-b border-border-subtle hover:bg-surface-elevated transition-fast group">
            <div className="flex items-center gap-2">
              <Globe size={14} className={h.status === "connected" ? "text-success" : "text-fg-muted"} />
              <div className="flex-1 min-w-0">
                <div className="text-xs font-medium text-fg-primary">{h.name}</div>
                <div className="text-[11px] text-fg-muted font-mono truncate">{h.user}@{h.host}:{h.port}</div>
                <div className="text-[10px] text-fg-muted mt-0.5">{h.type}</div>
              </div>
              ...
              {h.status === "connected" ? (
                ...
                <button onClick={() => connect(h.id)} disabled={h.status === "connecting"}
                  className="text-[10px] px-2 py-1 rounded bg-surface-elevated text-fg-muted hover:text-fg-primary disabled:opacity-50 transition-fast">
                  {h.status === "connecting" ? "Connecting..." : "Connect"}
                </button>
              )}
              <button onClick={() => removeHost(h.id)} aria-label="Remove remote host" className="p-1 text-fg-muted hover:text-error opacity-0 group-hover:opacity-100 transition-fast">
                <Trash2 size={11} />
              </button>
            </div>
          </div>
