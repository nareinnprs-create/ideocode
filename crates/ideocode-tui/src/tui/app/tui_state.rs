...
impl App {
    ...
    /// the header uses (session stub, `IDEOCODE_MODEL`, config default) so effort
    ...
    ///   [`AuthStatus`] snapshot (cached up to 60s) or a `IDEOCODE_RUNTIME_PROVIDER`
            ...
            let (session_count, client_count, session_names, has_activity) = if self.is_remote {
                ...
                let (status, detail) = match &self.status {
                    ...
                    ProcessingStatus::Installing => (
                        ...
                        Some("installing built-in engine".to_string()),
                    ),
                };
