...
server but not every interrupted session continues. It correlates three sources:
  ...
  1. ~/.IDEOCODE/reload-recovery/*.json durable recovery intent records
  2. ~/.IDEOCODE/logs/IDEOCODE-*.log recovery lifecycle messages
  3. ~/.IDEOCODE/sessions/<session>.json coarse transcript markers
...
def IDEOCODE_home() -> pathlib.Path:
    return pathlib.Path(os.environ.get("IDEOCODE_HOME", pathlib.Path.home() / ".IDEOCODE"))


def log_files(log_dir: pathlib.Path, max_log_files: int) -> list[pathlib.Path]:
    files = sorted(log_dir.glob("IDEOCODE-*.log"), key=lambda p: p.stat().st_mtime, reverse=True)
...
def parse_trace_files(records: dict[tuple[str, str], IntentRecord], trace_dir: pathlib.Path) -> None:
    """Merge structured ~/.IDEOCODE/reload-traces/*.jsonl lifecycle events."""
    phase_to_kind = {
...
def main() -> int:
    ...
    parser.add_argument("--home", type=pathlib.Path, default=IDEOCODE_home(), help="IDEOCODE_HOME path")
    ...
    parser.add_argument("--max-log-files", type=int, default=3, help="Newest IDEOCODE logs to scan")
