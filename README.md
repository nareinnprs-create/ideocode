...
IDEOCODE is built to be as performant and resource efficient as possible. Every metric is optimized to the bone, which is important for scaling multi-session workflows. Here we sample a few metrics to show the difference: RAM usage and boot up.
  ...
  <tr>
    <td valign="top" align="center" width="50%">
      ...
      <table>
        ...
        <tbody>
          <tr>
            <td><strong>IDEOCODE (local embedding off)</strong></td>
            <td align="right"><strong>27.8 MB</strong></td>
            <td align="right">baseline</td>
          </tr>
          <tr>
            <td><strong>IDEOCODE</strong></td>
...
|---|---:|---:|---:|
| **IDEOCODE** | **14.0 ms** | 10.1–19.3 ms | baseline |
...
|---|---:|---:|---:|
| **IDEOCODE** | **48.7 ms** | 30.3–62.7 ms | baseline |
...
|---|---:|---:|
| **IDEOCODE (local embedding off)** | **~9.9 MB** | baseline |
| **IDEOCODE** | **~10.4 MB** | **1.1× more RAM** |
...
- `IDEOCODE v0.9.1888-dev (be386f2)`
- `pi 0.62.0`
- `codex-cli 0.120.0`
- `opencode 1.0.203`
...
The side panel is a place for auxiliary information. Tell your IDEOCODE agent to load a file into the side panel and see it update in real time, or tell your agent to write directly to the side panel, or use it as a diff viewer. The side panel (and chat) is able to render mermaid diagrams inline. 
<img width="2877" height="1762" alt="image" src="https://github.com/user-attachments/assets/6c7bec81-ef3f-434d-8a7b-d55f8a54e5cf" />

To make this possible, I created a new mermaid rendering library to render diagrams 1800x faster. It has no browser or Typescript dependency. See https://github.com/nareinnprs-create/mermaid-rs-renderer
...
IDEOCODE can render at over a thousand fps. Your monitor will not have the refresh rate to show you, but this means you will not have silly flicker problems. 

The custom scrollback implementation of IDEOCODE allows it to do much more than a native scrollback. However, it is a terminal-level limitation that I cannot have smooth, partial line scrolling with a custom scrollback. To fix this, I made my own terminal. Handterm https://github.com/nareinnprs-create/handterm implements a native scroll api, and also happens to be very effiecent. This is a work in progress. Scrolling is still well implemented for normal terminals.

IDEOCODE is left-aligned by default. You can switch to centered mode with the `Alt+C` hotkey, with the `/alignment` command, or in the config.

To disable emoji globally in TUI and CLI output, set `emoji = false` under `[display]` in `~/.IDEOCODE/config.toml`, or launch with `IDEOCODE_NO_EMOJI=1`. IDEOCODE replaces emoji with compact ASCII markers while preserving other Unicode text.
...
IDEOCODE works with subscription-backed OAuth flows and many provider integrations, so you can use the models you already pay for and still fall back to direct API providers when needed.

### Supported built-in login flows

- **Claude** (`IDEOCODE login --provider claude`)
- **OpenAI / ChatGPT / Codex** (`IDEOCODE login --provider openai`)
- **Google Gemini** (`IDEOCODE login --provider gemini`)
- **GitHub Copilot** (`IDEOCODE login --provider copilot`)
- **Azure OpenAI** (`IDEOCODE login --provider azure`)
- **Alibaba Cloud Coding Plan** (`IDEOCODE login --provider alibaba-coding-plan`)
- **Fireworks** (`IDEOCODE login --provider fireworks`)
- **MiniMax** (`IDEOCODE login --provider minimax`)
- **LM Studio** (`IDEOCODE login --provider lmstudio`)
- **Ollama** (`IDEOCODE login --provider ollama`)
- **Custom OpenAI-compatible endpoint** (`IDEOCODE login --provider openai-compatible`)

For custom OpenAI-compatible endpoints, IDEOCODE now prompts for the API base and supports local localhost servers without requiring an API key.

### Config-file setup for self-hosted endpoints and MCP

If you prefer to configure things by editing files instead of using the login UI, IDEOCODE supports both a custom OpenAI-compatible endpoint config and MCP config files.

#### OpenAI-compatible providers

Many hosted services speak the standard OpenAI `/v1/chat/completions` API. IDEOCODE talks to them through one shared OpenAI-compatible provider, so you can use almost any such endpoint without waiting for a dedicated integration.

There are two ways to set one up:

- **Built-in named profiles** — IDEOCODE ships ready-made profiles for several popular OpenAI-compatible services. Log in by id and IDEOCODE fills in the base URL and key environment variable for you:

  ```bash
  IDEOCODE login --provider <profile-id>
  # for example:
  IDEOCODE login --provider openrouter
  IDEOCODE login --provider deepseek
  IDEOCODE login --provider opencode      # OpenCode Zen
  IDEOCODE login --provider moonshotai
  ```

  Built-in OpenAI-compatible profile ids include: `openrouter`, `deepseek`, `zai`, `kimi`, `moonshotai`, `opencode` (OpenCode Zen), `opencode-go`, `302ai`, `baseten`, `cortecs`, `huggingface`, `nebius`, `scaleway`, `stackit`, and `firmware`. Each profile only sets the endpoint and key variable; you still pick the model with `/model` (or `--model`). Run `IDEOCODE login` with no provider to see the interactive list.

- **Any other endpoint** — point IDEOCODE at an arbitrary OpenAI-compatible API (hosted or local) with `IDEOCODE login --provider openai-compatible` or the scriptable `IDEOCODE provider add` command described below.

Useful environment overrides for these endpoints:

- `IDEOCODE_STREAM_IDLE_TIMEOUT_SECS` — raise the base streaming idle timeout (default 180s) for slow reasoning models that think silently before emitting tokens. High reasoning efforts scale this automatically (high 2x, xhigh 3x, max 4x). Also settable as `[provider] stream_idle_timeout_secs` in `config.toml`.
- Per-model `context_window` (alias `context_limit`) in a `[[providers.<name>.models]]` entry — set the context window when the endpoint has no usable `/v1/models` response, so IDEOCODE does not fall back to the generic 200k default.
...
For agents and scripts, the preferred path is the one-shot provider profile command. It writes a named profile to `~/.IDEOCODE/config.toml`, stores secrets in IDEOCODE's private app config directory when requested, and prints exact run/validation commands:
...
IDEOCODE --provider-profile my-api auth-test --prompt 'Reply exactly IDEOCODE_PROVIDER_SETUP_OK'

# Use it directly.
IDEOCODE --provider-profile my-api run 'hello'
```

For local servers that do not require auth:

```bash
IDEOCODE provider add local-vllm \
...
IDEOCODE --provider ollama --model llama3.2 run 'hello'
...
IDEOCODE --provider lmstudio --model '<model-id>' run 'hello'
```

Ollama and LM Studio both expose OpenAI-compatible `/v1/models` and `/v1/chat/completions` endpoints. IDEOCODE uses streaming chat completions, function/tool calling, and OpenAI-style image content for vision-capable local models. If a local server requires a token, enter it during `IDEOCODE login` or create a named profile with `--api-key-stdin`.
...
The generated profile can also be edited manually in `~/.IDEOCODE/config.toml`:
...
api_key_env = "IDEOCODE_PROVIDER_MY_API_API_KEY"
...
2. For built-in profiles (e.g. `nvidia-nim`) or any endpoint, via the `IDEOCODE_OPENAI_EXTRA_BODY` environment variable (a JSON object string). It can live in the provider's env file (`~/.config/IDEOCODE/nvidia-nim.env`) next to the API key:

   ```bash
   IDEOCODE_OPENAI_EXTRA_BODY={"chat_template_kwargs":{"thinking":true,"reasoning_effort":"high"}}
   ```

Keys from `extra_body` are merged last and override any IDEOCODE-generated body field with the same name (`IDEOCODE_OPENAI_EXTRA_BODY` wins over the config `extra_body` on key collisions). Invalid values are logged and ignored rather than failing the request.

The custom OpenAI-compatible provider reads overrides from environment variables or from an env file in IDEOCODE's app config directory. On Linux this is usually `~/.config/IDEOCODE/`, so the default file is usually:

```text
~/.config/IDEOCODE/openai-compatible.env
```

Example for a local or LAN vLLM server:

```bash
IDEOCODE_OPENAI_COMPAT_API_BASE=http://192.168.1.50:8000/v1
IDEOCODE_OPENAI_COMPAT_DEFAULT_MODEL=Qwen/Qwen3-Coder-30B-A3B-Instruct
...
- `IDEOCODE login --provider openai-compatible` can create or update this for you.
...
- `~/.IDEOCODE/mcp.json` for global MCP servers
- `.IDEOCODE/mcp.json` for project-local MCP servers
...
Both the canonical `mcpServers` key and IDEOCODE's historical `servers` key are accepted. IDEOCODE currently supports stdio (command-based) servers only; HTTP/SSE entries (`"type": "http"`/`"sse"`) are recognized and skipped with a log line.
...
On first run, IDEOCODE also tries to import MCP servers from `~/.claude.json` (falling back to the legacy `~/.claude/mcp.json`) and `~/.codex/config.toml` if `~/.IDEOCODE/mcp.json` does not exist yet.

For headless or SSH sessions, OAuth-style providers support `IDEOCODE login --provider <provider> --no-browser` (alias: `--headless`) so IDEOCODE prints the auth URL/QR and falls back to manual code or callback paste instead of trying to launch a local browser.
...
IDEOCODE login --provider openai --print-auth-url --json

# Step 2: complete later with the callback URL or auth code
IDEOCODE login --provider openai --callback-url 'http://localhost:1455/auth/callback?...'
IDEOCODE login --provider gemini --auth-code '...'
```
...
IDEOCODE login --provider copilot --print-auth-url --json
IDEOCODE login --provider copilot --complete

# Gmail/Google OAuth after credentials are already configured
IDEOCODE login --provider google --print-auth-url --google-access-tier readonly
IDEOCODE login --provider google --callback-url 'http://127.0.0.1:8456?...'
```

Pending scriptable login state is stored under `~/.IDEOCODE/pending-login/`, automatically expires, and stale entries are cleaned up when new scriptable logins start or resume.

For the built-in OpenAI login flow, IDEOCODE opens a local callback on
...
IDEOCODE also supports easy multi-account switching. Ran out of tokens on your first ChatGPT Pro subscription? /account and quickly switch to your second. 

---

## Customizability / Self-Dev

IDEOCODE is inventing a new form of customizability. One that doesn't limit you to what a plugin or extension can do. Tell your IDEOCODE agent to enter self dev mode, and it will start modifying its own source code. IDEOCODE is optimized to iterate on itself. There is significant infrastructure around self developement, which allows it to edit, build, and test its own source code, then reload its own binary and continue work in your (potentially many) sessions, fully automatically. 

It is reccomended that you use a frontier model for this. The IDEOCODE codebase is not a simple one, and weaker models can make subtle, breaking changes. GPT 5.5 or the latest available frontier model works well.
...
The devil is in the details. There are many undocumented optimizations and niceties that IDEOCODE implements. Some examples: 
...
IDEOCODE comes with instructions on how to set up Firefox Agent Bridge. Ask you agent to set it up, and then you will have browser automation in IDEOCODE as well. 

Agent grep is a grep tool I made for the IDEOCODE agent. It adds file strucuture information (ie the list of functions, their displacement, etc) to the grep return, so that the agent can infer more of what the file doesn without actually reading the file. It also implements a harness-level integration that adaptively truncates returns based on what the agent has already seen. This saves on context a lot. 
...
Resume sessions from different harnesses. Claude code broke on you? Resume the session from IDEOCODE and continue where you left off. Session resume is supported for codex, claude code, opencode, and pi. 
...
A native iOS application version of IDEOCODE is coming soon. This will allow you to work with IDEOCODE on your personal machine's environment from your phone, via Tailscale. Openclaw like features will be bundled with this iOS application. 
...
IDEOCODE supports interactive TUI use, non-interactive runs, persistent server/client workflows,
...
IDEOCODE includes a first-class built-in `browser` tool for browser control inside agent sessions.
...
```bash
IDEOCODE browser status
IDEOCODE browser setup
```
...
If you want another agent to set up IDEOCODE for you, give it this prompt:

```text
Set up IDEOCODE on this machine for me.
...
1. Detect the operating system, available package managers, and shell environment, then install IDEOCODE using the best matching command below instead of referring me somewhere else:
   ...
   - For local self-dev / refactor work on Linux x86_64, prefer:
     scripts/dev_cargo.sh build --release -p IDEOCODE --bin IDEOCODE
     scripts/dev_cargo.sh --print-setup
     scripts/install_release.sh

2. Verify that `IDEOCODE` is on my `PATH`.
3. Launch `IDEOCODE` once in a new terminal window/session to confirm it starts successfully.
4. Before attempting any interactive login flow, assess which providers are already available non-interactively and prefer those first. Check existing local credentials, config files, CLI sessions, and environment variables such as:
   - Claude: `~/.IDEOCODE/auth.json`, `~/.claude/.credentials.json`, `~/.local/share/opencode/auth.json`, `ANTHROPIC_API_KEY`
   - OpenAI: `~/.IDEOCODE/openai-auth.json`, `~/.codex/auth.json`, `OPENAI_API_KEY`
   - Gemini: `~/.IDEOCODE/gemini_oauth.json`, `~/.gemini/oauth_creds.json`
   - GitHub Copilot: existing auth under `~/.config/github-copilot/`
   - Azure OpenAI: `~/.config/IDEOCODE/azure-openai.env`, `AZURE_OPENAI_*`, or an existing `az login`
   - OpenRouter: `OPENROUTER_API_KEY`
   - Fireworks: `~/.config/IDEOCODE/fireworks.env`, `FIREWORKS_API_KEY`
   - MiniMax: `~/.config/IDEOCODE/minimax.env`, `MINIMAX_API_KEY`
   - NVIDIA NIM: `~/.config/IDEOCODE/nvidia-nim.env`, `NVIDIA_API_KEY`
   - Alibaba Cloud Coding Plan: existing IDEOCODE config/env if present
5. Prefer whichever provider is already configured and verify it with `IDEOCODE auth-test --all-configured` or a provider-specific auth test when appropriate.
6. Only if no usable provider is already configured, guide me through the minimal manual step needed:
   - Claude: `IDEOCODE login --provider claude`
   - GitHub Copilot: `IDEOCODE login --provider copilot`
   - OpenAI: `IDEOCODE login --provider openai`
   - Gemini: `IDEOCODE login --provider gemini`
   - Azure OpenAI: `IDEOCODE login --provider azure`
   - Fireworks: `IDEOCODE login --provider fireworks`
   - MiniMax: `IDEOCODE login --provider minimax`
   - NVIDIA NIM: `IDEOCODE login --provider nvidia-nim`
   - Alibaba Cloud Coding Plan: `IDEOCODE login --provider alibaba-coding-plan`
   - OpenRouter: help me set `OPENROUTER_API_KEY`
   - Anthropic direct API: help me set `ANTHROPIC_API_KEY`
7. After setup, run a simple smoke test with `IDEOCODE run "say hello"` and confirm it works.
8. If I want browser automation, also check `IDEOCODE browser status`. If browser automation is not ready, run `IDEOCODE browser setup`, verify the built-in `browser` tool works, and explain any remaining manual step.
...
This is intended to be a copy-paste bootstrap prompt for IDEOCODE itself or any other coding agent.
