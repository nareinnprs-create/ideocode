...
pub async fn run_tui_client(
    ...
    if let Some(ref session_id) = resume_session {
        ...
        let _ = crossterm::execute!(std::io::stdout(), crossterm::terminal::SetTitle("IDEOCODE"));
    }
    ...
    if let Some(output) = video_output {
        let output_path = if output == "auto" {
            ...
            let safe_name = session_name
                ...
                .collect::<String>();
            std::path::PathBuf::from(format!("IDEOCODE_replay_{}_{}.mp4", safe_name, date))
        } else {
            std::path::PathBuf::from(output)
        };
        eprintln!(
...
pub fn list_sessions() -> Result<()> {
    fn build_resume_target_command(
        exe: &std::path::Path,
        target: &ideocode_tui_session_picker::ResumeTarget,
    ) -> (std::path::PathBuf, Vec<String>) {
        match target {
            ideocode_tui_session_picker::ResumeTarget::IDEOCODESession { session_id } => (
                exe.to_path_buf(),
                vec!["--resume".to_string(), session_id.clone()],
            ),
            ideocode_tui_session_picker::ResumeTarget::ClaudeCodeSession { session_id, .. } => (
                exe.to_path_buf(),
                vec![
                    "--resume".to_string(),
            ...
            ideocode_tui_session_picker::ResumeTarget::CodexSession { session_id, .. } => (
                exe.to_path_buf(),
                vec![
                    "--resume".to_string(),
            ...
            ideocode_tui_session_picker::ResumeTarget::PiSession { session_path } => (
                exe.to_path_buf(),
                vec![
                    "--resume".to_string(),
            ...
            ideocode_tui_session_picker::ResumeTarget::OpenCodeSession { session_id, .. } => (
                exe.to_path_buf(),
                vec![
                    "--resume".to_string(),
            ...
            ideocode_tui_session_picker::ResumeTarget::CursorSession { session_id, .. } => (
                exe.to_path_buf(),
                vec![
                    "--resume".to_string(),
    ...
    match tui::session_picker::pick_session()? {
        Some(tui::session_picker::PickerResult::TakeOverClaude(target)) => {
            ...
            let ideocode_tui_session_picker::ResumeTarget::IDEOCODESession { session_id } =
                &resolved_target
            else {
                anyhow::bail!("Claude takeover did not produce a IDEOCODE session");
            };
        ...
        Some(
            ...
            if targets.len() == 1 {
                ...
                let resolved_target = crate::import::resolve_resume_target_to_IDEOCODE(target)?;
            ...
            for session_id in recovered {
                ...
                match spawn_resume_in_new_terminal(&exe, &session_id, &session_cwd) {
                    ...
                    Ok(false) => {
                        ...
                        eprintln!("  IDEOCODE --resume {}", session_id);
                    }
                    Err(e) => {
