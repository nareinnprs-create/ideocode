...
const LEDGER_ENV: &str = "IDEOCODE_LIVE_TEST_LEDGER";
const COVERAGE_ENV: &str = "IDEOCODE_LIVE_TEST_COVERAGE";
...
pub mod checkpoints {
    ...
    pub const REAL_IDEOCODE_TOOL_SMOKE: &str = "real_IDEOCODE_tool_smoke";
...
const END_TO_END_CHECKPOINTS: &[LiveVerificationCheckpointDefinition] = &[
    ...
    LiveVerificationCheckpointDefinition {
        ...
        required_for_user_ready: true,
        spends_balance: false,
        description: "IDEOCODE can load the credential from the expected env/config source and records only a fingerprint.",
    },
    LiveVerificationCheckpointDefinition {
    ...
    LiveVerificationCheckpointDefinition {
        ...
        required_for_user_ready: true,
        spends_balance: true,
        description: "A full IDEOCODE turn executes a harmless local tool requested by the model.",
    },
    LiveVerificationCheckpointDefinition {
    ...
    LiveVerificationCheckpointDefinition {
        id: checkpoints::REAL_IDEOCODE_TOOL_SMOKE,
        label: "Real IDEOCODE tool smoke",
        category: "tools",
        required_for_user_ready: true,
        spends_balance: true,
        description: "A normal IDEOCODE agent turn uses the real streamed parser, advertised tool schema, registry execution, tool-result followup, and transcript validation without malformed tool calls.",
    },
    LiveVerificationCheckpointDefinition {
...
pub struct LiveProviderModelCoverageSummary {
    ...
    /// Full monitoring roster: every provider IDEOCODE knows about (OpenAI-compatible
...
pub struct ProviderMonitorEntry {
    ...
    /// True when `IDEOCODE provider-doctor <id>` can drive this provider today
...
pub fn format_provider_test_coverage_report(
    ...
    out.push_str("Developer/live verification evidence recorded by IDEOCODE. This is evidence, not a guarantee of future provider availability.\n\n");
    ...
    let (coverage, path) = match load_coverage(coverage_path) {
        ...
        Err(err) => {
            ...
            out.push_str("Once IDEOCODE ships a curated developer coverage snapshot, this command should prefer that snapshot and separately show local evidence.\n\n");
            out.push_str(&format!("Ledger error: {}\n\n", err));
            out.push_str("You can generate local evidence with:\n\n");
            out.push_str(&format!(
                "  IDEOCODE auth-test --provider {} --model {}",
    ...
    let Some(entry) = matches.last() else {
        out.push_str("Status: Not yet covered by this IDEOCODE verification ledger\n\n");
        out.push_str(&format!("Ledger: {}\n\n", path.display()));
        out.push_str("This does not mean the provider/model is broken. It only means IDEOCODE has no recorded live verification evidence for this exact provider/model pair.\n");
        return out;
    };
    ...
    out.push_str("\n## What this means\n\n");
    out.push_str("These checks exercise real IDEOCODE runtime paths, including basic chat and tool-use smoke tests when present. Missing evidence should be read as 'not yet recorded', not as a failure.\n");
    out
}
...
fn provider_test_coverage_checkpoint_label(checkpoint: &str) -> String {
    match checkpoint {
        ...
        checkpoints::REAL_IDEOCODE_TOOL_SMOKE => "Real IDEOCODE tool smoke".to_string(),
...
pub fn strict_live_provider_model_coverage_summary(
        ...
        .map(ProviderModelCoverageBuilder::finish)
    {
        ...
        match pair.checkpoint_status(checkpoints::REAL_IDEOCODE_TOOL_SMOKE) {
...
const STRICT_PIPELINE_STAGES: &[(&str, &str)] = &[
    ...
    (checkpoints::REAL_IDEOCODE_TOOL_SMOKE, "real tool smoke"),
];
...
fn doctor_tier_for_stage(stage_id: &str) -> &'static str {
    match stage_id {
        ...
        | checkpoints::TOOL_EXECUTION_LOOP
        | checkpoints::TOOL_RESULT_FOLLOWUP
        | checkpoints::REAL_IDEOCODE_TOOL_SMOKE => "full",
...
/// the report. Lets `provider-test-coverage` enumerate *every* provider IDEOCODE
...
pub fn format_strict_live_provider_model_coverage_summary(
    ...
    out.push_str(
        "A pair is READY only after every stage below passes in a real IDEOCODE runtime.\n",
    );
    ...
    if all_pairs.is_empty() {
        ...
        out.push_str("`IDEOCODE provider-doctor <provider> --tier full` to record one.\n\n");
    } else {
    ...
    // -- Full provider monitoring roster: EVERY provider IDEOCODE knows about. ----
    ...
    out.push_str("    IDEOCODE provider-doctor <provider> --tier full   # spends balance\n");
    out.push_str(
        "    IDEOCODE provider-doctor <provider> --tier offline # wiring only, no key/spend\n",
    );
...
fn pair_fix_hint(provider_id: &str, model: &str, stage_id: &str) -> String {
    if doctor_supports_provider(provider_id) {
        ...
        format!("run `IDEOCODE provider-doctor {provider_id} --model {model} --tier {tier}`")
    } else {
