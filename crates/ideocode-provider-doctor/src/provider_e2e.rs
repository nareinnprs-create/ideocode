...
//! - [`DoctorTier::Offline`]: no API key, no network, no spend. Validates IDEOCODE's
...
pub enum DoctorTier {
    /// No key, no network, no spend. Validates IDEOCODE-side wiring only.
    Offline,
...
pub async fn run_claude_native_e2e(
    ...
    // `IDEOCODE_RUNTIME_PROVIDER=claude-api` would silently test the API-key path
        ...
        checks.push(DoctorCheck::failed(
            ...
            format!(
                "could not select the Claude OAuth credential path: {error}. \
                 Run `IDEOCODE login --provider claude` to mint a fresh OAuth token."
            ),
        ));
        return Ok(finish_report(
            provider_id,
...
/// Google OAuth tokens minted by `IDEOCODE login --provider antigravity`; the
...
/// accepts every schema construct IDEOCODE emits), then any Gemini model, then any
                ...
                checks.push(DoctorCheck::failed(
                    ...
                    format!(
                        "could not resolve an Antigravity credential: {error}. \
                         Run `IDEOCODE login --provider antigravity` to sign in."
                    ),
                ));
                return Ok(finish_report(
                    provider_id,
...
impl NativeProviderKind {
    ...
    fn spec(self) -> NativeProviderSpec {
        match self {
            Self::OpenAi => NativeProviderSpec {
                ...
                auth_env_key: None,
                login_hint: "IDEOCODE login --provider openai",
            },
            Self::Gemini => NativeProviderSpec {
                ...
                auth_env_key: None,
                login_hint: "IDEOCODE login --provider gemini",
            },
            Self::Cursor => NativeProviderSpec {
                ...
                login_hint: "IDEOCODE login --provider cursor",
            },
            Self::Copilot => NativeProviderSpec {
                ...
                auth_env_key: None,
                login_hint: "IDEOCODE login --provider copilot",
            },
            Self::Bedrock => NativeProviderSpec {
                ...
                login_hint: "IDEOCODE login --provider bedrock",
            },
            Self::IDEOCODE => NativeProviderSpec {
                provider_id: "IDEOCODE",
                label: "IDEOCODE Subscription",
                ...
                auth_source: "IDEOCODE subscription API key (IDEOCODE_API_KEY)",
                auth_env_key: Some("IDEOCODE_API_KEY"),
                login_hint: "IDEOCODE login --provider IDEOCODE",
            },
            Self::Azure => NativeProviderSpec {
                ...
                login_hint: "IDEOCODE login --provider azure",
            },
        }
    }
    ...
    fn cheapest_model(self, catalog: &[String]) -> Option<String> {
        ...
        let cheap_markers: &[&str] = match self {
            ...
            Self::IDEOCODE => &["mini", "flash", "haiku", "lite", "nano"],
...
struct NativeProviderSpec {
    ...
    label: &'static str,
    /// IDEOCODE-side routing/activation contract for the wiring checkpoints.
    contract: WiringContract,
    ...
    /// `IDEOCODE login` hint surfaced when the credential cannot be resolved.
...
/// The IDEOCODE-side wiring a given compat profile is expected to activate.
///
...
/// and IDEOCODE remaps them to their native runtimes. The doctor must assert the
...
mod tests {
    ...
    fn native_provider_kind_maps_every_generic_id() {
        for (id, expected) in [
            ...
            ("IDEOCODE", NativeProviderKind::IDEOCODE),
    ...
    fn native_provider_specs_are_self_consistent() {
        ...
        for kind in [
            ...
            if kind == NativeProviderKind::IDEOCODE {
                assert!(
                    spec.contract.switch_prefix.is_empty(),
                    "IDEOCODE switches must use bare managed model ids"
                );
            } else {
                assert!(
