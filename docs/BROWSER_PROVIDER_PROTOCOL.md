...
Status: draft
Owner: IDEOCODE
Audience: IDEOCODE core, browser bridge authors, adapter authors

## Why this exists

IDEOCODE should expose a single first-class `browser` tool while remaining compatible with multiple browser automation backends:
...
The protocol in this document defines the **normalized contract** between IDEOCODE and a browser provider.

This is intentionally **not** a demand that every bridge speak exactly the same native command language. Instead:

- IDEOCODE defines a **core semantic layer** it can rely on
...
1. **One first-class tool in IDEOCODE**
...
3. **Capability negotiation**
   - IDEOCODE should know what each provider can and cannot do.
...
- **browser tool**: the user/model-facing IDEOCODE tool.
...
- **browser session**: the provider's isolated session or attachment scope for a IDEOCODE session.
...
- direct Rust trait calls inside IDEOCODE
...
IDEOCODE should not care whether a provider uses tabs, contexts, profiles, or remote targets internally.
It only needs a stable handle it can reuse.

### `session.ensure`

Creates or reuses a browser session for a IDEOCODE session.

Request:

```json
{
  "client_session_id": "IDEOCODE_session_123",
  "browser_preference": "auto",
  "isolation": "per_IDEOCODE_session",
  "attach": "prefer",
  "persist": true,
  "metadata": {
    "owner": "agent",
...
IDEOCODE must not assume identifier shape or encode browser semantics into them.

---

## Normalized core methods

These are the semantics IDEOCODE can rely on.
...
{
  "snapshot": {
    "format": "IDEOCODE.page_snapshot.v1",
    "root": {
      "node_id": "n1",
      "role": "document",
      "name": "Example Domain",
...
### Rule 3: IDEOCODE core should only rely on normalized methods by default
...
- a IDEOCODE-side adapter knows how to use them safely
...
Semantics or qualities that influence IDEOCODE behavior:
...
## Recommended IDEOCODE integration policy

The IDEOCODE `browser` tool should:
...
4. Should `page.snapshot` support multiple named formats beyond `IDEOCODE.page_snapshot.v1`?
