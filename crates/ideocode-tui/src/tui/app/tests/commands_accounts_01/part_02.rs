...
fn test_subscription_command_shows_ideocode_status_scaffold() {
    ...
    crate::env::remove_var(crate::subscription_catalog::IDEOCODE_API_BASE_ENV);
    ...
    assert_eq!(msg.role, "system");
    assert!(msg.content.contains("IDEOCODE Subscription Status"));
    assert!(msg.content.contains("/login IDEOCODE"));
...
fn test_subscribe_command_shows_pitch_with_plans_and_next_step() {
    ...
    assert_eq!(msg.role, "system");
    assert!(msg.content.contains("Subscribe to IDEOCODE"));
