...
/// Cached auth status plus the `IDEOCODE_HOME` it was computed under.
///
/// Auth probes read credential files relative to `IDEOCODE_HOME`. Tests swap
/// `IDEOCODE_HOME` to per-test temp dirs, and a status computed under one home
...
/// CLI tools don't get installed/uninstalled while IDEOCODE is running, so caching
...
pub fn browser_suppressed(cli_no_browser: bool) -> bool {
    cli_no_browser
        ...
        || env_truthy("IDEOCODE_NO_BROWSER")
...
/// location no installed or self-dev IDEOCODE binary ever runs from.
///
...
/// multiple browser tabs. Set `IDEOCODE_ALLOW_BROWSER_IN_TESTS=1` to opt out
...
pub fn running_in_test_harness() -> bool {
    ...
    *IN_TEST_HARNESS.get_or_init(|| {
        if env_truthy("IDEOCODE_ALLOW_BROWSER_IN_TESTS") {
            return false;
        }
        std::env::current_exe()
            .ok()
...
fn auth_timing_logging_enabled() -> bool {
    env_truthy("IDEOCODE_AUTH_TIMING")
}
...
fn log_auth_status_snapshot(event: &str, status: &AuthStatus) {
    crate::logging::auth_event(
        ...
        "all",
        &[
            ("IDEOCODE", auth_state_label(status.IDEOCODE)),
...
impl AuthStatus {
    ...
    pub fn has_any_available(&self) -> bool {
        self.anthropic.state == AuthState::Available
            || self.IDEOCODE == AuthState::Available
    ...
    /// IDEOCODE believes credentials are available/expired/missing without leaking
    ...
    pub fn log_snapshot(&self, surface: &str) {
        crate::logging::event_info(
            ...
            vec![
                ...
                ("IDEOCODE", self.IDEOCODE.label().to_string()),
    ...
    pub fn state_for_key(&self, key: LoginProviderAuthStateKey) -> AuthState {
        match key {
            ...
            LoginProviderAuthStateKey::IDEOCODE => self.IDEOCODE,
        ...
        let (
            ...
            crate::provider_catalog::LoginProviderTarget::IDEOCODE => {
                let (source, detail) = summarize_sources(vec![
                    env_source(crate::subscription_catalog::IDEOCODE_API_KEY_ENV),
                    config_source(
                        crate::subscription_catalog::IDEOCODE_API_KEY_ENV,
                        crate::subscription_catalog::IDEOCODE_ENV_FILE,
                        "~/.config/IDEOCODE/IDEOCODE-subscription.env",
                    ),
                ]);
                (
                    source,
            ...
            crate::provider_catalog::LoginProviderTarget::OpenRouter => {
                let (source, detail) = summarize_sources(vec![
                    ...
                    config_source(
                        "OPENROUTER_API_KEY",
                        "openrouter.env",
                        "~/.config/IDEOCODE/openrouter.env",
                    ),
            ...
            crate::provider_catalog::LoginProviderTarget::ClaudeApiKey => {
                ...
                // config file (`~/.config/IDEOCODE/anthropic.env`), *not* an env
                // var and *not* `~/.IDEOCODE/auth.json` (which holds the separate
            ...
            crate::provider_catalog::LoginProviderTarget::Azure => {
                let (source, detail) = summarize_sources(vec![
                    ...
                    config_source(
                        ...
                        "~/.config/IDEOCODE/azure-openai.env",
                    ),
                ]);
                (
                    source,
            ...
            crate::provider_catalog::LoginProviderTarget::Bedrock => {
                let (source, detail) = summarize_sources(vec![
                    ...
                    config_source(
                        ...
                        "~/.config/IDEOCODE/bedrock.env",
                    ),
                    env_source("AWS_PROFILE"),
                    env_source("IDEOCODE_BEDROCK_PROFILE"),
                    ...
                    crate::provider_catalog::active_named_provider_profile_credential_source()
                {
                    ...
                    summarize_sources(vec![
                        ...
                        config_source(
                            &resolved.api_key_env,
                            &resolved.env_file,
                            format!("~/.config/IDEOCODE/{}", resolved.env_file),
                        ),
...
fn assessment_for_key(
    ...
    match key {
        ...
        LoginProviderAuthStateKey::IDEOCODE
...
fn anthropic_oauth_source(status: &AuthStatus) -> Option<(AuthCredentialSource, String)> {
    ...
    {
        return Some((
            AuthCredentialSource::IDEOCODEManagedFile,
            "~/.IDEOCODE/auth.json".to_string(),
        ));
    }
...
fn openai_oauth_source(status: &AuthStatus) -> Option<(AuthCredentialSource, String)> {
        ...
        .is_empty()
    {
        return Some((
            AuthCredentialSource::IDEOCODEManagedFile,
            "~/.IDEOCODE/openai-auth.json".to_string(),
        ));
    }
