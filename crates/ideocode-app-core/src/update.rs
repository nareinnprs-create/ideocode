...
/// who opens IDEOCODE a few dozen times an hour exhausted the bucket and then saw
...
fn source_build_repo_dir() -> Result<PathBuf> {
    Ok(source_build_root()?.join("IDEOCODE"))
}

pub fn should_auto_update() -> bool {
    if std::env::var("IDEOCODE_NO_AUTO_UPDATE").is_ok() {
        return false;
    }
...
/// Build IDEOCODE from source by cloning/pulling the repo and running cargo build
fn build_from_source() -> Result<PathBuf> {
    let started = Instant::now();
    let build_dir = source_build_root()?;
    fs::create_dir_all(&build_dir)?;

    let repo_dir = build_dir.join("IDEOCODE");
    ...
    if repo_dir.join(".git").exists() {
        ...
        let output = std::process::Command::new("git")
            .args([
                "clone", "--depth", "1", "--branch", "main", &clone_url, "IDEOCODE",
            ])
...
pub fn download_and_install_blocking_with_progress(
    ...
    let temp_path = temp_dir.join(format!("IDEOCODE-update-{}", std::process::id()));
    ...
    if asset.name.ends_with(".tar.gz") {
        ...
        for entry in archive.entries()? {
            ...
            entry.unpack(&dest)?;
            if file_name.starts_with("IDEOCODE") && !file_name.ends_with(".bin") {
                extracted_binary = Some(dest);
            }
        }
        let Some(extracted_binary) = extracted_binary else {
            anyhow::bail!("Could not find IDEOCODE binary inside tar.gz archive");
        };
...
mod tests {
    ...
    fn test_verify_asset_checksum_text_accepts_matching_digest() {
        ...
        let contents = format!("{}  IDEOCODE-linux-x86_64.tar.gz\n", digest);
        verify_asset_checksum_text(&contents, "IDEOCODE-linux-x86_64.tar.gz", bytes).unwrap();
    }
    ...
    fn test_verify_asset_checksum_text_rejects_mismatch() {
        ...
        let contents = format!("{}  IDEOCODE-linux-x86_64.tar.gz\n", wrong);
        let err = verify_asset_checksum_text(&contents, "IDEOCODE-linux-x86_64.tar.gz", b"actual")
            .unwrap_err()
            .to_string();
    ...
    fn test_parse_sha256sums_rejects_invalid_digest() {
        let err = parse_sha256sums("not-a-sha  IDEOCODE-linux-x86_64.tar.gz\n")
            .unwrap_err()
            .to_string();
