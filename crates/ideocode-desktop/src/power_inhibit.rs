...
/// Best-effort inhibitor that keeps laptops awake while IDEOCODE is actively
        ...
        match build_inhibit_command(platform).spawn() {
            ...
            Err(error) => {
                crate::desktop_log::error(format_args!(
                    "IDEOCODE-desktop: failed to acquire power inhibitor: {error}"
                ));
                self.available = false;
            }
        }
        ...
        if let Some(mut child) = self.child.take() {
            if let Err(error) = child.kill() {
                crate::desktop_log::warn(format_args!(
                    "IDEOCODE-desktop: failed to stop power inhibitor process: {error}"
                ));
            }
            if let Err(error) = child.wait() {
                crate::desktop_log::warn(format_args!(
                    "IDEOCODE-desktop: failed to reap power inhibitor process: {error}"
                ));
            }
        }
    }
...
fn build_linux_systemd_inhibit_command() -> Command {
    ...
    command
        ...
        .arg("--who=IDEOCODE")
        .arg("--why=IDEOCODE is streaming or processing active work")
        .arg("sleep")
        .arg("infinity")
        .stdin(Stdio::null())
        .stdout(Stdio::null())
...
mod tests {
    ...
    fn linux_inhibitor_blocks_sleep_and_lid_switch() {
        ...
        assert!(args.contains(&"--who=IDEOCODE".to_string()));
