...
fn auth_full_specs(auth: &AuthStatus) -> Vec<(String, AuthState)> {
    ...
    fn dual_method_label(
        ...
        let runtime_provider = std::env::var("IDEOCODE_RUNTIME_PROVIDER").ok();
...
fn version_display_candidates() -> Vec<String> {
    let full = format!("IDEOCODE {}", semver());
    let core = format!("IDEOCODE {}", semver_core());
    let minor = format!("IDEOCODE {}", semver_minor());
...
mod tests {
    ...
    fn ensure_test_IDEOCODE_home_if_unset() {
        ...
        crate::env::set_var("IDEOCODE_HOME", path);
    }
    ...
    fn header_model_display_name_sweeps_real_model_catalog() {
        ...
        // prettify_model_id, over the model ids IDEOCODE actually routes.
        let cases = [
            // Anthropic
    ...
    fn header_provider_auth_tag_reports_active_credential_for_openai() {
        ...
        let prev = std::env::var_os("IDEOCODE_RUNTIME_PROVIDER");
        ...
        if let Some(value) = prev {
            crate::env::set_var("IDEOCODE_RUNTIME_PROVIDER", value);
        }
    }
    ...
    fn header_provider_auth_tag_honors_runtime_selection_and_oauth_first() {
        ...
        crate::env::set_var("IDEOCODE_RUNTIME_PROVIDER", "claude-api");
        assert_eq!(header_provider_auth_tag("anthropic", &both), "api-key");

        // Explicit OAuth selection.
        crate::env::set_var("IDEOCODE_RUNTIME_PROVIDER", "claude");
    ...
    fn build_persistent_header_prefers_configured_model_during_remote_connect() {
        let _guard = crate::storage::lock_test_env();
        let prev_model = std::env::var_os("IDEOCODE_MODEL");
        let prev_provider = std::env::var_os("IDEOCODE_PROVIDER");
        crate::env::set_var("IDEOCODE_MODEL", "gpt-5.4");
        crate::env::set_var("IDEOCODE_PROVIDER", "openai");
        ...
        if let Some(prev_model) = prev_model {
            crate::env::set_var("IDEOCODE_MODEL", prev_model);
        } else {
            crate::env::remove_var("IDEOCODE_MODEL");
        }
        if let Some(prev_provider) = prev_provider {
            crate::env::set_var("IDEOCODE_PROVIDER", prev_provider);
        } else {
            crate::env::remove_var("IDEOCODE_PROVIDER");
        }
    }
    ...
    fn build_header_lines_omits_placeholder_provider_label_when_unknown() {
        ...
        // sibling test that sets IDEOCODE_MODEL=gpt-5.4 mid-flight leaks into this
    ...
    fn build_header_lines_hides_secondary_placeholder_during_brief_connecting_phase() {
        // Same env sensitivity as the placeholder test above: IDEOCODE_MODEL /
        // IDEOCODE_PROVIDER mutations from sibling tests change what renders.
        let _guard = crate::storage::lock_test_env();
        let app = crate::tui::app::App::new_for_remote(None);
    ...
    fn auth_status_line_marks_active_credential_when_both_configured() {
        ...
        let rendered_with = |runtime: Option<&str>| {
            match runtime {
                Some(value) => crate::env::set_var("IDEOCODE_RUNTIME_PROVIDER", value),
                None => crate::env::remove_var("IDEOCODE_RUNTIME_PROVIDER"),
            }
