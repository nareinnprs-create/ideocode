...
fn parse_account_command(trimmed: &str) -> Option<Result<AccountCommand, String>> {
    ...
    if let Some(provider) = resolve_account_provider_descriptor(first) {
        ...
        let parsed = match subcommand {
            ...
            "status" if provider.id == "IDEOCODE" => AccountCommand::IDEOCODEStatus,
            "manage" if provider.id == "IDEOCODE" => AccountCommand::IDEOCODEManage,
            "logout" if provider.id == "IDEOCODE" => AccountCommand::IDEOCODELogout,
...
fn save_openai_compat_setting(app: &mut App, setting: OpenAiCompatSetting, value: Option<&str>) {
    ...
    let (env_key, normalized_value) = match setting {
        OpenAiCompatSetting::ApiBase => {
            ...
            ("IDEOCODE_OPENAI_COMPAT_API_BASE", normalized)
        }
...
mod tests {
    ...
    fn parse_native_ideocode_account_actions() {
        ...
        assert!(matches!(
            parse_account_command("/account IDEOCODE status"),
            Some(Ok(AccountCommand::IDEOCODEStatus))
        ));
        assert!(matches!(
            parse_account_command("/account IDEOCODE manage"),
            Some(Ok(AccountCommand::IDEOCODEManage))
        ));
        assert!(matches!(
            parse_account_command("/account IDEOCODE logout"),
            Some(Ok(AccountCommand::IDEOCODELogout))
        ));
    }
    ...
    fn render_auth_doctor_markdown_includes_recovery_steps() {
        ...
        assert!(markdown.contains("IDEOCODE login --provider openai"));
        assert!(markdown.contains("Review current state: IDEOCODE auth status --json"));
    }
}
