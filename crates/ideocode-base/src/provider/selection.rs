...
impl MultiProvider {
    ...
    pub(super) fn initial_provider_from_env() -> Option<ActiveProvider> {
        let explicit = std::env::var("IDEOCODE_INITIAL_PROVIDER_EXPLICIT")
            .ok()
            .is_some_and(|value| {
                matches!(
        ...
        std::env::var("IDEOCODE_ACTIVE_PROVIDER")
            .ok()
    ...
    pub fn default_model_selection_from_route(
        ...
        let provider_key = match &api_method_kind {
            ModelRouteApiMethod::IDEOCODESubscription => Some("IDEOCODE".to_string()),
    ...
    pub fn model_switch_request_for_session_route(
            ...
            .filter(|api_method| !api_method.is_empty())
        {
            match ModelRouteApiMethod::parse(api_method) {
                ModelRouteApiMethod::IDEOCODESubscription => return model.to_string(),
