...
def default_log_path() -> Path:
    ...
    if xdg:
        return Path(xdg) / "IDEOCODE" / "desktop" / "performance.log"
    return Path.home() / ".cache" / "IDEOCODE" / "desktop" / "performance.log"
...
def summarize(events: list[dict[str, Any]], scope: str, malformed: int, top: int) -> dict[str, Any]:
    ...
    no_paint = top_events(events, "IDEOCODE-desktop-no-paint-profile", ("payload", "gap_ms"), top)
    frames = top_events(events, "IDEOCODE-desktop-frame-profile", ("payload", "worst_wall_ms"), top)
    session_events = top_events(events, "IDEOCODE-desktop-session-event-profile", ("payload", "ui_queue_delay_ms"), top)
    card_loads = top_events(events, "IDEOCODE-desktop-session-cards-load-profile", ("payload", "loaded_in_ms"), top)
    single_card_loads = top_events(events, "IDEOCODE-desktop-session-card-refresh-profile", ("payload", "loaded_in_ms"), top)
    preference_saves = top_events(events, "IDEOCODE-desktop-preferences-save-profile", ("payload", "saved_in_ms"), top)
    restores = top_events(events, "IDEOCODE-desktop-crashed-sessions-restore-profile", ("payload", "elapsed_ms"), top)
...
def print_text(summary: dict[str, Any], path: Path) -> None:
    print("IDEOCODE Desktop performance report")
    print(f"log: {path}")
