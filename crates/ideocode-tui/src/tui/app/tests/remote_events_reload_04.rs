...
/// Run `f` with the shared env lock held and `IDEOCODE_MODEL`/`IDEOCODE_PROVIDER`
...
/// config `default_model` (shared test home) or exported `IDEOCODE_MODEL` would
...
fn test_info_widget_local_direct_api_runtime_shows_cost_based_usage() {
    ...
    for (runtime_provider, provider_name, model, expected_auth) in cases {
        crate::env::set_var("IDEOCODE_RUNTIME_PROVIDER", runtime_provider);
    ...
    crate::env::set_var("IDEOCODE_RUNTIME_PROVIDER", "IDEOCODE");
    crate::env::remove_var("IDEOCODE_OPENROUTER_ALLOW_NO_AUTH");
    ...
    crate::env::set_var("IDEOCODE_RUNTIME_PROVIDER", "openai-compatible");
...
fn test_anthropic_api_cost_accounts_for_split_cache_tokens() {
    ...
    let _guard = crate::storage::lock_test_env();
    let saved_runtime = std::env::var_os("IDEOCODE_RUNTIME_PROVIDER");
