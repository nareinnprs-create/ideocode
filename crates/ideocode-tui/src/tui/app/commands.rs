...
pub(super) fn is_auto_poke_connectivity_error(error: &str) -> bool {
    // Delegate to the shared connectivity classifier (IDEOCODE-app-core's
        ...
        Ok(result) => {
            ...
            match result {
                Ok(prepared) => {
                    ...
                    let socket = std::env::var("IDEOCODE_SOCKET").ok();
...
fn handle_subagent_command(app: &mut App, trimmed: &str) -> bool {
    ...
    if app.is_remote {
        app.push_display_message(DisplayMessage::error(
            "/subagent requires a live IDEOCODE server connection in remote mode.".to_string(),
        ));
        return true;
    }
...
/// and macOS shortcuts, and any conflicts with IDEOCODE's own keybindings.
...
pub(super) fn handle_log_command(app: &mut App, trimmed: &str) -> bool {
    ...
    crate::logging::info(&format!(
        "IDEOCODE_LOG_MARK id={} session={} provider={} model={} cwd={} note={}",
        marker_id,
        app.session.id,
        app.provider_name(),
        app.provider_model(),
    ...
    let mut message = format!(
        "Log mark written: {}\n\nAgents can search ~/.IDEOCODE/logs/ for IDEOCODE_LOG_MARK or this marker id.",
        marker_id
    );
    if !note.is_empty() {
...
Step 1/4: Tell IDEOCODE where to connect.
...
Security model
  - IDEOCODE stores this host/user target so you can run /ssh {} later.
  - IDEOCODE does not ask for or store your SSH password.
  - If a password is needed, it will be typed into your system ssh prompt, not into IDEOCODE.
...
IDEOCODE will ask for the SSH target, then use your system SSH client for authentication. IDEOCODE never stores SSH passwords."
                    .to_string(),
            ));
        }
        Ok(config) => {
            ...
            lines.push("".to_string());
            lines.push("Security: IDEOCODE stores targets only, never SSH passwords.".to_string());
...
IDEOCODE verified that {} is reachable through your system SSH client.

What this means:
  - Authentication is handled by OpenSSH / your SSH agent.
  - IDEOCODE did not see or store your password.
  - The SSH connection setup is ready for remote IDEOCODE tools.

Next implementation step: start the remote IDEOCODE server over this verified SSH connection.",
...
IDEOCODE could not connect without an interactive login, so it opened a separate terminal running your system ssh command.

What to expect in that terminal
  1. OpenSSH may ask for your password or two-factor prompt.
  2. You type credentials into OpenSSH, not into IDEOCODE.
  3. After login, SSH creates a temporary background control socket.
  4. The terminal verifies that socket before closing.

Security model
  - IDEOCODE cannot read what you type in the SSH terminal.
  - IDEOCODE stores only the target {}.
...
IDEOCODE could not open a terminal automatically. Run this command yourself:

  ssh -f -M -S {} -N {}

Type your password into that SSH prompt if asked. IDEOCODE will not see or store it.",
            profile.name,
            crate::ssh_remote::control_socket_path(&profile.name)
                .map(|p| p.display().to_string())
                .unwrap_or_else(|_| "~/.IDEOCODE/ssh-control/remote.sock".to_string()),
            profile.ssh_target
        ))),
        Err(error) => app.push_display_message(DisplayMessage::error(format!(
...
fn git_command_repo_dir(app: &App) -> Result<PathBuf, String> {
    if let Some(path) = active_working_dir(app) {
        ...
        return Err(format!(
            "Unable to run /git: session working directory {} is not accessible from this IDEOCODE client.",
            path.display()
        ));
    }
...
pub(super) fn build_triage_prompt(focus: &str) -> String {
    let mut prompt = String::from(
        ...
        Hard rules: every public comment, issue reply, or PR description you post MUST end with a clear agent attribution line like '--- *— IDEOCODE agent (automated triage), on behalf of @<repo-owner>*' so it can never be mistaken for the human. \
...
fn build_test_verification_prompt(claim: &str) -> String {
    ...
    format!(
        "Run IDEOCODE's /test verification orchestrator for: {target}\n\n\
...
pub(super) fn handle_dictation_command(app: &mut App, trimmed: &str) -> bool {
    ...
    if trimmed.starts_with("/dictate ") || trimmed.starts_with("/dictation ") {
        app.push_display_message(DisplayMessage::error(
            "Usage: /dictate\nConfigure [dictation] in ~/.IDEOCODE/config.toml to customize command, mode, hotkey, and timeout."
                .to_string(),
        ));
        return true;
    }
...
fn ensure_swarm_prompt_edit_path(
    ...
    let project_path = project_dir.join(".IDEOCODE").join("swarm-prompt.md");
...
pub(super) fn handle_swarm_prompt_command(app: &mut App, trimmed: &str) -> bool {
    ...
    let ideocode_dir = match crate::storage::ideocode_dir() {
        ...
        Err(error) => {
            app.push_display_message(DisplayMessage::error(format!(
                "Failed to locate the IDEOCODE config directory: {}",
                error
            )));
            return true;
        }
        ...
        .spawn()
    {
        Ok(_) => {
            app.push_display_message(DisplayMessage::system(format!(
                "Opening the active swarm routing prompt in {}:\n{}\n\nChanges apply after restarting or reloading IDEOCODE because running agent tool registries cache the prompt.",
                editor,
                path.display()
            )));
    ...
    if trimmed == "/config edit" {
        ...
        if let Some(path) = Config::path() {
            ...
            app.push_display_message(DisplayMessage {
                role: "system".to_string(),
                content: format!(
                    "Opening config in editor...\n{} {}\n\n*Restart IDEOCODE after editing for changes to take effect.*",
                    editor,
                    path.display()
                ),
                tool_calls: vec![],
