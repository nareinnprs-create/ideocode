...
pub async fn run() -> Result<()> {
    ...
    std::thread::Builder::new()
        .name("IDEOCODE-memlog-cleanup".to_string())
    ...
    std::thread::Builder::new()
        .name("IDEOCODE-session-bak-prune".to_string())
        .spawn(crate::session::prune_old_session_backups)
        .ok();
    logging::info("IDEOCODE starting");
    ...
    // provider registry. These crates sit downstream of IDEOCODE-base (so
...
/// Register provider runtimes that live downstream of `IDEOCODE-base` with the
...
fn parse_and_prepare_args() -> Result<Args> {
    ...
    if args.trace {
        crate::env::set_var("IDEOCODE_TRACE", "1");
    }
...
fn report_main_error(error: &anyhow::Error) {
    ...
    if let Some(session_id) = terminal::get_current_session() {
        ...
        output::stderr_info(format!("  IDEOCODE --resume {}", session_id));
        output::stderr_blank_line();
    }
}
...
mod tests {
    ...
    fn auto_install_allowed_without_live_terminal() {
        let args = parse_args(&["IDEOCODE", "login"]);
    ...
    fn auto_install_respects_explicit_disable_even_without_terminal() {
        let mut args = parse_args(&["IDEOCODE", "login"]);
    ...
    fn update_command_still_skips_background_check_before_auto_install_logic() {
        let args = parse_args(&["IDEOCODE", "update"]);
    ...
    fn hidden_spawn_hotkey_argument_is_global_and_preserves_canonical_text() {
        let args = parse_args(&["IDEOCODE", "--spawn-hotkey", "shift+cmd+'", "self-dev"]);
