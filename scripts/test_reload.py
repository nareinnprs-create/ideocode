...
def IDEOCODE_debug_socket():
    """Find the active IDEOCODE debug socket path."""
    runtime_dir = os.environ.get("XDG_RUNTIME_DIR") or f"/run/user/{os.getuid()}"
    return os.path.join(runtime_dir, "IDEOCODE-debug.sock")
...
def dbg(command, session_id=None, timeout=TIMEOUT_SECS):
    ...
    return _send_recv(IDEOCODE_debug_socket(), req, timeout=timeout)
...
def test_reload_context_roundtrip():
    IDEOCODE_dir = pathlib.Path.home() / ".IDEOCODE"
    ctx_path = IDEOCODE_dir / "reload-context.json"
...
def test_reload_info_file():
    IDEOCODE_dir = pathlib.Path.home() / ".IDEOCODE"
    info_path = IDEOCODE_dir / "reload-info"
...
def test_canary_binary_path():
    ...
    manifest_path = home / ".IDEOCODE" / "build-manifest.json"
    ...
    if canary_hash:
        canary_binary = home / ".IDEOCODE" / "builds" / "canary" / "IDEOCODE"
...
def test_multiple_sessions_responsive():
    ...
    try:
        for i in range(N):
            s = create_session(f"/tmp/IDEOCODE-test-{i}")
            sessions.append(s)
...
def check_server_up():
    dbg_sock = IDEOCODE_debug_socket()
    if not os.path.exists(dbg_sock):
        raise RuntimeError(
            f"Debug socket not found at {dbg_sock}.\n"
            "  Start IDEOCODE server first:\n"
            "    IDEOCODE serve   (or just launch IDEOCODE in the repo)"
        )
    try:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Test IDEOCODE selfdev reload mechanism",
    ...
    print("\n╔══════════════════════════════════════════════════════════╗")
    print("║      IDEOCODE selfdev reload test suite                    ║")
    print("╚══════════════════════════════════════════════════════════╝\n")
