...
const ALLOW_LEGACY_AUTH_ENV: &str = "IDEOCODE_ALLOW_CODEX_LEGACY_AUTH";
...
pub struct IDEOCODEOpenAiAuthFile {
...
fn relabel_accounts(auth: &mut IDEOCODEOpenAiAuthFile) -> bool {
...
pub fn load_auth_file() -> Result<IDEOCODEOpenAiAuthFile> {
    ...
    let mut auth = if path.exists() {
        ...
        IDEOCODEOpenAiAuthFile::default()
    };
...
fn load_IDEOCODE_credentials() -> Result<CodexCredentials> {
    ...
    let account = auth
        ...
        .context("No OpenAI accounts in IDEOCODE auth file")?;
...
/// the raw tokens plus a `last_refresh` timestamp. Without an expiry, IDEOCODE
...
pub(crate) fn expires_at_from_access_token(access_token: &str) -> Option<i64> {
    ...
    // `exp` is in seconds since epoch; the rest of IDEOCODE tracks expiry in millis.
