...
/// spawn hooks and spawned terminals as `IDEOCODE_SPAWN_*` env vars so external
...
pub struct SessionSpawnContext {
    ...
    /// Extra `IDEOCODE_SPAWN_*` env entries (e.g. swarm/coordinator ids).
...
/// `IDEOCODE_FOCUS_SESSION_ID` and `IDEOCODE_FOCUS_TITLE` env vars.
...
/// var is exported natively and under a `IDEOCODE_CLIENT_<NAME>` alias.
pub fn focus_session_via_hook_with_env(
    ...
    cmd.args(args)
        .env("IDEOCODE_FOCUS_SESSION_ID", session_id)
        .env("IDEOCODE_FOCUS_TITLE", title)
        .stdin(std::process::Stdio::null())
        .stdout(std::process::Stdio::null())
        .stderr(std::process::Stdio::null());
    for (key, value) in client_terminal_env {
        cmd.env(key, value);
        cmd.env(format!("IDEOCODE_CLIENT_{key}"), value);
    }
...
fn focus_title_best_effort(title: &str) {
    ...
    cmd.arg("-c")
        .arg(
            "sleep 0.4; \
             if command -v wmctrl >/dev/null 2>&1; then wmctrl -a \"$IDEOCODE_WINDOW_TITLE\" >/dev/null 2>&1 && exit 0; fi; \
             if command -v xdotool >/dev/null 2>&1; then xdotool search --name \"$IDEOCODE_WINDOW_TITLE\" windowactivate >/dev/null 2>&1 && exit 0; fi; \
             exit 0",
        )
        .env("IDEOCODE_WINDOW_TITLE", title)
        .stdin(Stdio::null())
        .stdout(Stdio::null())
        .stderr(Stdio::null());
...
fn resume_terminal_candidates_windows() -> Vec<String> {
    std::env::var("IDEOCODE_RESUME_TERMINAL")
        .ok()
        .map(|value| {
            value
                .split(',')
