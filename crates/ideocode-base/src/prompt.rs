...
/// creating `~/.IDEOCODE/swarm-prompt.md` (global) or `./.IDEOCODE/swarm-prompt.md`
...
/// project `./.IDEOCODE/swarm-prompt.md`, then global `~/.IDEOCODE/swarm-prompt.md`,
...
/// `mission` module in the upper `IDEOCODE-app-core` layer; the asset lives here
...
pub fn build_system_prompt_full_with_capabilities(
    ...
    // Add optional prompt overlays from ~/.IDEOCODE/ and ./.IDEOCODE/
    ...
    // Add optional preferred-tool guidance from ~/.IDEOCODE/ and ./.IDEOCODE/
...
impl SelfDevProductContext {
    fn from_working_dir(working_dir: Option<&Path>) -> Self {
        ...
        if path.contains("/crates/IDEOCODE-desktop") || path.ends_with("crates/IDEOCODE-desktop") {
            Self::Desktop
        } else {
            Self::Tui
        }
...
/// Load optional prompt overlay markdown from ~/.IDEOCODE/ and ./.IDEOCODE/
fn load_prompt_overlay_files_from_dir(working_dir: Option<&Path>) -> (Option<String>, usize) {
    ...
    let project_dir = working_dir.unwrap_or(Path::new("."));
    if let Some((content, size)) = load_file(
        &project_dir.join(".IDEOCODE").join("prompt-overlay.md"),
        "Project Prompt Overlay (.IDEOCODE/prompt-overlay.md)",
    ) {
        total_chars += size;
        contents.push(content);
    }
...
/// Load optional preferred-tool guidance from ~/.IDEOCODE/ and ./.IDEOCODE/
fn load_preferred_tools_files_from_dir(working_dir: Option<&Path>) -> (Option<String>, usize) {
    ...
    let project_dir = working_dir.unwrap_or(Path::new("."));
    if let Some((content, size)) = load_file(
        &project_dir.join(".IDEOCODE").join("preferred-tools.md"),
        "Project Preferred Tools (.IDEOCODE/preferred-tools.md)",
    ) {
        total_chars += size;
        contents.push(content);
    }
