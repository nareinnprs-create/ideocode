...
fn spawn_ideocode_server_with_diagnostics() -> Result<()> {
    ...
    let mut child = command.spawn().context("failed to spawn IDEOCODE serve")?;
    ...
    std::thread::Builder::new()
        .name("IDEOCODE-desktop-serve-wait".to_string())
        .spawn(move || match child.wait() {
            Ok(status) if status.success() => crate::desktop_log::info(format_args!(
                "IDEOCODE-desktop: IDEOCODE serve child pid={pid} exited with {status}"
            )),
            Ok(status) => crate::desktop_log::error(format_args!(
                "IDEOCODE-desktop: IDEOCODE serve child pid={pid} exited with {status}"
            )),
            Err(error) => crate::desktop_log::warn(format_args!(
                "IDEOCODE-desktop: failed to wait for IDEOCODE serve child pid={pid}: {error}"
            )),
        })
        .context("failed to spawn IDEOCODE serve wait logger")?;
    ...
    R: Read + Send + 'static,
{
    if let Err(error) = std::thread::Builder::new()
        .name(format!("IDEOCODE-desktop-serve-{stream_name}"))
        .spawn(move || {
            let reader = BufReader::new(pipe);
            for line in reader.lines() {
                match line {
                    Ok(line) => crate::desktop_log::info(format_args!(
                        "IDEOCODE-desktop: IDEOCODE serve pid={pid} {stream_name}: {}",
                        crate::desktop_log::truncate_for_log(&line, 4096)
                    )),
                    Err(error) => {
                        crate::desktop_log::warn(format_args!(
                            "IDEOCODE-desktop: failed reading IDEOCODE serve pid={pid} {stream_name}: {error}"
                        ));
                        break;
                    }
                }
    ...
    {
        crate::desktop_log::warn(format_args!(
            "IDEOCODE-desktop: failed to spawn IDEOCODE serve {stream_name} logger: {error}"
        ));
    }
}
...
pub(super) fn connect_server_with_retry_path(
    ...
    match last_error {
        Some(error) => Err(error).with_context(|| {
            format!(
                "timed out connecting to IDEOCODE server at {}",
                socket_path.display()
            )
        }),
        None => anyhow::bail!("timed out connecting to IDEOCODE server"),
    }
}
...
pub(super) fn validate_reload_socket_path(
    ...
    if trimmed.is_empty() {
        anyhow::bail!("IDEOCODE server advertised an empty reload socket path");
    }

    let new_socket_path = PathBuf::from(trimmed);
    if !new_socket_path.is_absolute() {
        anyhow::bail!(
            "IDEOCODE server advertised non-absolute reload socket path {}",
    ...
    if !metadata.file_type().is_socket() {
        anyhow::bail!(
            "IDEOCODE server advertised reload path that is not a socket: {}",
            new_socket_path.display()
        );
    }
    ...
    if metadata.uid() != effective_uid {
        anyhow::bail!(
            "IDEOCODE server advertised reload socket owned by uid {}, expected uid {}: {}",
            metadata.uid(),
            effective_uid,
            new_socket_path.display()
        );
    ...
    let current_parent = current_socket_path.parent().with_context(|| {
        format!(
            "current IDEOCODE socket path has no parent: {}",
    ...
    let current_parent = current_parent.canonicalize().with_context(|| {
        format!(
            "failed to canonicalize current IDEOCODE socket directory {}",
    ...
    if current_parent != new_parent {
        anyhow::bail!(
            "IDEOCODE server advertised reload socket outside current socket directory: {} (current directory {})",
            new_socket_path.display(),
            current_parent.display()
        );
    }
...
pub(super) fn read_session_id_from_events(
    ...
    while started.elapsed() < timeout {
        ...
        match reader.read_line(&mut line) {
            Ok(0) => anyhow::bail!("IDEOCODE server disconnected before assigning a session"),
            Ok(_) => {
                let value = parse_server_event_line(&line, "waiting for session id")?;
                if value.get("type").and_then(Value::as_str) == Some("session") {
                    let Some(session_id) = value.get("session_id").and_then(Value::as_str) else {
                        anyhow::bail!("IDEOCODE server sent malformed session event");
                    };
                ...
                if value.get("type").and_then(Value::as_str) == Some("error") {
                    ...
                    crate::desktop_log::error(format_args!(
                        "IDEOCODE-desktop: IDEOCODE server rejected fresh session: {}",
                        crate::desktop_log::truncate_for_log(message, 2048)
                    ));
                    anyhow::bail!("IDEOCODE server rejected fresh session: {message}");
                }
            ...
            Err(error) => return Err(error).context("failed to read IDEOCODE server event"),
        }
    }

    anyhow::bail!("timed out waiting for IDEOCODE server session id")
}
...
pub(super) fn read_session_id_from_state(
    ...
    while started.elapsed() < timeout {
        line.clear();
        match reader.read_line(&mut line) {
            Ok(0) => anyhow::bail!("IDEOCODE server disconnected before returning state"),
            Ok(_) => {
                ...
                {
                    let Some(session_id) = value.get("session_id").and_then(Value::as_str) else {
                        anyhow::bail!("IDEOCODE server sent malformed state event");
                    };
                    ...
                    && value.get("id").and_then(Value::as_u64) == Some(state_request_id)
                {
                    let message = value
                        ...
                        .unwrap_or("unknown server error");
                    crate::desktop_log::error(format_args!(
                        "IDEOCODE-desktop: IDEOCODE server rejected state request id={state_request_id}: {}",
                        crate::desktop_log::truncate_for_log(message, 2048)
                    ));
                    anyhow::bail!("IDEOCODE server rejected state request: {message}");
                }
            }
            Err(error)
                if matches!(
    ...
    anyhow::bail!("timed out waiting for IDEOCODE server state")
}
...
pub(super) fn read_model_changed(
    ...
    while started.elapsed() < timeout {
        line.clear();
        match reader.read_line(&mut line) {
            Ok(0) => anyhow::bail!("IDEOCODE server disconnected before switching model"),
            Ok(_) => {
                    ...
                    && value.get("id").and_then(Value::as_u64) == Some(request_id)
                {
                    let message = value
                        ...
                        .unwrap_or("unknown server error");
                    crate::desktop_log::error(format_args!(
                        "IDEOCODE-desktop: IDEOCODE server rejected model switch id={request_id}: {}",
                        crate::desktop_log::truncate_for_log(message, 2048)
                    ));
                    anyhow::bail!("IDEOCODE server rejected model switch: {message}");
                }
            }
            Err(error)
                if matches!(
    ...
    anyhow::bail!("timed out waiting for IDEOCODE server model switch")
}
...
pub(super) fn read_model_catalog(
    ...
    while started.elapsed() < timeout {
        line.clear();
        match reader.read_line(&mut line) {
            Ok(0) => anyhow::bail!("IDEOCODE server disconnected before loading model catalog"),
            Ok(_) => {
                    ...
                    && value.get("id").and_then(Value::as_u64) == Some(request_id)
                {
                    ...
                    anyhow::bail!("IDEOCODE server returned malformed model catalog");
                }
                forward_non_done_server_event(event_tx, &value);
                if value.get("type").and_then(Value::as_str) == Some("error")
                    && value.get("id").and_then(Value::as_u64) == Some(request_id)
                {
                    let message = value
                        ...
                        .unwrap_or("unknown server error");
                    crate::desktop_log::error(format_args!(
                        "IDEOCODE-desktop: IDEOCODE server rejected model catalog request id={request_id}: {}",
                        crate::desktop_log::truncate_for_log(message, 2048)
                    ));
                    anyhow::bail!("IDEOCODE server rejected model catalog request: {message}");
                }
            }
            Err(error)
                if matches!(
    ...
    anyhow::bail!("timed out waiting for IDEOCODE server model catalog")
}
...
pub(super) fn read_control_response(
    ...
    while started.elapsed() < timeout {
        line.clear();
        match reader.read_line(&mut line) {
            Ok(0) => anyhow::bail!("IDEOCODE server disconnected while {action_label}"),
            Ok(_) => {
                ...
                if event_type == "error" && id_matches {
                    let message = value
                        ...
                        .unwrap_or("unknown server error");
                    crate::desktop_log::error(format_args!(
                        "IDEOCODE-desktop: IDEOCODE server rejected {action_label} id={request_id}: {}",
                        crate::desktop_log::truncate_for_log(message, 2048)
                    ));
                    anyhow::bail!("IDEOCODE server rejected {action_label}: {message}");
                }
            }
            Err(error)
                if matches!(
    ...
    anyhow::bail!("timed out waiting for IDEOCODE server while {action_label}")
}
                    ...
                    match event_type {
                        ...
                        Some("error") => {
                            let message = value
                                ...
                                .unwrap_or("unknown server error");
                            anyhow::bail!(
                                "IDEOCODE server rejected cancel request {cancel_request_id}: {message}"
                            );
                        }
                        _ => {}
                    }
...
fn parse_server_event_line(line: &str, context: &str) -> Result<Value> {
    match serde_json::from_str::<Value>(line.trim()) {
        ...
        Err(error) => {
            crate::desktop_log::error(format_args!(
                "IDEOCODE-desktop: failed to parse IDEOCODE server event while {context}: {error}; line={}",
                crate::desktop_log::truncate_for_log(line.trim(), 512)
            ));
            Err(error).context("failed to parse IDEOCODE server event")
        }
    }
}
...
fn validate_server_event_value(value: &Value, context: &str) -> Result<()> {
    let Some(object) = value.as_object() else {
        anyhow::bail!("IDEOCODE server sent non-object event while {context}");
    };
    let Some(event_type) = object.get("type").and_then(Value::as_str) else {
        anyhow::bail!("IDEOCODE server sent event without string type while {context}");
    };
    ...
    match event_type {
        "stdin_request" => {
            ...
            {
                anyhow::bail!(
                    "IDEOCODE server sent stdin_request with non-string prompt while {context}"
                );
            }
            if value
                .get("is_password")
            ...
            {
                anyhow::bail!(
                    "IDEOCODE server sent stdin_request with non-boolean is_password while {context}"
                );
            }
        }
        "reloading"
        ...
        {
            anyhow::bail!(
                "IDEOCODE server sent reloading event with non-string new_socket while {context}"
            );
        }
        _ => {}
    }
...
fn require_non_empty_event_string(
        ...
        .is_some_and(|value| !value.trim().is_empty())
    {
        ...
        anyhow::bail!(
            "IDEOCODE server sent {event_type} event without non-empty string {field} while {context}"
        );
    }
}
