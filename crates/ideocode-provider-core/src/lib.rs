...
pub trait Provider: Send + Sync {
    ...
    /// Returns true if IDEOCODE should use its own compaction for this provider.
...
impl CredentialMode {
    /// Resolve the runtime-env pin (IDEOCODE_*_AUTH / route aliases) for a
...
/// Native tool result to send back to provider bridges that delegate tool execution to IDEOCODE.
...
/// Canonical User-Agent for generic outbound IDEOCODE HTTP requests.
pub const IDEOCODE_USER_AGENT: &str = concat!("IDEOCODE/", env!("CARGO_PKG_VERSION"));
    ...
    CLIENT
        .get_or_init(|| {
            reqwest::Client::builder()
                .user_agent(IDEOCODE_USER_AGENT)
                ...
                .build()
                .unwrap_or_else(|err| {
                    eprintln!("IDEOCODE: failed to build shared provider HTTP client: {err}");
                        ...
                        .build()
                    {
                        ...
                        Err(fallback_err) => {
                            eprintln!(
                                "IDEOCODE: failed to build fallback provider HTTP client: {fallback_err}"
                            );
                            reqwest::Client::new()
                        }
                    }
...
impl RuntimeKey {
    pub fn from_api_method(api_method: &ModelRouteApiMethod, _provider_label: &str) -> Self {
        match api_method {
            ModelRouteApiMethod::IDEOCODESubscription => Self::IDEOCODESubscription,
    ...
    pub fn stable_id(&self) -> String {
        match self {
            Self::IDEOCODESubscription => "IDEOCODE-subscription".to_string(),
...
impl RouteSelection {
    ...
    pub fn routed_model_spec(&self) -> String {
        ...
        match &self.runtime_key {
            RuntimeKey::IDEOCODESubscription => model.to_string(),
...
impl ModelRouteApiMethod {
    ...
    pub fn display_label(&self) -> String {
        match self {
            Self::IDEOCODESubscription => "subscription".to_string(),
