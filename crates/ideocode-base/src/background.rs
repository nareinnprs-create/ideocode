...
impl BackgroundTaskManager {
    ...
    fn publish_task_started_activity(
        ...
        Bus::global().publish(BusEvent::UiActivity(crate::bus::UiActivity::background(
            ...
            format!(
                "**Background task started** `{}` · `{}`\n\nIDEOCODE is running this in the background. Progress, checkpoints, and completion will appear here.",
                task_id, safe_label
            ),
    ...
    /// every IDEOCODE process on the machine:
    ...
    pub async fn adopt(
        ...
        tool_name: &str,
        session_id: &str,
        handle: JoinHandle<Result<ideocode_tool_types::ToolOutput>>,
    ) -> BackgroundTaskInfo {
    ...
    fn progress_wait_result(task: TaskStatusFile) -> BackgroundTaskWaitResult {
        let event_record = task.event_history.last().cloned();
        let progress_event = task
            .progress
            .as_ref()
            .map(|progress| BackgroundTaskProgressEvent {
