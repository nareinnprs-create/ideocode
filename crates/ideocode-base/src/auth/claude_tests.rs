...
fn ideocode_auth_file_roundtrip() {
    ...
    let parsed: IDEOCODEAuthFile = serde_json::from_str(&json).unwrap();
...
fn ideocode_auth_file_legacy_migration_format() {
    ...
    let parsed: IDEOCODEAuthFile = serde_json::from_str(legacy_json).unwrap();
...
fn update_account_profile_sets_email() {
    let mut auth = IDEOCODEAuthFile::default();
...
///   cargo test -p IDEOCODE-base --lib auth::claude::tests::live_keychain -- --ignored --nocapture
#[cfg(target_os = "macos")]
#[test]
