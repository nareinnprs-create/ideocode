...
/// full truecolor.
///
/// Overridable with `IDEOCODE_GLYPH_SAFE_MODE=on|off` (shared with the perf
...
mod tests {
    ...
    fn test_fragile_terminal_override_off_forces_truecolor() {
        ...
        temp_env_scope(
            &[
                ("IDEOCODE_GLYPH_SAFE_MODE", Some("off")),
    ...
    fn test_fragile_terminal_override_on_forces_quantize() {
        temp_env_scope(&[("IDEOCODE_GLYPH_SAFE_MODE", Some("on"))], || {
            assert!(fragile_glyph_cache_terminal());
        });
    }
    ...
    fn test_detect_color_capability_downgrades_on_fragile_override() {
        temp_env_scope(
            &[
                ("IDEOCODE_GLYPH_SAFE_MODE", Some("on")),
