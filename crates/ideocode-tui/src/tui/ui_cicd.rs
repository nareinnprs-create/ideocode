    ...
    pub fn refresh(&mut self) {
        ...
        let mut output = run_command(
            ...
            &[
                ...
                "10",
                "--json",
                "headBranch,status,conclusion,title,updatedAt",
            ],
        );
        if output.is_empty() {
            ...
            for run in arr {
                self.runs.push(CicdRun {
                    ...
                    status: run["conclusion"]
                        .as_str()
                        .unwrap_or(run["status"].as_str().unwrap_or("unknown"))
                        .to_string(),
...
pub fn render_cicd_panel(frame: &mut Frame, area: Rect, state: &CicdPanelState) {
        ...
        .enumerate()
    {
        ...
        lines.push(Line::from(vec![
            ...
            Span::styled(
                run.title.chars().take(50).collect::<String>(),
