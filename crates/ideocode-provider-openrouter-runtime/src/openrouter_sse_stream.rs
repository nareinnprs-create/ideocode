...
fn local_endpoint_troubleshooting_hint(api_base: &str, model: &str) -> &'static str {
    ...
    if lower.contains("localhost:11434") || lower.contains("127.0.0.1:11434") {
        return "Ollama hint: make sure `ollama serve` is running, the model is installed with `ollama pull <model>`, and run IDEOCODE with an installed model, for example `IDEOCODE --provider ollama --model llama3.2 run 'hello'`.";
    }

    if lower.contains("localhost:1234") || lower.contains("127.0.0.1:1234") {
        return "LM Studio hint: start the Local Server in LM Studio, load a chat model, and run IDEOCODE with the exact model id shown by LM Studio's /v1/models endpoint.";
    }
...
async fn stream_response(
    ...
    if send_openrouter_headers {
        req = req
            .header("HTTP-Referer", "https://github.com/IDEOCODE")
            .header("X-Title", "IDEOCODE");
    }
