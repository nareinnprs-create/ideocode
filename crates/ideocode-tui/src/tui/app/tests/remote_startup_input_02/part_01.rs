...
fn test_remote_hydrated_catalog_restores_missing_direct_bedrock_route() {
    with_temp_IDEOCODE_home(|| {
        ...
        match previous_enable {
            Some(value) => crate::env::set_var("IDEOCODE_BEDROCK_ENABLE", value),
            None => crate::env::remove_var("IDEOCODE_BEDROCK_ENABLE"),
        }
        crate::auth::AuthStatus::invalidate_cache();
...
fn test_remote_IDEOCODE_subscription_catalog_is_not_augmented_with_local_auth_routes() {
    with_temp_IDEOCODE_home(|| {
        ...
        app.remote_provider_name =
            Some(crate::subscription_catalog::IDEOCODE_PROVIDER_DISPLAY_NAME.to_string());
        ...
        let expected = crate::subscription_catalog::curated_models()
            .iter()
            .filter(|model| {
                crate::subscription_catalog::IDEOCODETier::Plus.allows(model.min_tier)
            })
            .map(|model| model.id)
            .collect::<std::collections::BTreeSet<_>>();
        assert_eq!(app.remote_model_options.len(), expected.len());
        assert!(app.remote_model_options.iter().all(|route| {
            route.provider == crate::subscription_catalog::IDEOCODE_PROVIDER_DISPLAY_NAME
...
fn test_remote_hydrated_catalog_adds_entitled_IDEOCODE_subscription_routes() {
    with_temp_IDEOCODE_home(|| {
        ...
        match previous_tier {
            Some(value) => crate::env::set_var(crate::subscription_catalog::IDEOCODE_TIER_ENV, value),
