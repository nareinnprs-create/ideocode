...
fn cloud_sessions_config_persists_secret_and_feeds_helper_env_without_args() {
    let _guard = crate::storage::lock_test_env();
    let _saved = SavedEnv::capture(&["IDEOCODE_HOME", "JADE_TOKEN_FOR_TEST"]);
...
fn cloud_sessions_sync_dry_run_reports_without_uploading_or_writing_state() {
    let _guard = crate::storage::lock_test_env();
    let _saved = SavedEnv::capture(&["IDEOCODE_HOME", "IDEOCODE_JADE_SESSIONS_HELPER"]);
    let temp = tempfile::tempdir().expect("tempdir");
    crate::env::set_var("IDEOCODE_HOME", temp.path());
    ...
    crate::env::set_var("IDEOCODE_JADE_SESSIONS_HELPER", &helper);
...
fn resolve_jade_sessions_helper_prefers_explicit_and_env_paths() {
    let _saved = SavedEnv::capture(&["IDEOCODE_JADE_SESSIONS_HELPER"]);
    crate::env::set_var("IDEOCODE_JADE_SESSIONS_HELPER", "/tmp/from-env.py");
