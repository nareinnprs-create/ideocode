...
//!   notice that it renders IDEOCODE poorly and suggest a modern terminal (Ghostty).
//! - Linux: create a .desktop launcher file.
//!
//! Each nudge can be dismissed permanently with "Don't ask again".
//! State is persisted in `~/.IDEOCODE/setup_hints.json`.
...
///   connection.
/// - 3: register three launch hotkeys instead of one. `Cmd+;` opens IDEOCODE in
///   `$HOME`, `Cmd+'` opens it in the last project directory, and `Cmd+Shift+'`
///   opens a self-dev session in the last IDEOCODE repo. Existing users are
...
///   `IDEOCODE-terminal-launch`, avoiding the generated shell-script hop on hotkey
///   press. Scripts/plan are still written for compatibility and diagnostics.
/// - 6: direct launches pass `--spawn-hotkey` into the new IDEOCODE process so
...
/// global hotkey reads this at fire time to reopen IDEOCODE there.
#[cfg(any(test, target_os = "macos", target_os = "linux", windows))]
fn mac_hotkey_last_dir_file() -> Result<PathBuf> {
    Ok(mac_hotkey_support_dir()?.join("last_dir"))
}

/// File holding the last IDEOCODE *repository* directory the user worked in. The
...
/// Load the `[launch_hotkeys]` table from `~/.IDEOCODE/config.toml`.
///
...
/// its own `Cmd+;` hotkey). When `dir` is inside a IDEOCODE repo, the repo root is
...
fn mac_hotkey_launch_agent_path() -> Result<PathBuf> {
    ...
    Ok(home
        .join("Library")
        .join("LaunchAgents")
        .join("com.IDEOCODE.hotkey.plist"))
}
...
<dict>
    <key>Label</key>
    <string>com.IDEOCODE.hotkey</string>
    ...
    <dict>
        <key>IDEOCODE_PREFERRED_TERMINAL</key>
...
/// Launch a new IDEOCODE window in the user's preferred macOS terminal, passing
/// `extra_args` (e.g. `["--resume", "<session-id>"]`) to the IDEOCODE invocation.
///
...
pub fn launch_ideocode_in_macos_terminal(extra_args: &[String]) -> Result<()> {
    ...
    let status = std::process::Command::new("sh")
        ...
        .arg(&command)
        .status()
        .context("failed to launch terminal for IDEOCODE")?;
    if !status.success() {
        anyhow::bail!(
...
fn install_macos_hotkey_listener(
    ...
    let status = std::process::Command::new("launchctl")
        ...
        .status()
        .context("failed to load IDEOCODE LaunchAgent")?;
    if !status.success() {
...
fn startup_hints_for_launch(_state: &SetupHintsState) -> Option<StartupHints> {
    ...
    let spawn_notice = if !_state.hotkey_configured || _state.startup_spawn_hint_dismissed {
        ...
        Some(format!(
            "Cmd+; launches a new IDEOCODE in your home directory from anywhere, system-wide (opens in {}). Cmd+' reopens your last project; Cmd+Shift+' opens a self-dev session.",
...
fn macos_terminal_notice(
    ...
    let message = "The built-in macOS Terminal.app renders IDEOCODE poorly (slow, limited colors, no inline images). Consider a modern terminal such as Ghostty, iTerm2, or Alacritty for a much better experience.".to_string();

    Some(StartupHints::with_status_and_display(
        "Tip: Terminal.app renders IDEOCODE poorly. Try Ghostty, iTerm2, or Alacritty.".to_string(),
        "Terminal",
        message,
    ))
}
...
/// Manual `IDEOCODE setup-hotkey` command.
///
...
pub fn run_setup_hotkey(
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        let terminal = effective_macos_terminal();
        eprintln!("\x1b[1mIDEOCODE setup-hotkey\x1b[0m");
        eprintln!();
        eprintln!("  Preferred terminal: {}", terminal.label());
        eprintln!("  Installing a LaunchAgent with three system-wide IDEOCODE launch hotkeys.");
        eprintln!();
        ...
        match install_macos_hotkey_listener(Some(terminal)) {
            Ok(installed_terminal) => {
                ...
                let _ = state.save();
                eprintln!(
                    "  \x1b[32m✓\x1b[0m Created launch hotkeys → {} + IDEOCODE",
        ...
        if let Some(comp) = detect_linux_compositor() {
            ...
            match install_linux_launch_hotkeys(comp) {
                Ok(changed) => {
                    if changed {
                        eprintln!(
                            "  \x1b[32m✓\x1b[0m Installed IDEOCODE launch hotkeys into your {} config.",
                            comp.name()
                        );
                    } else {
                        eprintln!(
                            "  \x1b[32m✓\x1b[0m IDEOCODE launch hotkeys already up to date in your {} config.",
                            comp.name()
                        );
                    }
                    state.hotkey_configured = true;
...
pub(crate) fn active_primary_launch_hotkey() -> Option<(String, String)> {
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        let exe_path = std::env::current_exe()
            ...
            .unwrap_or_else(|_| "IDEOCODE".to_string());
...
fn run_macos_hotkey_listener() -> Result<()> {
    ...
    // launches IDEOCODE on key-down, then hand the thread to the event loop so the
    ...
    // to a cwd + IDEOCODE argv. Older versions dispatched through generated shell
    ...
    if launch_for_id.is_empty() {
        anyhow::bail!("failed to register any IDEOCODE launch hotkey");
    }
    ...
    GlobalHotKeyEvent::set_event_handler(Some(move |event: GlobalHotKeyEvent| {
        ...
        if let Some(launch) = launch_for_id.get(&event.id) {
            ...
            let command = TerminalCommand::new(&exe_path, args)
                .fresh_spawn()
                .kind("hotkey")
                .spawn_env("IDEOCODE_SPAWN_LABEL", launch.label.clone());
            match spawn_command_in_new_terminal_with(&command, &cwd, |cmd| cmd.spawn().map(|_| ()))
            {
                Ok(true) => {}
                Ok(false) => {
                    macos_hotkey_log("failed to launch IDEOCODE: no terminal candidate worked")
                }
                Err(err) => macos_hotkey_log(&format!("failed to launch IDEOCODE: {err}")),
            }
        }
    }));

    macos_hotkey_log("macOS IDEOCODE launch hotkeys registered; entering event loop");
    ...
    macos_hotkey_log("macOS IDEOCODE launch hotkey event loop exited");
    Ok(())
}
...
/// Log a hotkey-listener diagnostic to both the IDEOCODE log and stderr.
///
...
pub fn maybe_show_setup_hints() -> Option<StartupHints> {
    ...
    // `IDEOCODE setup-launcher` explicitly.
...
fn macos_launch_hotkeys_notice(state: &SetupHintsState) -> Option<StartupHints> {
    ...
    Some(StartupHints::with_status_and_display(
        ...
        "Launch hotkeys",
        format!("Configured IDEOCODE launch hotkeys:\n{}", lines.join("\n")),
    ))
}
...
/// Config file IDEOCODE manages for a flat (`#`-commented) compositor config.
...
/// Directory for IDEOCODE's hidden KDE launcher desktop files.
#[cfg(target_os = "linux")]
...
/// The config file IDEOCODE would manage for the *current* session's compositor.
#[cfg(target_os = "linux")]
...
/// The sentinel that marks IDEOCODE's managed region in `path` for `comp`.
#[cfg(target_os = "linux")]
...
/// Whether IDEOCODE's launch hotkeys are already installed for `comp`.
#[cfg(target_os = "linux")]
fn linux_hotkeys_installed(comp: linux_env::LinuxCompositor) -> bool {
    use linux_env::LinuxCompositor;
    match comp {
        LinuxCompositor::Gnome => gnome_keybinding_list().contains("/IDEOCODE-launch-"),
        LinuxCompositor::Cinnamon => {
            dconf_read("/org/cinnamon/desktop/keybindings/custom-list").contains("IDEOCODE-launch-")
        }
        LinuxCompositor::Mate => {
            dconf_list("/org/mate/desktop/keybindings/").contains("IDEOCODE-launch-")
        }
        LinuxCompositor::Xfce => xfce_shortcut_commands_text().contains("/launch_IDEOCODE_"),
        LinuxCompositor::Kde => kde_globalshortcutsrc_path()
            .and_then(|p| std::fs::read_to_string(p).ok())
            .map(|text| text.contains("[services][IDEOCODE-launch-"))
            .unwrap_or(false),
...
/// Pick a terminal emulator to launch IDEOCODE in on Linux. Honors `$TERMINAL`,
...
/// scripts written under `~/.IDEOCODE/hotkey/`, so the config never embeds shell
...
/// xfconf-query. Stale IDEOCODE entries bound to accelerators we no longer use
...
fn install_xfce_launch_hotkeys() -> Result<bool> {
    ...
    // Remove stale IDEOCODE entries bound to accelerators we no longer use.
    for line in existing.lines() {
        let Some((prop, value)) = line.split_once(char::is_whitespace) else {
            continue;
        };
        if value.trim().contains("/launch_IDEOCODE_") && !wanted.iter().any(|(p, _)| p == prop) {
...
/// `~/.IDEOCODE/hotkey/` and return the chord -> script binds. The scripts `cd`
/// into the target (with `$HOME` fallback for stale/dynamic dirs) and exec the
/// user's terminal running IDEOCODE, so compositor bind lines stay trivial.
#[cfg(target_os = "linux")]
fn write_linux_launch_scripts() -> Result<Vec<linux_env::ScriptBind>> {
    ...
    for entry in resolved {
        ...
        let script_body = format!(
            "#!/bin/sh\n# Auto-generated by IDEOCODE setup-hotkey; re-run it to refresh.\n{cd}exec {exec}\n",
            cd = entry.cd_prefix,
        );
        let script_path = hotkey_dir.join(&entry.script_file_name);
...
/// Timestamped backup matching the `.bak-IDEOCODE-*` convention, taken before
...
fn linux_launch_hotkeys_notice(state: &SetupHintsState) -> Option<StartupHints> {
    ...
    Some(StartupHints::with_status_and_display(
        ...
        "Launch hotkeys",
        format!(
            "Configured IDEOCODE launch hotkeys ({}): {} {}",
            comp.name(),
            lines.join("; "),
            footer
        ),
...
/// - Once the user has learned at least one binding and has launched IDEOCODE at
...
/// Check whether IDEOCODE's keybindings conflict with shortcuts owned by the
...
/// Whether the current terminal triggers IDEOCODE's glyph-safe color quantization
/// (macOS VS Code integrated terminal / Apple Terminal). Mirrors the detection
/// in `IDEOCODE-tui-style`'s color module and `IDEOCODE-app-core::perf` so the
...
/// shown the first time IDEOCODE launches in a fragile-glyph terminal. Discloses
...
its glyph cache under IDEOCODE's full-color animations, rendering letters as boxes. \
IDEOCODE automatically quantizes colors to the 256-palette here to keep text readable; \
the only tradeoff is slightly reduced color fidelity. Animations still run. \
For full color, use Ghostty, iTerm2, kitty, or WezTerm, or set IDEOCODE_GLYPH_SAFE_MODE=off."
        .to_string();
    (
        Some(StartupHints::with_status_and_display(
...
/// Manual `IDEOCODE setup-launcher` command.
pub fn run_setup_launcher() -> Result<()> {
    #[cfg(target_os = "macos")]
    {
        let mut state = SetupHintsState::load();
        eprintln!("\x1b[1mIDEOCODE setup-launcher\x1b[0m");
        eprintln!();
        ...
        match install_macos_app_launcher() {
            Ok((app_dir, terminal)) => {
                ...
                eprintln!(
                    "  \x1b[32m✓\x1b[0m Spotlight/Launchpad/Dock will launch IDEOCODE in {}",
                    terminal.label()
                );
                eprintln!();
                eprintln!("  Tip: pin IDEOCODE.app to your Dock or launch it with Cmd+Space.");
                return Ok(());
            }
            Err(e) => {
                eprintln!("  \x1b[31m✗\x1b[0m Failed: {}", e);
    ...
    #[cfg(windows)]
    {
        ...
        match create_windows_desktop_shortcut(&mut state) {
            Ok(()) => {
                eprintln!("  \x1b[32m✓\x1b[0m Created desktop shortcut: IDEOCODE.lnk");
                Ok(())
            }
            Err(e) => {
                eprintln!("  \x1b[31m✗\x1b[0m Failed: {}", e);
...
/// Create a desktop shortcut/launcher for IDEOCODE.
///
/// - macOS: creates a IDEOCODE.app bundle in ~/Applications/
/// - Windows uses [`windows_setup::create_windows_desktop_shortcut`] via
///   `IDEOCODE setup-launcher` instead (PowerShell/COM is too slow for the
///   startup path).
