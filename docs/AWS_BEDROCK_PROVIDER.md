...
IDEOCODE supports two Bedrock auth styles:

- **Bedrock API key / bearer token**: easiest for local onboarding. IDEOCODE stores the token in its config env file and sends it through the AWS SDK as `AWS_BEARER_TOKEN_BEDROCK`.
...
```bash
IDEOCODE login --provider bedrock
```

This saves `AWS_BEARER_TOKEN_BEDROCK` and `IDEOCODE_BEDROCK_REGION` to `~/.config/IDEOCODE/bedrock.env`.
...
export AWS_REGION=us-east-1
# Optional IDEOCODE-specific overrides:
export IDEOCODE_BEDROCK_PROFILE=my-profile
export IDEOCODE_BEDROCK_REGION=us-east-1
```

If you rely on instance/container metadata credentials and have no local profile env vars, opt in explicitly:

```bash
export IDEOCODE_BEDROCK_ENABLE=1
export AWS_REGION=us-east-1
```
...
For AWS CLI console-login profiles, IDEOCODE can also use credentials exported by:

```bash
aws configure export-credentials --profile my-profile --format env-no-export
```

IDEOCODE does not store these exported session credentials; it asks the AWS CLI profile provider when the Bedrock provider initializes.
...
If you enable STS validation with `IDEOCODE_BEDROCK_VALIDATE_STS=1`, allow `sts:GetCallerIdentity`.

## Run IDEOCODE with Bedrock

```bash
IDEOCODE --provider bedrock --model anthropic.claude-3-5-sonnet-20241022-v2:0
```

or:

```bash
IDEOCODE --model bedrock:anthropic.claude-3-5-sonnet-20241022-v2:0
```

Inference profile IDs/ARNs are accepted as model IDs, for example:

```bash
IDEOCODE --model bedrock:us.anthropic.claude-3-5-sonnet-20241022-v2:0
```
...
```bash
export IDEOCODE_BEDROCK_MAX_TOKENS=4096
export IDEOCODE_BEDROCK_TEMPERATURE=0.2
export IDEOCODE_BEDROCK_TOP_P=0.9
export IDEOCODE_BEDROCK_STOP_SEQUENCES='</done>,STOP'
```

## Model discovery

IDEOCODE will use a static Bedrock model list immediately. When model prefetch/catalog refresh runs, it calls `ListFoundationModels` and `ListInferenceProfiles`, then caches results in IDEOCODE's config directory. Cached Bedrock catalogs are region-scoped; if you switch `IDEOCODE_BEDROCK_REGION`/`AWS_REGION`, IDEOCODE ignores the old-region cache and refreshes for the new region.
...
```bash
IDEOCODE_BEDROCK_LIVE_TEST=1 \
AWS_PROFILE=my-profile \
AWS_REGION=us-east-1 \
cargo test -p IDEOCODE --lib provider::bedrock::tests::bedrock_live_smoke_test -- --ignored
```
...
- Missing region: set `AWS_REGION` or `IDEOCODE_BEDROCK_REGION`.
