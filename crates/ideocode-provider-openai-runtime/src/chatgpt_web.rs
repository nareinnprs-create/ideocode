...
const TOOL_CALL_START: &str = "<IDEOCODE_tool_call>";
const TOOL_CALL_END: &str = "</IDEOCODE_tool_call>";
...
/// state, so parallel IDEOCODE agents do not cross streams.
...
impl ChatGptWebState {
    ...
    async fn run_turn(
        ...
        if !status.ready {
            anyhow::bail!(
                "Firefox Browser Agent Bridge is not ready. Run `IDEOCODE browser status`, start Firefox, and log in at chatgpt.com"
            );
        }
        ...
        match (result, cleanup) {
            ...
            (Ok(_), Err(cleanup_err)) => Err(cleanup_err.context(
                "GPT-5.6 Pro answered, but IDEOCODE could not securely close its browser tab",
            )),
...
fn next_owned_tab_name() -> String {
    ...
    let millis = SystemTime::now()
        ...
        .unwrap_or_default()
        .as_millis();
    format!("IDEOCODE-chatgpt-web-{millis}-{sequence}")
}
...
"#,
    ...
    if preparation.get("onboarding").and_then(Value::as_bool) == Some(true) {
        anyhow::bail!(
            "ChatGPT is waiting for a workspace onboarding choice. Open chatgpt.com in Firefox and finish onboarding; IDEOCODE will not merge or move your personal chat history automatically"
        );
    }
    if preparation.get("signedOut").and_then(Value::as_bool) == Some(true) {
        anyhow::bail!(
            "Firefox is not logged in to ChatGPT. Log in at chatgpt.com, then retry the IDEOCODE turn"
        );
    }
...
"#,
    ...
    if verification.get("temporary").and_then(Value::as_bool) != Some(true) {
        anyhow::bail!(
            "ChatGPT did not enter Temporary Chat. Refusing to send the IDEOCODE system prompt into persistent web history"
        );
    }
    Ok(())
}
...
async fn poll_for_response(tab_id: u64, tx: &mpsc::Sender<Result<StreamEvent>>) -> Result<String> {
    let timeout_secs = std::env::var("IDEOCODE_CHATGPT_WEB_TIMEOUT_SECS")
        .ok()
...
"#,
        ...
        if Instant::now() >= deadline {
            anyhow::bail!(
                "GPT-5.6 Pro web response timed out after {} seconds (override with IDEOCODE_CHATGPT_WEB_TIMEOUT_SECS)",
                timeout_secs
            );
        }
...
async fn emit_response(
    ...
    if let Some(parsed) = parse_tool_call(response)? {
        if !advertised_tools.iter().any(|name| name == &parsed.name) {
            anyhow::bail!(
                "GPT-5.6 Pro requested unknown IDEOCODE tool '{}'; advertised tools were: {}",
                parsed.name,
...
fn parse_tool_call(response: &str) -> Result<Option<ParsedToolCall>> {
    ...
    if !trimmed.starts_with(TOOL_CALL_START) || !trimmed.ends_with(TOOL_CALL_END) {
        anyhow::bail!(
            "GPT-5.6 Pro mentioned a IDEOCODE tool-call envelope without emitting it as the entire response"
        );
    }
    ...
    let payload: Value = serde_json::from_str(payload_text.trim())
        .context("GPT-5.6 Pro emitted invalid JSON in a IDEOCODE tool-call envelope")?;
    let name = payload
        .get("name")
        .and_then(Value::as_str)
        .map(str::trim)
        .filter(|name| !name.is_empty())
        .ok_or_else(|| anyhow::anyhow!("IDEOCODE tool-call envelope is missing a non-empty name"))?
        .to_string();
    let input = payload.get("input").cloned().unwrap_or_else(|| json!({}));
    if !input.is_object() {
        anyhow::bail!("IDEOCODE tool-call envelope input must be a JSON object");
    }
...
fn build_web_prompt(
    ...
    prompt.push_str("# IDEOCODE system instructions\n\n");
    prompt.push_str(system);
    prompt.push_str("\n\n# IDEOCODE tools available for this turn\n\n");
...
# Mandatory IDEOCODE web-transport protocol

Act as the assistant for the conversation above and obey the IDEOCODE system instructions.
...
mod tests {
    ...
    fn tool_call_parser_accepts_valid_exact_envelope() {
        let parsed = parse_tool_call(
            "  <IDEOCODE_tool_call>{\"name\":\"bash\",\"input\":{\"command\":\"pwd\",\"intent\":\"Check cwd\"}}</IDEOCODE_tool_call>\n",
        )
        .unwrap()
        .unwrap();
    ...
    fn tool_call_parser_rejects_incomplete_envelope() {
        let err = parse_tool_call("<IDEOCODE_tool_call>{\"name\":\"bash\"}")
            .err()
    ...
    fn tool_call_parser_rejects_quoted_or_suffixed_envelope() {
        for response in [
            "Example: <IDEOCODE_tool_call>{\"name\":\"bash\",\"input\":{}}</IDEOCODE_tool_call>",
            "<IDEOCODE_tool_call>{\"name\":\"bash\",\"input\":{}}</IDEOCODE_tool_call> done",
        ] {
    ...
    fn tool_call_parser_rejects_non_object_input() {
        let err = parse_tool_call(
            "<IDEOCODE_tool_call>{\"name\":\"bash\",\"input\":[\"not\",\"an\",\"object\"]}</IDEOCODE_tool_call>",
        )
        .err()
    ...
    fn tool_call_parser_allows_marker_text_inside_json_string_values() {
        let parsed = parse_tool_call(
            "<IDEOCODE_tool_call>{\"name\":\"bash\",\"input\":{\"command\":\"printf '</IDEOCODE_tool_call>'\"}}</IDEOCODE_tool_call>",
        )
        .unwrap()
        .unwrap();
        assert_eq!(parsed.input["command"], "printf '</IDEOCODE_tool_call>'");
    }
