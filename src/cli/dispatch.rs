...
fn is_file_controlled_debug_client() -> bool {
    std::env::var_os("IDEOCODE_DEBUG_CMD_PATH").is_some()
}
...
pub(crate) async fn run_main(mut args: Args) -> Result<()> {
        ...
        .filter(|value| !value.is_empty())
    {
        ...
        crate::env::set_var("IDEOCODE_PROVIDER_PROFILE_NAME", profile_name);
    ...
    if let Some(tool_profile) = args.tool_profile.as_deref() {
        crate::env::set_var("IDEOCODE_TOOL_PROFILE", tool_profile);
    }
    if let Some(tools) = args.tools.as_deref() {
        crate::env::set_var("IDEOCODE_TOOLS", tools);
    }
    if let Some(disabled_tools) = args.disabled_tools.as_deref() {
        crate::env::set_var("IDEOCODE_DISABLED_TOOLS", disabled_tools);
    }
    if args.disable_base_tools {
        crate::env::set_var("IDEOCODE_DISABLE_BASE_TOOLS", "1");
    }
    ...
    match args.command {
        ...
        Some(Command::Server { action }) => match action {
            ServerCommand::Start { json } => {
                ...
                if json {
                    ...
                    println!("IDEOCODE server is running.");
                }
            }
        ...
        Some(Command::ProviderTestCoverage {
            ...
            let colorize = std::io::stdout().is_terminal()
                ...
                && std::env::var_os("IDEOCODE_NO_COLOR").is_none();
        ...
        match resolve_resume_id(&resume_id) {
            ...
            Err(e) => {
                match resume_resolution_failure_action(&resume_id, |key| std::env::var_os(key)) {
                    ...
                    // itself with `--resume <id>` and `IDEOCODE_RESUMING=1`. In the
                    ...
                    // making IDEOCODE unusable after an auto-update (issue #328).
                    ...
                    ResumeResolutionFailureAction::Exit => {
                        ...
                        if !output::quiet_enabled() {
                            eprintln!("\nUse `IDEOCODE --resume` to list available sessions.");
                        }
                        std::process::exit(1);
                    }
                }
...
where
    F: Fn(&str) -> Option<V>,
{
    if var_os("IDEOCODE_RESUMING").is_some() {
        ResumeResolutionFailureAction::DeferToServer
    } else {
        ResumeResolutionFailureAction::Exit
    }
...
async fn run_default_command(args: Args) -> Result<()> {
    ...
    // hotkeys can reopen IDEOCODE in the last project directory (Cmd+') and the
    // last IDEOCODE repo for self-dev (Cmd+Shift+'). Best-effort; ignored unless a
    ...
    if !server_running {
        ...
        if server::reap_stale_socket_if_dead(&server::socket_path()).await {
            output::stderr_info("Removed a stale IDEOCODE socket from a previous server.");
        }
...
pub(crate) async fn maybe_prompt_server_bootstrap_login(
    ...
    let cli_bootstrap_requested = std::env::var_os("IDEOCODE_CLI_BOOTSTRAP_LOGIN").is_some();
    ...
    // `IDEOCODE_CLI_BOOTSTRAP_LOGIN` env var.
...
pub(crate) async fn spawn_server(
    ...
    if client_requested_selfdev {
        cmd.env("IDEOCODE_DEBUG_CONTROL", "1");
    }
    ...
    cmd.env("IDEOCODE_DEFERRED_AUTH_BOOTSTRAP", "1");
