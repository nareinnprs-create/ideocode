...
pub async fn send_email(request: SendEmailRequest<'_>) -> Result<()> {
    ...
    if let Some(cid) = request.cycle_id {
        let msg_id = format!("<ambient-{}@IDEOCODE>", cid);
...
pub fn poll_imap_once(host: &str, port: u16, user: &str, pass: &str) -> Result<Vec<ReplyAction>> {
    ...
    let reply_search = session.search("UNSEEN HEADER In-Reply-To \"@IDEOCODE>\"")?;
    let button_search = session.search("UNSEEN SUBJECT \"[IDEOCODE-perm:\"")?;
    ...
    for message in messages.iter() {
            ...
            && let Some(parsed) = mail_parser::MessageParser::default().parse(body)
        {
            ...
            let cycle_id = if in_reply_to.contains("@IDEOCODE>") {
                in_reply_to
                    .trim_start_matches("<ambient-")
                    .trim_end_matches("@IDEOCODE>")
                    .to_string()
            } else if let Some(start) = subject.find("[IDEOCODE-perm:") {
                let rest = &subject[start + "[IDEOCODE-perm:".len()..];
...
pub fn build_permission_email_html(
    ...
    let approve_subj_raw = format!("[IDEOCODE-perm:{}] Approved", request_id);
    let deny_subj_raw = format!("[IDEOCODE-perm:{}] Denied", request_id);
...
mod tests {
    ...
    fn test_build_permission_email_html() {
        let html = build_permission_email_html(
            ...
            "Touch Cargo.toml",
            "req_123",
            "IDEOCODE@example.com",
        );
        assert!(html.contains("Permission Request"));
        assert!(html.contains("req_123"));
        assert!(html.contains("mailto:IDEOCODE@example.com"));
    }
}
