...
///
/// Uses a fast O(n) scan of `~/.IDEOCODE/active_pids/` (typically 0-5 files)
...
// Active PID tracking
// ---------------------------------------------------------------------------
// Lightweight files in ~/.IDEOCODE/active_pids/<session_id> containing the PID.
...
/// IDEOCODE store under a stable `imported_<tool>_<provider_id>` stem. Resuming the
...
/// was still sitting in `~/.IDEOCODE/sessions` (issue #336).
///
...
mod batch_crash_tests {
    ...
    -> anyhow::Result<()> {
        ...
        // No external OpenCode store exists under IDEOCODE_HOME/external, so the only
