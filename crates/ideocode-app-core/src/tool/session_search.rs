...
struct SearchInput {
    ...
    /// Restrict IDEOCODE sessions by saved/bookmarked flag.
    #[serde(default)]
    saved: Option<bool>,
    /// Restrict IDEOCODE sessions by debug flag.
    #[serde(default)]
    debug: Option<bool>,
    /// Restrict IDEOCODE sessions by canary flag.
    #[serde(default)]
    canary: Option<bool>,
    /// Restrict source: IDEOCODE, claude, codex, pi, opencode, cursor, or all.
    #[serde(default)]
    ...
    /// Scan every available IDEOCODE session instead of the recent indexed subset.
    #[serde(default)]
...
/// Covers the IDEOCODE store plus the external stores (claude/codex/pi/cursor).
pub fn spawn_recent_index_warmup() {
    tokio::task::spawn_blocking(|| {
        ...
        .unwrap_or_else(|err| {
            crate::logging::info(&format!("IDEOCODE session index warmup skipped: {err}"));
            0
        });
    ...
    fn parameters_schema(&self) -> Value {
        json!({
            ...
            "properties": {
                ...
                "saved": {
                    "type": "boolean",
                    "description": "Restrict IDEOCODE sessions by saved/bookmarked flag."
                },
                "debug": {
                    "type": "boolean",
                    "description": "Restrict IDEOCODE sessions by debug/test flag."
                },
                "canary": {
                    "type": "boolean",
                    "description": "Restrict IDEOCODE sessions by canary flag."
                },
                "source": {
                    "type": "string",
                    "enum": ["all", "IDEOCODE", "claude", "codex", "pi", "opencode", "cursor"],
                ...
                "exhaustive": {
                    "type": "boolean",
                    "description": "Search every available IDEOCODE session instead of the recent indexed subset. Slower, but useful for deep recall."
                }
            },
            "required": ["query"]
        })
...
fn normalize_source_filter(raw: Option<&str>) -> std::result::Result<Option<String>, String> {
    ...
    match normalized.as_str() {
        "all" => Ok(None),
        "IDEOCODE" | "claude" | "claude-code" | "codex" | "pi" | "opencode" | "cursor" => {
            Ok(Some(normalized.replace("claude-code", "claude")))
        }
        _ => Err(format!(
            "source must be one of all, IDEOCODE, claude, codex, pi, opencode, or cursor; received {source}."
        )),
    }
}
...
fn IDEOCODE_index_candidates(
    ...
    let index_path = index_dir()?.join("session_search_IDEOCODE_index_v2.bin");
...
fn collect_external_jsonl_source(
    ...
    // Like the IDEOCODE path, cap how many candidate files get fully parsed.
...
fn IDEOCODE_session_matches_filters(session: &Session, options: &SearchOptions) -> bool {
    ...
    if !provider_matches(session.provider_key.as_deref(), "IDEOCODE", options) {
        return false;
    }
