...
pub fn atomic_symlink_swap(src: &Path, dst: &Path, temp: &Path) -> std::io::Result<()> {
    ...
    #[cfg(windows)]
    {
        ...
        let old = dst.parent().unwrap_or_else(|| Path::new(".")).join(format!(
            ".IDEOCODE-launcher-old-{operation_id}{}",
            dst.extension()
...
mod tests {
    ...
    fn windows_swap_replaces_existing_launcher_via_staged_file() {
        ...
        let dst = dir.path().join("IDEOCODE.exe");
        let temp = dir.path().join(".IDEOCODE-current");
        ...
        assert!(
            !dir.path()
                .join(format!(".IDEOCODE-launcher-old-{}.exe", std::process::id()))
                .exists()
        );
    }
}
