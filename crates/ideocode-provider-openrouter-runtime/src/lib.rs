...
//! - Manual pinning: Set IDEOCODE_OPENROUTER_PROVIDER or use model@Provider syntax
...
const OPENROUTER_TRANSPORT_STATE_ENV: &str = "IDEOCODE_OPENROUTER_TRANSPORT_STATE";
...
fn configured_api_base() -> String {
    let raw = std::env::var("IDEOCODE_OPENROUTER_API_BASE")
        .ok()
...
fn configured_api_key_name() -> String {
    let raw = std::env::var("IDEOCODE_OPENROUTER_API_KEY_NAME")
        .ok()
        .map(|v| v.trim().to_string())
        .filter(|v| !v.is_empty())
...
fn configured_env_file_name() -> String {
    let raw = std::env::var("IDEOCODE_OPENROUTER_ENV_FILE")
        .ok()
        .map(|v| v.trim().to_string())
        .filter(|v| !v.is_empty())
...
fn provider_features_enabled(api_base: &str) -> bool {
    if let Ok(raw) = std::env::var("IDEOCODE_OPENROUTER_PROVIDER_FEATURES") {
...
fn model_catalog_enabled() -> bool {
    if let Ok(raw) = std::env::var("IDEOCODE_OPENROUTER_MODEL_CATALOG") {
        if let Some(value) = parse_env_bool(&raw) {
            return value;
        }
        ideocode_base::logging::warn(&format!(
...
fn configured_auth_header_mode() -> AuthHeaderMode {
    let Some(raw) = std::env::var("IDEOCODE_OPENROUTER_AUTH_HEADER")
        .ok()
...
fn configured_auth_header_name() -> HeaderName {
    let raw = std::env::var("IDEOCODE_OPENROUTER_AUTH_HEADER_NAME")
        .ok()
        .map(|v| v.trim().to_string())
        .filter(|v| !v.is_empty())
...
fn configured_dynamic_bearer_provider() -> Option<String> {
    std::env::var("IDEOCODE_OPENROUTER_DYNAMIC_BEARER_PROVIDER")
        .ok()
        .map(|v| v.trim().to_ascii_lowercase())
        .filter(|v| !v.is_empty())
}
...
fn configured_allow_no_auth() -> bool {
    std::env::var("IDEOCODE_OPENROUTER_ALLOW_NO_AUTH")
        .ok()
...
pub enum OpenRouterTransportState {
    ...
    OpenRouterApiKey,
    /// IDEOCODE subscription access currently reuses the OpenRouter HTTP slot, but is
    /// not user BYOK/OpenRouter billing.
    IDEOCODESubscription,
...
impl OpenRouterTransportState {
    pub fn from_current_env(runtime_provider: Option<&str>) -> Self {
        ...
        if matches!(runtime_provider.as_deref(), Some("IDEOCODE")) {
            return Self::IDEOCODESubscription;
        }
...
pub fn maybe_schedule_standard_openrouter_catalog_refresh(context: &'static str) -> bool {
    ...
    // when IDEOCODE_OPENROUTER_* env vars are set by an active named profile
    // (e.g. NVIDIA NIM via `[providers.mynvidia]`, which sets
    // IDEOCODE_OPENROUTER_API_BASE to the NVIDIA endpoint): that profile owns the
...
pub struct OpenRouterProvider {
    ...
    /// `IDEOCODE_OPENAI_EXTRA_BODY` env/env-file value.
...
impl OpenRouterProvider {
    ...
    fn configured_max_tokens(profile_id: Option<&str>) -> Option<u32> {
        if let Ok(raw) = std::env::var("IDEOCODE_OPENROUTER_MAX_TOKENS") {
    ...
    /// 2. The `IDEOCODE_OPENAI_EXTRA_BODY` env var (or the same key inside the
    ...
    pub fn runtime_display_name(&self) -> String {
        ...
        if !self.supports_provider_features {
            ...
            if std::env::var("IDEOCODE_RUNTIME_PROVIDER")
                .ok()
    ...
    /// scoped `IDEOCODE_API_KEY`, then routes through the OpenAI-compatible
    ...
    fn thinking_override() -> Option<bool> {
        let raw = std::env::var("IDEOCODE_OPENROUTER_THINKING").ok()?;
    ...
    pub fn new() -> Result<Self> {
        ...
        let profile_id = std::env::var("IDEOCODE_OPENROUTER_CACHE_NAMESPACE")
            .ok()
            .map(|value| value.trim().to_ascii_lowercase())
            .filter(|value| !value.is_empty())
        ...
        let static_context_limits = profile_id
            ...
            .unwrap_or_default();
        let static_models = std::env::var("IDEOCODE_OPENROUTER_STATIC_MODELS")
            .ok()
            .map(|raw| {
                raw.lines()
                    .map(str::trim)
    ...
    pub fn new_openai_compatible_profile_runtime(
        ...
        let auth = match load_api_key_from_env_or_config(&resolved.api_key_env, &resolved.env_file)
        {
            ...
            None => {
                ...
                anyhow::bail!(
                    "{} credentials not available. {} not found in environment or {}. Run `IDEOCODE login --provider {}` first.",
            ...
            return match provider.as_str() {
                "azure" => {
                    if ideocode_base::auth::azure::has_configuration() {
                        ...
                        anyhow::bail!(
                            "Azure OpenAI is configured for Entra ID, but Azure settings are incomplete. Run `IDEOCODE login --provider azure`."
                        )
                    }
                }
                other => anyhow::bail!(
                    "Unsupported IDEOCODE_OPENROUTER_DYNAMIC_BEARER_PROVIDER '{}'.",
                    other
                ),
            };
        }
