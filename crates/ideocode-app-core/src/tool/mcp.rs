    ...
    async fn list_servers(&self) -> Result<ToolOutput> {
        ...
        if servers.is_empty() && configured.is_empty() {
            return Ok(ToolOutput::new(
                ...
                Or add servers to ~/.IDEOCODE/mcp.json or .IDEOCODE/mcp.json and use {\"action\": \"reload\"}.\n\
    ...
    async fn reload_config(&self, session_id: &str) -> Result<ToolOutput> {
        ...
        if config.servers.is_empty() {
            ...
            return Ok(ToolOutput::new(
                ...
                Add servers to ~/.IDEOCODE/mcp.json (global) or .IDEOCODE/mcp.json (project):\n\
...
mod tests {
    ...
    fn create_test_tool() -> McpManagementTool {
        ...
        // would load the developer's real ~/.IDEOCODE/mcp.json, and list output
    ...
    impl LocalMcpConfigGuard {
        fn new(content: &str) -> std::io::Result<Self> {
            let path = PathBuf::from(".IDEOCODE/mcp.json");
            let dir = path
                .parent()
