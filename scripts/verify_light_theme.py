...
color usage. On the white background run, IDEOCODE should detect a light theme
...
IDEOCODE = os.path.expanduser("~/.IDEOCODE/builds/current/IDEOCODE")
...
def run_capture(bg: str, seconds: float = 12.0) -> bytes:
    ...
    for k in ("IDEOCODE_THEME", "TERM_PROGRAM", "TERM_PROGRAM_VERSION", "TMUX", "STY", "SSH_TTY", "SSH_CONNECTION", "SSH_CLIENT", "KITTY_WINDOW_ID", "GHOSTTY_RESOURCES_DIR", "WEZTERM_PANE", "ZELLIJ"):
        env.pop(k, None)
...
def main() -> int:
    ...
    # IDEOCODE's dim_color() is rgb(80,80,80); its lightness flip is (175,175,175).
