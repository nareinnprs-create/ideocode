...
```
phone (IDEOCODE iOS app / Termius, connected through Tailscale)
  │  WebSocket :7643 (pair token auth, tailnet-only)
  ▼
EC2 IDEOCODE server ──instance role──▶ AWS Bedrock (Opus 4.6 default)
  ▲
  │ tap wake link (API Gateway → wake lambda, bearer token from URL fragment)
  │ "Pair this phone" button (lambda → SSM Run Command → `IDEOCODE pair`)
  │
AWS Budget $10 ──▶ SNS IDEOCODE-guard-stop ──▶ breaker lambda ──▶ stop instance
                         └──▶ email
```
...
|---|---|---|
| `units/IDEOCODE-serve.service` | `~ec2-user/.config/systemd/user/` (user unit, linger on) | IDEOCODE daemon + gateway, restart always |
| `units/IDEOCODE-pair.service` | `/etc/systemd/system/` | Legacy local pairing HTTP service, now disabled after migration to SSM |
| `units/idle-autostop.{service,timer}` | `/etc/systemd/system/` | 5-min check, poweroff after 30 min idle |
| `idle-autostop.sh` | `/usr/local/bin/` | idle = no gateway/SSH clients AND IDEOCODE has no outbound :443 (not streaming) |
| `IDEOCODE-pair-service.py` | `/usr/local/bin/` | Legacy tailnet-only fallback; normal pairing now uses SSM |
| `wake-lambda.py` | Lambda `IDEOCODE-phone-wake` (behind API Gateway `8c3wp4cbag`) | wake page: starts instance, polls SSM health, generates pair codes through SSM |
| `breaker-lambda.py` | Lambda `IDEOCODE-guard-breaker` | stops the instance; subscribed to SNS `IDEOCODE-guard-stop` |
...
- `~/.IDEOCODE/config.toml`: `[provider]` default bedrock/Opus 4.6, `[gateway] enabled = true, port 7643, bind 0.0.0.0`
  (note: `~/.IDEOCODE/config.toml`, NOT `~/.config/IDEOCODE/`)
- `~/.bashrc` env: `IDEOCODE_BEDROCK_ENABLE=1`, `AWS_REGION=us-east-1`,
  `IDEOCODE_BEDROCK_MODEL=us.anthropic.claude-opus-4-6-v1`, `IDEOCODE_GATEWAY_HOST=100.109.78.41`
- Helpers: `~/bin/jc` (IDEOCODE with bedrock), `~/bin/phone` (attach-or-create tmux IDEOCODE)
...
| `IDEOCODE-bedrock-tokens-warn` | >3M input tokens / 15 min | email |
| `IDEOCODE-bedrock-tokens-stop` | >10M input tokens / 15 min, 2 periods | breaker stops instance + email |
| AWS budget `IDEOCODE-dev-monthly-cost` | forecast >50% / actual >80% | email |
| AWS budget `IDEOCODE-dev-monthly-cost` | actual >100% of $10 | SNS breaker stops instance + email |
...
1. Bookmark the wake link (`https://<api-id>.execute-api.us-east-1.amazonaws.com/#t=<token>`, token stored at `~/.IDEOCODE/IDEOCODE-phone-wake-token` on the workstation). The URL fragment is not sent in HTTP requests; JavaScript exchanges it for an `Authorization: Bearer` header and keeps it in session storage.
2. Tap it: the Lambda starts the instance and polls EC2/SSM every 5 s until ready.
3. Tap "Pair this phone" → Lambda runs `IDEOCODE pair` through SSM → 6-digit code + `IDEOCODE://` deep link → opens the iOS app paired to `100.109.78.41:7643`.
...
- The EC2 security group has no public ingress. Gateway and SSH access are tailnet-only through `IDEOCODE-phone` (`100.109.78.41`).
...
- The deployment access key was rotated and the prior key is inactive. Daily maintenance can use the tested `IDEOCODE-operator` profile and `IDEOCODEOperator` role, which cannot create IAM users or terminate instances. `jade-deploy` retains its administrator attachment only as a temporary recovery path until an independent root/MFA login is verified; see `IAM-LEAST-PRIVILEGE.md`.
...
2. Install IDEOCODE, Tailscale, tmux, and git. Join the tailnet as `IDEOCODE-phone`; set the IDEOCODE gateway host to `100.109.78.41`.
3. Copy `units/IDEOCODE-serve.service` and the idle-autostop unit/script; enable linger and the required services. The legacy pair service is not required.
4. Deploy `wake-lambda.py` as `waker.py`, set `WAKE_TOKEN`, `INSTANCE_ID`, and `IDEOCODE_GATEWAY_HOST=100.109.78.41`, and grant scoped EC2/SSM permissions. API Gateway HTTP API routes to the Lambda.
5. Create the SNS topics/subscriptions and connect the $10 Budget's 100% actual notification to `IDEOCODE-guard-stop`.
