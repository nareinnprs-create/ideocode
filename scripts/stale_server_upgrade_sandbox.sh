...
SANDBOX="$(mktemp -d /tmp/IDEOCODE-stale-sandbox.XXXXXX)"
export IDEOCODE_HOME="$SANDBOX/home"
export IDEOCODE_RUNTIME_DIR="$SANDBOX/runtime"
# Hard isolation: pin the socket explicitly so we can NEVER touch the real
# global daemon at /run/user/<uid>/IDEOCODE.sock.
export IDEOCODE_SOCKET="$SANDBOX/runtime/IDEOCODE.sock"
# Make the new client's clean release version comparable (debug build is dirty).
export IDEOCODE_TEST_CLIENT_VERSION_OVERRIDE="v0.22.0 (sandbox)"
mkdir -p "$IDEOCODE_HOME" "$IDEOCODE_RUNTIME_DIR"

BUILDS="$IDEOCODE_HOME/builds"
...
cp "$OLD_DIR/IDEOCODE-linux-x86_64.bin" "$OLD_DIR/libssl.so.10" \
   "$OLD_DIR/libcrypto.so.10" "$BUILDS/versions/0.14.6/"
cat > "$BUILDS/versions/0.14.6/IDEOCODE" <<'WRAP'
#!/usr/bin/env sh
set -eu
real=$0
...
WRAP
chmod +x "$BUILDS/versions/0.14.6/IDEOCODE"

# --- Install the NEW binary as version 0.22.0 (newer mtime) ----------------
cp "$NEW_BIN" "$BUILDS/versions/0.22.0/IDEOCODE"
touch -d "+1 minute" "$BUILDS/versions/0.22.0/IDEOCODE"

# --- Field state: shared-server -> OLD, stable/current -> NEW --------------
ln -sf "../versions/0.14.6/IDEOCODE" "$BUILDS/shared-server/IDEOCODE"
echo "0.14.6" > "$BUILDS/shared-server-version"
ln -sf "../versions/0.22.0/IDEOCODE" "$BUILDS/stable/IDEOCODE"
echo "0.22.0" > "$BUILDS/stable-version"
ln -sf "../versions/0.22.0/IDEOCODE" "$BUILDS/current/IDEOCODE"
...
cleanup() {
  ...
  pkill -f "$BUILDS/versions/0.14.6/IDEOCODE-linux-x86_64.bin" 2>/dev/null || true
  pkill -f "$BUILDS/versions/0.22.0/IDEOCODE" 2>/dev/null || true
  rm -rf "$SANDBOX"
}
trap cleanup EXIT
...
"$BUILDS/shared-server/IDEOCODE" --no-update --provider antigravity serve \
  >"$SANDBOX/server.log" 2>&1 &
SERVER_PID=$!
# Wait for the socket to appear.
for _ in $(seq 1 40); do
  [ -S "$IDEOCODE_SOCKET" ] && break
  sleep 0.25
done
sleep 1
...
# --- 2) New client: IDEOCODE server reload (repairs channel, then reloads) ----
log "Running NEW client: IDEOCODE server reload"
