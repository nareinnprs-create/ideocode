...
IDEOCODE uses a **single-server, multi-client** architecture. One server process
...
│                                                                             │
│  IDEOCODE serve                                                                │
│  ├── Unix socket:  /run/user/$UID/IDEOCODE.sock                                │
│  ├── Debug socket: /run/user/$UID/IDEOCODE-debug.sock                          │
│  ├── Registry:     ~/.IDEOCODE/servers.json                                    │
...
The server name persists across reloads via the registry (`~/.IDEOCODE/servers.json`).
...
```
  ...
  ─────                          ───────                     ──────
  IDEOCODE (first run)              IDEOCODE (subsequent)          /reload
       │                              │                          │
...
When you run `IDEOCODE`, it checks if a server is already running:

1. **Server exists**: connect directly as a client
2. **No server**: spawn `IDEOCODE serve` as a detached daemon (with `setsid`),
...
`IDEOCODE serve --server-name <name>` or the `IDEOCODE_SERVER_NAME` environment
variable. The optional `IDEOCODE_SERVER_DISPLAY_NAME` environment variable is also
...
```bash
IDEOCODE --socket /tmp/IDEOCODE.sock -C /local/checkout --remote-working-dir /remote/checkout
```
...
```
/run/user/$UID/
├── IDEOCODE.sock          # Main communication socket
└── IDEOCODE-debug.sock    # Debug/testing socket
```

## Self-Dev Mode

When running `IDEOCODE` inside the IDEOCODE repository:

1. Auto-detects the repo and enables self-dev mode
2. Connects to the normal shared IDEOCODE server
...
| Scenario | Behavior |
|----------|----------|
| First `IDEOCODE` run | Spawns server daemon, connects |
| Subsequent `IDEOCODE` | Connects to existing server |
...
| Resume session | `IDEOCODE --resume fox` reconnects to existing session |
