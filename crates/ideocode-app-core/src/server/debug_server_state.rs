    ...
    let (summary, actions) = match decision.primary_cause {
        "runaway_live_session_population" => (
            ...
            vec![
                serde_json::json!({
                    ...
                    "commands": ["IDEOCODE debug 'swarm:list'", "IDEOCODE debug 'server:memory-incident'"]
                }),
                serde_json::json!({
                    "priority": 2,
                ...
                serde_json::json!({
                    ...
                    "commands": ["IDEOCODE debug 'server:memory-incident'", "python scripts/analyze_runtime_memory_log.py --days 1"]
                }),
                serde_json::json!({
                    "priority": 4,
        ...
        "unattributed_live_heap" => (
            ...
            vec![
                serde_json::json!({
                    ...
                    "commands": ["IDEOCODE debug 'server:memory'", "python scripts/analyze_runtime_memory_log.py --days 1"]
                }),
                serde_json::json!({
                    "priority": 2,
                    "action": "Escalate to a jemalloc-prof build and heap dump if attribution remains below 50% of live heap.",
                    "commands": ["IDEOCODE debug 'allocator:profile:on'", "IDEOCODE debug 'allocator:profile:dump /tmp/IDEOCODE-server.heap'"]
                }),
            ],
        ),
        ...
        _ => (
            ...
            vec![serde_json::json!({
                ...
                "commands": ["IDEOCODE debug 'server:memory-incident'"]
            })],
        ),
    };
