...
impl RunPlanReporter {
    ...
    /// Record an explicit checkpoint (a IDEOCODE_CHECKPOINT-style milestone) on
...
/// The `IDEOCODE login` invocation most likely to fix a credential wave for
...
fn credential_login_fix_hint(provider: Option<&str>) -> String {
    ...
    format!("`IDEOCODE login --provider {target}`")
}
...
impl CommunicateTool {
    pub fn new() -> Self {
        ...
        let description = if swarm_prompt.is_empty() {
            ...
            format!(
                "{BASE_DESCRIPTION}\n\nSwarm prompt (user-tunable via ~/.IDEOCODE/swarm-prompt.md):\n{swarm_prompt}"
            )
        };
        Self { description }
    }
