...
impl DaemonSession {
    ...
    async fn read_event(&self) -> Result<ServerEvent> {
        ...
        if n == 0 {
            anyhow::bail!("IDEOCODE daemon disconnected");
        }
        let event = serde_json::from_str(&line)
            .with_context(|| format!("failed to decode IDEOCODE daemon event: {}", line.trim_end()))?;
        Ok(event)
    }
}
            ...
            _ if method.starts_with('_') => {
                if let Some(id) = message.id {
                    self.write_error_value(
                        ...
                        format!("Unsupported IDEOCODE ACP extension method: {method}"),
                    )
                    .await?;
                }
            }
        ...
        match self.create_new_session(cwd).await {
            ...
            Err(err) => {
                self.write_error_value(
                    ...
                    format!("Failed to create IDEOCODE session: {err:#}"),
                )
                .await?;
            }
        }
            ...
            .await
        {
            ...
            Err(err) => {
                self.write_error_value(
                    id,
                    JSONRPC_INTERNAL_ERROR,
                    format!("Failed to attach IDEOCODE session '{session_id}': {err:#}"),
                )
                .await?;
            }
        }
    ...
    fn map_event(&mut self, event: ServerEvent) -> Vec<Value> {
        match event {
            ...
            ServerEvent::Compaction { trigger, .. } if self.profile.is_extended() => vec![json!({
                ...
                "content": {
                    "type": "text",
                    "text": format!("\n[IDEOCODE compacted context: {trigger}]\n"),
                }
            })],
...
fn ensure_no_acp_mcp_servers(params: &Value) -> std::result::Result<(), String> {
    match params.get("mcpServers") {
        ...
        Some(_) => Err(
            "ACP mcpServers are not supported yet; configure MCP servers in IDEOCODE config.toml"
                .to_string(),
        ),
    }
}
...
pub(crate) async fn run_acp_command(
    ...
    if !explicit_tool_profile {
        crate::env::set_var("IDEOCODE_TOOL_PROFILE", acp_config.tool_profile.trim());
...
mod tests {
    ...
    fn cwd_must_be_absolute() {
        ...
        // Use a platform-absolute path: on Windows a bare `/tmp` is
        // drive-relative (rooted but not absolute) and would be rejected.
        let absolute = std::env::temp_dir();
        let params = json!({"cwd": absolute.to_str().expect("utf8 temp dir")});
