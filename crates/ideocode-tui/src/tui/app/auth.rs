...
impl App {
    fn open_auth_browser(url: &str) -> bool {
        // Honors --no-browser/NO_BROWSER/IDEOCODE_NO_BROWSER and never opens real
    ...
    fn record_oauth_preflight(
        ...
        if matches!(callback_available, Some(false)) {
            ...
            if let Some(target) = callback_target {
                notices.push(format!(
                    "Local callback target {} is unavailable, so IDEOCODE is using manual-safe paste completion instead.",
                    target
                ));
            } else {
                notices.push(
                    "The local callback listener is unavailable, so IDEOCODE is using manual-safe paste completion instead."
                        .to_string(),
                );
            }
        }
        if !notices.is_empty() {
            notices.push(format!(
                "If login still fails, run IDEOCODE auth doctor {} for a guided diagnosis.",
                provider_id
            ));
        }
        notices.join("\n")
    ...
    pub(super) fn show_IDEOCODE_subscription_status(&mut self) {
        ...
        let mut message = String::from("IDEOCODE Subscription Status\n\n");
        message.push_str(&format!(
            "  - Credentials: {}\n",
            if configured_key {
                "configured"
            } else {
                "not configured (/login IDEOCODE)"
            }
        ));
        message.push_str(&format!(
            "  - Router base: {}{}\n",
        ...
        for model in crate::subscription_catalog::curated_models() {
            ...
            let tier_suffix = if model.min_tier == crate::subscription_catalog::IDEOCODETier::Plus {
                String::new()
            } else {
        ...
        if configured_key {
            ...
            message.push_str("\nLog in with /login IDEOCODE to see account usage and tier.");
        }
                    ...
                    match crate::subscription_api::fetch_subscription_me().await {
                        ...
                        Err(error) => {
                                ...
                                == Some(&crate::subscription_api::AccountApiError::Unauthorized)
                            {
                                ...
                                "IDEOCODE Account Status\n\nThe saved account key was revoked or expired. Local credentials were cleared. Use /account IDEOCODE login to sign in again."
                                    .to_string()
                            } else {
                                format!(
                                    "IDEOCODE Account Status\n\nCould not load /v1/me: {}\n\nThe local credential was retained. Retry /account IDEOCODE status, open /account IDEOCODE manage, or use /account IDEOCODE logout.",
                                    error
                                )
                            };
                            crate::bus::Bus::global().publish(crate::bus::BusEvent::UiActivity(
                                crate::bus::UiActivity::background(
                                    Some(session_id),
                                    message,
                                    Some("IDEOCODE account status unavailable"),
                                ),
                            ));
                        }
                    }
    ...
    pub(super) fn show_auth_status(&mut self) {
        ...
        message.push_str(
            "\nUse /login <provider> to authenticate. /login IDEOCODE is for curated IDEOCODE subscription access; /account opens the provider/account management center, /account <provider> settings shows provider-specific controls, and /auth doctor or /account <provider> doctor shows recovery steps.",
        );
        self.push_display_message(DisplayMessage::system(message));
    }
    ...
    pub(super) fn start_logout_all(&mut self) {
        ...
        Self::clear_api_key_logout_summary(
            &mut summary,
            &mut errors,
            "IDEOCODE subscription API key",
            crate::subscription_catalog::IDEOCODE_API_KEY_ENV,
            crate::subscription_catalog::IDEOCODE_ENV_FILE,
        );
        for env_key in [
            crate::subscription_catalog::IDEOCODE_API_BASE_ENV,
            crate::subscription_catalog::IDEOCODE_ACCOUNT_ID_ENV,
            crate::subscription_catalog::IDEOCODE_ACCOUNT_EMAIL_ENV,
            crate::subscription_catalog::IDEOCODE_TIER_ENV,
        ] {
            if let Err(err) = crate::provider_catalog::save_env_value_to_env_file(
                ...
                errors.push(format!("IDEOCODE subscription {}: {}", env_key, err));
            }
        }
        ...
        match provider.target {
            ...
            crate::provider_catalog::LoginProviderTarget::Google => {
                ...
                self.push_display_message(DisplayMessage::error(
                    "Google/Gmail login is only available from the CLI right now. Run IDEOCODE login --provider google."
                        .to_string(),
                ));
            }
        }
    ...
    fn start_IDEOCODE_login(&mut self) {
        ...
        self.set_status_notice("IDEOCODE account: requesting browser approval");
        let session_id = self.session.id.clone();
        let Ok(handle) = tokio::runtime::Handle::try_current() else {
            self.push_display_message(DisplayMessage::error(
                "IDEOCODE account login requires the async runtime.".to_string(),
            ));
            return;
        };
        handle.spawn(async move {
            ...
            let device = match crate::subscription_api::request_device_authorization(
                &client,
                &api_base,
                Some(crate::subscription_catalog::IDEOCODETier::Pro),
            )
            .await
            {
                Ok(device) => device,
                Err(error) => {
                    publish(
                        format!(
                            "IDEOCODE Account Login\n\nCould not start browser approval: {}\n\nRetry /account IDEOCODE login. No credential was saved.",
                            error
                        ),
                        "IDEOCODE account login failed",
                    );
                    return;
                }
            };
            ...
            publish(
                format!(
                    "IDEOCODE Account Login\n\n{}\n\nApprove the request in the same browser. IDEOCODE is waiting for the single-use exchange.{}",
                ...
                "IDEOCODE account: waiting for browser approval",
            );
                ...
                loop {
                    ...
                    .await
                    {
                        ...
                        Ok(TokenPollOutcome::Denied) => break Err(
                            "IDEOCODE account login was canceled or denied in the browser."
                                .to_string(),
                        ),
            ...
            let approved = match approved {
                ...
                Err(error) => {
                    publish(
                        format!("IDEOCODE Account Login\n\n{error}\n\nRetry /account IDEOCODE login."),
                        "IDEOCODE account login stopped",
                    );
                    return;
                }
            };
            ...
            if let Err(error) = crate::subscription_catalog::persist_account_credentials(
                ...
                publish(
                    ...
                    "IDEOCODE account credential save failed",
                );
                return;
            }
            crate::auth::AuthStatus::invalidate_cache();
            ...
            .await
            {
                ActivationOutcome::Active(me) => publish(
                    format!(
                        "IDEOCODE Account Ready\n\n{} is active for {}.\n\nStatus: /account IDEOCODE status\nManage: /account IDEOCODE manage\nLogout: /account IDEOCODE logout",
                        me.parsed_tier()
                            .map(|tier| tier.display_name().to_string())
                            .unwrap_or(me.tier),
                        me.email
                    ),
                    "IDEOCODE account plan active",
                ),
                ActivationOutcome::Canceled(_) => publish(
                    "IDEOCODE Account Login\n\nCheckout was canceled. The valid account key remains saved, but no paid plan is active.\n\nStatus: /account IDEOCODE status\nManage: /account IDEOCODE manage\nLogout: /account IDEOCODE logout".to_string(),
                    "IDEOCODE account plan not active",
                ),
                ActivationOutcome::TimedOut { last_error_was_offline } => publish(
                    ...
                    "IDEOCODE account activation pending",
                ),
                ActivationOutcome::Revoked | ActivationOutcome::Denied => {
                    let _ = crate::subscription_catalog::clear_account_credentials();
                    publish(
                        "IDEOCODE Account Login\n\nThe issued key was revoked or denied during activation checks. Local credentials were cleared. Retry /account IDEOCODE login.".to_string(),
                        "IDEOCODE account key rejected",
                    );
                }
            }
        });
        ...
        let Ok(handle) = tokio::runtime::Handle::try_current() else {
            ...
            match result {
                Ok(()) => self.push_display_message(DisplayMessage::system(
                    "IDEOCODE account credentials and cache were cleared locally. Remote revocation could not be attempted without the async runtime."
                        .to_string(),
                )),
                Err(error) => self.push_display_message(DisplayMessage::error(format!(
                    "Failed to clear local IDEOCODE account credentials: {error}"
                ))),
            }
            return;
        };
        handle.spawn(async move {
            ...
            crate::bus::Bus::global().publish(crate::bus::BusEvent::UiActivity(
                crate::bus::UiActivity::background(
                    Some(session_id),
                    message,
                    Some("IDEOCODE account logout complete"),
                ),
            ));
        });
    }
    ...
    fn start_api_key_login(
        ...
        let provider_id = openai_compatible_profile
            ...
            .unwrap_or_else(|| match key_name {
                crate::subscription_catalog::IDEOCODE_API_KEY_ENV => "IDEOCODE".to_string(),
    ...
    fn start_azure_login(&mut self) {
        self.push_display_message(DisplayMessage::system(
            "Azure OpenAI Login\n\n\
             IDEOCODE uses Azure OpenAI's /openai/v1 API with either Microsoft Entra ID or an API key.\n\n\
    ...
    fn start_cursor_login(&mut self) {
        ...
        self.push_display_message(DisplayMessage::system(
            "Cursor API Key\n\n\
             ...
             IDEOCODE will save it securely and use the native Cursor HTTPS transport.\n\n\
    ...
    pub(super) fn handle_login_input(&mut self, pending: PendingLogin, input: String) {
        ...
        match pending {
            ...
            PendingLogin::ApiKeyProfile {
                ...
                // it proves the input was received and which env var/file IDEOCODE
                ...
                let save_result: anyhow::Result<()> =
                    ...
                    } else if key_name == crate::subscription_catalog::IDEOCODE_API_KEY_ENV {
                        (|| {
                    ...
                    Ok(()) => {
                        ...
                        if key_name == crate::provider::bedrock::API_KEY_ENV {
                            ...
                            if let Some(default_model) = default_model.as_deref() {
                                crate::env::set_var("IDEOCODE_BEDROCK_MODEL", default_model);
                            }
                        }
                        ...
                        let guidance = if key_name == crate::subscription_catalog::IDEOCODE_API_KEY_ENV
                        {
                            ...
                            if resolved.requires_api_key {
                                "Fetching models now. IDEOCODE will switch to an accessible model returned by the live catalog and show the catalog diff when discovery finishes. If the model list looks stale, run /refresh-model-list.".to_string()
                            } else {
                                format!(
                                    "Local endpoint configured at {}. Fetching models now; IDEOCODE will switch to an accessible model returned by the live catalog and show the catalog diff when discovery finishes. If the model list looks stale, run /refresh-model-list.",
                                    endpoint.as_deref().unwrap_or(resolved.api_base.as_str()),
                                )
                            }
                        } else if key_name == crate::provider::bedrock::API_KEY_ENV {
                            "You can now use /model to switch to Bedrock models. TUI onboarding saved region us-east-2; for a different region, run IDEOCODE login --provider bedrock from a terminal.".to_string()
                    ...
                    Ok(()) => {
                        ...
                        Bus::global().publish(BusEvent::LoginCompleted(LoginCompleted {
                            ...
                            message: "Cursor API key saved.\n\n\
                             Stored at ~/.config/IDEOCODE/cursor.env.\n\
                             IDEOCODE will use it with the native Cursor HTTPS transport."
                                .to_string(),
                        }));
                    }
                    Err(e) => {
    ...
    pub(super) fn onboarding_should_prefer_strongest_model(&self) -> bool {
        ...
        let runtime_provider_explicit = std::env::var("IDEOCODE_INITIAL_PROVIDER_EXPLICIT")
            .ok()
            .is_some_and(|value| {
    ...
    pub(super) fn start_openai_compatible_post_login_activation(
        ...
        crate::bus::Bus::global().publish(crate::bus::BusEvent::UiActivity(
            crate::bus::UiActivity::catalog(
                Some(self.session.id.clone()),
                format!(
                    "{} Model Discovery Started\n\nSaved credentials are active. IDEOCODE is fetching the live model catalog, will only switch to a model returned by that catalog, and will show what changed when discovery finishes.",
                    provider_label
                ),
                        ...
                        if let Some(model) = selected {
                            ...
                            crate::bus::Bus::global().publish(crate::bus::BusEvent::UiActivity(
                                crate::bus::UiActivity::catalog(
                                    Some(session_id),
                                    format!(
                                        "{} Model Discovery Still Updating\n\nSaved credentials are active, but this local refresh pass did not find a selectable {} route yet. IDEOCODE is still processing the auth-change catalog refresh and will switch once provider routes are available. If the model list still looks stale after the auth catalog update, run /refresh-model-list.",
                    ...
                    Err(error) => {
                        ...
                        crate::bus::Bus::global().publish(crate::bus::BusEvent::UiActivity(
                            crate::bus::UiActivity::catalog(
                                Some(session_id),
                                format!(
                                    "{} Model Discovery Still Updating\n\nSaved credentials are active, but this local refresh pass failed before the server auth-change catalog refresh finished. IDEOCODE is still processing the auth-change catalog refresh and will switch once provider routes are available. If the model list still looks stale after the auth catalog update, run /refresh-model-list.\n\nLocal refresh error: {}",
