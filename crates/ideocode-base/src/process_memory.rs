...
/// Retention trim threshold in bytes, from `IDEOCODE_HEAP_RETENTION_TRIM_MB`
...
pub fn retention_trim_threshold_bytes() -> u64 {
    parse_retention_trim_threshold(
        std::env::var("IDEOCODE_HEAP_RETENTION_TRIM_MB")
            .ok()
            .as_deref(),
    )
}
...
fn default_heap_profile_path() -> Result<PathBuf> {
    let base = crate::storage::ideocode_dir()?
        .join("profiles")
        .join("heap");
