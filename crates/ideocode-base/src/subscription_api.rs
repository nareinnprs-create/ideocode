...
use crate::subscription_catalog::{self, IDEOCODETier};
...
impl SubscriptionMe {
    pub fn parsed_tier(&self) -> Option<IDEOCODETier> {
        IDEOCODETier::parse(&self.tier)
    }
...
impl fmt::Display for AccountApiError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::Unauthorized => write!(f, "the IDEOCODE account key is revoked or expired"),
            ...
            Self::Http { status, code } => match code {
                ...
                None => write!(f, "IDEOCODE account API returned HTTP {status}"),
            },
            Self::InvalidResponse(detail) => {
                write!(
                    f,
                    "IDEOCODE account API returned an invalid response: {detail}"
                )
            }
        }
    }
...
pub fn configured_api_base() -> String {
    subscription_catalog::configured_api_base()
        .unwrap_or_else(|| subscription_catalog::DEFAULT_IDEOCODE_API_BASE.to_string())
        .trim_end_matches('/')
        .to_string()
}
...
pub async fn fetch_subscription_me() -> Result<SubscriptionMe> {
    let api_key = subscription_catalog::configured_api_key()
        .context("no IDEOCODE account credential configured (run `IDEOCODE account login`)")?;
...
mod tests {
    ...
    async fn legacy_device_response_is_explained_without_echoing_body() {
        ...
        let error = request_device_authorization(&client(), &base, Some(IDEOCODETier::Pro))
            .await
