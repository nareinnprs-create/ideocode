...
//! - Listens on `~/.IDEOCODE/IDEOCODE-api.sock` (or `IDEOCODE_API_SOCKET`).
//! - For each API client, dials the legacy daemon socket (`IDEOCODE_SOCKET` or
//!   `~/.IDEOCODE/IDEOCODE.sock`) and speaks `subscribe`/`message`/... on its
//!   behalf.
...
fn IDEOCODE_home() -> PathBuf {
    ...
    if let Ok(dir) = std::env::var("XDG_RUNTIME_DIR") {
        ...
        if runtime.join("IDEOCODE.sock").exists() {
            return runtime;
        }
    }
    let home = std::env::var("HOME").unwrap_or_else(|_| ".".to_string());
    PathBuf::from(home).join(".IDEOCODE")
}
...
async fn handle_api_client(stream: UnixStream, legacy_socket: PathBuf) -> Result<()> {
    ...
    let hello_ok = ServerFrame::reply(
        ...
        ApiEvent::HelloOk {
            ...
            server: format!("IDEOCODE-harness-api-bridge/{}", env!("CARGO_PKG_VERSION")),
