...
impl AntigravityProvider {
    ...
    /// by `IDEOCODE login --provider antigravity`; there is no API-key path. This
    ...
    fn resolve_model_for_request(&self, model: &str) -> String {
        ...
        // Claude models on the Cloud Code backend currently reject IDEOCODE's tool
    ...
    async fn generate_content(
        ...
        // construct. Gemini-native accepts everything IDEOCODE emits, so Gemini
    ...
    async fn complete(
        ...
        tokio::spawn(async move {
            ...
            if let Some(content) = candidate.content {
                ...
                // calls through the standard ToolUseStart/End path so IDEOCODE
