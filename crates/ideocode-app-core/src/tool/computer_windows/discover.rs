...
const ACTIONS: &[(&str, &str, &str)] = &[
    ...
    ("observe", "ocr", "Optical character recognition: extract text from the screen. Options: x,y,w,h region."),
    ("observe", "ui", "Dump the accessibility (UI Automation) control tree."),
    ...
    ("mouse", "click", "Left-click at (x,y). Defaults to current cursor position."),
    ("mouse", "double_click", "Double-click at (x,y)."),
    ("mouse", "right_click", "Right-click at (x,y)."),
    ("mouse", "drag", "Click-drag from (x,y) to (to_x, to_y) with smooth interpolation."),
    ("mouse", "scroll", "Scroll at (x,y) by dx (horizontal), dy (vertical) ticks."),
    ("keyboard", "type", "Type text at the current focused element."),
    ("keyboard", "key", "Press a key chord, e.g. ctrl+c, alt+tab, win+r."),
    ("apps", "list_apps", "List running processes with visible windows (name, PID, title)."),
    ("apps", "activate_app", "Bring an app's window to the foreground by process name."),
    ("apps", "quit_app", "Terminate an app by process name."),
    ("windows", "list_windows", "List all visible top-level windows (handle, title, class)."),
    ("windows", "focus_window", "Alias for activate_app: bring window to foreground."),
    ("windows", "move_window", "Move an app's window to absolute screen coordinates (x,y)."),
    ("windows", "resize_window", "Resize an app's window to width (w) x height (h)."),
    ("windows", "minimize_window", "Minimize an app's window."),
    ("windows", "close_window", "Gracefully close an app's main window."),
    ("clipboard", "get_clipboard", "Read the current clipboard text content."),
    ("clipboard", "set_clipboard", "Set the clipboard text content."),
    ("scripting", "run_powershell", "Execute an arbitrary PowerShell script and return output."),
];
...
fn param_schema(action: &str) -> Value {
    match action {
        "screenshot" | "cursor" | "list_apps" | "list_windows" | "ui" | "get_clipboard" => json!({}),
        ...
        "activate_app" | "quit_app" | "focus_window" | "minimize_window" | "close_window" => json!({
        ...
        "run_powershell" => json!({ "script": {"type":"string","description":"PowerShell script source"} }),
        _ => json!({}),
    }
}
