...
WATCHED_CRATES = ["IDEOCODE-base", "IDEOCODE-app-core", "IDEOCODE-tui", "IDEOCODE"]
...
def target_state_violations(stats_by_name: dict[str, CrateStats]) -> list[str]:
    ...
    tui = stats_by_name.get("IDEOCODE-tui")
    if tui and "IDEOCODE-app-core" in tui.normal_workspace_deps:
        violations.append("target-state: IDEOCODE-tui still directly depends on IDEOCODE-app-core")

    app_core = stats_by_name.get("IDEOCODE-app-core")
    if app_core and "IDEOCODE-base" in app_core.normal_workspace_deps:
        violations.append("target-state: IDEOCODE-app-core still directly depends on IDEOCODE-base")

    base = stats_by_name.get("IDEOCODE-base")
    if base:
        for dep in base.normal_workspace_deps:
            if dep in {
                "IDEOCODE-azure-auth",
                "IDEOCODE-provider-gemini",
                "IDEOCODE-provider-openai",
                "IDEOCODE-provider-openrouter",
                "IDEOCODE-notify-email",
                "IDEOCODE-build-support",
            }:
                violations.append(f"target-state: IDEOCODE-base still depends on leaf/runtime crate {dep}")
        for dep in base.normal_external_deps:
            if dep.startswith("aws-") or dep in {"aws-types"}:
                violations.append(f"target-state: IDEOCODE-base still depends directly on AWS crate {dep}")

    for crate in stats_by_name.values():
        for glob in crate.glob_reexports:
            if crate.name in {"IDEOCODE", "IDEOCODE-app-core", "IDEOCODE-tui"}:
