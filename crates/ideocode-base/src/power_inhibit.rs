...
/// Best-effort inhibitor that keeps the machine awake while IDEOCODE is actively
...
impl PowerInhibitor {
    ...
    /// unless the legacy `IDEOCODE_DISABLE_POWER_INHIBIT` env escape hatch is set.
    ///
...
impl WindowsPowerGuard {
    fn acquire() -> io::Result<Self> {
        ...
        let thread = thread::Builder::new()
            .name("IDEOCODE-power-inhibit".to_string())
