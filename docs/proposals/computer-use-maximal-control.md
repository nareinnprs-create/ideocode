...
`AXIsProcessTrustedWithOptions(prompt=true)` (adds IDEOCODE to the list + shows the
...
Important: the permission attaches to the **host binary/terminal** running IDEOCODE.
For a stable experience we likely want a signed IDEOCODE.app with a fixed bundle id so
...
6. Signed IDEOCODE.app bundle for durable permissions.
...
2. **Requests** what it can programmatically:
   - `AXIsProcessTrustedWithOptions(prompt=true)` — shows the Accessibility dialog
     and pre-adds IDEOCODE to the list (toggled off).
...
- Ship/install IDEOCODE as a **signed `.app` bundle with a stable bundle id**
  (e.g. `com.IDEOCODE.app`) and a Designated Requirement, so the Accessibility /
