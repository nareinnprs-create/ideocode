...
/// Ensure a single background `IDEOCODE menubar` helper is running on macOS so the
/// session-count indicator shows up automatically for every macOS user without
/// them needing to run `IDEOCODE menubar` by hand.
///
/// This is a best-effort, fire-and-forget singleton: it records the helper's
/// PID in the *global* `~/.IDEOCODE/menubar.pid` (see [`global_menubar_dir`]) and
...
/// hard against sandboxed IDEOCODE processes (tests, self-dev, onboarding) ever
/// spawning a helper: each such process runs with a throwaway `$IDEOCODE_HOME`,
...
pub fn ensure_menubar_helper_running() {
    ...
    if std::env::var_os("IDEOCODE_NO_MENUBAR").is_some() {
        return;
    }

    // Sandboxed IDEOCODE (tests / self-dev / onboarding, anything with a throwaway
    // `$IDEOCODE_HOME`) must never manage the real user's global menu bar.
...
/// IDEOCODE process for this user, so this state must live at a fixed location
/// that does **not** depend on `$IDEOCODE_HOME`. Sandboxes (tests, self-dev,
...
fn global_menubar_dir() -> Option<std::path::PathBuf> {
    ...
    let dir = home.join(".IDEOCODE");
    let _ = std::fs::create_dir_all(&dir);
    Some(dir)
}

/// True when this process is a sandboxed IDEOCODE that must not own the real
/// user's global menu bar. A throwaway `$IDEOCODE_HOME` (anything other than the
/// real `~/.IDEOCODE`) or an explicit test/temp marker means "sandbox".
#[cfg(target_os = "macos")]
...
/// - A `$IDEOCODE_HOME` that differs from the real `~/.IDEOCODE` is a sandbox home.
...
fn is_menubar_sandbox(
    ...
    // No explicit override: the real user's default `~/.IDEOCODE`.
...
mod macos {
    ...
    /// `~/.IDEOCODE/menubar.lock` (see [`super::global_menubar_dir`]) so the lock
    /// is shared across every IDEOCODE process for this OS user, including ones
    /// running with a sandboxed `$IDEOCODE_HOME`. The menu bar itself is a single
    ...
    const STATUS_ITEM_AUTOSAVE: &str = "IDEOCODE-menubar";
    ...
    define_class!(
           ...
           #[name = "IDEOCODEMenubarHandler"]
           struct MenuHandler;

           impl MenuHandler {
               /// Open the clicked session (stored in the item's representedObject)
               /// in a new terminal window via `IDEOCODE --resume <id>`.
    ...
    pub(super) fn run_status_item_app() {
        let mtm = MainThreadMarker::new()
            .expect("IDEOCODE menubar must run on the main thread (the process entry point)");
        ...
        // stray `IDEOCODE menubar` invoked directly inside a test harness should
        // still never realize a status item.
        if super::env_truthy("IDEOCODE_TEST_SESSION") || super::env_truthy("IDEOCODE_TEMP_SERVER") {
            return;
        }
        ...
        // bypassed entirely (e.g. a self-dev `target/.../IDEOCODE` and the
        // installed `~/.local/bin/IDEOCODE` both spawn helpers, or a reload
        ...
        // Follow the system's Light/Dark setting explicitly. `IDEOCODE` runs as a
        ...
        if let Some(button) = status_item.button(mtm) {
            let icon = NSImage::imageWithSystemSymbolName_accessibilityDescription(
                ...
                Some(ns_string!("IDEOCODE sessions")),
            );
        ...
        let quit_item = unsafe {
            NSMenuItem::initWithTitle_action_keyEquivalent(
                NSMenuItem::alloc(mtm),
                ns_string!("Quit IDEOCODE menu bar"),
    ...
    ///
    /// `IDEOCODE` runs as a bare executable without an `Info.plist` app bundle, so
...
mod tests {
    ...
    fn session_menu_item_title_loads_persisted_todo_title() {
        ...
        let temp = tempfile::tempdir().expect("create temporary IDEOCODE_HOME");
    ...
    fn menubar_sandbox_detection() {
        ...
        let real = Path::new("/Users/me/.IDEOCODE");
        ...
        assert!(!is_menubar_sandbox(
            false,
            false,
            Some(OsStr::new("/Users/me/.IDEOCODE")),
            Some(real),
        ));
        ...
        assert!(is_menubar_sandbox(
            false,
            false,
            Some(OsStr::new("/private/tmp/IDEOCODE-e2e-home-xyz")),
            Some(real),
        ));
