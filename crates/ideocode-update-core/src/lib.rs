...
mod tests {
    ...
    fn summarize_update_error_maps_known_causes() {
        ...
        assert_eq!(
            summarize_update_error("Checksum mismatch for IDEOCODE: expected a, got b"),
            "download failed checksum verification"
        );
        assert_eq!(
            summarize_update_error("No asset found for platform: IDEOCODE-linux-x86_64"),
    ...
    fn sha256sums_accepts_standard_and_binary_lines() {
        let checksums = parse_sha256sums(
            "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa  IDEOCODE-linux-x86_64\n\
             bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb *IDEOCODE-macos-aarch64\n",
        )
        .unwrap();
        assert_eq!(
            checksums.get("IDEOCODE-linux-x86_64").map(String::as_str),
            Some("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa")
        );
        assert_eq!(
            checksums.get("IDEOCODE-macos-aarch64").map(String::as_str),
    ...
    fn checksum_verification_accepts_matching_digest() {
        ...
        let sums = format!("{}  IDEOCODE-linux-x86_64\n", digest);
        verify_asset_checksum_text(&sums, "IDEOCODE-linux-x86_64", bytes).unwrap();
    }

    #[test]
    fn checksum_verification_rejects_mismatch() {
        let sums = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa  IDEOCODE-linux-x86_64\n";
        let err = verify_asset_checksum_text(sums, "IDEOCODE-linux-x86_64", b"hello")
            .unwrap_err()
            .to_string();
    ...
    fn sha256sums_rejects_invalid_digest() {
        let err = parse_sha256sums("not-a-digest  IDEOCODE\n")
            .unwrap_err()
            .to_string();
