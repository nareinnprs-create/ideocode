...
impl GeminiProvider {
    ...
    pub fn new() -> Self {
        let model = std::env::var("IDEOCODE_GEMINI_MODEL").unwrap_or_else(|_| DEFAULT_MODEL.into());
        let provider = Self {
    ...
    /// cloudcode-pa tier. Set `IDEOCODE_GEMINI_FORCE_OAUTH=1` to pin OAuth even when
    /// a key is present.
    fn auth_mode() -> GeminiAuthMode {
        let force_oauth = std::env::var("IDEOCODE_GEMINI_FORCE_OAUTH")
            .map(|value| {
...
fn gemini_http_client() -> reqwest::Client {
    reqwest::Client::builder()
        .user_agent("IDEOCODE/1.0 (gemini)")
        .http1_only()
