...
pub(crate) fn log_desktop_session_event_batch_profile(
    ...
    emit_desktop_profile_event(
        "IDEOCODE-desktop-session-event-profile",
        serde_json::json!({
...
pub(crate) fn log_desktop_session_card_refresh_profile(
    ...
    emit_desktop_profile_event(
        "IDEOCODE-desktop-session-card-refresh-profile",
        serde_json::json!({
...
pub(crate) fn log_desktop_session_cards_load_profile(
    ...
    emit_desktop_profile_event(
        "IDEOCODE-desktop-session-cards-load-profile",
        serde_json::json!({
...
pub(crate) fn log_desktop_preferences_save_profile(
    ...
    emit_desktop_profile_event(
        "IDEOCODE-desktop-preferences-save-profile",
        serde_json::json!({
...
pub(crate) fn log_desktop_crashed_sessions_restore_profile(
    ...
    emit_desktop_profile_event(
        "IDEOCODE-desktop-crashed-sessions-restore-profile",
        serde_json::json!({
            "restored": restored,
            "errors": errors,
...
impl DesktopFrameProfiler {
    ...
    pub(crate) fn report(&mut self, now: Instant) {
        if let Some(worst) = self.worst.as_ref() {
            emit_desktop_profile_event(
                "IDEOCODE-desktop-frame-profile",
                serde_json::json!({
...
impl DesktopInteractionLatencyProfiler {
    pub(crate) fn new() -> Self {
        ...
        let budget = duration_millis_env(
            "IDEOCODE_DESKTOP_INPUT_LATENCY_BUDGET_MS",
    ...
    pub(crate) fn observe_presented(&mut self, frame: &DesktopRenderFrameResult) {
        ...
        eprintln!(
            "IDEOCODE-desktop-latency-profile {}",
            serde_json::json!({
                "kind": pending.kind,
...
impl DesktopNoPaintWatchdog {
    ...
    pub(crate) fn observe_active_tick(
        ...
        if report_due {
            ...
            emit_desktop_profile_event(
                "IDEOCODE-desktop-no-paint-profile",
                serde_json::json!({
...
pub(crate) fn desktop_profile_stderr_enabled() -> bool {
    std::env::var_os("IDEOCODE_DESKTOP_PROFILE_STDERR").is_none_or(env_flag_enabled)
}
    ...
    DESKTOP_PROFILE_LOG_TX
        .get_or_init(|| {
            ...
            match std::thread::Builder::new()
                .name("IDEOCODE-desktop-profile-log".to_string())
                .spawn(move || {
                    let mut file = path.and_then(|path| {
                            ...
                            && let Err(error) = std::fs::create_dir_all(parent)
                        {
                            desktop_log::error(format_args!(
                                "IDEOCODE-desktop: failed to create profile log directory {}: {error}",
                                parent.display()
                            ));
                            return None;
                        }
                        match OpenOptions::new().create(true).append(true).open(&path) {
                            ...
                            Err(error) => {
                                desktop_log::error(format_args!(
                                    "IDEOCODE-desktop: failed to open profile log {}: {error}",
                                    path.display()
                                ));
                                None
                            }
                    ...
                    while let Ok(line) = rx.recv() {
                        ...
                        {
                            desktop_log::error(format_args!(
                                "IDEOCODE-desktop: failed to write profile log: {error}"
                            ));
                            file = None;
                        }
                    }
                ...
                Err(error) => {
                    desktop_log::error(format_args!(
                        "IDEOCODE-desktop: failed to start profile logger: {error:#}"
                    ));
                    None
                }
            }
    ...
    if let Some(tx) = desktop_profile_log_sender() {
            ...
            .is_err()
        {
            desktop_log::warn(format_args!(
                "IDEOCODE-desktop: failed to queue profile event {event}, logger is closed"
            ));
        }
    }
}
...
pub(crate) fn log_desktop_slow_interaction(
    ...
    emit_desktop_profile_event(
        "IDEOCODE-desktop-interaction-profile",
        serde_json::json!({
            "kind": kind,
