...
fn cached_grouped_sessions_round_trip_from_disk() {
    ...
    let _scan_limit = EnvVarGuard::set_str("IDEOCODE_SESSION_PICKER_MAX_SESSIONS", "100");
    let _include_saved = EnvVarGuard::set_str("IDEOCODE_SESSION_PICKER_INCLUDE_OLD_SAVED", "0");
    ...
    let session = SessionInfo {
        ...
        server_name: None,
        server_icon: None,
        source: SessionSource::IDEOCODE,
        resume_target: ResumeTarget::IDEOCODESession {
...
fn load_sessions_includes_saved_sessions_beyond_scan_limit() {
    ...
    let temp = tempfile::tempdir().expect("temp dir");
    let _home = EnvVarGuard::set_path("IDEOCODE_HOME", temp.path());
    let _scan_limit = EnvVarGuard::set_str("IDEOCODE_SESSION_PICKER_MAX_SESSIONS", "50");
...
fn benchmark_real_resume_loading_phases() {
    ...
    eprintln!(
        concat!(
            ...
            "IDEOCODE_summary={}ms IDEOCODE_loaded={} skipped_empty={} skipped_imported={} summary_errors={} ",
...
#[test]
#[ignore = "developer benchmark: scans the real IDEOCODE_HOME session directory"]
...
fn onboarding_scoped_loader_returns_only_codex_sessions() {
    ...
    assert!(
        ...
        "scoped Codex load must not include IDEOCODE/other-CLI sessions"
    );
}
...
fn hidden_debug_sessions_do_not_consume_default_resume_budget() {
    ...
    // loader stopped after the first 50, leaving no ordinary IDEOCODE sessions for
