...
valid_conversion_id() {
  printf '%s' "${IDEOCODE_INSTALL_CONVERSION_ID:-}" |
...
report_install_funnel() {
  ...
  outcome="$2"
  failure_stage="${3:-}"
  [ "${IDEOCODE_NO_TELEMETRY:-}" != "1" ] || return 0
  ...
  payload=$(printf '{"id":"%s","event":"install_funnel","version":"%s","os":"%s","arch":"%s","conversion_id":"%s","stage":"%s","outcome":"%s","source":"installer","install_method":"shell","failure_stage":"%s"}' \
    "$IDEOCODE_INSTALL_CONVERSION_ID" \
  ...
  curl -fsS --max-time 2 -H 'Content-Type: application/json' \
    --data "$payload" https://telemetry.IDEOCODE.sh/v1/event >/dev/null 2>&1 || true
}
...
persist_install_conversion_id() {
  ...
  [ "${DO_NOT_TRACK:-}" != "1" ] || return 0
  valid_conversion_id || return 0
  IDEOCODE_home="${IDEOCODE_HOME:-$HOME/.IDEOCODE}"
  mkdir -p "$IDEOCODE_home" 2>/dev/null || return 0
  (umask 077; printf '%s\n' "$IDEOCODE_INSTALL_CONVERSION_ID" > "$IDEOCODE_home/install_conversion_id") \
    2>/dev/null || return 0
  chmod 600 "$IDEOCODE_home/install_conversion_id" 2>/dev/null || true
}
...
case "$OS" in
  Linux)
    case "$ARCH" in
      x86_64)  ARTIFACT="IDEOCODE-linux-x86_64" ;;
      aarch64|arm64) ARTIFACT="IDEOCODE-linux-aarch64" ;;
      *)       err "Unsupported Linux architecture: $ARCH" ;;
    esac
    ;;
  Darwin)
    case "$ARCH" in
      arm64)   ARTIFACT="IDEOCODE-macos-aarch64" ;;
      x86_64)  ARTIFACT="IDEOCODE-macos-x86_64" ;;
  ...
  MINGW*|MSYS*|CYGWIN*)
    ...
    case "$WINDOWS_ARCH" in
      x86_64) ARTIFACT="IDEOCODE-windows-x86_64" ;;
      aarch64) ARTIFACT="IDEOCODE-windows-aarch64" ;;
...
if [ "$IS_WINDOWS" = true ]; then
  INSTALL_DIR="${IDEOCODE_INSTALL_DIR:-$LOCALAPPDATA/IDEOCODE/bin}"
else
...
# are visible immediately. IDEOCODE.sh keeps a static copy of the latest published
...
if [ -z "$VERSION" ]; then
  ...
  if valid_release_tag "$GITHUB_VERSION"; then
    ...
    info "GitHub release lookup unavailable; using cached IDEOCODE.sh metadata ($VERSION)."
  fi
fi
...
if [ "$IS_WINDOWS" = true ]; then
  EXE=".exe"
  builds_dir="$LOCALAPPDATA/IDEOCODE/builds"
else
  EXE=""
...
launcher_path="$INSTALL_DIR/IDEOCODE${EXE}"
...
if [ -n "$EXISTING" ]; then
  if echo "$EXISTING" | grep -qF "${VERSION#v}"; then
    info "IDEOCODE $VERSION is already installed — reinstalling"
  else
    info "Updating IDEOCODE $EXISTING → $VERSION"
  fi
else
  info "Installing IDEOCODE $VERSION"
fi
...
for candidate in "$ARTIFACT.tar.gz" "$ARTIFACT$EXE"; do
  while IFS= read -r base; do
    ...
    if curl -fsSL --retry 2 --connect-timeout 10 \
      "${base%/}/$candidate" -o "$tmpdir/IDEOCODE.download" 2>/dev/null; then
...
if [ -n "$download_mode" ]; then
  ...
  ACTUAL_SHA256=$(sha256_file "$tmpdir/IDEOCODE.download") \
    ...
    if [ -x "$termux_glibc_linker" ]; then
      if command -v patchelf >/dev/null 2>&1; then
        patchelf --set-interpreter "$termux_glibc_linker" "$dest_version_dir/$bin_name" \
          || err "Failed to patch IDEOCODE ELF interpreter for Termux glibc"
        info "Patched Termux glibc ELF interpreter: $termux_glibc_linker"
      else
        info "Termux detected: install patchelf with 'pkg install patchelf' and rerun this installer if IDEOCODE fails to start."
      fi
    else
      info "Termux detected: install glibc with 'pkg install glibc' if IDEOCODE fails due to a missing dynamic linker."
    fi
  fi
fi
...
if [ "${IDEOCODE_SKIP_SERVER_RELOAD:-}" != "1" ]; then
  ...
  if [ -x "$reload_bin" ]; then
    if "$reload_bin" server reload </dev/null >/dev/null 2>&1; then
      info "Reloaded the running IDEOCODE server onto $VERSION (if one was active)."
    fi
  fi
fi
...
if [ "$IS_WINDOWS" = true ]; then
  ...
  # cmd, Git Bash, Windows Terminal) finds IDEOCODE without manual setup. This is
  # the Git Bash (`curl | sh`) counterpart of install.ps1's Set-IDEOCODEUserPath:
  # read the user PATH, drop stale IDEOCODE launcher entries (case- and trailing-
  ...
  if command -v powershell.exe >/dev/null 2>&1; then
    ...
    if [ "$new_user_path" = "$current_user_path" ]; then
    ...
    elif IDEOCODE_NEW_USER_PATH="$new_user_path" powershell.exe -NoProfile -NonInteractive -Command \
      ...
      win_broadcast_ps=$(cat <<'IDEOCODE_PS_BROADCAST_EOF'
$sig = '[DllImport("user32.dll", SetLastError = true, CharSet = CharSet.Auto)] public static extern IntPtr SendMessageTimeout(IntPtr hWnd, uint Msg, UIntPtr wParam, string lParam, uint fuFlags, uint uTimeout, out UIntPtr lpdwResult);'
$type = Add-Type -MemberDefinition $sig -Name 'IDEOCODEEnvBroadcast' -Namespace Win32 -PassThru
[UIntPtr]$result = [UIntPtr]::Zero
$type::SendMessageTimeout([IntPtr]0xffff, 0x001A, [UIntPtr]::Zero, 'Environment', 2, 5000, [ref]$result) | Out-Null
IDEOCODE_PS_BROADCAST_EOF
)
  ...
  echo ""
  info "✅ IDEOCODE $VERSION installed successfully!"
  echo ""
  if [ "$win_path_persisted" = true ]; then
    info "Added $win_install_dir to your user PATH. New terminals will find IDEOCODE automatically."
  fi
  if command -v IDEOCODE >/dev/null 2>&1; then
    info "Run 'IDEOCODE' to get started."
  else
    echo "  To start using IDEOCODE in THIS terminal right now, run:"
    echo ""
    printf '    \033[1;32mexport PATH="%s:$PATH" && IDEOCODE\033[0m\n' "$INSTALL_DIR"
    if [ "$win_path_persisted" != true ]; then
      echo ""
      echo "  To add IDEOCODE to PATH permanently (PowerShell):"
      echo ""
  ...
  if [ "$(uname -s)" = "Darwin" ]; then
    if [ "$hotkey_setup_ready" = true ]; then
      info "Global hotkey ready: Cmd+; launches a new IDEOCODE from anywhere, system-wide"
    else
      info "Tip: run 'IDEOCODE setup-hotkey' so Cmd+; launches IDEOCODE system-wide on macOS"
    fi
  fi
  ...
  if command -v IDEOCODE >/dev/null 2>&1; then
    ...
    echo "  To start using IDEOCODE right now, run:"
    echo ""
    printf '    \033[1;32mexport PATH="%s:\$PATH" && IDEOCODE\033[0m\n' "$INSTALL_DIR"
    echo ""
    echo "  Future terminal sessions will have IDEOCODE on PATH automatically."
  fi
fi
