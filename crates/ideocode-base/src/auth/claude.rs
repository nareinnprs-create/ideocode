...
/// Represents a named Anthropic OAuth account stored in IDEOCODE's auth.json.
...
fn relabel_accounts(auth: &mut IDEOCODEAuthFile) -> bool {
...
pub fn load_auth_file() -> Result<IDEOCODEAuthFile> {
    ...
    let mut auth: IDEOCODEAuthFile = crate::storage::read_json(&path)
        .with_context(|| format!("Could not read IDEOCODE credentials from {:?}", path))?;
...
/// Falls through Claude Code -> IDEOCODE accounts -> OpenCode, preferring non-expired tokens.
pub fn load_credentials() -> Result<ClaudeCredentials> {
    ...
    // prompt, so it is read once at import time and copied into IDEOCODE's own
    ...
    if let Ok(creds) = load_IDEOCODE_credentials() {
        ...
        expired_candidates.push(("IDEOCODE", creds));
    }
    ...
    anyhow::bail!("No Claude OAuth credentials found (checked Claude Code, IDEOCODE, OpenCode)")
}

/// Load credentials for a specific IDEOCODE account by label.
...
fn load_IDEOCODE_credentials() -> Result<ClaudeCredentials> {
    ...
    let account = auth
        ...
        .context("No anthropic accounts in IDEOCODE auth.json")?;
...
/// them into a IDEOCODE-managed Anthropic account so future loads and token
/// refreshes are served from IDEOCODE's own `auth.json`.
///
...
/// rotates into IDEOCODE's store on refresh, so the copy stays usable after the
