...
//!
//! Data persists to `~/.IDEOCODE/provider_activity.json` and is shared across
...
//!   - `openrouter`, `IDEOCODE`, `copilot`, `gemini`, `cursor`, `bedrock`,
...
pub fn display_name_for_source_key(source_key: &str) -> String {
    ...
    match source_key {
        ...
        "IDEOCODE" => "IDEOCODE subscription".to_string(),
...
/// `"NVIDIA NIM"`) plus the optional `IDEOCODE_RUNTIME_PROVIDER` key onto a
...
mod tests {
    ...
    fn record_use_and_spend_roundtrip_under_IDEOCODE_home() {
        ...
        let _home = EnvVarGuard::set("IDEOCODE_HOME", temp.path().as_os_str());
