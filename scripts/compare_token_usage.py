...
The goal is to verify that IDEOCODE's token consumption is within expected bounds
compared to the official Claude Code CLI.

NOTE: IDEOCODE typically uses FEWER tokens than Claude CLI because:
1. IDEOCODE has a smaller/simpler system prompt
2. IDEOCODE registers fewer tools (Claude CLI has many built-in tools)
3. Different prompt caching behavior

The test PASSES if IDEOCODE uses fewer tokens OR at most 50% more tokens.
...
Requirements:
    - IDEOCODE built and in PATH or at target/release/IDEOCODE
...
def find_IDEOCODE_binary() -> str:
    """Find the IDEOCODE binary."""
    # Check target/release first
    repo_root = Path(__file__).parent.parent
    release_binary = repo_root / "target" / "release" / "IDEOCODE"
    if release_binary.exists():
        return str(release_binary)

    # Check PATH
    result = subprocess.run(["which", "IDEOCODE"], capture_output=True, text=True)
    if result.returncode == 0:
        return result.stdout.strip()

    raise FileNotFoundError("IDEOCODE binary not found. Run 'cargo build --release' first.")
...
def run_IDEOCODE(prompt: str, workdir: str, IDEOCODE_binary: str, model: str = "claude-opus-4-5-20251101") -> RunResult:
    """Run IDEOCODE and capture token usage from trace output."""
    try:
        # Create a temporary IDEOCODE_HOME to avoid polluting user's sessions
        with tempfile.TemporaryDirectory() as tmpdir:
            env = os.environ.copy()
            env["IDEOCODE_HOME"] = tmpdir
            env["IDEOCODE_TRACE"] = "1"
...
def compare_usage(claude_result: RunResult, IDEOCODE_result: RunResult, verbose: bool = False) -> dict:
    """Compare token usage between Claude CLI and IDEOCODE."""
    c = claude_result.usage
    j = IDEOCODE_result.usage
...
def print_comparison(comparison: dict, prompt: str, verbose: bool = False):
    ...
    c = comparison["claude"]
    j = comparison["IDEOCODE"]
    d = comparison["diff"]
    p = comparison["pct_diff"]

    print(f"\n{'Metric':<20} {'Claude':<15} {'IDEOCODE':<15} {'Diff':<15} {'% Diff':<10}")
    print("-" * 75)
...
def run_test_suite(verbose: bool = False, runs: int = 1) -> list:
    ...
    IDEOCODE_binary = find_IDEOCODE_binary()
    print(f"Using IDEOCODE binary: {IDEOCODE_binary}")
    ...
    with tempfile.TemporaryDirectory() as workdir:
        for prompt in prompts:
            for run_num in range(runs):
                ...
                print("  Running IDEOCODE...", end=" ", flush=True)
                IDEOCODE_result = run_IDEOCODE(prompt, workdir, IDEOCODE_binary)
                if IDEOCODE_result.success:
                    print(f"OK ({IDEOCODE_result.usage.total} tokens)")
                else:
                    print(f"FAILED: {IDEOCODE_result.error}")
                    if verbose:
                        print(f"    Output: {IDEOCODE_result.output[:200]}")

                if claude_result.success and IDEOCODE_result.success:
                    comparison = compare_usage(claude_result, IDEOCODE_result, verbose)
                    results.append({
                        "prompt": prompt,
                        "run": run_num + 1,
...
def summarize_results(results: list) -> bool:
    ...
    total_IDEOCODE = sum(r["comparison"]["IDEOCODE"]["total"] for r in results)
    total_diff = total_IDEOCODE - total_claude
    ...
    total_IDEOCODE_io = sum(
        r["comparison"]["IDEOCODE"]["input"] + r["comparison"]["IDEOCODE"]["output"]
        for r in results
    )

    if total_claude > 0:
        pct_diff = ((total_IDEOCODE - total_claude) / total_claude) * 100
    else:
        pct_diff = 0
    ...
    print(f"IDEOCODE:      {total_IDEOCODE}")
    print(f"Difference: {total_diff:+} ({pct_diff:+.1f}%)")

    print(f"\n--- Input + Output only (excluding cache) ---")
    print(f"Claude CLI: {total_claude_io}")
    print(f"IDEOCODE:      {total_IDEOCODE_io}")
    if total_claude_io > 0:
        io_pct_diff = ((total_IDEOCODE_io - total_claude_io) / total_claude_io) * 100
        print(f"Difference: {total_IDEOCODE_io - total_claude_io:+} ({io_pct_diff:+.1f}%)")

    # Check if within acceptable bounds
    # IDEOCODE using fewer tokens is always good (negative diff)
    # IDEOCODE using more tokens is acceptable up to MAX_OVERHEAD_PCT
    MAX_OVERHEAD_PCT = 50  # Allow up to 50% more tokens (for different system prompts)

    passed = True
    if pct_diff <= 0:
        print(f"\n✅ PASS: IDEOCODE uses {abs(pct_diff):.1f}% fewer tokens than Claude CLI")
    elif pct_diff <= MAX_OVERHEAD_PCT:
        print(f"\n✅ PASS: IDEOCODE uses {pct_diff:.1f}% more tokens (within {MAX_OVERHEAD_PCT}% threshold)")
    else:
        print(f"\n❌ FAIL: IDEOCODE uses {pct_diff:.1f}% more tokens (exceeds {MAX_OVERHEAD_PCT}% threshold)")
        passed = False

    # Per-prompt breakdown
    print("\nPer-prompt breakdown:")
    print(f"{'Prompt':<40} {'Claude':<10} {'IDEOCODE':<10} {'Diff':<10}")
    print("-" * 70)

    for r in results:
        prompt = r["prompt"][:37] + "..." if len(r["prompt"]) > 40 else r["prompt"]
        c_total = r["comparison"]["claude"]["total"]
        j_total = r["comparison"]["IDEOCODE"]["total"]
...
def main():
    parser = argparse.ArgumentParser(
        description="Compare token usage between IDEOCODE and Claude Code CLI"
    )
    parser.add_argument(
        "--verbose", "-v",
        action="store_true",
    ...
    print("Token Usage Comparison: IDEOCODE vs Claude Code CLI")
    print("=" * 50)
