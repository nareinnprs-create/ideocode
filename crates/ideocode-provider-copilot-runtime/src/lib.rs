...
impl CopilotApiProvider {
    ...
    fn env_premium_mode() -> u8 {
        match std::env::var("IDEOCODE_COPILOT_PREMIUM").ok().as_deref() {
    ...
    fn startup_prefetch_grace_ms() -> u64 {
        std::env::var("IDEOCODE_COPILOT_PREFETCH_STARTUP_GRACE_MS")
            .ok()
    ...
    /// If IDEOCODE_COPILOT_MODEL is set, this is a no-op (user override).
