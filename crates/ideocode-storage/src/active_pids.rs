...
/// List active session IDs currently tracked in `~/.IDEOCODE/active_pids`.
...
/// Live snapshot of how many IDEOCODE sessions are running, and how many of those
/// are actively streaming a model response right now. Used by the menu bar
/// indicator (`IDEOCODE menubar`) and any other presence UI.
...
mod tests {
    ...
    /// Serialize tests that mutate `IDEOCODE_HOME`.
