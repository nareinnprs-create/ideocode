...
pub async fn run_debug_command(
    ...
    if !crate::transport::is_socket_path(&debug_socket) {
        ...
        eprintln!("\nMake sure:");
        eprintln!("  1. A IDEOCODE server is running (IDEOCODE or IDEOCODE serve)");
        eprintln!("  2. debug_socket is enabled in ~/.IDEOCODE/config.toml");
        eprintln!("     [display]");
        eprintln!("     debug_socket = true");
        eprintln!("\nOr use 'IDEOCODE debug start' to start a server.");
        eprintln!("Use 'IDEOCODE debug list' to see running servers.");
...
async fn debug_list_servers() -> Result<()> {
    ...
    if servers.is_empty() {
        println!("No running IDEOCODE servers found.");
        println!("\nStart one with: IDEOCODE debug start");
        return Ok(());
    }

    println!("Running IDEOCODE servers:\n");
    ...
    println!("  IDEOCODE debug -s /path/to/socket.sock sessions");
...
async fn debug_start_server(arg: &str, socket_path: Option<String>) -> Result<()> {
            ...
            .is_ok()
    {
        ...
        eprintln!("Use 'IDEOCODE debug list' to see all servers.");
        return Ok(());
    }
    ...
    eprintln!("Starting IDEOCODE server...");
    ...
    if crate::transport::is_socket_path(&debug_socket) {
        ...
        eprintln!("⚠ Debug socket not enabled. Add to ~/.IDEOCODE/config.toml:");
        eprintln!("  [display]");
        eprintln!("  debug_socket = true");
    }
