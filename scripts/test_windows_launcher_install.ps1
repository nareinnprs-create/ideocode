...
$originalImportOnly = $env:IDEOCODE_INSTALL_PS1_IMPORT_ONLY
$testRoot = Join-Path $env:TEMP ("IDEOCODE-windows-launcher-tests-{0}" -f ([guid]::NewGuid().ToString('N')))
...
try {
    ...
    $installDir = Join-Path $env:LOCALAPPDATA 'IDEOCODE\bin'
    $launcherPath = Join-Path $installDir 'IDEOCODE.exe'

    Write-Host 'test_launcher_path_localappdata'
    Assert-Equal $installDir (Get-DefaultIDEOCODEInstallDir) 'default installer path should live under LOCALAPPDATA\IDEOCODE\bin'
    Assert-Equal $launcherPath (Join-Path (Get-DefaultIDEOCODEInstallDir) 'IDEOCODE.exe') 'launcher path should be LOCALAPPDATA\IDEOCODE\bin\IDEOCODE.exe'
    ...
    $pathUpdate = Resolve-IDEOCODEPathUpdate -InstallDir $installDir -CurrentPath $currentPath
    ...
    Assert-PathCount $pathUpdate.Path $installDir 1 'updated PATH should contain exactly one IDEOCODE launcher dir'
    ...
    $secondUpdate = Resolve-IDEOCODEPathUpdate -InstallDir $installDir -CurrentPath $pathUpdate.Path
    ...
    $mockUpdate = Set-IDEOCODEUserPath -InstallDir $installDir -CurrentPath 'C:\Tools' -SetUserPathAction $setPathAction -BroadcastAction $broadcastAction
    ...
    $noChangeUpdate = Set-IDEOCODEUserPath -InstallDir $installDir -CurrentPath $script:appliedPath -SetUserPathAction $setPathAction -BroadcastAction $broadcastAction
    ...
    Assert-Equal 'v0.47.0' (ConvertFrom-IDEOCODEVersionOutput 'IDEOCODE v0.47.0 (f7f5898c)') 'local artifact version parser should accept normal IDEOCODE --version output'
    Assert-Equal 'v0.47.0' (ConvertFrom-IDEOCODEVersionOutput 'IDEOCODE collects anonymous usage statistics. IDEOCODE v0.47.0 (f7f5898c)') 'local artifact version parser should accept fresh-profile telemetry before version output'
    Assert-Equal $null (ConvertFrom-IDEOCODEVersionOutput 'not a IDEOCODE binary') 'local artifact version parser should reject unrelated output'
    $freshProfileBinary = Join-Path $testRoot 'fresh-profile-version.cmd'
    Set-Content -LiteralPath $freshProfileBinary -Value "@echo off`r`n>&2 echo IDEOCODE collects anonymous usage statistics.`r`necho IDEOCODE v0.47.0 (f7f5898c)" -NoNewline
    Assert-Equal 'v0.47.0' (Get-IDEOCODEVersionFromBinary $freshProfileBinary) 'binary version probe should tolerate a successful fresh-profile telemetry notice on stderr'

    Write-Host 'test_windows_architecture_detection_prefers_native_arm64'
    Assert-Equal 'IDEOCODE-windows-x86_64' (Resolve-IDEOCODEWindowsArtifact @('X64', 'AMD64')) 'x64 Windows should select the x64 release asset'
    Assert-Equal 'IDEOCODE-windows-aarch64' (Resolve-IDEOCODEWindowsArtifact @('Arm64')) 'native ARM64 Windows should select the ARM64 release asset'
    Assert-Equal 'IDEOCODE-windows-aarch64' (Resolve-IDEOCODEWindowsArtifact @('X64', 'AMD64', 'ARM64')) 'emulated x64 PowerShell on Windows ARM64 should prefer the native ARM64 release asset'
    Assert-Equal $null (Resolve-IDEOCODEWindowsArtifact @('x86', 'unknown')) 'unsupported architectures should not silently select an asset'
    ...
    $manifest = "$digest  nested/path/IDEOCODE-windows-x86_64.exe"
    $manifestBytes = [System.Text.Encoding]::UTF8.GetBytes($manifest)
    Assert-Equal $manifest (ConvertFrom-IDEOCODEWebContent -Content $manifest) 'web response decoder should preserve string content'
    Assert-Equal $manifest (ConvertFrom-IDEOCODEWebContent -Content $manifestBytes) 'web response decoder should decode Windows PowerShell 5.1 byte-array content as UTF-8'
    Assert-Equal $digest (Get-IDEOCODESha256FromManifest -ManifestText (ConvertFrom-IDEOCODEWebContent -Content $manifestBytes) -AssetName 'IDEOCODE-windows-x86_64.exe') 'checksum parser should accept a manifest decoded from a byte-array web response'
    Assert-Equal $digest (Get-IDEOCODESha256FromManifest -ManifestText $manifest -AssetName 'IDEOCODE-windows-x86_64.exe') 'checksum parser should match release assets by file name'
    Assert-Equal $null (Get-IDEOCODESha256FromManifest -ManifestText $manifest -AssetName 'missing.exe') 'checksum parser should fail closed when the requested asset is absent'
    Assert-Equal $digest (Assert-IDEOCODEFileChecksum -FilePath $checksumFile -ExpectedSha256 $digest -AssetName 'IDEOCODE-windows-x86_64.exe') 'checksum validation should accept the matching digest'
    $checksumThrew = $false
    try {
        Assert-IDEOCODEFileChecksum -FilePath $checksumFile -ExpectedSha256 ('0' * 64) -AssetName 'IDEOCODE-windows-x86_64.exe' | Out-Null
    } catch {
        $checksumThrew = $true
    }
    ...
    $armManifest = "$digest  nested/path/IDEOCODE-windows-aarch64.exe"
    Assert-Equal $digest (Get-IDEOCODESha256FromManifest -ManifestText $armManifest -AssetName 'IDEOCODE-windows-aarch64.exe') 'checksum parser should match the Windows ARM64 release asset'
    ...
    Assert-True ($installText.Contains('"--locked", "-p", "IDEOCODE", "--bin", "IDEOCODE"')) 'source-build fallback should compile only the locked IDEOCODE binary target'

    Write-Host 'test_hotkey_shortcut_script_is_valid_powershell'
    $shortcutScript = Get-IDEOCODEHotkeyShortcutScript -StartupShortcutPath "C:\Users\Test User\AppData\Roaming\IDEOCODE's hotkey.lnk" -IDEOCODEExePath "C:\Program Files\IDEOCODE's bin\IDEOCODE.exe"
    ...
    Assert-True ($installText.Contains('IDEOCODE_WINDOWS_SETUP_SKIP_PROCESS_LIFECYCLE')) 'isolated verification should be able to create shortcut files without stopping or spawning real user listeners'

    Write-Host 'test_upgrade_replaces_launcher_no_extra_path'
    $sourceDir = Join-Path $testRoot 'sources'
    New-Item -ItemType Directory -Path $sourceDir -Force | Out-Null
    $sourceV1 = Join-Path $sourceDir 'IDEOCODE-v1.exe'
    $sourceV2 = Join-Path $sourceDir 'IDEOCODE-v2.exe'
    Set-Content -Path $sourceV1 -Value 'version-one' -NoNewline
    Set-Content -Path $sourceV2 -Value 'version-two' -NoNewline
    Install-IDEOCODELauncher -SourcePath $sourceV1 -LauncherPath $launcherPath | Out-Null
    Install-IDEOCODELauncher -SourcePath $sourceV2 -LauncherPath $launcherPath | Out-Null
    Assert-Equal 'version-two' (Get-Content -Path $launcherPath -Raw) 'upgrade should replace launcher contents with the new build'
    $tempLaunchers = @(Get-ChildItem -LiteralPath $installDir -Filter '.IDEOCODE-launcher-*.tmp.exe' -Force -ErrorAction SilentlyContinue)
    Assert-Equal 0 $tempLaunchers.Count 'launcher upgrade should clean temporary files'
    $upgradePath = Resolve-IDEOCODEPathUpdate -InstallDir $installDir -CurrentPath $pathUpdate.Path
    ...
    $runningLauncher = Join-Path $runningDir 'IDEOCODE.exe'
    ...
    try {
        ...
        Install-IDEOCODELauncher -SourcePath $replacementLauncher -LauncherPath $runningLauncher | Out-Null
        ...
        $runningBackups = @(Get-ChildItem -LiteralPath $runningDir -Filter '.IDEOCODE-launcher-old-*.exe' -Force -ErrorAction SilentlyContinue)
    ...
    Remove-IDEOCODEStaleLauncherBackups -LauncherDir $runningDir
    $runningBackups = @(Get-ChildItem -LiteralPath $runningDir -Filter '.IDEOCODE-launcher-old-*.exe' -Force -ErrorAction SilentlyContinue)
    ...
    $rollbackLauncher = Join-Path $rollbackDir 'IDEOCODE.exe'
    ...
    $rollbackThrew = $false
    try {
        Install-IDEOCODELauncher -SourcePath $rollbackSource -LauncherPath $rollbackLauncher | Out-Null
    } catch {
        $rollbackThrew = $true
    } finally {
    ...
    Assert-Equal 0 @(Get-ChildItem -LiteralPath $rollbackDir -Filter '.IDEOCODE-launcher-*.tmp.exe' -Force -ErrorAction SilentlyContinue).Count 'rollback should remove temporary launcher files'
    Assert-Equal 0 @(Get-ChildItem -LiteralPath $rollbackDir -Filter '.IDEOCODE-launcher-old-*.exe' -Force -ErrorAction SilentlyContinue).Count 'rollback should restore rather than retain the previous launcher backup'

    Write-Host 'test_uninstall_removes_launcher_and_only_IDEOCODE_path'
    $removeCurrentPath = "$installDir;C:\Keep;$installVariant;C:\Keep"
    $removeUpdate = Resolve-IDEOCODEPathUpdate -InstallDir $installDir -CurrentPath $removeCurrentPath -RemoveOnly
    Assert-Equal 'C:\Keep;C:\Keep' $removeUpdate.Path 'uninstall path cleanup should remove only IDEOCODE-managed entries and preserve unrelated entries'
    Assert-Equal 2 $removeUpdate.RemovedManagedEntries 'uninstall path cleanup should remove all IDEOCODE launcher dir variants'
    Assert-PathCount $removeUpdate.Path $installDir 0 'uninstall path cleanup should leave no IDEOCODE launcher dir entries'
    ...
    if ($null -eq $originalImportOnly) { Remove-Item Env:IDEOCODE_INSTALL_PS1_IMPORT_ONLY -ErrorAction SilentlyContinue } else { $env:IDEOCODE_INSTALL_PS1_IMPORT_ONLY = $originalImportOnly }
