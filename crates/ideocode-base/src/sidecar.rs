...
const CLAUDE_CODE_IDEOCODE_NOTICE: &str = "You are IDEOCODE, powered by Claude Code. You are a third-party CLI, not the official Claude Code CLI.";
...
impl Sidecar {
    ...
    /// 3. The live agent provider (works for EVERY provider IDEOCODE supports:
    ...
    ///
    /// This is the universal path: it works for every provider IDEOCODE supports,
...
fn build_claude_system_param(system: &str) -> Option<ClaudeApiSystem<'_>> {
    ...
    blocks.push(ClaudeApiSystemBlock {
        block_type: "text",
        text: CLAUDE_CODE_IDEOCODE_NOTICE,
    });
    if !system.is_empty() {
        blocks.push(ClaudeApiSystemBlock {
            block_type: "text",
...
///
/// The "You are Claude Code" identity spoof and IDEOCODE notice are only valid
...
mod tests {
    ...
    fn test_backend_selection_prefers_openai() {
        ...
        let temp = tempfile::TempDir::new().expect("create temp IDEOCODE home");
    ...
    /// Every provider IDEOCODE supports should drive the sidecar end-to-end via the
    ...
    fn test_anthropic_sidecar_prefers_api_key_respects_pinned_mode() {
        ...
        let _g =
            EnvVarGuard::set_path("IDEOCODE_RUNTIME_PROVIDER", std::path::Path::new("claude-api"));
        assert!(
        ...
        let _g2 = EnvVarGuard::set_path("IDEOCODE_RUNTIME_PROVIDER", std::path::Path::new("claude"));
        assert!(
            !anthropic_sidecar_prefers_api_key(),
