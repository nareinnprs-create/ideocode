...
#
# This script:
# 1. Starts a fresh IDEOCODE instance in a new terminal
...
echo "🎬 IDEOCODE Recording Replay"
...
# Start IDEOCODE in a new kitty window
echo "🚀 Starting IDEOCODE..."
kitty --title "IDEOCODE-replay" -e bash -c "cd $(pwd) && ~/.cargo/bin/IDEOCODE; read -p 'Press Enter to close...'" &
KITTY_PID=$!
sleep 2  # Wait for IDEOCODE to start

# Focus the new window
sleep 0.5
# Find and focus the IDEOCODE-replay window
WINDOW_ID=$(niri msg windows 2>/dev/null | grep -B5 "IDEOCODE-replay" | grep -oP 'Window ID \K\d+' | head -1)
if [ -n "$WINDOW_ID" ]; then
