...
It should not require a IDEOCODE release.

A IDEOCODE code change is required when the sponsor needs:
...
- `crates/IDEOCODE-app-core/src/tool/discover.rs`;
- `crates/IDEOCODE-base/src/sponsors.rs`;
- `crates/IDEOCODE-base/src/sponsors/provenance.rs`;
- `crates/IDEOCODE-tui/src/tui/app/sponsor_disclosure.rs`; and
- `TELEMETRY.md`.
...
5. The setup does not bypass IDEOCODE's confirmation requirements. Signups,
...
`crates/IDEOCODE-base/src/sponsors.rs`. Category values are lowercase slugs.
...
5. Ship the IDEOCODE change before publishing entries that rely on the category.
...
measurement-sensitive. IDEOCODE records discovery provenance only when a later MCP
...
`x-IDEOCODE-discovery-benchmark: 1`. The service must retain that marker with its
...
suggestions are sent only to IDEOCODE maintainers. The schema tells the model to
...
marked `x-IDEOCODE-discovery-benchmark: 1` remain stored with
...
receipt. Suggestions go only to IDEOCODE maintainers, never to sponsors. Receipt
...
- suggest requires a prior successful browse in the same category, persists one
  actionable proposal per browse, and sends it only to IDEOCODE maintainers;
...
Then validate through IDEOCODE by pointing a test config at staging:
...
In a disposable IDEOCODE session, browse the category, select the sponsor or submit
...
```bash
cargo test -p IDEOCODE-app-core tool::discover
cargo test -p IDEOCODE-base sponsors
cargo test -p IDEOCODE-base discovery_provenance
cargo test -p IDEOCODE-tui sponsor_disclosure
cargo check -p IDEOCODE
```
...
- [ ] Staging IDEOCODE validation passes, including disclosure and confirmation.
