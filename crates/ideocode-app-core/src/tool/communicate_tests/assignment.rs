...
async fn communicate_assign_task_can_spawn_fallback_agent() {
    ...
    let socket_path = runtime_dir.path().join("IDEOCODE.sock");
    let _runtime = EnvGuard::set("IDEOCODE_RUNTIME_DIR", runtime_dir.path());
    let _socket = EnvGuard::set("IDEOCODE_SOCKET", &socket_path);
    let _debug = EnvGuard::set("IDEOCODE_DEBUG_CONTROL", "1");
