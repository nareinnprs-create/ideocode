...
/// paths for OpenAI (codex-spark) and Claude (haiku) OAuth, but IDEOCODE also runs
...
pub struct MultiProvider {
    ...
    /// `IDEOCODE-provider-copilot-runtime` and is instantiated through
    ...
    /// `IDEOCODE-provider-antigravity-runtime` and is instantiated through
    /// `external::instantiate_external_provider`.
    antigravity: RwLock<Option<Arc<dyn Provider>>>,
    /// Gemini provider (hot-swappable after login). Held as `dyn Provider`:
    /// the concrete runtime lives downstream in `IDEOCODE-provider-gemini-runtime`
    ...
    /// `dyn Provider`: the concrete runtime lives downstream in
    /// `IDEOCODE-provider-cursor-runtime` and is instantiated through
    /// `external::instantiate_external_provider`.
...
impl MultiProvider {
    ...
    fn routes_memo_key(&self) -> String {
        ...
        format!(
            "{}|{}|{}|{:?}|{}|{}|{}|{}",
            // Scope by home so sandboxes (tests, IDEOCODE_HOME switches) never
            // share catalogs that were built from different credential files.
            std::env::var("IDEOCODE_HOME").unwrap_or_default(),
    ...
    fn activity_source_key(&self, provider: ActiveProvider) -> String {
        match provider {
            ...
            ActiveProvider::OpenRouter => {
                ...
                // IDEOCODE subscription, and direct OpenAI-compatible profiles.
                let label = self
                    .active_openrouter_execution_provider()
                    .map(|execution| execution.runtime_display_name())
                    .unwrap_or_else(|| "OpenRouter".to_string());
                let runtime = std::env::var("IDEOCODE_RUNTIME_PROVIDER").ok();
        ...
        match provider {
            ActiveProvider::Claude => {
                ...
                if let Some(anthropic) = self.anthropic_provider() {
                    ...
                    anyhow::bail!(
                        "Claude credentials not available. Run `IDEOCODE login --provider claude` first."
                    );
                }
            ...
            ActiveProvider::OpenAI => {
                let Some(openai) = self.openai_provider() else {
                    ...
                    anyhow::bail!(
                        "OpenAI credentials not available. Run `IDEOCODE login --provider openai` first."
                    );
                };
            ...
            ActiveProvider::Copilot => {
                let Some(copilot) = self.copilot_provider() else {
                    anyhow::bail!(
                        "GitHub Copilot credentials not available. Run `IDEOCODE login --provider copilot` first."
                    );
                };
            ...
            ActiveProvider::Antigravity => {
                let Some(antigravity) = self.antigravity_provider() else {
                    anyhow::bail!(
                        "Antigravity credentials not available. Run `IDEOCODE login --provider antigravity` first."
                    );
                };
            ...
            ActiveProvider::Gemini => {
                let Some(gemini) = self.gemini_provider() else {
                    anyhow::bail!(
                        "Gemini credentials not available. Run `IDEOCODE login --provider gemini` first."
                    );
                };
            ...
            ActiveProvider::Cursor => {
                let Some(cursor) = self.cursor_provider() else {
                    anyhow::bail!(
                        "Cursor credentials not available. Run `IDEOCODE login --provider cursor` first."
                    );
                };
            ...
            ActiveProvider::OpenRouter => {
                ...
                let Some(openrouter) = self.openrouter_provider() else {
                    anyhow::bail!(
                        "OpenRouter/OpenAI-compatible credentials not available. Set the configured API key or run `IDEOCODE login --provider openrouter` first."
                    );
                };
    ...
    fn set_model_on_openai_compatible_profile(
        ...
        if !crate::provider_catalog::openai_compatible_profile_is_configured(profile) {
            anyhow::bail!(
                "{} credentials not available. Run `IDEOCODE login --provider {}` first.",
...
impl Provider for MultiProvider {
    ...
    fn handles_tools_internally(&self) -> bool {
        match self.active_provider() {
            ActiveProvider::Claude => {
                // Direct API does NOT handle tools internally - IDEOCODE executes them
            ...
            ActiveProvider::Cursor => self
                ...
                .map(|o| o.handles_tools_internally())
                .unwrap_or(false),
            ActiveProvider::Bedrock => false, // IDEOCODE executes Bedrock tool calls
            ActiveProvider::OpenRouter => false, // IDEOCODE executes tools
        }
    }
