...
const STATUS_COLORS = { idle: "text-fg-muted", running: "text-accent", completed: "text-success", failed: "text-error" };
    ...
    <div className="flex flex-col h-full" role="region" aria-label="Sub-agents">
      ...
      <div className="px-3 py-2 border-b border-border-subtle space-y-1.5">
        ...
        <textarea placeholder="What should the subagent do?" value={taskInput} onChange={(e) => setTaskInput(e.target.value)} rows={2}
          ...
          className="w-full p-1.5 text-xs bg-surface-elevated border border-border-subtle rounded text-fg-primary focus:outline-none focus:border-accent resize-none" />
          ...
          agents.map((a) => (
            <div key={a.id} className="border-b border-border-subtle group">
              <div onClick={() => setExpandedId(expandedId === a.id ? null : a.id)}
                ...
                className="px-3 py-2 hover:bg-surface-elevated cursor-pointer transition-fast flex items-center gap-2">
                <ChevronRight size={12} className={`text-fg-muted transition-transform ${expandedId === a.id ? "rotate-90" : ""}`} />
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-medium text-fg-primary">{a.name}</span>
                    <span className={`text-[10px] font-medium ${STATUS_COLORS[a.status]}`}>{a.status}</span>
                    {a.status === "running" && <Loader2 size={11} className="text-accent animate-spin" />}
                  </div>
                  <div className="text-[11px] text-fg-muted truncate">{a.task}</div>
                </div>
              ...
              {expandedId === a.id && (
                <div className="px-6 pb-2 text-[11px] text-fg-secondary">
                  <div className="mb-1 font-medium">Task:</div>
                  <div className="whitespace-pre-wrap bg-surface-elevated p-2 rounded">{a.task}</div>
                  {a.error && (
                  ...
                  {a.result && (<>
                    ...
                    <div className="whitespace-pre-wrap bg-surface-elevated p-2 rounded">{a.result}</div>
                  </>)}
                  {a.status === "failed" && (
                    <button onClick={() => retry(a.id)}
                      className="mt-2 flex items-center gap-1 px-2 py-1 text-[11px] text-accent hover:bg-surface-elevated rounded transition-fast">
                      Retry
                    </button>
                  )}
                </div>
