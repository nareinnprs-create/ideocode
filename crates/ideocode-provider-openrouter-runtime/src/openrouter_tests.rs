...
fn isolate_openrouter_autodetect_env() -> Vec<EnvVarGuard> {
    let mut guards = vec![
        EnvVarGuard::remove("IDEOCODE_OPENROUTER_API_BASE"),
        EnvVarGuard::remove("IDEOCODE_OPENROUTER_API_KEY_NAME"),
        EnvVarGuard::remove("IDEOCODE_OPENROUTER_ENV_FILE"),
        EnvVarGuard::remove("IDEOCODE_OPENROUTER_DYNAMIC_BEARER_PROVIDER"),
        EnvVarGuard::remove("IDEOCODE_OPENROUTER_MODEL"),
        EnvVarGuard::remove("IDEOCODE_OPENROUTER_CACHE_NAMESPACE"),
        EnvVarGuard::remove("IDEOCODE_OPENROUTER_ALLOW_NO_AUTH"),
        EnvVarGuard::remove("IDEOCODE_OPENROUTER_TRANSPORT_STATE"),
        EnvVarGuard::remove("IDEOCODE_OPENROUTER_PROVIDER_FEATURES"),
        EnvVarGuard::remove("IDEOCODE_OPENROUTER_MODEL_CATALOG"),
        EnvVarGuard::remove("IDEOCODE_OPENROUTER_AUTH_HEADER"),
        EnvVarGuard::remove("IDEOCODE_OPENROUTER_AUTH_HEADER_NAME"),
        EnvVarGuard::remove("IDEOCODE_OPENROUTER_STATIC_MODELS"),
        EnvVarGuard::remove("IDEOCODE_ACTIVE_PROVIDER"),
        EnvVarGuard::remove("IDEOCODE_RUNTIME_PROVIDER"),
        EnvVarGuard::remove("IDEOCODE_NAMED_PROVIDER_PROFILE"),
        EnvVarGuard::remove("IDEOCODE_PROVIDER_PROFILE_NAME"),
        EnvVarGuard::remove("IDEOCODE_PROVIDER_PROFILE_ACTIVE"),
        EnvVarGuard::remove("IDEOCODE_OPENAI_COMPAT_API_BASE"),
        EnvVarGuard::remove("IDEOCODE_OPENAI_COMPAT_API_KEY_NAME"),
        EnvVarGuard::remove("IDEOCODE_OPENAI_COMPAT_ENV_FILE"),
        EnvVarGuard::remove("IDEOCODE_OPENAI_COMPAT_SETUP_URL"),
        EnvVarGuard::remove("IDEOCODE_OPENAI_COMPAT_DEFAULT_MODEL"),
...
fn kimi_for_coding_tool_call_message_includes_reasoning_content() {
    let _lock = ENV_LOCK.lock();
    let _thinking = EnvVarGuard::remove("IDEOCODE_OPENROUTER_THINKING");
    let (api_base, request_rx) = spawn_single_response_chat_server();
    let provider = OpenRouterProvider {
        api_base,
...
fn comtegra_profile_uses_endpoint_default_max_tokens() {
    let _lock = ENV_LOCK.lock();
    let _override = EnvVarGuard::remove("IDEOCODE_OPENROUTER_MAX_TOKENS");
...
fn max_tokens_env_overrides_profile_default() {
    let _lock = ENV_LOCK.lock();
    let _override = EnvVarGuard::set("IDEOCODE_OPENROUTER_MAX_TOKENS", "4096");
...
fn autodetected_profile_seeds_default_model_and_cache_namespace() {
    ...
    assert_eq!(
        std::env::var("IDEOCODE_OPENROUTER_CACHE_NAMESPACE")
            .ok()
            .as_deref(),
        Some("zai")
    );
...
fn live_openrouter_models() -> Vec<String> {
    std::env::var("IDEOCODE_LIVE_OPENROUTER_MODELS")
        .or_else(|_| std::env::var("IDEOCODE_OPENROUTER_MODEL"))
...
async fn live_openrouter_unified_reasoning_smoke() -> Result<()> {
    ...
    let effort = std::env::var("IDEOCODE_LIVE_OPENROUTER_REASONING_EFFORT")
        .unwrap_or_else(|_| "low".to_string());
    let max_tokens = std::env::var("IDEOCODE_LIVE_OPENROUTER_MAX_TOKENS")
        .ok()
...
fn direct_deepseek_profile_uses_static_1m_context_when_catalog_is_absent() {
    let _lock = ENV_LOCK.lock();
    let _base = EnvVarGuard::set("IDEOCODE_OPENROUTER_API_BASE", "https://api.deepseek.com");
    let _key_name = EnvVarGuard::set("IDEOCODE_OPENROUTER_API_KEY_NAME", "DEEPSEEK_API_KEY");
    let _api_key = EnvVarGuard::set("DEEPSEEK_API_KEY", "test");
    let _namespace = EnvVarGuard::set("IDEOCODE_OPENROUTER_CACHE_NAMESPACE", "deepseek");
    let _model = EnvVarGuard::set("IDEOCODE_OPENROUTER_MODEL", "deepseek-v4-flash");
    let _catalog = EnvVarGuard::set("IDEOCODE_OPENROUTER_MODEL_CATALOG", "0");
...
fn ideocode_subscription_runtime_has_explicit_display_and_route_identity() {
    ...
    let provider = OpenRouterProvider::new().expect("build IDEOCODE subscription runtime");
    assert_eq!(provider.runtime_display_name(), "IDEOCODE Subscription");
    assert_eq!(Provider::display_name(&provider), "IDEOCODE Subscription");
...
fn non_subscription_runtimes_keep_existing_display_and_route_identity() {
    ...
    let _key_name = EnvVarGuard::set("IDEOCODE_OPENROUTER_API_KEY_NAME", "GENERIC_API_KEY");
    let _provider_features = EnvVarGuard::set("IDEOCODE_OPENROUTER_PROVIDER_FEATURES", "0");
    let _transport = EnvVarGuard::set("IDEOCODE_OPENROUTER_TRANSPORT_STATE", "direct-compatible");
...
fn resolve_extra_body_returns_none_when_unset() {
    let _lock = ENV_LOCK.lock();
    let _guard = EnvVarGuard::remove("IDEOCODE_OPENAI_EXTRA_BODY");
...
fn resolve_extra_body_ignores_invalid_env_json() {
    let _lock = ENV_LOCK.lock();
    let _guard = EnvVarGuard::set("IDEOCODE_OPENAI_EXTRA_BODY", "not-json");
    assert!(OpenRouterProvider::resolve_extra_body(None, "nonexistent.env").is_none());
}
...
fn resolve_extra_body_ignores_non_object_env_json() {
    let _lock = ENV_LOCK.lock();
    let _guard = EnvVarGuard::set("IDEOCODE_OPENAI_EXTRA_BODY", "[1,2,3]");
    assert!(OpenRouterProvider::resolve_extra_body(None, "nonexistent.env").is_none());
}
...
/// `IDEOCODE run` (which binds profile_id in-process) works fine.
#[test]
