...
//! providers, including every OpenAI-compatible profile IDEOCODE ships.
//!
//! This module mirrors the OpenRouter catalog pattern:
//!   - a 24h disk cache under `~/.IDEOCODE/cache/models_dev_pricing.json`,
...
/// from, so changing `IDEOCODE_HOME` (tests, multi-home setups) never serves
...
pub fn schedule_refresh() {
    ...
    // IDEOCODE_FORCE_PRICING_REFRESH=1 re-enables the fetch for manual e2e checks
    // (e.g. `cargo run --example pricing_e2e_check`, which builds with
    // test-support unified in).
    let forced = std::env::var_os("IDEOCODE_FORCE_PRICING_REFRESH").is_some();
    if !forced
        && (cfg!(any(test, feature = "test-support"))
            || std::env::var_os("IDEOCODE_DISABLE_PRICING_REFRESH").is_some())
    {
        return;
    }
