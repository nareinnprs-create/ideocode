...
/// Reads the last-seen commit hash from ~/.IDEOCODE/last_seen_changelog,
...
pub(super) fn get_unseen_changelog_entries() -> &'static Vec<String> {
    ...
    ENTRIES.get_or_init(|| {
        ...
        let state_file = dirs::home_dir()
            .map(|h| h.join(".IDEOCODE").join("last_seen_changelog"))
            .unwrap_or_else(|| std::path::PathBuf::from(".IDEOCODE/last_seen_changelog"));
