...
impl LogLevel {
    ...
    fn is_enabled(self) -> bool {
        !matches!(self, Self::Debug) || std::env::var("IDEOCODE_TRACE").is_ok()
    }
}
...
impl Logger {
    fn new() -> Option<Self> {
        ...
        let path = log_dir.join(format!("IDEOCODE-{}.log", date));
    ...
    fn write(&mut self, level: &str, message: &str) {
        ...
        if let Err(err) = self.file.write_all(line.as_bytes()) {
            eprintln!("IDEOCODE logger write failed: {err}");
            return;
        }
        if let Err(err) = self.file.flush() {
            eprintln!("IDEOCODE logger flush failed: {err}");
        }
    }
}
...
pub fn init() {
    ...
    // `IDEOCODE-*.log` files, so it is safe to run unconditionally.
    if !CLEANUP_STARTED.swap(true, Ordering::SeqCst) {
        std::thread::Builder::new()
            .name("IDEOCODE-log-cleanup".to_string())
...
/// Log a debug message (only if IDEOCODE_TRACE is set)
#[expect(
    clippy::collapsible_if,
    reason = "Debug logging keeps env gating and logger access explicit"
)]
pub fn debug(message: &str) {
    if std::env::var("IDEOCODE_TRACE").is_ok() {
        if let Ok(mut guard) = LOGGER.lock() {
            if let Some(logger) = guard.as_mut() {
...
fn structured_json_enabled() -> bool {
    matches!(
        std::env::var("IDEOCODE_LOG_JSON").as_deref(),
...
pub fn log_path() -> Option<PathBuf> {
    let log_dir = log_dir()?;
    let date = Local::now().format("%Y-%m-%d");
    Some(log_dir.join(format!("IDEOCODE-{}.log", date)))
}

/// Remove daily `IDEOCODE-*.log` / `IDEOCODE-desktop-*.log` files older than 7 days.
///
...
fn cleanup_old_logs_in(log_dir: &std::path::Path, now: chrono::DateTime<Local>) {
    ...
    for entry in entries.flatten() {
            ...
            && let Ok(modified) = metadata.modified()
        {
            ...
            {
                eprintln!("IDEOCODE logger cleanup failed: {err}");
            }
        }
    }
}
...
mod tests {
    ...
    fn cleanup_removes_only_old_ideocode_logs() {
        ...
        let old_log = write("IDEOCODE-2000-01-01.log", true);
        let old_desktop = write("IDEOCODE-desktop-2000-01-01.log", true);
        // Recent log file that SHOULD survive.
        let new_log = write("IDEOCODE-2099-01-01.log", false);
        ...
        assert!(!old_log.exists(), "old IDEOCODE log should be deleted");
        assert!(!old_desktop.exists(), "old desktop log should be deleted");
        assert!(new_log.exists(), "recent IDEOCODE log must survive");
