...
//! Self-development tool - manage canary builds when working on IDEOCODE itself
...
impl BuildRequest {
    ...
    fn terminal_history_limit() -> usize {
        std::env::var("IDEOCODE_SELFDEV_REQUEST_HISTORY_LIMIT")
            .ok()
...
impl SelfDevTool {
    ...
    pub fn description_for(is_selfdev: bool) -> &'static str {
        if is_selfdev {
            "Manage self-dev builds, tests, and reloads while working on IDEOCODE itself."
        } else {
            "Enter self-dev mode to work on IDEOCODE itself. Also sets up the dev \
             environment, reloads IDEOCODE to a newer build, and locates IDEOCODE config/paths."
        }
    }
            ...
            json!({
                ...
                "properties": {
                    ...
                    "target": {
                        ...
                        "description": "Build target for action=build. auto chooses from changed paths; tui builds IDEOCODE; desktop builds IDEOCODE-desktop; all builds both."
                    },
                    "command": {
                        "type": "string",
            ...
            json!({
                ...
                "properties": {
                    ...
                    "action": {
                        ...
                        "description": "Action. `enter` spawns a self-dev session (optionally seeded with `prompt`); `setup` checks/installs the dev prerequisites (rust toolchain, git, repo clone); `reload` restarts IDEOCODE into a newer installed build; `status` shows build/version state; `find-config` locates IDEOCODE config and key paths."
                    },
                    "prompt": {
                        "type": "string",
...
impl Tool for SelfDevTool {
    ...
    async fn execute(&self, input: Value, ctx: ToolContext) -> Result<ToolOutput> {
        ...
        let result = match action.as_str() {
            ...
            // access only make sense once you are working on IDEOCODE itself.
            "build" => {
                self.do_build(
                    params.reason,
                    params.target,
...
impl SelfDevTool {
    fn is_test_session() -> bool {
        std::env::var("IDEOCODE_TEST_SESSION")
            .map(|value| {
    ...
    fn reload_timeout_secs() -> u64 {
        std::env::var("IDEOCODE_SELFDEV_RELOAD_TIMEOUT_SECS")
            .ok()
    ...
    fn build_reload_wait_secs() -> u64 {
        std::env::var("IDEOCODE_SELFDEV_BUILD_WAIT_SECS")
            .ok()
            .and_then(|raw| raw.trim().parse::<u64>().ok())
            .filter(|secs| *secs > 0)
            .unwrap_or(1800)
    ...
    fn launch_binary() -> Result<std::path::PathBuf> {
        build::client_update_candidate(true)
            ...
            .ok_or_else(|| anyhow::anyhow!("Could not resolve IDEOCODE executable to launch"))
    }
