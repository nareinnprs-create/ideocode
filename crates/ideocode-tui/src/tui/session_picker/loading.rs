...
fn session_scan_limit() -> usize {
    std::env::var("IDEOCODE_SESSION_PICKER_MAX_SESSIONS")
        .ok()
...
fn include_old_saved_sessions_on_initial_load() -> bool {
    std::env::var("IDEOCODE_SESSION_PICKER_INCLUDE_OLD_SAVED")
        .ok()
...
pub fn load_sessions() -> Result<Vec<SessionInfo>> {
    ...
    let (mut sessions, external_sessions) = std::thread::scope(|scope| {
        ...
        // default list appear to jump from a handful of IDEOCODE rows straight to old
