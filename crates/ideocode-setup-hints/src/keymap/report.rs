...
/// counts, and any conflicts with IDEOCODE's configured bindings.
pub fn render_report(cfg: &KeybindingsConfig, snapshot: &KeymapSnapshot) -> String {
    ...
    if term_count == 0 && sys_count == 0 && app_count == 0 {
        out.push_str(
            "\nNo machine bindings were discovered. IDEOCODE can read Ghostty bindings, macOS\n\
    ...
    if conflicts.is_empty() {
        out.push_str("No conflicts found between your IDEOCODE keybindings and the machine.\n");
    } else {
        out.push_str(&format!(
        ...
        out.push_str(
            "These keys may be captured by your terminal, macOS, or another app (window\n\
             manager, launcher) before IDEOCODE sees them.\n\
             To fix: rebind the IDEOCODE action in ~/.IDEOCODE/config.toml under [keybindings],\n\
...
fn render_conflict_block(c: &Conflict) -> String {
    ...
    format!(
        "  ⚠ {key}\n      IDEOCODE: {action} ({field} = \"{raw}\")\n      taken by {interceptor}\n",
        key = c.IDEOCODE.chord.display(),
        action = c.IDEOCODE.action,
        field = c.IDEOCODE.field,
        raw = c.IDEOCODE.raw,
...
pub fn render_status_line(cfg: &KeybindingsConfig, snapshot: &KeymapSnapshot) -> Option<String> {
    ...
    let keys: Vec<String> = conflicts
        .iter()
        .map(|c| c.IDEOCODE.chord.display())
        .collect::<std::collections::BTreeSet<_>>()
        .into_iter()
        .collect();
    Some(format!(
