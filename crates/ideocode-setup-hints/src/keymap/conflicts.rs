...
/// One configured IDEOCODE binding, tied back to its config field.
#[derive(Debug, Clone)]
pub struct IDEOCODEBinding {
...
/// Enumerate IDEOCODE's configured bindings as comparable chords. Bindings that are
...
/// Find conflicts between IDEOCODE's configured bindings and the discovered
/// machine bindings in `snapshot`.
///
/// Conflicts are deduplicated per `(IDEOCODE field, interceptor chord, source)` so
...
mod tests {
    ...
    fn detects_window_manager_prompt_jump_conflict() {
        ...
        // focus, shadowing IDEOCODE's built-in prompt navigation fallback.
        let cfg = KeybindingsConfig::default();
        let snapshot = snapshot_with(vec![
        ...
        assert!(
            conflicts
                .iter()
                .all(|c| c.IDEOCODE.field.contains("built-in fallback")),
    ...
    fn no_conflict_when_chords_differ() {
        ...
        // IDEOCODE has no cmd+t binding by default.
    ...
    fn disabled_binding_is_not_reported() {
        ...
        assert!(
            !conflicts
                .iter()
                .any(|c| c.IDEOCODE.field == "keybindings.model_switch_next")
        );
    }
