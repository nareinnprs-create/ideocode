...
MAIN_SOCK = f"/run/user/{os.getuid()}/IDEOCODE.sock"
DEBUG_SOCK = f"/run/user/{os.getuid()}/IDEOCODE-debug.sock"
...
def get_server_pid():
    ...
    try:
        ...
        for line in result.stdout.splitlines():
            if "IDEOCODE.sock" in line and "LISTEN" in line:
                parts = line.split()
                return int(parts[1])
    except:
        pass
    
    # Fallback: find the oldest IDEOCODE process (likely the server)
    try:
        result = subprocess.run(
            ["pgrep", "-o", "IDEOCODE"], capture_output=True, text=True, timeout=5
        )
...
print_header(f"IDEOCODE Stress Test: {NUM_INSTANCES} sessions")
...
if not os.path.exists(MAIN_SOCK):
    ...
    print("  Start a server with: IDEOCODE serve &")
    sys.exit(1)
...
baseline_procs = int(subprocess.run(["pgrep", "-c", "IDEOCODE"], capture_output=True, text=True).stdout.strip() or "0")
...
print_stat("Total IDEOCODE procs", baseline_procs)
...
for i in range(1, NUM_INSTANCES + 1):
    t0 = time.monotonic()
    resp = debug_cmd(f"create_session:/tmp/IDEOCODE-stress-{i}", timeout=30)
    t1 = time.monotonic()
...
peak_procs = int(subprocess.run(["pgrep", "-c", "IDEOCODE"], capture_output=True, text=True).stdout.strip() or "0")
...
print_stat("Total IDEOCODE procs", peak_procs)
...
final_procs = int(subprocess.run(["pgrep", "-c", "IDEOCODE"], capture_output=True, text=True).stdout.strip() or "0")
