    ...
    match choice {
        ProviderChoice::Auto => {
            if options.uses_scriptable_flow()? {
                anyhow::bail!(
                    "Scriptable login flags require an explicit provider. Use `IDEOCODE login --provider <provider> ...`."
                );
            }
            ...
            if !io::stdin().is_terminal() {
                anyhow::bail!(
                    "`IDEOCODE login --provider auto` requires an interactive terminal. Use `IDEOCODE login --provider <provider>` in non-interactive mode."
                );
            }
...
/// Best-effort: tell a running IDEOCODE server that on-disk auth has changed so it
...
fn login_azure_flow() -> Result<()> {
    ...
    eprintln!(
        "Reference: OpenCode supports Azure OpenAI with Entra credentials. IDEOCODE uses Azure OpenAI's newer `/openai/v1` API with either Microsoft Entra ID or an API key.\n"
    );
    ...
    if use_entra {
        ...
        eprintln!(
            "That means IDEOCODE can authenticate via `az login`, managed identity, or Azure environment credentials."
        );
    } else {
...
fn login_openai_compatible_flow(
    ...
    if !resolved.requires_api_key && resolved.default_model.is_none() {
        match resolved.id.as_str() {
            "ollama" => {
                eprintln!(
                    "Next step: install a model with `ollama pull llama3.2`, then run `IDEOCODE --provider ollama --model llama3.2 run 'hello'`."
                );
            }
            "lmstudio" => {
                eprintln!(
                    "Next step: load a chat model in LM Studio's Local Server, then run IDEOCODE with that exact model id, for example `IDEOCODE --provider lmstudio --model <model-id> run 'hello'`."
                );
            }
            _ => {
                eprintln!(
                    "Next step: run IDEOCODE with a model available on this endpoint, for example `IDEOCODE --provider {} --model <model-id> run 'hello'`.",
                    resolved.id
                );
            }
        }
...
fn login_cursor_flow() -> Result<()> {
    ...
    eprintln!("IDEOCODE will use the native Cursor HTTPS transport.");
...
async fn login_antigravity_flow(no_browser: bool) -> Result<()> {
    ...
    eprintln!(
        "IDEOCODE will authenticate directly with Google Antigravity; the Antigravity desktop app is not required."
    );
    eprintln!(
        "If browser launch fails, or you pass `--no-browser`, IDEOCODE will prompt for the callback URL instead."
    );
    eprintln!(
...
async fn login_gemini_flow(no_browser: bool) -> Result<()> {
    ...
    eprintln!(
        "If browser launch fails, or you pass `--no-browser`, IDEOCODE will prompt for the manual authorization code."
    );
    eprintln!(
        ...
        Err(_) => {
            ...
            match input.trim() {
                ...
                "3" => {
                    ...
                    eprintln!("   - Fill in app name (e.g. 'IDEOCODE') and your email");
