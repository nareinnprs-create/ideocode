...
fn test_env(dir: &tempfile::TempDir) -> EnvGuard {
    ...
    let previous = std::env::var_os("IDEOCODE_RUNTIME_DIR");
    crate::env::set_var("IDEOCODE_RUNTIME_DIR", dir.path());
    EnvGuard {
        _lock: lock,
        runtime: previous,
    }
...
fn state_dir_is_durable_not_runtime() {
    ...
    // With IDEOCODE_RUNTIME_DIR pinned, the state dir stays sandboxed but must
