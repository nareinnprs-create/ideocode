...
pub enum AppRuntimeMode {
    /// Normal product TUI. The client renders state owned by the IDEOCODE server.
    RemoteClient,
...
fn active_runtime_provider_key() -> Option<String> {
    std::env::var("IDEOCODE_RUNTIME_PROVIDER")
        .ok()
...
/// An interactive "let a IDEOCODE agent merge the diverged update for you" offer.
///
...
/// IDEOCODE session, pre-loaded with a prompt to reconcile the branches, instead of
...
pub struct App {
    ...
    // Interactive "spawn a IDEOCODE agent to merge the diverged update" offer shown
    ...
    /// to target the agent repair brief (`IDEOCODE auth-test --provider X`). `None`
    /// when unknown.
...
// The cache-relevant projection lives in `IDEOCODE-message-types` (re-exported
// through `crate::message`) so this local path and the server event path in
// `IDEOCODE-app-core::agent::kv_cache_request_event` hash messages identically.
