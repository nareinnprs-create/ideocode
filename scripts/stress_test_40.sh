...
NUM_INSTANCES=${1:-40}
IDEOCODE_BIN="${IDEOCODE_BIN:-$(which IDEOCODE)}"
LOG_DIR="/tmp/IDEOCODE-stress-test-$(date +%s)"
mkdir -p "$LOG_DIR"

MAIN_SOCK="/run/user/$(id -u)/IDEOCODE.sock"
DEBUG_SOCK="/run/user/$(id -u)/IDEOCODE-debug.sock"

echo "========================================="
echo " IDEOCODE Stress Test: $NUM_INSTANCES instances"
echo "========================================="
echo "Binary: $IDEOCODE_BIN"
...
snapshot() {
    ...
    # IDEOCODE process count and total RSS
    local IDEOCODE_procs=$(pgrep -c IDEOCODE 2>/dev/null || echo 0)
    local total_rss=0
    local total_vms=0
    for pid in $(pgrep IDEOCODE 2>/dev/null); do
    ...
    echo "IDEOCODE_processes: $IDEOCODE_procs" >> "$LOG_DIR/snapshots.log"
    ...
    echo "[$label] procs=$IDEOCODE_procs rss=${total_rss}KB($(( total_rss / 1024 ))MB) server_rss=$(awk '/^VmRSS:/{print $2}' /proc/${server_pid:-0}/status 2>/dev/null || echo '?')KB fds=$(ls /proc/${server_pid:-0}/fd 2>/dev/null | wc -l) threads=$(ls /proc/${server_pid:-0}/task 2>/dev/null | wc -l)"
}
...
if ! [ -S "$MAIN_SOCK" ]; then
    echo "ERROR: No IDEOCODE server running at $MAIN_SOCK"
    echo "Start one with: IDEOCODE serve &"
    exit 1
fi
...
BASELINE_RSS=$(pgrep IDEOCODE 2>/dev/null | while read pid; do awk '/^VmRSS:/{print $2}' /proc/$pid/status 2>/dev/null; done | paste -sd+ | bc 2>/dev/null || echo 0)
BASELINE_PROCS=$(pgrep -c IDEOCODE 2>/dev/null || echo 0)
...
(
    while true; do
        ts=$(date +%s)
        IDEOCODE_procs=$(pgrep -c IDEOCODE 2>/dev/null || echo 0)
        total_rss=0
        for pid in $(pgrep IDEOCODE 2>/dev/null); do
        ...
        echo "$ts,$IDEOCODE_procs,$total_rss,$server_rss,$server_fds,$server_threads,$cpu_load"
        sleep 1
    done
...
# --- Spawn instances ---
echo "=== Spawning $NUM_INSTANCES IDEOCODE instances ==="
PIDS=()
SPAWN_TIMES=()

# Use script to give each instance a pty (IDEOCODE requires tty)
for i in $(seq 1 $NUM_INSTANCES); do
    ...
    script -q -c "$IDEOCODE_BIN --no-update --no-selfdev" /dev/null \
...
POST_PROCS=$(pgrep -c IDEOCODE 2>/dev/null || echo 0)
POST_RSS=0
for pid in $(pgrep IDEOCODE 2>/dev/null); do
    rss=$(awk '/^VmRSS:/{print $2}' /proc/$pid/status 2>/dev/null || echo 0)
