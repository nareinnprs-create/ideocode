...
/// Convert Claude Code content blocks to IDEOCODE ContentBlocks
...
fn reuse_existing_imported_session(session_id: &str) -> bool {
    // An imported snapshot becomes a normal IDEOCODE continuation as soon as the
    ...
    // doing so can discard IDEOCODE-only turns and journal state. New imports are
...
pub fn resolve_resume_target_to_IDEOCODE(
    ...
    let session_id = match target {
        ResumeTarget::IDEOCODESession { session_id } => {
            return Ok(ResumeTarget::IDEOCODESession {
                session_id: session_id.clone(),
            });
        }
    ...
    Ok(ResumeTarget::IDEOCODESession { session_id })
}
...
/// Explicitly hand a currently-running Claude Code session over to IDEOCODE.
///
/// This is deliberately separate from normal resume. It first imports the
/// current transcript into a fresh durable IDEOCODE session, then gracefully stops
...
fn take_over_live_claude_session_with_timeout(
    ...
    // Use a normal memorable IDEOCODE session ID so the handed-off conversation
    ...
    if stop_outcome == crate::claude_live::StopLiveClaudeOutcome::ExitUnconfirmed {
        crate::session_list_cache::invalidate();
        anyhow::bail!(
            "Claude Code process {} was asked to exit, but its exit was not confirmed; prepared IDEOCODE session {} was preserved and can be resumed",
            live.pid,
            takeover_id
        );
    }
    ...
    if let Some(err) = final_refresh_error {
        ...
        return Err(err).with_context(|| {
            format!(
                "Claude Code exited, but its final transcript could not be refreshed; prepared IDEOCODE session {takeover_id} was preserved"
            )
        });
    }
    ...
    crate::session_list_cache::invalidate();
    Ok(ResumeTarget::IDEOCODESession {
...
fn finalize_imported_session(
    ...
    // Never overwrite a IDEOCODE-side continuation with a shorter external
