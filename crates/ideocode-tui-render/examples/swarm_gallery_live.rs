...
//!
//! Run with:
//!   cargo run --profile selfdev -p IDEOCODE-tui-render --example swarm_gallery_live
//!
//! Controls:
//!   q / Esc      quit
//!   + / -        more / fewer agents
    ...
    let pool: Vec<(&str, Option<&'static str>, Vec<&str>)> = vec![
        (
            ...
            vec![
                ...
                "Reading crates/IDEOCODE-app-core/src/auth.rs",
        ...
        (
            ...
            None,
            vec![
                "Editing crates/IDEOCODE-base/src/config.rs",
...
fn draw(f: &mut Frame, workers: &[MockWorker], max_pct: usize, paused: bool) {
    ...
    let help = vec![
        ...
        Line::from(""),
        Line::from(vec![Span::styled(
            "  The band above is the same renderer the IDEOCODE TUI shows above your chat",
