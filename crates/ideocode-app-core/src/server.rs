...
const SERVER_NAME_ENV: &str = "IDEOCODE_SERVER_NAME";
const SERVER_DISPLAY_NAME_ENV: &str = "IDEOCODE_SERVER_DISPLAY_NAME";
...
impl Server {
    ...
    fn spawn_power_inhibitor(swarm_members: Arc<RwLock<HashMap<String, SwarmMember>>>) {
        ...
        if !inhibitor.is_available() {
            ...
            crate::logging::info(
                "power_inhibit: unavailable (unsupported platform or IDEOCODE_DISABLE_POWER_INHIBIT set); not monitoring",
            );
            return;
        }
    ...
    pub async fn run(&self) -> Result<()> {
        // Ensure socket directory exists (for named sockets like /run/user/1000/IDEOCODE/)
