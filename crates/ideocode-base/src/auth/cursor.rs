...
// `IDEOCODE_CURSOR_CLIENT_VERSION` if Cursor moves the floor again.
...
pub fn has_authenticated_cli_session() -> bool {
    let command = std::env::var_os("IDEOCODE_CURSOR_CLI_PATH")
...
pub fn cursor_direct_client_version() -> String {
    std::env::var("IDEOCODE_CURSOR_CLIENT_VERSION")
        .ok()
...
/// Save a Cursor API key to `~/.config/IDEOCODE/cursor.env`.
...
/// Remove the saved Cursor API key from `~/.config/IDEOCODE/cursor.env` and the
...
pub fn cursor_auth_file_path() -> Result<PathBuf> {
    ...
    {
        // Honor IDEOCODE_HOME isolation (used by the onboarding sandbox and tests)
        ...
        // `$IDEOCODE_HOME/external/...`, so a fresh-install sandbox would show only
