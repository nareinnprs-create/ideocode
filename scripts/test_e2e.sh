#!/bin/bash
# End-to-end test script for IDEOCODE
...
echo "=== E2E Testing Script for IDEOCODE ==="
echo ""

# Test 1: Check binary exists and runs
echo "Test 1: Check IDEOCODE binary..."
if command -v IDEOCODE &> /dev/null; then
    echo "✓ IDEOCODE binary found"
    IDEOCODE --version
else
    echo "✗ IDEOCODE binary not found"
    exit 1
fi
...
if [[ "${IDEOCODE_REAL_PROVIDER:-0}" == "1" ]]; then
    echo ""
    echo "Test 7: Real provider smoke (IDEOCODE_REAL_PROVIDER=1)..."
    scripts/real_provider_smoke.sh
    echo "✓ Real provider smoke passed"
fi

if [[ "${IDEOCODE_REAL_AUTH_TEST:-0}" == "1" ]]; then
    echo ""
    echo "Test 8: Auth E2E validation (IDEOCODE_REAL_AUTH_TEST=1)..."
...
echo "  IDEOCODE        # Start TUI mode"
echo "  IDEOCODE server # Start server mode"
echo "  IDEOCODE client # Connect to server"
