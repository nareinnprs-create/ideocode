...
MODEL="${IDEOCODE_OPENRELAY_TEST_MODEL:-gpt-5.6-sol}"
PROVIDER="${IDEOCODE_OPENRELAY_TEST_PROVIDER:-openai}"
...
PROMPT="${IDEOCODE_OPENRELAY_TEST_PROMPT:-$DEFAULT_PROMPT}"
OUTPUT="${IDEOCODE_OPENRELAY_TEST_OUTPUT:-$REPO_ROOT/target/openrelay-discovery-test/latest.ndjson}"
SERVER_LOG="${IDEOCODE_OPENRELAY_TEST_SERVER_LOG:-$REPO_ROOT/target/openrelay-discovery-test/server.jsonl}"
...
Runs a local-only OpenRelay Discovery fixture and an isolated IDEOCODE agent test.
Environment overrides:
  IDEOCODE_BIN, IDEOCODE_OPENRELAY_TEST_MODEL, IDEOCODE_OPENRELAY_TEST_PROVIDER
  IDEOCODE_OPENRELAY_TEST_PROMPT, IDEOCODE_OPENRELAY_TEST_OUTPUT
EOF
    exit 0
    ;;
  --print-prompt)
...
[[ -x "$IDEOCODE_BIN" ]] || { printf 'IDEOCODE binary is not executable: %s\n' "$IDEOCODE_BIN" >&2; exit 1; }

mkdir -p "$(dirname "$OUTPUT")" "$(dirname "$SERVER_LOG")"
test_root="$(mktemp -d "${TMPDIR:-/tmp}/IDEOCODE-openrelay-discovery.XXXXXX")"
...
for name in openai-auth.json auth.json auth-refresh-state.json auth-validation.json provider_activity.json; do
  if [[ -f "$HOME/.IDEOCODE/$name" ]]; then
    cp -p "$HOME/.IDEOCODE/$name" "$test_home/$name"
  fi
done
...
printf 'IDEOCODE_PROGRESS {"percent":10,"message":"Local Discovery fixture ready"}\n'
printf 'IDEOCODE_PROGRESS {"percent":20,"message":"OpenRelay public RPC verified"}\n'

set +e
IDEOCODE_HOME="$test_home" \
IDEOCODE_RUNTIME_DIR="$runtime_dir" \
IDEOCODE_DISCOVERY_BENCHMARK=1 \
...
printf 'IDEOCODE_PROGRESS {"percent":90,"message":"Validating agent trace"}\n'
set +e
...
if [[ $status -ne 0 ]]; then
  printf 'IDEOCODE exited with status %s\n' "$status" >&2
  exit "$status"
fi
...
printf 'IDEOCODE_CHECKPOINT {"message":"OpenRelay Discovery test passed"}\n'
