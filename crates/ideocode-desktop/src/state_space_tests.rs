...
//!
//! Run with:
//!   cargo test -p IDEOCODE-desktop state_space -- --nocapture
...
fn walk_budget() -> WalkBudget {
    WalkBudget {
        max_depth: env_usize("IDEOCODE_WALK_DEPTH", MAX_DEPTH),
        max_unique_states: env_usize("IDEOCODE_WALK_STATES", MAX_UNIQUE_STATES_PER_SEED),
        seed_time_budget: Duration::from_secs(env_usize(
            "IDEOCODE_WALK_SECONDS",
