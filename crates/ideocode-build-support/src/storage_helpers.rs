...
fn resolve_builds_dir(
    ...
    if is_windows && let Some(local_app_data) = local_app_data {
        ...
        // the supported Windows layout under %LOCALAPPDATA%\IDEOCODE\builds.
        // Durable user state and logs still live under ~/.IDEOCODE.
...
mod tests {
    ...
    fn windows_builds_use_local_app_data() {
        let resolved = resolve_builds_dir(
            ...
            PathBuf::from("/home/test/.IDEOCODE"),
            true,
        );

        assert_eq!(resolved, PathBuf::from("/local-app-data/IDEOCODE/builds"));
    }
    ...
    fn ideocode_home_override_wins_on_windows() {
        ...
        assert_eq!(resolved, PathBuf::from("/isolated-IDEOCODE/builds"));
    }
    ...
    fn non_windows_builds_stay_under_ideocode_home() {
        ...
        assert_eq!(resolved, PathBuf::from("/home/test/.IDEOCODE/builds"));
    }
}
