...
export IDEOCODE_IN_DEV_CARGO=1
...
configure_build_tmpdir() {
  local candidate="${IDEOCODE_BUILD_TMPDIR:-}"
  if [[ -n "$candidate" ]]; then
    ...
    return
  elif [[ -n "${IDEOCODE_SCRATCH_DIR:-}" ]]; then
    candidate="$IDEOCODE_SCRATCH_DIR/cargo-tmp"
    build_tmpdir_status="IDEOCODE-scratch"
  elif [[ -n "${IDEOCODE_HOME:-}" ]]; then
    candidate="$IDEOCODE_HOME/scratch/cargo-tmp"
    build_tmpdir_status="IDEOCODE-home"
  elif [[ -n "${HOME:-}" ]]; then
    candidate="$HOME/.IDEOCODE/scratch/cargo-tmp"
    build_tmpdir_status="home-scratch"
  else
    candidate="$repo_root/target/IDEOCODE-scratch/cargo-tmp"
...
maybe_enable_sccache() {
  ...
  # caller explicitly forces it via IDEOCODE_SCCACHE=1/on/force.
  local force_sccache="${IDEOCODE_SCCACHE:-auto}"
  case "$force_sccache" in
    1|true|yes|on|force) force_sccache="1" ;;
    0|false|no|off|never)
      sccache_status="disabled-by-IDEOCODE-sccache"
      log "sccache disabled by IDEOCODE_SCCACHE"
      return
      ;;
    *) force_sccache="auto" ;;
  esac
  ...
  if [[ "$force_sccache" != "1" ]] && build_is_incremental "$@"; then
    ...
    log "sccache skipped for incremental build (it cannot cache incremental units; set IDEOCODE_SCCACHE=on to force, or use a non-incremental profile)"
    return
  fi
...
validate_feature_profile() {
  local profile="${IDEOCODE_DEV_FEATURE_PROFILE:-default}"
  case "$profile" in
    ""|default|minimal|none|pdf|embeddings|full)
      ;;
    *)
      printf 'error: unsupported IDEOCODE_DEV_FEATURE_PROFILE=%s (expected default|minimal|pdf|embeddings|full)\n' "$profile" >&2
      exit 1
      ;;
  esac
}
...
selfdev_low_memory_default_needed() {
  ...
  # On small no-swap machines, earlyoom can terminate the root IDEOCODE rustc
...
#
# After the monolith was split into the IDEOCODE-base/app-core/tui/cli crate DAG,
# the largest single rustc unit is IDEOCODE-base, which has grown back to a
...
# coarse sample, and IDEOCODE-base keeps growing) so a fresh build under load backs
...
# backs off further. An explicit CARGO_BUILD_JOBS / IDEOCODE_BUILD_JOBS always
# wins, and non-Linux hosts fall back to the cargo/.cargo default.
select_build_jobs() {
  # Respect an explicit override from either env var.
  local override="${IDEOCODE_BUILD_JOBS:-${CARGO_BUILD_JOBS:-}}"
  if [[ -n "$override" ]]; then
  ...
  # rustc unit (IDEOCODE-base, ~1.6 GiB RSS sampled) so an idle machine uses nearly
  # every core while a memory-pressured one backs off before earlyoom kills a
  # build. Tunable per host via IDEOCODE_BUILD_MIB_PER_JOB.
  mib_per_job_default=1792
  mib_per_job="${IDEOCODE_BUILD_MIB_PER_JOB:-$mib_per_job_default}"
  ...
  if (( jobs < cpus )); then
    log "limiting cargo to ${jobs} job(s) under memory pressure (${mem_available_mib}MiB available, ~${mib_per_job}MiB/job); override with IDEOCODE_BUILD_JOBS"
  fi
}

# Keep `IDEOCODE-build-meta`'s embedded git metadata in lockstep with HEAD without
# making it watch `.git` mtimes.
#
# `IDEOCODE-build-meta/build.rs` deliberately does NOT declare `.git/HEAD`/`.git/index`
...
# `cargo:rerun-if-env-changed=IDEOCODE_BUILD_GIT_HASH` (and _DATE), so cargo reruns
...
# export IDEOCODE_BUILD_GIT_DIRTY here: the dirty flag flips on every edit and would
...
export_git_build_metadata() {
  ...
  if [[ -n "${IDEOCODE_BUILD_GIT_HASH:-}" ]]; then
    git_meta_status="external:${IDEOCODE_BUILD_GIT_HASH}"
    return
  fi
  ...
  export IDEOCODE_BUILD_GIT_HASH="$hash"
  date="$(git -C "$repo_root" log -1 --format=%ci 2>/dev/null || true)"
  if [[ -n "$date" ]]; then
    export IDEOCODE_BUILD_GIT_DATE="$date"
  fi
...
maybe_configure_low_memory_selfdev() {
  ...
  local mode="${IDEOCODE_SELFDEV_LOW_MEMORY:-auto}"
  case "$mode" in
    1|true|yes|on|force)
      ;;
    0|false|no|off|never)
    ...
    auto|"")
      ...
      ;;
    *)
      printf 'error: unsupported IDEOCODE_SELFDEV_LOW_MEMORY=%s (expected auto|on|off)\n' "$mode" >&2
      exit 1
      ;;
  esac
...
# builds. The IDEOCODE monoliths (IDEOCODE-base/app-core/tui) are ~80% single-threaded
...
#
# Controls:
#   IDEOCODE_PARALLEL_FRONTEND=auto|0|1   (default auto)
#   IDEOCODE_FRONTEND_THREADS=<n>         (default 4; diminishing returns past 4)
#   IDEOCODE_DEV_TOOLCHAIN=<name>         (default: nightly when present)
...
dev_nightly_toolchain() {
  ...
  if [[ -n "${IDEOCODE_DEV_TOOLCHAIN:-}" ]]; then
    printf '%s\n' "$IDEOCODE_DEV_TOOLCHAIN"
    return 0
  fi
  local tc
...
configure_parallel_frontend() {
  local requested="${IDEOCODE_PARALLEL_FRONTEND:-auto}"
  local forced="false"
  case "$requested" in
    0|false|no|off)
  ...
  # (`IDEOCODE_PARALLEL_FRONTEND=1`) opts dev/test in too, accepting that potential
  # cache contention.
  local profile
  ...
  local threads="${IDEOCODE_FRONTEND_THREADS:-4}"
...
configure_linux_linker() {
  local requested_mode="${IDEOCODE_FAST_LINKER:-auto}"
  local mode="$requested_mode"

  case "$mode" in
    auto)
      # Prefer mold over lld: on this repo's large statically-linked binary
      # (~300 MB .text), mold links the IDEOCODE bin in ~2.0s vs lld's ~2.9s
    ...
    lld|mold|system)
      ;;
    *)
      printf 'error: unsupported IDEOCODE_FAST_LINKER=%s (expected auto|lld|mold|system)\n' "$mode" >&2
      exit 1
      ;;
  esac
...
print_setup() {
  if [[ -n "${IDEOCODE_DEV_FEATURE_PROFILE:-}" && "${IDEOCODE_DEV_FEATURE_PROFILE}" != "default" ]]; then
    feature_profile_status="${IDEOCODE_DEV_FEATURE_PROFILE}"
  fi
  cat <<EOF
repo_root=$repo_root
os=$(uname -s)
...
build_git_hash=${IDEOCODE_BUILD_GIT_HASH:-<unset>}
...
remote_connect_timeout() {
  local value="${IDEOCODE_REMOTE_CONNECT_TIMEOUT:-5}"
...
remote_tcp_timeout() {
  ...
  local value="${IDEOCODE_REMOTE_TCP_TIMEOUT:-1}"
...
remote_recovery_tcp_timeout() {
  ...
  local value="${IDEOCODE_REMOTE_RECOVERY_TCP_TIMEOUT:-0.3}"
  if [[ ! "$value" =~ ^[0-9]+([.][0-9]+)?$ ]]; then
    value=0.3
  fi
  printf '%s\n' "$value"
...
remote_down_cache_ttl() {
  ...
  local value="${IDEOCODE_REMOTE_DOWN_TTL:-300}"
  if [[ ! "$value" =~ ^[0-9]+$ ]]; then
    value=300
  fi
  printf '%s\n' "$value"
...
remote_down_cache_path() {
  local remote="${IDEOCODE_REMOTE_HOST:-}"
  local key
  ...
  printf '%s/IDEOCODE-remote-down-%s\n' "${TMPDIR:-/tmp}" "$key"
}
...
remote_cargo_preflight() {
  local remote="${IDEOCODE_REMOTE_HOST:-}"
  if [[ -z "$remote" ]]; then
    log "remote cargo requested but IDEOCODE_REMOTE_HOST is not configured"
    return 1
  fi

  local ssh_bin="${IDEOCODE_REMOTE_SSH_BIN:-ssh}"
  ...
  # disabled via IDEOCODE_REMOTE_TCP_PROBE=0.
  local tcp_probe="${IDEOCODE_REMOTE_TCP_PROBE:-1}"
  case "$tcp_probe" in
  ...
  local server_alive_interval="${IDEOCODE_REMOTE_SERVER_ALIVE_INTERVAL:-10}"
  local server_alive_count="${IDEOCODE_REMOTE_SERVER_ALIVE_COUNT_MAX:-1}"
  local output
  if ! output=$("$ssh_bin" \
    -o BatchMode=yes \
    ...
    "$remote" "printf 'IDEOCODE-remote-ok\\n'" 2>&1); then
...
remote_cargo_fallback_mode() {
  local mode="${IDEOCODE_REMOTE_CARGO_FALLBACK:-local}"
  case "$mode" in
    local|1|true|yes|on)
      printf 'local\n'
      ;;
    error|fail|0|false|no|off|never)
      ...
      ;;
    *)
      printf 'error: unsupported IDEOCODE_REMOTE_CARGO_FALLBACK=%s (expected local|error)\n' "$mode" >&2
      exit 2
      ;;
  esac
}
...
run_local_cargo() {
  if cargo_test_has_explicit_filter "${cargo_argv[@]}" && [[ "${IDEOCODE_DEV_CARGO_ALLOW_ZERO_TESTS:-0}" != "1" ]]; then
    local output_file
    output_file=$(mktemp "${TMPDIR:-/tmp}/IDEOCODE-dev-cargo.XXXXXX")
    local status=0
    ...
    if [[ "$status" -eq 0 ]] \
      ...
      printf 'dev_cargo: explicit cargo test filter matched zero tests; check the test path/name or set IDEOCODE_DEV_CARGO_ALLOW_ZERO_TESTS=1 to allow this intentionally\n' >&2
      rm -f "$output_file"
      return 97
    fi
    rm -f "$output_file"
...
if [[ "${IDEOCODE_REMOTE_CARGO:-0}" == "1" ]]; then
  ...
  if [[ "$(remote_cargo_fallback_mode)" == "local" ]]; then
    log "remote cargo unavailable; falling back to local cargo (set IDEOCODE_REMOTE_CARGO_FALLBACK=error to fail instead)"
  else
