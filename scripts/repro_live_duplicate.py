...
Everything runs in a throwaway IDEOCODE_HOME / runtime dir / socket, so the
...
def main() -> int:
    ...
    root = Path(tempfile.mkdtemp(prefix="IDEOCODE-repro314-"))
    home = root / "home"
    run = root / "run"
    ...
    print(f"  home   : {home}")
    print(f"  socket : {env['IDEOCODE_SOCKET']}")
