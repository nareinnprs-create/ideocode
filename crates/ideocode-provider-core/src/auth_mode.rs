...
//!
//! Historically IDEOCODE encoded that decision as free-form strings spread across
//! several overlapping vocabularies:
//!
//! | concept              | runtime env (`IDEOCODE_RUNTIME_PROVIDER`) | route / stable-id    | CLI `--provider` | model prefix     |
//! |----------------------|----------------------------------------|----------------------|------------------|------------------|
...
impl AuthRoute {
    ...
    /// Parse a dual-auth token from *any* of IDEOCODE's overlapping vocabularies
    ...
    /// Canonical `IDEOCODE_RUNTIME_PROVIDER` value that pins this route.
...
/// Read `IDEOCODE_RUNTIME_PROVIDER` and return the dual-auth route it pins, if any.
pub fn runtime_env_auth_route() -> Option<AuthRoute> {
    let value = std::env::var("IDEOCODE_RUNTIME_PROVIDER").ok()?;
    AuthRoute::parse(&value)
}

/// Read `IDEOCODE_RUNTIME_PROVIDER` and resolve the dual-auth mode it pins for a
