...
pub fn socket_path() -> PathBuf {
    ...
    crate::storage::runtime_dir().join("IDEOCODE.sock")
}
...
pub async fn connect_socket(path: &std::path::Path) -> Result<Stream> {
    match Stream::connect(path).await {
        ...
        Err(err) if err.kind() == std::io::ErrorKind::ConnectionRefused && path.exists() => {
            anyhow::bail!(
                "Socket exists but refused the connection at {}. Retry, or remove it after confirming no IDEOCODE server is running.",
                path.display()
            )
        }
...
///
/// Safety: a live daemon holds an exclusive `flock` on `IDEOCODE-daemon.lock` for
...
pub async fn reap_stale_socket_if_dead(path: &std::path::Path) -> bool {
    ...
    crate::logging::warn(&format!(
        "Reaping stale IDEOCODE socket with no live listener at {}",
        path.display()
    ));
...
pub(super) fn daemon_lock_path() -> PathBuf {
    crate::storage::runtime_dir().join("IDEOCODE-daemon.lock")
}
...
pub(super) fn acquire_daemon_lock() -> Result<DaemonLockGuard> {
    ...
    try_acquire_daemon_lock(&path)?.ok_or_else(|| {
        anyhow::anyhow!(
            "Another IDEOCODE server process is already running for runtime dir {}",
...
pub fn set_socket_path(path: &str) {
    crate::env::set_var("IDEOCODE_SOCKET", path);
}
...
/// `IDEOCODE_READY_FD`, and awaits a single byte on the read end. The server
...
pub async fn spawn_server_notify(cmd: &mut std::process::Command) -> Result<std::process::Child> {
    ...
    cmd.env("IDEOCODE_READY_FD", write_fd.to_string());
...
pub(super) fn server_start_matches_existing_server(stderr_output: &str) -> bool {
    stderr_output.contains("Another IDEOCODE server process is already running")
...
pub(super) fn format_server_start_error(
    ...
    if stderr_output.trim().is_empty() {
        format!(
            "Server exited before signalling ready ({}). Check logs at ~/.IDEOCODE/logs/",
            status
        )
    } else {
        format!(
...
/// Write a single byte to the fd in `IDEOCODE_READY_FD` and close it.
...
pub(super) fn signal_ready_fd() {
    #[cfg(unix)]
    {
        ...
        if let Ok(fd_str) = std::env::var("IDEOCODE_READY_FD") {
            crate::env::remove_var("IDEOCODE_READY_FD");
