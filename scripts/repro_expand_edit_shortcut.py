...
closes that gap by driving the **real IDEOCODE binary** under a pseudo-terminal and
...
Everything runs in a throwaway IDEOCODE_HOME / runtime dir / socket. The user's
...
# protocol IDEOCODE turns on at startup):
...
# ──────────────────────────────────────────────────────────────────────────────
# A PTY-driven IDEOCODE client. This is the real live-render input path; bytes
...
def launch_client(binary: str, env: dict, session_id: str, name: str,
    ...
    cenv["IDEOCODE_DEBUG_CMD_PATH"] = str(debug_cmd)
    cenv["IDEOCODE_DEBUG_RESPONSE_PATH"] = str(debug_resp)
    proc = subprocess.Popen(
        [
            binary,
            "--no-update",
            "--no-selfdev",
            "--socket", env["IDEOCODE_SOCKET"],
            "--resume", session_id,
        ],
...
def main() -> int:
    ...
    default_bin = Path.home() / ".IDEOCODE" / "builds" / "current" / "IDEOCODE"
    ...
    root = Path(tempfile.mkdtemp(prefix="IDEOCODE-expand-edit-"))
    home = root / "home"
    run = root / "run"
    ...
    env = os.environ.copy()
    env["IDEOCODE_HOME"] = str(home)
    env["IDEOCODE_RUNTIME_DIR"] = str(run)
    env["IDEOCODE_SOCKET"] = str(run / "IDEOCODE.sock")
    ...
    debug_sock = run / "IDEOCODE-debug.sock"
    ...
    server = subprocess.Popen(
        [binary, "serve", "--socket", env["IDEOCODE_SOCKET"], "--debug-socket",
    ...
    rc = 3
    try:
        wait_for_socket(Path(env["IDEOCODE_SOCKET"]))
