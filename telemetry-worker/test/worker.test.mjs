...
test("web_vital validates, caps, stores, and appends firehose fields", async () => {
  ...
  const response = await worker.fetch(
    postRequest(makeWebBody({
      ...
      url: "https://IDEOCODE.sh/private?token=secret",
    })),
...
test("OPTIONS preflight from IDEOCODE.sh echoes the origin", async () => {
  const response = await worker.fetch(
    new Request(EVENT_URL, {
      method: "OPTIONS",
      headers: { Origin: "https://IDEOCODE.sh" },
    }),
    { DB: makeDb() },
    makeCtx(),
  );
  assert.equal(response.headers.get("Access-Control-Allow-Origin"), "https://IDEOCODE.sh");
