...
IDEOCODE_RUNTIME_DIR so it does not interfere with the user's real server, logs, or
credentials.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("binary", nargs="?", default="./target/release/IDEOCODE")
...
def measure_server_startup(binary: str, runs: int) -> list[float]:
    times: list[float] = []
    for _ in range(runs):
        root = tempfile.mkdtemp(prefix="IDEOCODE-server-bench-")
        env = isolated_env(root)
        socket_path = env["IDEOCODE_SOCKET"]
        proc = None
        try:
            start = time.perf_counter()
