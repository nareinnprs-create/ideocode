...
pub(super) fn draw_help_overlay(frame: &mut Frame, area: Rect, scroll: usize, app: &dyn TuiState) {
    ...
    lines.push(help_entry(
        "/subscription",
        "Inspect IDEOCODE subscription scaffold",
    ));
    lines.push(help_entry(
        "/subscribe",
        "Why and how to subscribe to IDEOCODE",
    ));
    ...
    lines.push(help_entry("/quit", "Exit IDEOCODE"));
    ...
    if let Some(label) = crate::tui::keybind::load_new_terminal_key().label {
        lines.push(key_entry(
            &label,
            "Spawn new IDEOCODE session in a new terminal",
        ));
    }
...
pub(super) fn draw_model_status_overlay(
    ...
    lines.push(Line::from(Span::styled(
        "  Live verification evidence for provider/model behavior in IDEOCODE",
        dim_style,
    )));
    lines.push(Line::from(""));
...
fn model_status_line_style(raw: &str, default: Style) -> Style {
    ...
    // and `IDEOCODE provider-test-coverage` stay color-consistent.
...
pub(super) fn draw_debug_overlay(
    ...
    render_overlay_box(frame, chunks[4], "spacer", Color::Gray);
    render_overlay_box(frame, chunks[5], "notification", Color::Green);
    render_overlay_box(frame, chunks[6], "inline_ui", Color::Blue);
    render_overlay_box(frame, chunks[8], "input", Color::Green);
