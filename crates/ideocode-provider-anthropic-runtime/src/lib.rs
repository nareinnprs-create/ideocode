...
pub struct AnthropicProvider {
    ...
    /// Explicit `IDEOCODE_ANTHROPIC_MAX_TOKENS` override. When unset, the output
...
impl AnthropicProvider {
    ...
    /// `IDEOCODE_RUNTIME_PROVIDER` happens to be in the current process (e.g. a
    /// self-dev session may have it set to `claude-api`). This also updates
    /// `IDEOCODE_RUNTIME_PROVIDER` so any provider instances the probes build
    ...
    pub fn new() -> Self {
        let model = std::env::var("IDEOCODE_ANTHROPIC_MODEL").unwrap_or_else(|_| {
    ...
    async fn get_access_token(&self) -> Result<(String, bool)> {
        ...
        // IDEOCODE OAuth account may still deserialize successfully even after its
    ...
    async fn get_oauth_access_token(&self) -> Result<(String, bool)> {
        ...
        {
            anyhow::bail!(
                "Claude OAuth credentials are missing the required user:inference scope (scopes: {}). Run `IDEOCODE login --provider claude` to mint a fresh Claude.ai OAuth token, or import/use a fresh Claude Code login.",
        ...
        if fresh_creds.expires_at <= now {
            anyhow::bail!(
                "Claude OAuth token is expired and no usable refresh token is available. Run `IDEOCODE login --provider claude` to refresh OAuth credentials"
            );
        }
        ...
        .await
        {
            ...
            Err(e) => {
                        ...
                        Err(refresh_err) => {
                            let _ = tx
                                .send(Err(anyhow::anyhow!(
                                    "{}\n\nAutomatic Claude OAuth refresh failed: {}\nRun `IDEOCODE login --provider claude` (preferred) or `claude`, then retry.",
                                    e,
                                    refresh_err
                                )))
                                .await;
                ...
                if is_oauth && is_oauth_auth_error(&error_str) {
                    let _ = tx
                        .send(Err(anyhow::anyhow!(
                            "{}\n\nClaude OAuth authentication failed. Run `IDEOCODE login --provider claude` (preferred) or `claude`, then retry.",
                            e
                        )))
                        .await;
                } else {
            ...
            if let Ok(parsed) = serde_json::from_str::<MessageStartEvent>(&event.data) {
                ...
                if let Some(usage) = parsed.message.usage {
                    ...
                    if let Some(tier) = usage.service_tier.as_deref() {
                        ...
                        if std::env::var("IDEOCODE_LOG_SERVICE_TIER").is_ok() {
