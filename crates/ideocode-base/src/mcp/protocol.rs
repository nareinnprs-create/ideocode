...
pub struct McpServerConfig {
    /// Command for stdio servers. Empty for HTTP/SSE servers, which IDEOCODE does
    ...
    /// URL for HTTP/SSE servers (Claude Code compat). Unused by IDEOCODE today.
...
impl McpServerConfig {
    /// IDEOCODE currently only supports stdio (command-based) MCP servers. A config
...
pub struct McpConfig {
    ...
    /// IDEOCODE's historical `servers` key.
...
impl McpConfig {
    ...
    /// Only runs if ~/.IDEOCODE/mcp.json doesn't exist yet.
    #[expect(
    ...
    /// order: `.IDEOCODE/mcp.json`, then `.mcp.json` (Claude Code project config),
    ...
    fn load_project_locals(project_root: &std::path::Path) -> Self {
        ...
        for relative in [".IDEOCODE/mcp.json", ".mcp.json", ".claude/mcp.json"] {
    ...
    /// Load from default locations (merges IDEOCODE global + local, local overrides),
    ...
    /// (`.IDEOCODE/mcp.json`, `.mcp.json`, `.claude/mcp.json`, and the per-project
    ...
    pub fn load_for_dir(project_dir: Option<&std::path::Path>) -> Self {
        ...
        // IDEOCODE only supports stdio servers today. Drop HTTP/SSE entries (common
