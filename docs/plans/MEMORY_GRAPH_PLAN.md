...
## Two operating modes (gate: `agents.memory_sidecar_enabled`, env `IDEOCODE_MEMORY_SIDECAR_ENABLED`, default off)
...
Benchmark (Mode 1, private ~/IDEOCODE-memory-bench, Sonnet judge):
...
Benchmark-driven results (Sonnet judge, 28 judged queries, IDEOCODE self-dev corpus):
...
- Shared: IDEOCODE-base/src/memory_rerank.rs (prompt + parse + rerank_candidates),
...
~/IDEOCODE-memory-bench/results/BASELINE_SUMMARY.md):
