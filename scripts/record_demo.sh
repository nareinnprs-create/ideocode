...
#
# This script:
# 1. Opens a fresh IDEOCODE in a new kitty window
# 2. Starts wf-recorder on that window
# 3. Sends the prompt to IDEOCODE
...
DEMO_DIR="/tmp/IDEOCODE-demo/$DEMO_NAME"
OUTPUT_DIR="$repo_root/assets/demos"
SOCK=$(ls /tmp/kitty.sock* 2>/dev/null | head -1)

mkdir -p "$DEMO_DIR" "$OUTPUT_DIR"

echo "=== IDEOCODE Demo Recorder ==="
echo "Demo: $DEMO_NAME"
echo "Prompt: $PROMPT"
echo "Working dir: $DEMO_DIR"
echo ""

# Step 1: Launch IDEOCODE in a new kitty OS window
echo "[1/5] Launching IDEOCODE..."
...
DEMO_WIN_ID=$(niri msg windows 2>/dev/null | grep -B5 "IDEOCODE-demo-$DEMO_NAME" | grep "Window ID" | awk '{print $3}' | tr -d ':')
...
# Step 4: Type the prompt into IDEOCODE
...
for os_win in data:
    for tab in os_win.get('tabs', []):
        for win in tab.get('windows', []):
            if 'IDEOCODE-demo-$DEMO_NAME' in win.get('title', ''):
                print(win['id'])
                sys.exit(0)
")
...
if [ -n "$KITTY_WIN_ID" ]; then
    ...
    kitten @ --to unix:$SOCK send-text --match "title:IDEOCODE-demo-$DEMO_NAME" "$PROMPT"
    sleep 0.5
    kitten @ --to unix:$SOCK send-text --match "title:IDEOCODE-demo-$DEMO_NAME" $'\r'
fi
...
# Poll for completion - check if IDEOCODE is still processing
