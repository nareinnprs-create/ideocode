...
/// Get the IDEOCODE repository directory
...
///
/// Release archives install a tiny `IDEOCODE` wrapper script alongside the real
/// `IDEOCODE-<platform>.bin` payload (the wrapper sets `LD_LIBRARY_PATH` and execs
...
fn infer_selfdev_build_target(repo_dir: &Path) -> SelfDevBuildTarget {
    ...
    for line in text.lines() {
        ...
        if path == "Cargo.toml" || path == "Cargo.lock" || path.starts_with(".cargo/") {
            desktop = true;
            other = true;
        } else if path.starts_with("crates/IDEOCODE-desktop/") {
            desktop = true;
...
///
/// Defaults to `~/.local/bin` on Unix, `%LOCALAPPDATA%\IDEOCODE\bin` on Windows.
/// Overridable with `IDEOCODE_INSTALL_DIR`.
pub fn launcher_dir() -> Result<PathBuf> {
    if let Some(custom) = non_empty_env_path("IDEOCODE_INSTALL_DIR") {
        return Ok(custom);
    }

    if let Some(sandbox_home) = non_empty_env_path("IDEOCODE_HOME") {
    ...
    #[cfg(windows)]
    {
        if let Ok(local) = std::env::var("LOCALAPPDATA") {
            return Ok(PathBuf::from(local).join("IDEOCODE").join("bin"));
        }
        Ok(home_dir()?
            .join("AppData")
            .join("Local")
...
/// Path to the launcher binary (`~/.local/bin/IDEOCODE` by default).
...
mod tests {
    ...
    fn repo_fixture(git_file: bool) -> tempfile::TempDir {
        ...
        if git_file {
            std::fs::write(temp.path().join(".git"), "gitdir: /tmp/IDEOCODE-test-git\n")
                .expect("git file");
        } else {
    ...
    fn find_repo_in_ancestors_finds_workspace_from_crate_dir() {
        ...
        let crate_dir = repo.path().join("crates").join("IDEOCODE-build-support");
    ...
    fn release_install_fixture() -> (tempfile::TempDir, PathBuf, PathBuf) {
        ...
        let wrapper = temp.path().join("IDEOCODE");
        let payload = temp.path().join("IDEOCODE-linux-x86_64.bin");
        std::fs::write(
            &wrapper,
    ...
    fn resolve_binary_payload_follows_channel_symlink_to_wrapper() {
        // The real layout: builds/<channel>/IDEOCODE -> versions/<v>/IDEOCODE (wrapper).
        ...
        let link = channel_dir.join("IDEOCODE");
        #[cfg(unix)]
    ...
    fn resolve_binary_payload_keeps_real_binary() {
        ...
        let temp = tempfile::TempDir::new().expect("temp install");
        let binary = temp.path().join("IDEOCODE");
        std::fs::write(&binary, vec![0x7fu8; 8192]).expect("binary");
        std::fs::write(temp.path().join("IDEOCODE-linux-x86_64.bin"), [0u8; 8]).expect("bin");
        assert_eq!(
    ...
    fn resolve_binary_payload_keeps_small_script_without_payload() {
        let temp = tempfile::TempDir::new().expect("temp install");
        let script = temp.path().join("IDEOCODE");
