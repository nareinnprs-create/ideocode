...
This document describes a proposed evolution of IDEOCODE's UI architecture from the
...
Today, IDEOCODE is effectively organized like this:
...
- a independent IDEOCODE window attached to one session
...
- current independent IDEOCODE window
...
2. Client spawns a independent IDEOCODE client attached to the same session.
...
This can initially be provided through existing IDEOCODE control channels such as:
...
- Preserve current `IDEOCODE --resume <session>` workflow.
...
- Should workspace mode be a new command (`IDEOCODE workspace`) or a runtime mode of
  the normal client?
...
This gives IDEOCODE a portable built-in multi-session workspace without sacrificing
