...
pub struct IDEOCODEProvider {
    inner: MultiProvider,
    selected_model: Arc<RwLock<String>>,
}

impl IDEOCODEProvider {
    pub fn new() -> Self {
    ...
    fn entitled_models_for(
        tier: crate::subscription_catalog::IDEOCODETier,
    ...
    fn model_routes_for(tier: crate::subscription_catalog::IDEOCODETier) -> Vec<ModelRoute> {
...
impl Default for IDEOCODEProvider {
    fn default() -> Self {
        Self::new()
    }
}
...
#[async_trait]
impl Provider for IDEOCODEProvider {
    async fn complete(
        &self,
        messages: &[Message],
    ...
    fn name(&self) -> &str {
        crate::subscription_catalog::IDEOCODE_PROVIDER_DISPLAY_NAME
    }
...
mod tests {
    ...
    fn IDEOCODE_provider_enables_subscription_runtime_mode() {
        ...
        runtime.block_on(async {
            let provider = IDEOCODEProvider::new();
    ...
    fn IDEOCODE_provider_exposes_only_explicit_subscription_routes() {
        ...
        assert!(plus_routes.iter().all(|route| {
            route.provider == "IDEOCODE Subscription"
                && route.api_method == "IDEOCODE-subscription"
                && route.available
        }));
        assert_eq!(
            IDEOCODEProvider::entitled_models_for(IDEOCODETier::Plus)
                .map(|model| model.id.to_string())
                .collect::<Vec<_>>(),
            expected_models
                .iter()
