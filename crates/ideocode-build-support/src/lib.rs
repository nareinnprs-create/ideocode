...
pub fn smoke_test_server_binary(binary: &Path) -> Result<()> {
    ...
    let socket_path = temp.path().join("IDEOCODE-smoke.sock");
    let stderr_path = temp.path().join("IDEOCODE-smoke.stderr.log");
