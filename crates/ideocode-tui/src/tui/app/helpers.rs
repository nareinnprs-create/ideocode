...
/// NO_BROWSER/IDEOCODE_NO_BROWSER and refuses to open anything from test binaries
...
pub(crate) fn open_path_or_url_detached(
    ...
    if crate::auth::browser_suppressed(false) {
        return Err(std::io::Error::other(
            "opening files/URLs is suppressed (NO_BROWSER/IDEOCODE_NO_BROWSER or test harness)",
        ));
    }
...
pub(super) fn debug_cmd_path() -> PathBuf {
    if let Ok(path) = std::env::var("IDEOCODE_DEBUG_CMD_PATH") {
        return PathBuf::from(path);
    }
    std::env::temp_dir().join("IDEOCODE_debug_cmd")
}

/// Debug response file path
pub(super) fn debug_response_path() -> PathBuf {
    if let Ok(path) = std::env::var("IDEOCODE_DEBUG_RESPONSE_PATH") {
        return PathBuf::from(path);
    }
    std::env::temp_dir().join("IDEOCODE_debug_response")
}
...
/// Spawn a new terminal window that resumes a IDEOCODE session.
...
/// Build the terminal command used to spawn a brand-new IDEOCODE session.
...
fn build_fresh_session_command(socket: Option<&str>) -> crate::terminal_launch::TerminalCommand {
    ...
    crate::terminal_launch::TerminalCommand::new(&exe, args)
        .title("IDEOCODE · new session".to_string())
        .kind("new-terminal")
        .fresh_spawn()
}

/// Spawn a brand-new IDEOCODE session in a new terminal window, staying on the
...
fn resumed_window_title(session_id: &str) -> String {
    ...
    {
        format!("IDEOCODE/{} {}", server_info.name, session_label)
    } else {
        format!("IDEOCODE {}", session_label)
    };
...
pub(super) fn clipboard_image() -> Option<(String, String)> {
    ...
    #[cfg(target_os = "macos")]
    {
        let temp_path = std::env::temp_dir().join("IDEOCODE_clipboard.png");
        let script = format!(
