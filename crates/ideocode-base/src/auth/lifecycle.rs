...
fn globally_preferred_model_rank(model: &str) -> (u8, usize) {
    ...
    // Some catalogs expose the clean release id instead of IDEOCODE's Sol route.
...
/// order. The Claude/OpenAI subscription default bias mirrors IDEOCODE's global
...
fn route_matches_activation(route: &ModelRoute, activation: &AuthActivationResult) -> bool {
    ...
    match provider_id {
        ...
        "IDEOCODE" => {
            // IDEOCODE subscription routes deliberately keep their managed public
            // identity even though the runtime reuses OpenRouter transport code.
            return matches!(
                api_method,
                crate::provider::ModelRouteApiMethod::IDEOCODESubscription
            );
        }
        "azure-openai" => {
...
fn api_key_env_bindings_for_provider(provider_id: &str) -> Vec<(String, String)> {
    match provider_id {
        ...
        "IDEOCODE" => vec![
            (
                crate::subscription_catalog::IDEOCODE_API_KEY_ENV.to_string(),
                crate::subscription_catalog::IDEOCODE_ENV_FILE.to_string(),
            ),
            (
                crate::subscription_catalog::IDEOCODE_API_BASE_ENV.to_string(),
                crate::subscription_catalog::IDEOCODE_ENV_FILE.to_string(),
            ),
        ],
        "bedrock" => vec![
...
/// process (issue #453).
///
/// `/login` persists API keys to the per-provider env file under the IDEOCODE
...
fn direct_provider_activation(provider_id: &str) -> Option<ProviderActivation> {
    let (runtime_id, active) = match normalized_login_provider_id(provider_id)? {
        ...
        "IDEOCODE" => (RuntimeProviderId::IDEOCODE, ActiveProvider::OpenRouter),
...
mod tests {
    ...
    fn direct_login_provider_ids_are_normalized_with_display_labels() {
        for (hint, normalized, label) in [
            ...
            ("subscription", "IDEOCODE", "IDEOCODE Subscription"),
    ...
    fn direct_login_provider_activation_sets_runtime_identity_and_active_provider() {
        // Sandbox IDEOCODE_HOME so activation's env-file credential sync (#453)
        // cannot read the developer's real ~/.config/IDEOCODE/*.env files and
        ...
        for (provider, runtime, active) in [
            ...
            crate::env::remove_var("IDEOCODE_INITIAL_PROVIDER_EXPLICIT");
            ...
            assert_eq!(
                std::env::var("IDEOCODE_RUNTIME_PROVIDER").as_deref(),
                Ok(runtime)
            );
            assert_eq!(
                std::env::var("IDEOCODE_ACTIVE_PROVIDER").as_deref(),
                Ok(active)
            );
            assert_eq!(
                std::env::var("IDEOCODE_INITIAL_PROVIDER_EXPLICIT").as_deref(),
                Ok("1")
            );
        }
    }
    ...
    fn direct_login_provider_descriptor_matrix_has_full_lifecycle_parity() {
        // Sandbox IDEOCODE_HOME for the same reason as the activation matrix
        ...
        for provider in crate::provider_catalog::login_providers() {
            let Some((normalized, runtime, active, switch_prefix)) = (match provider.target {
                crate::provider_catalog::LoginProviderTarget::IDEOCODE => {
                    Some(("IDEOCODE", "IDEOCODE", "openrouter", ""))
                }
    ...
    fn ideocode_auth_lifecycle_matches_only_managed_subscription_routes() {
        let activation = AuthActivationResult {
            ...
            expected_runtime: Some("IDEOCODE-subscription".to_string()),
            expected_catalog_namespace: Some("IDEOCODE-subscription".to_string()),
        };
        let routes = vec![
            ...
            route(
                ...
                crate::subscription_catalog::IDEOCODE_ROUTE_API_METHOD,
                true,
            ),
        ];
        ...
        assert!(
            report.ok(),
            "canonical IDEOCODE route should match: {report:?}"
        );
        assert_eq!(report.selectable_provider_routes, 1);
        assert_eq!(
            report.route_sample,
        ...
        assert_eq!(
            activation.model_switch_request("IDEOCODE Subscription", "gpt-5.5"),
            "gpt-5.5"
        );
    }
    ...
    fn post_auth_model_selection_keeps_catalog_order_for_unranked_providers() {
        ...
        // shared test-env lock (and a private IDEOCODE_HOME) or a sibling test's
    ...
    /// Claude flagship over the OpenAI flagship to mirror IDEOCODE's default model.
    #[test]
