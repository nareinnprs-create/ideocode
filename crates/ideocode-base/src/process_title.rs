...
/// identifies the session/connection, so do not repeat `IDEOCODE` or the memorable
...
pub fn set_server_title(server_name: &str) {
    set_title(compact_process_title("IDEOCODE:s:", Some(server_name)));
}
...
pub fn set_client_remote_display_title(server_name: &str, session_name: &str, is_selfdev: bool) {
    if server_name.is_empty() || server_name.eq_ignore_ascii_case("IDEOCODE") {
    ...
    fn terminal_window_title_omits_product_and_animal_names() {
        assert_eq!(
            terminal_window_title(
                ...
                Some("IDEOCODE Octopus"),
                false
            ),
            "🐙 resume window title"
        );
        assert_eq!(
            terminal_window_title("🐙", None, Some("IDEOCODE Octopus"), false),
            "🐙 IDEOCODE Octopus"
        );
        assert_eq!(
            terminal_window_title(
