...
impl App {
    ...
    pub fn debug_socket_path() -> std::path::PathBuf {
        crate::storage::runtime_dir().join("IDEOCODE-debug.sock")
    }
}
...
/// plus the curated list of IDEOCODE-endorsed skills (marking which are installed).
fn build_skills_report(app: &App) -> String {
    ...
    if app.is_remote && !app.remote_skills.is_empty() {
        ...
        if skills.is_empty() {
            out.push_str(
                "- none loaded\n  Add skills under ~/.IDEOCODE/skills/<name>/SKILL.md or ./.IDEOCODE/skills/<name>/SKILL.md\n",
            );
        } else {
            for skill in skills {
...
pub(super) fn handle_info_command(app: &mut App, trimmed: &str) -> bool {
    ...
    if trimmed == "/version" {
        ...
        let mut content = format!("IDEOCODE client: {}{}", version, is_canary);
        if app.is_remote {
