...
```bash
IDEOCODE --quiet --no-update --no-selfdev ...
```
...
```bash
IDEOCODE --quiet model list
IDEOCODE --quiet model list --json
IDEOCODE --quiet --provider openai model list --json
```
...
```bash
IDEOCODE --quiet provider list
IDEOCODE --quiet provider list --json
```

Inspect the currently requested and resolved provider/model selection:

```bash
IDEOCODE --quiet provider current
IDEOCODE --quiet --provider openai --model gpt-5.4 provider current --json
```

Verbose human summary:

```bash
IDEOCODE --quiet model list --verbose
```

## Run one prompt and return JSON

```bash
IDEOCODE --quiet run --json "Reply with exactly OK"
```

## Stream one prompt as NDJSON

```bash
IDEOCODE --quiet run --ndjson "Reply with exactly OK"
```
...
```bash
IDEOCODE --quiet auth status
IDEOCODE --quiet auth status --json
```
...
```bash
IDEOCODE --quiet version
IDEOCODE --quiet version --json
```
...
- `IDEOCODE model list` and `IDEOCODE run --json` do not require the TUI
- `IDEOCODE model list` does not require an already-running shared server
