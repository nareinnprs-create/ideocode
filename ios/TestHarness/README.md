...
- **`mock_gateway.py`** - a self-contained (stdlib-only) mock of the IDEOCODE
  server gateway. Speaks the exact wire protocol from
  `crates/IDEOCODE-base/src/gateway.rs` on one TCP port:
...
```bash
# Full pipeline, screenshot lands in $TMPDIR/IDEOCODE-ios-e2e/chat.png
...
python3 TestHarness/mock_gateway.py &        # or run a real `IDEOCODE` gateway
...
`Library/Application Support/IDEOCODE-servers.json` when the Keychain is
...
`IDEOCODEKit` (the platform-free client core) is fully unit-tested with `swift
