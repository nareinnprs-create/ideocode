...
fn first_three_launches_can_include_hotkey_notice_too() {
    ...
    // The notice should make clear the hotkey works globally, not just inside IDEOCODE.
...
fn default_resolved_hotkeys_match_legacy_three() {
    ...
    let home = launch_hotkeys::shell_command_for(&resolved[0], "/usr/local/bin/IDEOCODE");
    assert!(home.starts_with("cd \"$HOME\"; "));
    assert!(!home.contains("self-dev"));

    let last_dir = launch_hotkeys::shell_command_for(&resolved[1], "/usr/local/bin/IDEOCODE");
    assert!(last_dir.contains("cat '/home/u/.IDEOCODE/hotkey/last_dir'"));
    assert!(last_dir.contains("cd \"$HOME\""));

    let selfdev = launch_hotkeys::shell_command_for(&resolved[2], "/usr/local/bin/IDEOCODE");
    assert!(selfdev.contains("cat '/home/u/.IDEOCODE/hotkey/last_repo'"));
    assert!(selfdev.contains("'/usr/local/bin/IDEOCODE' 'self-dev';"));
}
...
fn baked_repo_hotkey_cds_into_fixed_dir() {
    ...
    let cmd = launch_hotkeys::shell_command_for(&resolved[0], "/usr/local/bin/IDEOCODE");
    assert!(cmd.contains("/Users/jeremy/IDEOCODE-github"));
...
fn mac_hotkey_launch_agent_plist_uses_valid_xml_quotes() {
    let plist = mac_hotkey_launch_agent_plist(
        "/Applications/IDEOCODE.app/Contents/MacOS/IDEOCODE",
        "/tmp/IDEOCODE-hotkey.out.log",
        "/tmp/IDEOCODE-hotkey.err.log",
        "ghostty",
    );
...
fn macos_terminal_notice_only_fires_for_default_terminal_app() {
    ...
    assert_eq!(
        ...
        Some("Tip: Terminal.app renders IDEOCODE poorly. Try Ghostty, iTerm2, or Alacritty.")
    );
    let (title, message) = hints.display_message.expect("expected display message");
    assert_eq!(title, "Terminal");
    assert!(message.contains("Terminal.app renders IDEOCODE poorly"));
...
fn glyph_safe_notice_shows_once_then_debounces() {
    ...
    assert!(body.contains("IDEOCODE_GLYPH_SAFE_MODE=off"));
