...
//!     Alt+Semicolon hotkey-overlay-title="IDEOCODE: home" { spawn "sh" "-c" "..."; }
//!     // <<< IDEOCODE launch hotkeys (managed) <<<
//! ```
...
pub(crate) const NIRI_BLOCK_BEGIN: &str = "// >>> IDEOCODE launch hotkeys (managed) >>>";
/// Closing sentinel for the managed bind region inside `binds { }`.
pub(crate) const NIRI_BLOCK_END: &str = "// <<< IDEOCODE launch hotkeys (managed) <<<";
...
pub(crate) struct NiriHotkey {
    pub chord: KeyChord,
    /// Concrete directory to launch IDEOCODE in (already resolved from any
...
/// Map a IDEOCODE modifier+key chord onto niri's KDL key syntax.
///
/// niri uses `+`-joined modifiers followed by an XKB key name, e.g.
/// `Alt+Semicolon`, `Super+Shift+Apostrophe`. We translate IDEOCODE's `cmd`
...
pub(crate) fn chord_to_niri_bind(chord: &KeyChord) -> Option<String> {
    ...
    // IDEOCODE `cmd` == macOS Command == Wayland Super.
    if chord.cmd {
        parts.push("Super");
    }
    if chord.ctrl {
...
/// Translate a canonical IDEOCODE key token into the XKB key name niri expects.
...
/// Build the `sh -c` command string a bind runs to open IDEOCODE in `dir`.
///
/// We `cd` into the directory (falling back to `$HOME` if it has since been
/// removed), then launch IDEOCODE via the user's terminal. The terminal is chosen
/// by `terminal` (e.g. `kitty`); we pass it the IDEOCODE executable directly.
fn launch_shell_command(
    ...
    // cd with $HOME fallback, then exec the terminal running IDEOCODE.
    format!(
...
pub(crate) fn render_niri_bind_line(
    ...
    let title = if hotkey.self_dev {
        format!("IDEOCODE: {} (self-dev)", hotkey.label)
    } else {
        format!("IDEOCODE: {}", hotkey.label)
    };
...
mod tests {
    ...
    fn renders_bind_line_with_cd_and_terminal() {
        let line = render_niri_bind_line(
            &hk("alt+;", "/home/jeremy/IDEOCODE", "IDEOCODE", true),
            "/home/jeremy/.local/bin/IDEOCODE",
            "kitty",
            "    ",
        )
        .unwrap();
        ...
        assert!(line.contains("hotkey-overlay-title=\"IDEOCODE: IDEOCODE (self-dev)\""));
        assert!(line.contains("spawn \"sh\" \"-c\""));
        assert!(line.contains("/home/jeremy/IDEOCODE"));
    ...
    fn render_block_wraps_sentinels() {
        ...
        .unwrap();
        assert!(block.starts_with("    // >>> IDEOCODE launch hotkeys (managed) >>>"));
        assert!(
            block
                .trim_end()
                .ends_with("// <<< IDEOCODE launch hotkeys (managed) <<<")
        );
