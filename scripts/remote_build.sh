...
#
# Defaults:
# - Config file: ~/.config/IDEOCODE/remote-build.env (override with IDEOCODE_REMOTE_CONFIG)
# - Host: IDEOCODE_REMOTE_HOST from env/config, or --host
# - Remote dir: .cache/remote-builds/IDEOCODE/<repo-name> (override with IDEOCODE_REMOTE_DIR or --remote-dir)
#
# Examples:
#   scripts/remote_build.sh --release
#   scripts/remote_build.sh test
#   scripts/remote_build.sh check --all-targets
#   scripts/remote_build.sh --host mybox --remote-dir ~/src/IDEOCODE test -- --nocapture
...
Options:
  ...
  --host HOST          Remote SSH host (default: $IDEOCODE_REMOTE_HOST from env/config; required if unset)
  --remote-dir DIR     Remote project directory (default: $IDEOCODE_REMOTE_DIR or .cache/remote-builds/IDEOCODE/<repo-name>)
...
Behavior:
  ...
  - For build sync-back, copies target/{debug|release}/<artifact> from remote to local
    (artifact defaults to 'IDEOCODE', or '--bin <name>' when provided)
  - Default config file is ~/.config/IDEOCODE/remote-build.env
EOF
}
...
REMOTE="${IDEOCODE_REMOTE_HOST:-}"
REMOTE_DIR="${IDEOCODE_REMOTE_DIR:-.cache/remote-builds/IDEOCODE/${REPO_NAME}}"
SSH_BIN="${IDEOCODE_REMOTE_SSH_BIN:-ssh}"
RSYNC_BIN="${IDEOCODE_REMOTE_RSYNC_BIN:-rsync}"
...
if [[ -z "$REMOTE" ]]; then
    echo "error: remote host not configured; set IDEOCODE_REMOTE_HOST or pass --host HOST" >&2
    exit 2
fi
...
SSH_SERVER_ALIVE_INTERVAL="${IDEOCODE_REMOTE_SERVER_ALIVE_INTERVAL:-10}"
SSH_SERVER_ALIVE_COUNT_MAX="${IDEOCODE_REMOTE_SERVER_ALIVE_COUNT_MAX:-1}"
SSH_OPTS=(
    -o BatchMode=yes
...
RSYNC_SSH_COMMAND="${IDEOCODE_REMOTE_RSYNC_SSH:-$SSH_BIN -o BatchMode=yes -o ConnectTimeout=$SSH_CONNECT_TIMEOUT -o ServerAliveInterval=$SSH_SERVER_ALIVE_INTERVAL -o ServerAliveCountMax=$SSH_SERVER_ALIVE_COUNT_MAX}"
...
if ! preflight_output="$(remote_ssh "printf 'IDEOCODE-remote-ok\\n'" 2>&1)"; then
    echo "error: remote host '$REMOTE' is not reachable within ${SSH_CONNECT_TIMEOUT}s" >&2
    echo "$preflight_output" >&2
    echo "hint: set IDEOCODE_REMOTE_CARGO=0 to force local cargo, or fix IDEOCODE_REMOTE_HOST/IDEOCODE_REMOTE_CONNECT_TIMEOUT." >&2
    exit 75
fi
...
if [[ "$SYNC_SOURCE" -eq 1 ]]; then
    ...
    trap 'rm -f "$metadata_file"' EXIT
    {
        ...
        printf 'git_tag=%s\n' "$local_git_tag"
        printf 'git_dirty=%s\n' "$local_git_dirty"
        printf 'changelog_raw<<IDEOCODE_CHANGELOG_EOF\n%s\nIDEOCODE_CHANGELOG_EOF\n' "$local_changelog_raw"
    } > "$metadata_file"
    "$RSYNC_BIN" -avz -e "$RSYNC_SSH_COMMAND" "$metadata_file" "$REMOTE:$REMOTE_DIR/.IDEOCODE-build-meta"
else
    echo ""
...
printf -v REMOTE_INNER_CMD 'cd %q && env IDEOCODE_BUILD_METADATA_FILE=.IDEOCODE-build-meta %s' "$REMOTE_DIR" "$REMOTE_CARGO_CMD"
