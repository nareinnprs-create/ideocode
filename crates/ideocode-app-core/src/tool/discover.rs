...
const DISCOVERY_REQUEST_ID_HEADER: &str = "x-IDEOCODE-discovery-request-id";
const DISCOVERY_BENCHMARK_HEADER: &str = "x-IDEOCODE-discovery-benchmark";
const DISCOVERY_SESSION_ID_HEADER: &str = "x-IDEOCODE-discovery-session-id";
const DISCOVERY_SESSION_METADATA_HEADER: &str = "x-IDEOCODE-discovery-session-metadata";
const DISCOVERY_SELF_DEV_HEADER: &str = "x-IDEOCODE-discovery-self-dev";
const DISCOVERY_DEBUG_HEADER: &str = "x-IDEOCODE-discovery-debug";
const DISCOVERY_CANARY_HEADER: &str = "x-IDEOCODE-discovery-canary";
const DISCOVERY_EXECUTION_MODE_HEADER: &str = "x-IDEOCODE-discovery-execution-mode";
const DISCOVERY_BUILD_CHANNEL_HEADER: &str = "x-IDEOCODE-discovery-build-channel";
const DISCOVERY_GIT_CHECKOUT_HEADER: &str = "x-IDEOCODE-discovery-git-checkout";
const DISCOVERY_CI_HEADER: &str = "x-IDEOCODE-discovery-ci";
const DISCOVERY_RAN_FROM_CARGO_HEADER: &str = "x-IDEOCODE-discovery-ran-from-cargo";
const DISCOVERY_BENCHMARK_ENV: &str = "IDEOCODE_DISCOVERY_BENCHMARK";
...
///
/// Disclosure contract: some providers may share revenue with IDEOCODE, but
...
impl Tool for DiscoverToolsTool {
    ...
    fn description(&self) -> &str {
        "Use proactively whenever fully completing the user's goal requires an external \
         ...
         vetted, and one-tool-call setup is enforced. Some providers are IDEOCODE partners and may \
         share revenue with IDEOCODE when a referred user becomes a customer. Partnership status \
        ...
        json!({
            ...
            "properties": {
                ...
                "query": {
                    ...
                    "description": "Required capability summary. Browse/select text may be sent to relevant partners for demand reporting. Suggest text goes only to IDEOCODE maintainers. Write a fresh summary instead of copying user text. Never include secrets, credentials, personal data, or private content."
                },
                "reason": {
                    "type": "string",
                    "minLength": DISCOVERY_REASON_MIN_CHARS,
                    "maxLength": DISCOVERY_REASON_MAX_CHARS,
                    "description": "Required rationale. For select, explain why the tool fits better than alternatives. For suggest, explain why browse results were unsuitable. Browse/select text may reach relevant partners; suggest text goes only to IDEOCODE maintainers. Never include private data."
                },
                "tool": {
                    "type": "string",
                ...
                "gap_evidence": {
                    "type": "string",
                    "maxLength": 500,
                    "description": "Optional concise explanation of which browse results were close and why they did not fit. Sent only to IDEOCODE maintainers."
                },
                "requirements": {
                    "type": "array",
                    "maxItems": 8,
                    "items": { "type": "string", "minLength": 3, "maxLength": 240 },
                    "description": "Optional concrete public constraints the catalog addition should satisfy. Sent only to IDEOCODE maintainers."
                },
                "prior_request_id": {
                    "type": "string",
    ...
    let mut request = context.provenance.apply(
        context
            ...
            .header(
                ...
                format!("IDEOCODE/{}", env!("CARGO_PKG_VERSION")),
            )
...
fn render_listing(category: &str, listing: &Value, request_id: &str) -> Result<String> {
    ...
    let mut out = format!(
        "Discoverable tools in '{category}' (IDEOCODE tool directory; recommendations must be based \
         only on fit; details: {}):\n",
...
fn render_suggestion(
    ...
    out.push_str(
        "\n\nStatus: received for IDEOCODE maintainer review. Suggestions are not sent to partners. This does not mean IDEOCODE has partnered with the tool or that it is approved or available.",
    );
    Ok(out)
}
...
fn render_selection(category: &str, tool_name: &str, listing: &Value) -> Result<String> {
    ...
    let blurb = tool.get("blurb").and_then(|v| v.as_str()).unwrap_or("");
    let mut out = format!(
        "Selected '{name}' from '{category}' (IDEOCODE tool directory; selection must be based only \
         on fit; details: {}):\n\n{name}: {blurb}",
        crate::sponsors::DISCOVERY_PARTNERS_URL
    );
    if let Some(url) = tool.get("url").and_then(|v| v.as_str()) {
...
mod tests {
    ...
    fn render_listing_includes_disclosure_and_tools() {
        ...
        assert!(out.contains("IDEOCODE tool directory"));
    ...
    fn schema_is_compact_and_self_contained() {
        ...
        assert!(description.contains("Some providers are IDEOCODE partners"));
        ...
        assert!(schema.contains("Suggest text goes only to IDEOCODE maintainers"));
    ...
    fn suggestion_validation_distinguishes_product_and_capability_gap() {
        ...
        known.product_url = Some("https://example.com/tool?via=IDEOCODE#setup".to_string());
    ...
    fn render_suggestion_is_clear_about_review_status_and_recipient() {
        ...
        assert!(out.contains("does not mean IDEOCODE has partnered with the tool"));
    }
    ...
    async fn execute_end_to_end_with_enabled_config_and_local_server() {
        ...
        assert!(output.output.contains("IDEOCODE tool directory"));
        assert!(
            output
                .output
